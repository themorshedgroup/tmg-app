    ...
    async function callCalendar(payload) {
      if (CAL_IS_DEV) return { ok: true, status: 200, data: (payload.action === 'create' || payload.action === 'team-event-create') ? { ok: true, id: 'dev-' + Math.random().toString(36).slice(2) } : { ok: true } };
    ...
    async function createOOOEvent(name, startISO, endISO) {
      ...
      const { ok, data } = await callCalendar({ action: 'team-event-create', event });
    ...
    async function deleteOOOEvent(eventId, calendarId) {
      if (!eventId) return;
      try { await callCalendar({ action: 'team-event-delete', eventId }); } catch (e) {}
    }
    ...
    function TimeOffTab({ dark, profile, isAdmin, onBack, hideHeader }) {
      ...
      const syncOOOToCalendar = async (row) => {
        try {
          ...
          const msg = ce.code === 'team_calendar_not_configured'
            ? 'Team Calendar sync isn’t set up yet — no OOO event was added.'
            : 'Couldn’t add it to the Team Calendar (' + (ce.message || ce) + '). You can add it there manually.';
          alert(msg);
          return false;
        }
      };
      const submit = async () => {
        ...
        try {
          if (editingId) {
            ...
            const row = await TimeOffDB.create({ start_at: draftStart, end_at: draftEnd, full_day: draftFull, reason, forUserId });
