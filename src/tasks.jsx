    ...
    const TASKS_EMBED = (() => { try { return window.self !== window.top; } catch (e) { return false; } })();
    ...
    const ROUTE_KIND_BY_SEG = { ctc: 'ctc_file', project: 'project', rock: 'rock' };
    const ROUTE_SEG_BY_KIND = { ctc_file: 'ctc', project: 'project', rock: 'rock' };
    const ROUTE_SURFACE_BY_SEG = { ctc: 'ctc', project: 'projects', rock: 'rocks' };
    ...
    const surfaceLabel = { my: 'My Tasks', ctc: 'CTC Files', projects: 'Projects', rocks: 'Rocks', accountability: 'My Accountabilities' };
    ...
    // in this repo).
    const docTitle = (label) => { document.title = label ? label + ' - TMG App' : 'TMG App'; };
    ...
    const PENDING_ROUTE = { kind: null, id: null, tab: null };
    ...
    function useWide(bp) { const b = bp || 700; const [w, setW] = useState(typeof window !== 'undefined' && window.innerWidth >= b); useEffect(() => { const f = () => setW(window.innerWidth >= b); window.addEventListener('resize', f); return () => window.removeEventListener('resize', f); }, []); return w; }
    ...
    function splitGeneratedBlocks(text) {
      ...
      while ((m = TMG_FILE_BLOCK_RE.exec(t))) {
        if (m.index > last) segments.push({ type: 'md', text: t.slice(last, m.index) });
        segments.push({ type: m[1], filename: m[2] || null, body: m[3] });
        last = m.index + m[0].length;
      }
      if (last < t.length) segments.push({ type: 'md', text: t.slice(last) });
      return segments.length ? segments : [{ type: 'md', text: t }];
    }
    ...
    function findMdTable(text) {
      ...
      for (let i = 0; i < lines.length - 1; i++) {
        const head = lines[i].trim(), sep = lines[i + 1].trim();
      ...
      if (segments.length === 1 && segments[0].type === 'md') {
        ...
        return (
          <React.Fragment>
            ...
            <FallbackFileActions text={segments[0].text} />
          </React.Fragment>
        );
      }
      return (
          ...
          {segments.map((seg, i) => {
            if (seg.type === 'md') {
              ...
              return html == null
                ...
                : <div key={i} className="md-body" dangerouslySetInnerHTML={{ __html: html }} />;
            }
            if (seg.type === 'image') return <GeneratedImageCard key={i} prompt={seg.body} />;
            return <GeneratedFileCard key={i} kind={seg.type} filename={seg.filename} body={seg.body} />;
          })}
        </React.Fragment>
      );
    }
    ...
    const GEN_FILE_META = {
      doc:    { icon: 'ti-file-type-doc', label: 'Word document',              ext: '.docx' },
      sheet:  { icon: 'ti-file-type-xls', label: 'Excel spreadsheet',          ext: '.xlsx' },
      slides: { icon: 'ti-file-type-ppt', label: 'PowerPoint presentation',    ext: '.pptx' },
    };
    function GeneratedFileCard({ kind, filename, body }) {
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, margin: '6px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 13px', borderRadius: 12, border: `1px solid ${C.border}`, background: C.surfaceHover }}>
            <i className={`ti ${meta.icon}`} style={{ fontSize: 22, color: C.gold, flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '0.84rem', fontWeight: 600, color: C.textPrimary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: C.fontSans }}>{name}</div>
              <div style={{ fontSize: '0.7rem', color: C.textSecondary, fontFamily: C.fontSans }}>{meta.label}</div>
            </div>
            <button onClick={download} disabled={busy} style={{ padding: '7px 12px', borderRadius: 9, border: 'none', background: C.navy, color: '#fff', fontSize: '0.76rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans, opacity: busy ? 0.6 : 1, flexShrink: 0 }}>{busy ? '…' : 'Download'}</button>
          </div>
          {err ? <div style={{ fontSize: '0.7rem', color: C.red, fontFamily: C.fontSans, padding: '0 2px' }}>{err}</div> : null}
        </div>
      );
    }
    ...
    function FallbackFileActions({ text }) {
      ...
      const go = async (kind) => {
        ...
        try {
          ...
          a.href = url; a.download = kind === 'sheet' ? 'TMG-Table.xlsx' : 'TMG-Notes.docx';
          document.body.appendChild(a); a.click();
          setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 400);
        } catch (e) { setErr(e.message || String(e)); }
        finally { setBusy(null); }
      };
      const btnStyle = { padding: '5px 10px', borderRadius: 8, border: `1px solid ${C.border}`, background: 'transparent', color: C.textSecondary, fontSize: '0.68rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans };
      return (
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6, marginTop: 6 }}>
          <button onClick={() => go('doc')} disabled={!!busy} style={{ ...btnStyle, opacity: busy ? 0.6 : 1 }}>{busy === 'doc' ? '…' : 'Download as Word'}</button>
          {tableRows ? <button onClick={() => go('sheet')} disabled={!!busy} style={{ ...btnStyle, opacity: busy ? 0.6 : 1 }}>{busy === 'sheet' ? '…' : 'Download as Excel'}</button> : null}
          {err ? <span style={{ fontSize: '0.68rem', color: C.red, fontFamily: C.fontSans }}>{err}</span> : null}
        </div>
      );
    }
    ...
    function GeneratedImageCard({ prompt }) {
      ...
      const generate = async () => {
        if (busy) return;
        setBusy(true); setErr(null);
        try { setUrl(await callGenerateImage(prompt, 'medium')); }
        catch (e) { setErr(e.message || String(e)); }
        finally { setBusy(false); }
      };
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, margin: '6px 0' }}>
          {url ? (
            <a href={url} download="generated-image.png" style={{ display: 'block' }}>
              <img src={url} alt={prompt} style={{ maxWidth: 280, maxHeight: 280, borderRadius: 12, display: 'block', border: `1px solid ${C.border}` }} />
            </a>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 13px', borderRadius: 12, border: `1px solid ${C.border}`, background: C.surfaceHover }}>
              <i className="ti ti-photo-plus" style={{ fontSize: 22, color: C.gold, flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '0.84rem', fontWeight: 600, color: C.textPrimary, fontFamily: C.fontSans }}>AI-generated image</div>
                <div style={{ fontSize: '0.7rem', color: C.textSecondary, fontFamily: C.fontSans, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{prompt}</div>
              </div>
              <button onClick={generate} disabled={busy} style={{ padding: '7px 12px', borderRadius: 9, border: 'none', background: C.navy, color: '#fff', fontSize: '0.76rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans, opacity: busy ? 0.6 : 1, flexShrink: 0 }}>{busy ? 'Generating…' : 'Generate image'}</button>
            </div>
          )}
          {err ? <div style={{ fontSize: '0.7rem', color: C.red, fontFamily: C.fontSans, padding: '0 2px' }}>{err}</div> : null}
        </div>
    ...
    const SYSTEM_PROMPT =
      "You are the TMG assistant, an AI helper for The Morshed Group, a real estate team. " +
      "Be warm, concise, and conversational — talk like a helpful colleague, not a report. " +
      "Help with general questions, drafting, and day-to-day tasks.\n\n" +
      "You can hand back an actual downloadable Word document, Excel spreadsheet, or PowerPoint " +
      "presentation. Only do this when the user clearly wants a file (e.g. \"make me a doc/deck/sheet\", " +
      "\"write this up as a Word doc\", \"put this in a spreadsheet\") — not for normal chat answers. " +
      "When you do, write a short one-line intro, then EXACTLY ONE fenced code block using one of these " +
      "language tags (include filename=\"...\" with a sensible name and the right extension), then an " +
      "optional short one-line outro:\n" +
      "- ```tmg-doc filename=\"Name.docx\"``` — content is Markdown: \"# \" for a heading, \"## \" for a " +
      "subheading, \"- \" for a bullet, **bold** for bold, blank lines between paragraphs.\n" +
      "- ```tmg-sheet filename=\"Name.xlsx\"``` — content is CSV. For more than one sheet, put a line " +
      "\"### Sheet: <name>\" before each sheet's CSV rows.\n" +
      "- ```tmg-slides filename=\"Name.pptx\"``` — content is one slide per block, separated by a line " +
      "containing only \"---\". Each slide's first line is its title; remaining lines starting with " +
      "\"- \" are bullets.\n\n" +
      "You can also offer to generate an actual AI image (a real picture, e.g. \"generate an image of a " +
      "modern living room\") using: ```tmg-image``` where the content is a single clear, detailed image-" +
      "generation prompt (no filename attribute needed). Only offer this when the user wants a picture " +
      "generated, not for anything else — generating an image costs real money per use, so never use it " +
      "speculatively or more than once per request.\n\n" +
      "Never mix these with your normal answer in the same block, and never use any of these tags unless " +
      "the user actually wants a file or image.";
    ...
    async function callAI(messages, sysOverride, feature = 'tasks') {
      const res = await fetch(AI_ENDPOINT, {
        ...
        body: JSON.stringify({ messages, system: sysOverride || SYSTEM_PROMPT, max_tokens: 4096, feature }),
      });
    ...
    async function fetchGoogleEvents(timeMin, timeMax, calendarIds) {
      const payload = { action: 'list', timeMin, timeMax };
      if (Array.isArray(calendarIds) && calendarIds.length) payload.calendarIds = calendarIds;
      const { ok, data } = await callCalendar(payload);
      if (!ok) { const err = new Error(data.error || 'calendar_error'); err.code = data.error; err.detail = data.detail || ''; throw err; }
      return data.events || [];
    }
    ...
    async function fetchTeamEvents(timeMin, timeMax) {
      try {
        const { ok, data } = await callCalendar({ action: 'team-events-list', timeMin, timeMax });
        if (!ok) return [];
    ...
    async function fetchCalendarList() {
      const { ok, data } = await callCalendar({ action: 'calendars' });
      if (!ok) return [];
      return Array.isArray(data.calendars) ? data.calendars : [];
    }
    function connectGoogleCalendar(refreshToken) {
      callCalendar({ action: 'connect', refresh_token: refreshToken }).catch(() => {});
    }
    // Create (mode 'create') or update (mode 'edit') an event; returns the new id.
    async function saveGoogleEvent({ mode, calendarId, eventId, event }) {
      const { ok, data } = await callCalendar({ action: mode === 'edit' ? 'update' : 'create', calendarId, eventId, event });
      if (!ok) { const err = new Error(data.error || 'save_failed'); err.code = data.error; throw err; }
      return data.id || null;
    }
    async function deleteGoogleEvent(calendarId, eventId) {
      const { ok, data } = await callCalendar({ action: 'delete', calendarId, eventId });
      if (!ok) { const err = new Error(data.error || 'delete_failed'); err.code = data.error; throw err; }
      return true;
    }
    // Mock Google Calendar responses for localhost/preview only.
    function devCalendarMock(payload) {
      if (payload.action === 'connect') return { ok: true };
      if (payload.action === 'create') return { ok: true, id: 'dev-' + Math.random().toString(36).slice(2) };
      if (payload.action === 'update') return { ok: true, id: payload.eventId };
      if (payload.action === 'delete') return { ok: true };
      if (payload.action === 'calendars') return { calendars: [
        { id: 'primary', summary: 'Hidenori Symon', primary: true, color: '#D50000', accessRole: 'owner', canWrite: true },
        { id: 'c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com', summary: 'TMG', color: '#F6BF26', accessRole: 'writer', canWrite: true },
        { id: 'ph#holiday@group.v.calendar.google.com', summary: 'Holidays in Philippines', color: '#3F51B5', accessRole: 'reader', canWrite: false },
        { id: 'us#holiday@group.v.calendar.google.com', summary: 'Holidays in United States', color: '#0B8043', accessRole: 'reader', canWrite: false },
      ] };
      if (payload.action === 'list') {
        const ids = Array.isArray(payload.calendarIds) && payload.calendarIds.length ? payload.calendarIds : ['primary'];
        const colorOf = { 'primary': '#D50000', 'c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com': '#F6BF26', 'ph#holiday@group.v.calendar.google.com': '#3F51B5', 'us#holiday@group.v.calendar.google.com': '#0B8043' };
        const base = payload.timeMin ? new Date(payload.timeMin) : new Date();
        const day0 = new Date(base); day0.setHours(0, 0, 0, 0);
        const iso = (d, h, m) => { const x = new Date(d); x.setHours(h, m || 0, 0, 0); return x.toISOString(); };
        const evs = [];
        for (let i = 0; i < 14; i++) {
          const d = new Date(day0); d.setDate(d.getDate() + i);
          if (ids.includes('primary') && i % 2 === 0) evs.push({ id: 'p' + i, title: 'Buyer call', start: iso(d, 9), end: iso(d, 9, 30), allDay: false, calendarId: 'primary', color: colorOf['primary'], colorId: null });
          if (ids.includes('c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com') && i % 3 === 0) evs.push({ id: 't' + i, title: 'TMG standup', start: iso(d, 14), end: iso(d, 15), allDay: false, calendarId: 'c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com', color: colorOf['c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com'], colorId: '5' });
        }
        ...
        const isoDate = (d) => { const p2 = (n) => String(n).padStart(2, '0'); return d.getFullYear() + '-' + p2(d.getMonth() + 1) + '-' + p2(d.getDate()); };
        const addD = (d, n) => { const x = new Date(d); x.setDate(x.getDate() + n); return x; };
        if (ids.includes('c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com')) {
          evs.push({ id: 'ooo-multi', title: 'OOO - Kyle Baird', start: isoDate(addD(day0, -3)), end: isoDate(addD(day0, 11)), allDay: true, calendarId: 'c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com', color: null, colorId: null });
          ['Maria Cruz', 'John Reyes', 'Ana Lopez'].forEach((name, idx) => {
            evs.push({ id: 'ooo-' + idx, title: 'OOO - ' + name, start: isoDate(addD(day0, 2)), end: isoDate(addD(day0, 3)), allDay: true, calendarId: 'c_u17la9j1annqi72em9qs3e8v44@group.calendar.google.com', color: null, colorId: null });
          });
        }
        if (ids.includes('us#holiday@group.v.calendar.google.com')) evs.push({ id: 'hol1', title: 'Independence Day', start: isoDate(addD(day0, 1)), end: isoDate(addD(day0, 2)), allDay: true, calendarId: 'us#holiday@group.v.calendar.google.com', color: null, colorId: null });
        return { events: evs };
      }
      ...
      if (payload.action === 'gmail_threads') {
        return { threads: DEV_GMAIL_THREADS.map(t => ({ id: t.id, permalink: 'https://mail.google.com/mail/u/0/#all/' + t.id, from: t.from, subject: t.subject, snippet: t.snippet, date: t.date, messageId: t.msgs[t.msgs.length - 1].id, messageCount: t.msgs.length })) };
      }
      if (payload.action === 'gmail_thread') {
        const t = DEV_GMAIL_THREADS.find(x => x.id === payload.threadId);
        if (!t) return { error: 'Thread not found.' };
        return { thread: { id: t.id, subject: t.subject, permalink: 'https://mail.google.com/mail/u/0/#all/' + t.id, messages: t.msgs } };
      }
    ...
    const DEV_GMAIL_THREADS = [
      { id: 'devA', from: 'Escrow Officer <escrow@firsttitle.example>', subject: 'RE: 888 Test Avenue — title order & prelim', date: 'Aug 3, 2026', snippet: 'Attaching the preliminary title report. Please review Schedule B exceptions before we proceed to binding…',
        msgs: [
          { id: 'devA-1', from: 'Symon Yongco <symon@themorshedgroup.com>', to: 'Escrow Officer <escrow@firsttitle.example>', date: 'Aug 2, 2026', subject: 'RE: 888 Test Avenue — title order & prelim', bodyText: 'Please open the title order for 888 Test Avenue. Buyer is financing, so we’ll need the prelim before the binding date on the 12th.', snippet: '' },
          { id: 'devA-2', from: 'Escrow Officer <escrow@firsttitle.example>', to: 'Symon Yongco, Kyle Baird', date: 'Aug 3, 2026', subject: 'RE: 888 Test Avenue — title order & prelim', bodyText: 'Attaching the preliminary title report. Please review Schedule B exceptions before we proceed to binding. Two easements to confirm; the rest is standard.', snippet: '' },
        ] },
      { id: 'devB', from: 'Kyle Baird <kyle@themorshedgroup.com>', subject: 'Lender appraisal timing', date: 'Aug 4, 2026', snippet: 'Confirmed the appraiser is scheduled for the 6th. Flagging in case it affects the inspection window…',
        msgs: [
          { id: 'devB-1', from: 'Kyle Baird <kyle@themorshedgroup.com>', to: 'Symon Yongco', date: 'Aug 4, 2026', subject: 'Lender appraisal timing', bodyText: 'Confirmed the appraiser is scheduled for the 6th. Flagging in case it affects the inspection window.', snippet: '' },
        ] },
      { id: 'devC', from: 'Buyer’s Agent <agent@buyerbrokerage.example>', subject: 'Inspection window — 888 Test Avenue', date: 'Aug 1, 2026', snippet: 'Buyer would like to schedule the inspection early next week. Are Tue/Wed open?…',
        msgs: [
          { id: 'devC-1', from: 'Buyer’s Agent <agent@buyerbrokerage.example>', to: 'Symon Yongco', date: 'Aug 1, 2026', subject: 'Inspection window — 888 Test Avenue', bodyText: 'Buyer would like to schedule the inspection early next week. Are Tue or Wed open?', snippet: '' },
        ] },
    ];
    ...
    const ConvDB = {
      ...
      async loadConversations() {
        ...
        if (error) { console.error('[ConvDB] load:', error.message); return []; }
        return (data || []).map(r => ({
          id: r.id, type: r.type, workRequest: r.work_request,
          title: r.title || 'New chat', pinned: !!r.pinned, archived: !!r.archived,
          updatedAt: r.updated_at,
        }));
      },

      async createConversation({ type, workRequest, title }) {
        if (!this.client()) {
          return { id: 'local-' + Date.now(), type, workRequest: workRequest || null, title: title || 'New chat', pinned: false, archived: false };
        }
        const { data, error } = await this.client().from('ai_conversations').insert({
          user_id: this.userId(),
          type: type || 'ai',
          work_request: workRequest || null,
          title: title || null,
        }).select().single();
        if (error) { console.error('[ConvDB] create:', error.message); throw error; }
        return { id: data.id, type: data.type, workRequest: data.work_request, title: data.title || 'New chat', pinned: false, archived: false };
      },
      ...
      async updateConversation(id, fields) {
        ...
        const { error } = await this.client().from('ai_conversations').update(mapped).eq('id', id);
        if (error) console.error('[ConvDB] update:', error.message);
      },

      async touchConversation(id) {
        if (!this.client() || String(id).startsWith('local-')) return;
        await this.client().from('ai_conversations').update({ updated_at: new Date().toISOString() }).eq('id', id);
      },
      ...
      async loadMessages(convId) {
        ...
        const { data, error } = await this.client()
          .from('ai_messages').select('*').eq('conversation_id', convId)
          .order('created_at', { ascending: true });
        if (error) { console.error('[ConvDB] loadMessages:', error.message); return []; }
        const rows = data || [];
        const out = [];
        for (const r of rows) {
          const atts = Array.isArray(r.attachments) ? r.attachments : [];
          for (const a of atts) { if (a.path) a.url = await this.signedUrl(a.path); }
          out.push({ role: r.role, content: r.content || '', attachments: atts });
        }
        return out;
      },
      ...
      async insertMessage(convId, role, content, attachments) {
        ...
        const slim = (attachments || []).map(a => ({
          kind: a.kind, name: a.name, mediaType: a.mediaType, path: a.path, size: a.size,
          ...(a.kind === 'text' ? { textContent: a.textContent } : {}),
        }));
      ...
      async uploadAttachment(file, convId) {
        ...
        const { error } = await this.client().storage.from('chat-attachments').upload(path, file, {
          contentType: file.type || 'application/octet-stream', upsert: false,
        });
        if (error) { console.error('[ConvDB] upload:', error.message); throw error; }
        return path;
      },

      async signedUrl(path) {
        if (!this.client() || !path) return null;
        const { data, error } = await this.client().storage.from('chat-attachments').createSignedUrl(path, 7200);
        if (error) { console.error('[ConvDB] signedUrl:', error.message); return null; }
    ...
    const ProfileDB = {
      ...
      async setCalendarPrefs(prefs) {
        if (!this.client()) return;
        const { error } = await this.client().from('profiles').update({ calendar_prefs: prefs }).eq('id', this.uid());
    ...
    function toApiContent(msg) {
      ...
      const blocks = [];
      for (const a of atts) {
        if (a.kind === 'image' && a.url)      blocks.push({ type: 'image',    source: { type: 'url', url: a.url } });
        else if (a.kind === 'pdf' && a.url)   blocks.push({ type: 'document', source: { type: 'url', url: a.url } });
    ...
    async function genDocxBlob(body) {
      ...
      const { Document, Packer, Paragraph, HeadingLevel } = d;
      const children = (body || '').split('\n').map(raw => {
        const l = raw.replace(/\r$/, '');
        if (/^#\s+/.test(l)) return new Paragraph({ text: l.replace(/^#\s+/, ''), heading: HeadingLevel.HEADING_1 });
        if (/^##\s+/.test(l)) return new Paragraph({ text: l.replace(/^##\s+/, ''), heading: HeadingLevel.HEADING_2 });
        if (/^-\s+/.test(l)) return new Paragraph({ text: l.replace(/^-\s+/, ''), bullet: { level: 0 } });
        if (!l.trim()) return new Paragraph({ text: '' });
        return new Paragraph({ children: mdInlineRuns(l) });
      });
    ...
    function genXlsxBlob(body) {
      ...
      const re = /^### Sheet: (.+)$/gm;
      const marks = []; let m;
      while ((m = re.exec(body || ''))) marks.push({ name: m[1].trim(), markStart: m.index, bodyStart: m.index + m[0].length });
      if (!marks.length) {
        const sub = XLSX.read(body || '', { type: 'string' });
        ...
        marks.forEach((mk, i) => {
          ...
          const sub = XLSX.read(chunk || ' ', { type: 'string' });
    ...
    async function genPptxBlob(body) {
      ...
      slides.forEach(raw => {
        ...
        slide.addText(title, { x: 0.5, y: 0.3, w: 9, h: 1, fontSize: 28, bold: true, color: '001A4A' });
        if (bullets.length) slide.addText(bullets.map(b => ({ text: b, options: { bullet: true, breakLine: true } })), { x: 0.5, y: 1.3, w: 9, h: 4.5, fontSize: 16, color: '333333' });
      });
      return pptx.write({ outputType: 'blob' });
    }

    const WORK_REQUESTS = [
      { key: 'kpis',                 label: 'Enter KPIs',                  short: 'KPIs' },
      { key: 'listing_presentation', label: 'Request Listing Presentation', short: 'Listing' },
    ];
    ...
    const TASK_PILL_INSTRUCTION =
`You are the TMG task assistant. Turn what the user tells you into ONE task on the team's shared task board. Have a short, natural conversation — ask only for what's missing and important, never interrogate. The user's message may begin with "To Do:" — treat that as a prefix and use the rest as the task.
...
- Keep the conversational part brief and friendly.`;
    ...
    const kpiTodayStr = () => { const t = new Date(); return t.getFullYear() + '-' + String(t.getMonth() + 1).padStart(2, '0') + '-' + String(t.getDate()).padStart(2, '0'); };
    ...
    const ALLOWED_OTHER_KPIS = [
      ...
      'Networking Event for Brett', 'Generate Clients/Relationships', 'Open House',
      'Pre-Hotzone Action', 'Professional Development',
    ];
    async function createAgentKpi(payload) {
      ...
      const record = { KPI_Date: payload.kpi_date || kpiTodayStr() };
      ...
      for (let i = 0; i < persons.length; i++) {
        ...
        if (p.contact_id) record['Person_' + n + '_Name'] = { id: p.contact_id };
      }
      ...
      const { ok, data } = await callZoho({ action: 'create_agent_kpi', record, owner_email: payload.owner_email || null });
    ...
    const KPI_NOTE_INSTRUCTION =
`You are the TMG KPI assistant. The agent pastes a short note describing one day of real-estate prospecting activity. Turn it into ONE normalized KPI submission. Keep replies short and friendly — never interrogate.
...
- Resolve relative dates using today's date given below. Do NOT use markdown formatting.`;
    ...
    function formatKpiPreview(p) {
      ...
      persons.forEach(function (x, i) {
        const parts = (x.kpis || []).map(function (k) { return k === 'Hotzone Action/s' ? ('Hotzone Action/s (' + (x.hotzone_count || 0) + ')') : k; });
        L.push('  ' + (i + 1) + '. ' + (x.name || '—') + ' — ' + (parts.length ? parts.join(', ') : '—'));
      });
      if (p.others && p.others.length) {
        L.push('');
        L.push('Other KPIs:');
        p.others.forEach(function (o) { L.push('  • ' + o.kpi + ': ' + o.count); });
      }
      return L.join('\n');
    }
    ...
    const FALLBACK_PILLS = [
      { id: 'fb-task',    label: 'Add To Do',            icon: 'ti-checklist',       colorLight: '#001A4A', colorDark: 'rgba(255,255,255,0.6)', type: 'task',   config: { instruction: TASK_PILL_INSTRUCTION, seed: 'To Do: ' }, enabled: true },
      { id: 'fb-kpis',    label: 'Enter KPI',            icon: 'ti-chart-bar',       colorLight: '#AD832F', colorDark: '#C9A45A',               type: 'prompt', config: { kpi: true, prompt: 'Enter KPI' }, enabled: true },
      { id: 'fb-listing', label: 'Listing Presentation', icon: 'ti-presentation',    colorLight: '#AD832F', colorDark: '#C9A45A',               type: 'prompt', config: { prompt: 'Listing Presentation' }, enabled: true },
      { id: 'fb-showing', label: 'Add Showing',          icon: 'ti-home',            colorLight: '#001A4A', colorDark: 'rgba(255,255,255,0.6)', type: 'prompt', config: { prompt: 'Add Showing' },          enabled: true },
      { id: 'fb-comm',    label: 'Commission Log',       icon: 'ti-currency-dollar', colorLight: '#0F6E56', colorDark: '#5DCAA5',               type: 'prompt', config: { prompt: 'Commission Log' },       enabled: true },
      { id: 'fb-score',   label: 'Team Scorecard',       icon: 'ti-users',           colorLight: '#AD832F', colorDark: '#C9A45A',               type: 'prompt', config: { prompt: 'Team Scorecard' },        enabled: true },
      { id: 'fb-eod',     label: 'EOD Report',           icon: 'ti-file-text',       colorLight: '#AD832F', colorDark: '#C9A45A',               type: 'prompt', config: { prompt: 'EOD Report' },            enabled: true },
      { id: 'fb-weekly',  label: 'Weekly Summary',       icon: 'ti-calendar-stats',  colorLight: '#001A4A', colorDark: 'rgba(255,255,255,0.6)', type: 'prompt', config: { prompt: 'Weekly Summary' },        enabled: true },
    ];
    ...
    const PILL_COLORS = [
      { l: '#AD832F', d: '#C9A45A' },                 // gold
      { l: '#001A4A', d: 'rgba(255,255,255,0.6)' },   // navy
      { l: '#0F6E56', d: '#5DCAA5' },                 // green
      { l: '#9A3B3B', d: '#E58A8A' },                 // red
      { l: '#5A3FA0', d: '#B79CEB' },                 // purple
    ];

    // Make a safe template key from a label/question.
    function slugKey(s) { return (s || '').toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 24); }
    ...
    const PillDB = {
      client() { return window.SupabaseAuth?._client || null; },
      uid() { return window.SupabaseAuth?._state?.session?.user?.id || null; },
      _map(r) { return { id: r.id, label: r.label, icon: r.icon, colorLight: r.color_light, colorDark: r.color_dark, type: r.type, config: r.config || {}, enabled: r.enabled, sortOrder: r.sort_order, roles: r.roles || [] }; },
      async list() {
        if (!this.client()) return null;
        const { data, error } = await this.client().from('work_pills').select('*').order('sort_order', { ascending: true }).order('created_at', { ascending: true });
        if (error) { console.error('[PillDB] list:', error.message); return null; }
        return (data || []).map(this._map);
      },
      async create(p) {
        if (!this.client()) return;
        const { error } = await this.client().from('work_pills').insert({
          label: p.label, icon: p.icon, color_light: p.colorLight, color_dark: p.colorDark,
          type: p.type, config: p.config || {}, enabled: p.enabled !== false, sort_order: p.sortOrder || 0, roles: p.roles || [], created_by: this.uid(),
        });
        if (error) { console.error('[PillDB] create:', error.message); throw error; }
      },
      async update(id, f) {
        if (!this.client()) return;
        const m = {};
        ...
        const { error } = await this.client().from('work_pills').update(m).eq('id', id);
        if (error) { console.error('[PillDB] update:', error.message); throw error; }
      },
      async remove(id) {
        if (!this.client()) return;
        const { error } = await this.client().from('work_pills').delete().eq('id', id);
        if (error) { console.error('[PillDB] remove:', error.message); throw error; }
      },
      async reorder(ids) {
        if (!this.client()) return;
        for (let i = 0; i < ids.length; i++) {
          const { error } = await this.client().from('work_pills').update({ sort_order: i }).eq('id', ids[i]);
          if (error) { console.error('[PillDB] reorder:', error.message); throw error; }
        }
      },
    };
    ...
    const CAL_TZ_OPTIONS = [
      { label: 'PH — Manila', tz: 'Asia/Manila' },
      { label: 'TX — Texas · Central', tz: 'America/Chicago' },
      { label: 'CA — Pacific', tz: 'America/Los_Angeles' },
      { label: 'CO — Mountain', tz: 'America/Denver' },
      { label: 'NY — Eastern', tz: 'America/New_York' },
      { label: 'UK — London', tz: 'Europe/London' },
      { label: 'UAE — Dubai', tz: 'Asia/Dubai' },
      { label: 'IN — Kolkata', tz: 'Asia/Kolkata' },
      { label: 'SG — Singapore', tz: 'Asia/Singapore' },
      { label: 'JP — Tokyo', tz: 'Asia/Tokyo' },
      { label: 'AU — Sydney', tz: 'Australia/Sydney' },
    ];
    const tzLabel = (tz) => { const o = CAL_TZ_OPTIONS.find(x => x.tz === tz); return o ? o.label : tzCity(tz); };
    ...
    const AccessDB = {
      ...
      async load() {
        if (!this.client()) return null;
        const { data, error } = await this.client().from('app_access').select('config').eq('id', 1).maybeSingle();
        if (error) { console.error('[AccessDB] load:', error.message); return null; }
        return (data && data.config) || null;
      },
      async save(config) {
        if (!this.client()) return;
        const { error } = await this.client().from('app_access').upsert({ id: 1, config, updated_at: new Date().toISOString() });
        if (error) { console.error('[AccessDB] save:', error.message); throw error; }
      },
    };
    ...
    const TASK_PRIORITY = [
      ...
      { id: 'high',   label: 'High',   color: '#DC2626' },  // red
    ];
    const TASK_RECUR = [
    ...
    const RECUR_COPY_FIELDS = [
      ...
      { id: 'people',      label: 'Assignees & assigners' },
      { id: 'decision',    label: 'Decision (question + options)' },
      { id: 'links',       label: 'Working URL & email' },
    ];
    ...
    const recurDesc     = (t) => {
      ...
      if (t.recurrence === 'custom') { const n = t.recur_interval || 1; const u = t.recur_unit || 'week'; return 'Every ' + n + ' ' + u + (n > 1 ? 's' : ''); }
      return recurLabel(t.recurrence);
    };

    const PROJECT_STATUS = [
      { id: 'on_track', label: 'On track', color: '#0F6E56', bg: '#E6F2EC', bgD: 'rgba(15,110,86,.14)' },
      { id: 'at_risk',  label: 'At risk',  color: '#AD832F', bg: '#F3EBDA', bgD: 'rgba(173,131,47,.14)' },
      { id: 'blocked',  label: 'Blocked',  color: '#9B1C1C', bg: '#FEE2E2', bgD: 'rgba(155,28,28,.16)' },
    ];
    ...
    const KIND_LABEL = {
      project:  { plural: 'Projects',  singular: 'Project',  article: 'project', groupLabel: 'Group',   groupPlaceholder: 'e.g. Q3 Rocks',                namePlaceholder: 'e.g. AI Feature Rollout' },
      ctc_file: { plural: 'CTC Files', singular: 'CTC File', article: 'file',    groupLabel: 'Deal Type', groupPlaceholder: 'e.g. Residential Seller', namePlaceholder: 'e.g. 5999 Ranch Rd 165' },
      rock:     { plural: 'Rocks',     singular: 'Rock',     article: 'rock',    groupLabel: 'Quarter', groupPlaceholder: 'e.g. Q3 2026',                 namePlaceholder: 'e.g. Close $12M in new brokerage volume' },
    };
    ...
    const TaskDB = {
      ...
      async loadAll(userId) {
        ...
        const peopleByTask = {};
        ids.forEach(id => { peopleByTask[id] = []; });
      ...
      async getById(id) {
        const c = this.client(); if (!c || !id) return null;
        const { data } = await c.from('tasks').select('*').eq('id', id).single();
        return data || null;
      },
      ...
      async loadPeopleFor(id) {
        ...
        const { data, error } = await c.from('task_people').select('task_id,user_id,role').eq('task_id', id);
        if (error) return null;
        return data || [];
      },

      async findOrCreateProject(name, user) {
        const c = this.client(); if (!c || !name || !name.trim()) return null;
        const nm = name.trim();
        const { data: ex } = await c.from('projects').select('id').ilike('name', nm).eq('archived', false).limit(1).maybeSingle();
        if (ex) return ex.id;
        const { data, error } = await c.from('projects').insert({ name: nm, created_by: user?.id || null }).select('id').single();
        if (error) { console.error('[TaskDB] project:', error.message); return null; }
        return data.id;
      },
      ...
      async create(fields, people, user) {
        const c = this.client(); if (!c) return null;
        const base = {
          title: fields.title, due_at: fields.due_at || null, project_id: fields.project_id || null,
          priority: fields.priority || 'medium', status: fields.status || 'todo',
          description: fields.description || null, context: fields.context || null,
          working_url: fields.working_url || null, email_link: fields.email_link || null,
          decision_question: fields.decision_question || null, recurrence: fields.recurrence || 'none',
        ...
        const wd = { weekly_priority: fields.weekly_priority || null, weekly_rank: fields.weekly_rank || null, daily_priority: fields.daily_priority || null, daily_rank: fields.daily_rank || null, decision_due_at: fields.decision_due_at || null, decision_due_has_time: fields.decision_due_has_time || null, recur_interval: fields.recur_interval || null, recur_unit: fields.recur_unit || null, recur_copy_fields: fields.recur_copy_fields || null, is_milestone: !!fields.is_milestone };
      ...
      async syncToZohoProjects(task, oldTask, user) {
        ...
        try {
          const { data: proj } = await c.from('projects')
            .select('record_type,zoho_sync_enabled,zoho_project_id,zoho_tasklist_id')
          ...
          let assigneeEmails = [], assigneeProfiles = [];
          const { data: assignees } = await c.from('task_people').select('user_id').eq('task_id', task.id).eq('role', 'assignee');
          if (assignees && assignees.length) {
            const { data: profs } = await c.from('profiles').select('email,first_name').in('id', assignees.map(a => a.user_id));
          ...
          const payload = { action: 'update_task', project_id: proj.zoho_project_id, task_id: task.zoho_task_id };
          let any = false;
          if (changed('title')) { payload.title = task.title; any = true; }
      ...
      async update(id, oldTask, fields, user, skipZohoSync) {
        ...
        if (error && /column|schema cache|PGRST204|42703/i.test((error.message || '') + (error.code || ''))) {
          ...
          ({ error } = await c.from('tasks').update(p2).eq('id', id));
        }
        if (error) { console.error('[TaskDB] update:', error.message); throw error; }
        const FL = { title: 'Title', due_at: 'Due date', project_id: 'Project', priority: 'Priority', status: 'Status', description: 'Description', context: 'Context', working_url: 'Working URL', email_link: 'Email link', decision_question: 'Decision question', decision_due_at: 'Decision due date', recurrence: 'Recurrence' };
        for (const k of Object.keys(fields)) {
          ...
          let msg;
          if (k === 'status') msg = 'Status: ' + statusLabel(oldTask.status) + ' → ' + statusLabel(fields.status);
        ...
        if (!skipZohoSync) this.syncToZohoProjects({ ...oldTask, ...fields, id }, oldTask, user);
      },
      ...
      async delete(id) {
        const c = this.client(); if (!c) return { error: 'Not connected.' };
        const { data: subtasks } = await c.from('tasks').select('id').eq('parent_task_id', id).limit(1);
        if (subtasks && subtasks.length) return { error: 'This task has subtasks — delete those first.' };
        await Promise.all([
          c.from('task_people').delete().eq('task_id', id),
          c.from('task_labels').delete().eq('task_id', id),
          c.from('task_activity').delete().eq('task_id', id),
          c.from('task_decision_options').delete().eq('task_id', id),
          c.from('task_assist').delete().eq('task_id', id),
          c.from('task_email_links').delete().eq('task_id', id),
        ]);
        await c.from('task_links').delete().or('predecessor_id.eq.' + id + ',successor_id.eq.' + id);
        const { error } = await c.from('tasks').delete().eq('id', id);
        if (error) { console.error('[TaskDB] delete:', error.message); return { error: error.message }; }
        return { error: null };
      },
      ...
      async spawnRecurrence(task, user) {
        const c = this.client(); if (!c) return;
        let nextDue = null;
        if (task.due_at) { const d = new Date(task.due_at); if (!isNaN(d)) {
          ...
          else if (task.recurrence === 'custom') { const n = task.recur_interval || 1; const u = task.recur_unit || 'week'; if (u === 'day') d.setDate(d.getDate() + n); else if (u === 'week') d.setDate(d.getDate() + n * 7); else d.setMonth(d.getMonth() + n); }
        ...
        const base = {
          title: task.title, due_at: nextDue, status: 'todo', recurrence: task.recurrence,
          recur_interval: task.recur_interval || null, recur_unit: task.recur_unit || null, recur_copy_fields: task.recur_copy_fields || null,
      ...
      async loadLabels(taskId) {
        const c = this.client(); if (!c) return [];
        const { data } = await c.from('task_labels').select('id,user_id,label,color').eq('task_id', taskId);
        return data || [];
      },
      async setLabels(taskId, userId, labels) {  // labels = [string] — replaces THIS user's labels only
        const c = this.client(); if (!c) return;
        await c.from('task_labels').delete().eq('task_id', taskId).eq('user_id', userId);
        const rows = (labels || []).map(l => (l || '').trim()).filter(Boolean).map(l => ({ task_id: taskId, user_id: userId, label: l }));
        if (rows.length) { const { error } = await c.from('task_labels').insert(rows); if (error) console.error('[TaskDB] setLabels:', error.message); }
      },

      // Decision options (Option A / B / C … with one optionally marked chosen).
      async loadDecisionOptions(taskId) {
        const c = this.client(); if (!c) return [];
        const { data } = await c.from('task_decision_options').select('*').eq('task_id', taskId).order('sort', { ascending: true });
        return data || [];
      },
      async setDecisionOptions(taskId, options) {  // options = [{ body, is_chosen }]
        const c = this.client(); if (!c) return;
        await c.from('task_decision_options').delete().eq('task_id', taskId);
        const rows = (options || []).map((o, i) => ({ task_id: taskId, body: (o.body || '').trim(), is_chosen: !!o.is_chosen, sort: i })).filter(r => r.body);
        if (rows.length) { const { error } = await c.from('task_decision_options').insert(rows); if (error) console.error('[TaskDB] setDecisionOptions:', error.message); }
      },
      ...
      async loadLinks(taskId) {
        const c = this.client(); if (!c) return { predecessors: [], successors: [] };
        const { data, error } = await c.from('task_links').select('id,predecessor_id,successor_id').or('predecessor_id.eq.' + taskId + ',successor_id.eq.' + taskId);
        if (error) { console.error('[TaskDB] loadLinks:', error.message); return { predecessors: [], successors: [] }; }
        const rows = data || [];
        ...
        if (otherIds.length) {
          ...
          tRows.forEach(t => { titles[t.id] = t.title; });
        }
        return {
          predecessors: rows.filter(r => r.successor_id === taskId),   // tasks that must come before this one
      ...
      async searchTasks(q, excludeId) {
        ...
        const { data, error } = await sel.order('created_at', { ascending: false }).limit(25);
        if (error) { console.error('[TaskDB] searchTasks:', error.message); return []; }
        return data || [];
      },
      async addLink(predecessorId, successorId) {
        const c = this.client(); if (!c) return;
        const { error } = await c.from('task_links').insert({ predecessor_id: predecessorId, successor_id: successorId });
      ...
      async myLabelNames(userId) {
        const c = this.client(); if (!c) return [];
        const { data } = await c.from('task_labels').select('label').eq('user_id', userId);
        return Array.from(new Set((data || []).map(r => r.label))).sort((a, b) => a.localeCompare(b));
      },
      async renameLabel(userId, oldLabel, newLabel) {
        const c = this.client(); if (!c || !newLabel.trim()) return;
        const { error } = await c.from('task_labels').update({ label: newLabel.trim() }).eq('user_id', userId).eq('label', oldLabel);
        if (error) console.error('[TaskDB] renameLabel:', error.message);
      },
      async deleteLabel(userId, label) {
        const c = this.client(); if (!c) return;
        const { error } = await c.from('task_labels').delete().eq('user_id', userId).eq('label', label);
      ...
      async loadProjects() {
        const c = this.client(); if (!c) return [];
        const { data } = await c.from('projects').select('id,name').eq('archived', false).order('name', { ascending: true });
        return data || [];
      },

      async loadActivity(taskId) {
        const c = this.client(); if (!c) return [];
        const { data, error } = await c.from('task_activity').select('*').eq('task_id', taskId).order('created_at', { ascending: true });
        if (error) { console.error('[TaskDB] activity:', error.message); return []; }
        return data || [];
      },

      async addActivity(taskId, kind, content, user, field) {
        const c = this.client(); if (!c) return null;
        const { data, error } = await c.from('task_activity').insert({
          task_id: taskId, kind, content, field: field || null,
          author_id: user?.id || null, author_name: user?.user_metadata?.full_name || user?.email || null,
        }).select().single();
        if (error) { console.error('[TaskDB] addActivity:', error.message); return null; }
        return data;
      },
      ...
      async loadAssist(taskId, user) {
        ...
        const { data, error } = await c.from('task_assist').select('*').eq('task_id', taskId).eq('user_id', user.id).order('created_at', { ascending: true });
        if (error) { console.error('[TaskDB] loadAssist:', error.message); return []; }
        return data || [];
      },
      async addAssist(taskId, role, content, user) {
        const c = this.client(); if (!c || !user) return null;
        const { data, error } = await c.from('task_assist').insert({ task_id: taskId, user_id: user.id, role, content }).select().single();
        if (error) { console.error('[TaskDB] addAssist:', error.message); return null; }
        return data;
      },
      async updateAssistContent(id, content) {
        const c = this.client(); if (!c) return;
        await c.from('task_assist').update({ content }).eq('id', id);
      },
      async publishAssist(rowItem, taskId, user) {
        const c = this.client(); if (!c) return;
        const act = await this.addActivity(taskId, 'ai', rowItem.content, user);
        await c.from('task_assist').update({ published: true, published_activity_id: act?.id || null }).eq('id', rowItem.id);
        return act;
      },
      async unpublishAssist(rowItem) {
        ...
        await c.from('task_assist').update({ published: false, published_activity_id: null }).eq('id', rowItem.id);
      },
      // Write the AI thread summary into context WITHOUT logging an activity (avoids feed noise).
      async setThreadSummary(taskId, text) {
        const c = this.client(); if (!c) return;
        await c.from('tasks').update({ context: text, updated_at: new Date().toISOString() }).eq('id', taskId);
      },
      async updateActivityContent(id, content) {
        const c = this.client(); if (!c) return;
        await c.from('task_activity').update({ content }).eq('id', id);
      },
    };
    ...
    const TaskEmailDB = {
      ...
      async loadAllByTask() {
        const c = this.client(); if (!c) return {};
        const { data } = await c.from('task_email_links').select('*').order('created_at', { ascending: true });
        const byTask = {};
        (data || []).forEach(l => { (byTask[l.task_id] = byTask[l.task_id] || []).push(l); });
        return byTask;
      },
      async listForTask(taskId) {
        const c = this.client(); if (!c || !taskId) return [];
        const { data } = await c.from('task_email_links').select('*').eq('task_id', taskId).order('created_at', { ascending: true });
        return data || [];
      },
      ...
      async attach(taskId, thread, user, autoLinked) {
        const c = this.client(); if (!c || !taskId || !thread?.id) return null;
        const { data, error } = await c.from('task_email_links').insert({
          task_id: taskId, thread_id: thread.id, message_id: thread.messageId || null,
          permalink: thread.permalink || ('https://mail.google.com/mail/u/0/#all/' + thread.id),
          from_addr: thread.from || null, subject: thread.subject || null, snippet: thread.snippet || null,
          email_date: thread.date ? new Date(thread.date).toISOString() : null,
          auto_linked: !!autoLinked, created_by: user?.id || null,
        }).select().single();
        if (error) {
      ...
      async remove(linkId) {
        const c = this.client(); if (!c || !linkId) return;
    ...
    const ProjectDB = {
      ...
      async loadFull(recordType) {
        ...
        const tasksByProj = {};
        (tRes.data || []).forEach(t => { if (t.project_id) (tasksByProj[t.project_id] = tasksByProj[t.project_id] || []).push({ ...t, _people: peopleByTask[t.id] || [] }); });
      ...
      async create(fields, user) {
        const c = this.client(); if (!c) return null;
        const base = { name: fields.name, record_type: fields.record_type || 'project', status: fields.status || 'on_track', outcome: fields.outcome || null, group_tag: fields.group_tag || null, owner_id: fields.owner_id || null, started_at: fields.started_at || null, target_date: fields.target_date || null, agent_id: fields.agent_id || null, zoho_deal_id: fields.zoho_deal_id || null, zoho_deal_name: fields.zoho_deal_name || null, created_by: user?.id || null };
        ...
        if (res.error && /column|schema cache|PGRST204|42703/i.test((res.error.message || '') + (res.error.code || ''))) {
          res = await c.from('projects').insert({ name: base.name, created_by: base.created_by }).select().single();
        }
      ...
      async update(id, patch) {
        const c = this.client(); if (!c) return;
        let { error } = await c.from('projects').update(patch).eq('id', id);
        if (error && /column|schema cache|PGRST204|42703/i.test((error.message || '') + (error.code || ''))) {
          const p2 = { ...patch }; ['record_type', 'status', 'outcome', 'group_tag', 'owner_id', 'started_at', 'target_date', 'agent_id', 'zoho_deal_id', 'zoho_deal_name'].forEach(k => delete p2[k]);
          if (Object.keys(p2).length) ({ error } = await c.from('projects').update(p2).eq('id', id));
        }
        if (error) console.error('[ProjectDB] update:', error.message);
        else this.syncToZoho(id, patch);
      },
      ...
      async syncToZoho(id, patch) {
        ...
        try {
          const { data: proj } = await c.from('projects')
            .select('record_type,zoho_sync_enabled,zoho_project_id')
            .eq('id', id).single();
          if (!proj || !ZOHO_SYNCABLE_KINDS.includes(proj.record_type) || !proj.zoho_sync_enabled || !proj.zoho_project_id) return;
          const payload = { action: 'update_project', project_id: proj.zoho_project_id };
      ...
      async archive(id) {
        const c = this.client(); if (!c) return;
        await c.from('projects').update({ archived: true }).eq('id', id);
      },
      ...
      async sidebarList() {
        ...
        const [pRes, sRes] = await Promise.all([
          c.from('projects').select('id,name,record_type').eq('archived', false).order('name', { ascending: true }),
          c.rpc('project_task_stats'),
        ]);
        const counts = {};
        (sRes.data || []).forEach(r => { if (r.project_id) counts[r.project_id] = r.total || 0; });
        return (pRes.data || []).map(p => ({ id: p.id, name: p.name, record_type: p.record_type || 'project', taskCount: counts[p.id] || 0 }));
      },
    };
    ...
    const CARD = { background: C.surface, boxShadow: '0 2px 8px rgba(0,26,74,0.07), 0 1px 2px rgba(0,26,74,0.04)', borderRadius: 14, padding: 16, marginBottom: 14 };
    ...
    function gmailComposeUrl(u) {
      ...
      const body = [
        (first ? 'Hi ' + first + ',' : 'Hi,'), '',
        'Good news — your TMG App account is now active.', '',
        'Open the app and sign in here:',
        APP_URL, '',
        'Tap "Continue with TMG Google" and use your @themorshedgroup.com account.', '',
        '— The Morshed Group'
      ].join('\n');
      return 'https://mail.google.com/mail/?view=cm&fs=1&to=' + encodeURIComponent(u.email || '') +
    ...
    function SettingsTab({ historySide, setHistorySide, dark, setDark, fontScale, setFontScale, embedded }) {
      const sideBtn = (val, label, icon) => {
        const on = historySide === val;
        return (
          <button onClick={() => setHistorySide(val)} style={{
            flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            padding: '10px', borderRadius: 10, cursor: 'pointer',
            border: `1px solid ${on ? C.navy : C.border}`,
            background: on ? C.navy : C.surface, color: on ? '#fff' : C.textSecondary,
            fontFamily: C.fontSans, fontSize: '0.85rem', fontWeight: on ? 600 : 500,
          }}>
      ...
      return (
        <div style={embedded ? { padding: '0 16px 16px' } : { padding: '20px 16px 100px', height: '100%', overflowY: 'auto' }}>
          {!embedded && <div style={{ fontSize: '1.5rem', fontWeight: 400, fontStyle: 'italic', color: C.navy, marginBottom: 24, fontFamily: C.fontDisplay }}>Settings</div>}

          {/* My Profile moved into ProfilePanel (cohesive profile-card redesign) */}

          {/* Appearance — dark mode + text size (standalone card, separate from Chat History) */}
          <div style={CARD}>
            <div style={{ fontSize: '0.72rem', fontWeight: 600, color: C.navy, letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: C.fontSans, marginBottom: 14 }}>Appearance</div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 18 }}>
              <span style={{ fontSize: '0.85rem', color: C.textPrimary, fontFamily: C.fontSans }}>Dark mode</span>
              <button onClick={() => setDark(d => !d)} title={dark ? 'Switch to light mode' : 'Switch to dark mode'} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.border}`, background: C.surface, color: C.textSecondary, cursor: 'pointer', fontFamily: C.fontSans, fontSize: '0.8rem', fontWeight: 500, flexShrink: 0 }}>
                <i className={`ti ${dark ? 'ti-sun' : 'ti-moon'}`} style={{ fontSize: 15 }} />{dark ? 'Light' : 'Dark'}
              </button>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 9 }}>
              <span style={{ fontSize: '0.85rem', color: C.textPrimary, fontFamily: C.fontSans }}>Text size</span>
              <span style={{ fontSize: '0.72rem', color: C.textMuted, fontFamily: C.fontSans, fontWeight: 600 }}>{Math.round((fontScale || 1) * 100)}%</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
              <span style={{ fontSize: 13, color: C.textMuted, fontFamily: C.fontSans, fontWeight: 700, flexShrink: 0 }}>A</span>
              <input type="range" min="0.85" max="1.35" step="0.05" value={fontScale || 1} onChange={e => setFontScale(parseFloat(e.target.value))} style={{ flex: 1, accentColor: C.navy, cursor: 'pointer' }} />
              <span style={{ fontSize: 22, color: C.textMuted, fontFamily: C.fontSans, fontWeight: 700, flexShrink: 0 }}>A</span>
            </div>
            <div style={{ fontSize: '0.72rem', color: C.textMuted, marginTop: 9, lineHeight: 1.5, fontFamily: C.fontSans }}>Adjusts the size of everything in the app.{(fontScale || 1) !== 1 && <React.Fragment> · <button onClick={() => setFontScale(1)} style={{ background: 'none', border: 'none', padding: 0, color: C.gold, cursor: 'pointer', fontFamily: C.fontSans, fontSize: '0.72rem', fontWeight: 600 }}>Reset to 100%</button></React.Fragment>}</div>
          </div>

          {/* Chat history */}
          <div style={CARD}>
            <div style={{ fontSize: '0.72rem', fontWeight: 600, color: C.navy, letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: C.fontSans, marginBottom: 6 }}>Chat History</div>
            <div style={{ fontSize: '0.78rem', color: C.textSecondary, marginBottom: 12, lineHeight: 1.5 }}>Which side the chat history panel opens from.</div>
            <div style={{ display: 'flex', gap: 8 }}>
              {sideBtn('left', 'Left', 'ti-layout-sidebar-left-expand')}
              {sideBtn('right', 'Right', 'ti-layout-sidebar-right-expand')}
            </div>
          </div>

          <button onClick={() => window.SupabaseAuth.signOut()} style={{
            width: '100%', padding: 12, background: 'none', border: `1px solid ${C.red}`,
            borderRadius: 12, color: C.red, fontSize: '15px', cursor: 'pointer', fontWeight: 500, fontFamily: C.fontSans,
          }}>Sign out</button>
        </div>
      );
    }
    ...
    const RM_STATUS = [
      { id: 'todo',       label: 'To Do',       bg: '#F1EFE8', text: '#5F5E5A', dot: '#888780' },
      { id: 'inprogress', label: 'In Progress', bg: '#E6F1FB', text: '#185FA5', dot: '#185FA5' },
      { id: 'completed',  label: 'Completed',   bg: '#E6F2EC', text: '#0F6E56', dot: '#0F6E56' },
      { id: 'stuck',      label: 'Stuck',       bg: '#FCEBEB', text: '#A32D2D', dot: '#A32D2D' },
    ];
    const RM_TYPES = [
      { id: 'mvp',    label: 'MVP',    bg: '#001A4A', text: '#FFFFFF' },
      { id: 'v2',     label: 'V2',     bg: '#F3EBDA', text: '#AD832F' },
      { id: 'future', label: 'Future', bg: '#F0EBE3', text: '#888888' },
    ];
    const rmStatus = (id) => RM_STATUS.find(s => s.id === id) || RM_STATUS[0];
    const rmType   = (id) => RM_TYPES.find(t => t.id === id) || RM_TYPES[0];
    const RM_DIFF = [
      { id: 'easy',   label: 'Easy',   bg: '#E6F2EC', text: '#0F6E56' },
      { id: 'normal', label: 'Normal', bg: '#FAEEDA', text: '#BA7517' },
      { id: 'hard',   label: 'Hard',   bg: '#FCEBEB', text: '#A32D2D' },
    ];
    ...
    function rmBucketKey(dateStr, groupBy) {
      const p = dateStr.split('-'); const y = +p[0], mo = +p[1]; // mo 1..12
    ...
    function rmGroupInfo(key, groupBy) {
      if (groupBy === 'month') {
        const p = key.split('-'); const y = +p[0], mo = +p[1];
        return { label: RM_MON[mo - 1] + ' ' + y, hint: '', dropDate: y + '-' + rmPad(mo) + '-01' };
      }
      const p = key.split('-Q'); const y = +p[0], q = +p[1];
      const sm = (q - 1) * 3; // 0-based start month
      return { label: 'Q' + q + ' ' + y, hint: RM_MON[sm] + ' – ' + RM_MON[sm + 2], dropDate: y + '-' + rmPad(sm + 1) + '-01' };
    }
    function rmNormalize(rows) {
      const parents = rows.filter(r => !r.parentId);
      const out = [];
      parents.forEach((p, pi) => {
        out.push({ ...p, parentId: null, order: pi });
        rows.filter(r => r.parentId === p.id).forEach((c, ci) => out.push({ ...c, order: ci }));
      });
      // keep any orphaned children as top-level so nothing is silently dropped
      rows.filter(r => r.parentId && !parents.some(p => p.id === r.parentId)).forEach((c, i) => out.push({ ...c, parentId: null, order: parents.length + i }));
      return out;
    }
    function rmBlocks(rows) {
      const blocks = []; let cur = null;
      rows.forEach(r => {
        if (!r.parentId) { cur = { pid: r.id, items: [r] }; blocks.push(cur); }
        else if (cur && r.parentId === cur.pid) { cur.items.push(r); }
        else { blocks.push({ pid: r.id, items: [r] }); cur = null; }
      });
      return blocks;
    }
    function rmGroups(rows, groupBy) {
      const parents = rows.filter(r => !r.parentId);
      const fam = (p) => ({ parent: p, children: rows.filter(r => r.parentId === p.id) });
      const families = parents.map(fam);
      if (groupBy === 'none') {
        const sorted = families.slice().sort((a, b) => {
          const da = a.parent.dueDate, db = b.parent.dueDate;
          if (!da && !db) return 0; if (!da) return 1; if (!db) return -1;
          return da < db ? -1 : da > db ? 1 : 0;
        });
        return [{ key: '__all', flat: true, families: sorted }];
      }
      const map = new Map();
      families.forEach(f => {
      ...
      const groups = dated.map(k => ({ key: k, info: rmGroupInfo(k, groupBy), families: map.get(k) }));
      if (map.has('__unscheduled')) groups.push({ key: '__unscheduled', unscheduled: true, families: map.get('__unscheduled') });
      return groups;
    }
    function rmGroupProgress(group) {
      let total = 0, done = 0;
      group.families.forEach(f => {
        total++; if (f.parent.status === 'completed') done++;
        f.children.forEach(c => { total++; if (c.status === 'completed') done++; });
      });
    ...
    function rmMoveFamily(rows, dragId, targetId, pos, newDue) {
      ...
      if (newDue !== undefined) dblock.items[0] = { ...dblock.items[0], dueDate: newDue };
      const ti = blocks.findIndex(b => b.pid === targetId);
      if (ti < 0) blocks.push(dblock);
      else blocks.splice(pos === 'after' ? ti + 1 : ti, 0, dblock);
    ...
    function rmMoveToGroupEnd(rows, dragId, newDue) {
      ...
      if (di < 0) return rows;
      const dblock = blocks.splice(di, 1)[0];
      dblock.items[0] = { ...dblock.items[0], dueDate: newDue };
      blocks.push(dblock);
      return rmNormalize(blocks.reduce((a, b) => a.concat(b.items), []));
    }
    function rmMigrate(oldArr) {
      const mapS = { todo: 'todo', in_progress: 'inprogress', stuck: 'stuck', done: 'completed' };
      const mapT = { mvp: 'mvp', vanity: 'future' };
      const rows = [];
      (oldArr || []).forEach(it => {
        const pid = it.id || ('p' + rows.length);
        rows.push({ id: pid, name: it.title || '', status: mapS[it.status] || 'todo', type: mapT[it.type] || 'mvp', dueDate: it.due || null, parentId: null, order: 0 });
        (it.subtasks || []).forEach(s => rows.push({ id: s.id || ('c' + rows.length), name: s.title || '', status: s.done ? 'completed' : 'todo', type: 'mvp', dueDate: null, parentId: pid, order: 0 }));
      });
      return rmNormalize(rows);
    }
    function rmSeed() {
      const DEF = [
        { n: 'AI Chat', s: 'completed', t: 'mvp', subs: [['AI Mode (general assistant)', 'completed'], ['Work Mode (guided pills)', 'completed'], ['Prompt pills', 'completed'], ['Form-type pills', 'completed'], ['Guided-steps pills', 'completed'], ['Chat history (pin / archive / rename / delete)', 'completed'], ['Attachments (files / photos / PDF)', 'completed'], ['Auto-grow message box', 'completed'], ['Claude AI proxy + usage tracking', 'completed']] },
        { n: 'Team Chat', s: 'todo', t: 'future', subs: [['Message threads preview', 'inprogress']] },
        { n: 'Calls', s: 'todo', t: 'future', subs: [['Recent calls preview', 'inprogress']] },
        { n: 'KPIs', s: 'todo', t: 'future', subs: [['Sales goals / KPI preview', 'inprogress']] },
        { n: 'Deals', s: 'todo', t: 'future', subs: [['Coming-soon placeholder', 'todo']] },
        { n: 'Shared Drives', s: 'completed', t: 'mvp', subs: [['8 Google Drive folder tiles', 'completed'], ['Admin + Agents folder links', 'completed']] },
        { n: 'Company Directory', s: 'completed', t: 'mvp', subs: [['Live roster (Supabase + curated)', 'completed'], ['Search', 'completed'], ['Role badges', 'completed'], ['Per-person profile pages', 'completed'], ['Clickable phone (tel:) / email (mailto:)', 'completed']] },
        { n: 'Exp Survey', s: 'completed', t: 'mvp', subs: [['Survey QR code', 'completed'], ['Survey link', 'completed']] },
        { n: 'SFFU (Showing Follow-Up)', s: 'inprogress', t: 'mvp', subs: [['Embedded app (iframe)', 'completed'], ['Add property / add showing', 'completed'], ['SMS follow-up engine (Quo)', 'inprogress']] },
        { n: 'Calendar', s: 'stuck', t: 'mvp', subs: [['Week view UI', 'completed'], ['Google Calendar sync', 'stuck']] },
        { n: 'Tasks', s: 'completed', t: 'mvp', subs: [['Statuses + priority', 'completed'], ['Subtasks', 'completed'], ['Recurrence (daily / weekly / monthly)', 'completed'], ['Assignees + labels', 'completed'], ['Decision tasks', 'completed'], ['Activity log', 'completed']] },
        { n: 'Profile & Status', s: 'completed', t: 'mvp', subs: [['Avatar chip + presence dot', 'completed'], ['Status (emoji + text)', 'completed'], ['Set away (green / amber)', 'completed'], ['Profile details + edit', 'completed']] },
        { n: 'Admin', s: 'completed', t: 'mvp', subs: [['Manage Users (roles / access)', 'completed'], ['Tab Access (per-role)', 'completed'], ['Work Mode Builder', 'completed'], ['Usage stats', 'completed']] },
        { n: 'Settings', s: 'completed', t: 'mvp', subs: [['Dark mode', 'completed'], ['Text-size slider', 'completed'], ['Chat history side', 'completed'], ['Sign out', 'completed']] },
        { n: 'Roadmap', s: 'inprogress', t: 'mvp', subs: [['Quarter grouping', 'completed'], ['Month progress tiles', 'completed'], ['Drag to reschedule', 'completed'], ['Subtasks', 'completed']] },
        { n: 'Auth / Account', s: 'completed', t: 'mvp', subs: [['Google SSO', 'completed'], ['Email + OTP / MFA', 'completed'], ['Access gating (pending / active)', 'completed']] },
      ];
      let i = 0;
      const dueFor = () => { const off = ((i * 50 + 23) % 163); i++; const d = new Date(2026, 0, 1 + off); return d.getFullYear() + '-' + rmPad(d.getMonth() + 1) + '-' + rmPad(d.getDate()); };
      const rows = [];
      DEF.forEach((tab, ti) => {
        const pid = 't' + ti;
        rows.push({ id: pid, name: tab.n, status: tab.s, type: tab.t, dueDate: dueFor(), parentId: null, order: 0 });
        (tab.subs || []).forEach((sub, si) => rows.push({ id: pid + '-' + si, name: sub[0], status: sub[1], type: sub[2] || tab.t, dueDate: dueFor(), parentId: pid, order: 0 }));
      });
      return rmNormalize(rows);
    }
    ...
    function rmEnsureAdditions(rows) {
      ...
      const addChild = (pid, name) => { if (pid && !hasChild(pid, name)) out.push({ id: pid + '-x' + out.length, name: name, status: 'todo', type: 'mvp', dueDate: null, parentId: pid, order: 0 }); };
      const ai = topByName('AI Chat', 't0'); if (ai) addChild(ai.id, 'Projects (Files + Contacts)');
      const tc = topByName('Team Chat', 't1'); if (tc) { ['Direct Messaging', 'Group Chat', 'Assigned Tasks'].forEach(n => addChild(tc.id, n)); }
      let crm = topByName('CRM', 'crm');
      if (!crm) { out.push({ id: 'crm', name: 'CRM', status: 'todo', type: 'mvp', dueDate: null, parentId: null, order: 0 }); crm = out[out.length - 1]; }
      addChild(crm.id, 'Contacts'); addChild(crm.id, 'Deals');
    ...
    function DateField({ value, onChange, dark, inp, bord, ink, sub, placeholder }) {
      const gold = dark ? '#C9A45A' : '#AD832F';
      const navy = '#001A4A';
      const J = C.fontSans;
      const [open, setOpen] = useState(false);
      const ddRef = useCloseOnOutside(open, () => setOpen(false));
      const parse = (v) => { if (!v) return null; const p = String(v).split('-'); if (p.length !== 3) return null; const d = new Date(+p[0], +p[1] - 1, +p[2]); return isNaN(d.getTime()) ? null : d; };
      const pad = (n) => String(n).padStart(2, '0');
      const toISO = (d) => `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
      const same = (a, b) => a && b && a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate();
      const selected = parse(value);
      const today = new Date(); today.setHours(0, 0, 0, 0);
      const [cursor, setCursor] = useState(() => { const b = selected || new Date(); return new Date(b.getFullYear(), b.getMonth(), 1); });
      useEffect(() => { if (open) { const b = parse(value) || new Date(); setCursor(new Date(b.getFullYear(), b.getMonth(), 1)); } }, [open]);
      ...
      const navBtn = { width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 7, border: `1px solid ${bord}`, background: dark ? '#06101F' : '#F7F4EE', color: gold, cursor: 'pointer', padding: 0, fontSize: 13 };
      const display = selected ? selected.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : (placeholder || 'Select date');
      return (
        <div ref={ddRef} style={{ flex: '1 1 150px', minWidth: 0 }}>
          <div onClick={() => setOpen(o => !o)} style={{ ...inp, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', gap: 8 }}>
            <span style={{ color: selected ? ink : sub, fontFamily: J, fontSize: '0.86rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{display}</span>
            <i className="ti ti-calendar" style={{ fontSize: 15, color: gold, flexShrink: 0 }} />
          </div>
          {open && (
            <div style={{ marginTop: 6, background: panelBg, border: `1px solid ${bord}`, borderRadius: 12, padding: 10, boxShadow: dark ? '0 10px 30px rgba(0,0,0,0.4)' : '0 10px 30px rgba(0,26,74,0.12)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ fontFamily: C.fontDisplay, fontSize: '1.05rem', fontWeight: 600, fontStyle: 'italic', color: dark ? '#EFE9DC' : navy }}>{cursor.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <button type="button" title="Previous month" onClick={() => stepMonth(-1)} style={navBtn}><i className="ti ti-chevron-left" /></button>
                  <button type="button" title="Next month" onClick={() => stepMonth(1)} style={navBtn}><i className="ti ti-chevron-right" /></button>
                  <div style={{ width: 1, height: 16, background: bord, margin: '0 2px' }} />
                  <button type="button" title="Year ascending" onClick={() => stepYear(1)} style={navBtn}><i className="ti ti-chevron-up" /></button>
                  <button type="button" title="Year descending" onClick={() => stepYear(-1)} style={navBtn}><i className="ti ti-chevron-down" /></button>
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2, marginBottom: 3 }}>
                {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((w, i) => <div key={i} style={{ textAlign: 'center', fontFamily: J, fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.04em', color: sub, padding: '2px 0' }}>{w}</div>)}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2 }}>
                {cells.map((d, i) => {
                  ...
                  return (
                    <button type="button" key={i} onClick={() => { onChange(toISO(d)); setOpen(false); }} style={{
                      height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: J,
                      fontSize: '0.8rem', fontWeight: isSel ? 700 : 500,
                      ...
                      borderRadius: 8, cursor: 'pointer', padding: 0, lineHeight: 1, WebkitAppearance: 'none', outline: 'none',
                    }}>{d.getDate()}</button>
                  );
                })}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 9, paddingTop: 8, borderTop: `1px solid ${bord}` }}>
                <button type="button" onClick={() => { onChange(''); setOpen(false); }} style={{ fontFamily: J, fontSize: '0.72rem', fontWeight: 600, color: sub, background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}>Clear</button>
                <button type="button" onClick={() => { onChange(toISO(today)); setOpen(false); }} style={{ fontFamily: J, fontSize: '0.72rem', fontWeight: 700, color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}>Today</button>
              </div>
            </div>
          )}
        </div>
    ...
    function CopyLinkBtn({ dark, hash, title }) {
      const [copied, setCopied] = useState(false);
      const bord = dark ? '#152545' : '#E4DFD4', sub = dark ? 'rgba(255,255,255,0.5)' : '#6B6B6B';
      const go = async () => {
      ...
      return (
        <button onClick={go} title={title || 'Copy a link straight to this view'} style={{ fontSize: 12, fontWeight: 400, letterSpacing: '0.08em', textTransform: 'uppercase', color: copied ? '#0F6E56' : sub, background: dark ? '#0A1730' : '#fff', border: `1px solid ${copied ? '#0F6E5655' : bord}`, borderRadius: 6, padding: '7px 12px', cursor: 'pointer', fontFamily: C.fontSans, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5 }}>
          <i className={`ti ${copied ? 'ti-check' : 'ti-link'}`} style={{ fontSize: 13 }} />{copied ? 'Copied' : 'Link'}
        </button>
      );
    }
    ...
    function PeopleDropdown({ dark, team, arr, set, placeholder }) {
      const gold = dark ? '#C9A45A' : '#AD832F';
      const J = C.fontSans;
      const [open, setOpen] = useState(false);
      const ddRef = useCloseOnOutside(open, () => setOpen(false));
      const bord = dark ? '#152545' : '#E4DFD4', ink = dark ? '#fff' : '#001A4A', sub = dark ? 'rgba(255,255,255,0.5)' : '#6B6B6B';
      const inp = { width: '100%', padding: '9px 11px', background: dark ? '#06101F' : '#F7F4EE', border: `1px solid ${bord}`, borderRadius: 6, color: ink, fontSize: 14, outline: 'none', fontFamily: J, boxSizing: 'border-box' };
      ...
      return (
        <div ref={ddRef}>
          <div onClick={() => setOpen(o => !o)} style={{ ...inp, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', gap: 8 }}>
            <span style={{ color: arr.length ? ink : sub, fontFamily: J, fontSize: '0.86rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{display}</span>
            <i className={`ti ti-chevron-${open ? 'up' : 'down'}`} style={{ fontSize: 15, color: gold, flexShrink: 0 }} />
          </div>
          {open && (
            <div style={{ marginTop: 6, background: dark ? '#0A1730' : '#FFFFFF', border: `1px solid ${bord}`, borderRadius: 12, padding: 6, boxShadow: dark ? '0 10px 30px rgba(0,0,0,0.4)' : '0 10px 30px rgba(0,26,74,0.12)', maxHeight: 220, overflowY: 'auto' }}>
              {!team.length && <div style={{ padding: '8px 6px', fontSize: '0.74rem', color: sub, fontFamily: J }}>No teammates loaded.</div>}
              {team.map(m => { const on = arr.includes(m.id); return (
                <div key={m.id} onClick={() => toggle(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 8px', borderRadius: 8, cursor: 'pointer', background: on ? (dark ? 'rgba(201,164,90,0.12)' : '#F3EBDA') : 'transparent' }}>
                  <i className={`ti ${on ? 'ti-square-check-filled' : 'ti-square'}`} style={{ fontSize: 16, color: on ? gold : sub, flexShrink: 0 }} />
                  <span style={{ fontSize: '0.82rem', color: ink, fontFamily: J }}>{m.name}</span>
                </div>
              ); })}
              {arr.length > 0 && (
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 4, paddingTop: 6, borderTop: `1px solid ${bord}` }}>
                  <button type="button" onClick={() => set([])} style={{ fontFamily: J, fontSize: '0.72rem', fontWeight: 600, color: sub, background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}>Clear</button>
                </div>
              )}
            </div>
          )}
    ...
    function TaskForm({ dark, task, team, user, onSave, onCancel }) {
      const t = task || {};
      ...
      useEffect(() => {
        TaskDB.loadProjects().then(rows => {
          ...
          if (!t._projectName && t.project_id) { const m = list.find(p => p.id === t.project_id); if (m && m.name) setProject(prev => prev || m.name); }
        });
      }, []);
      useEffect(() => {
        if (!t.id) return;
        TaskDB.loadLabels(t.id).then(rows => setMyLabels((rows || []).filter(r => r.user_id === uid).map(r => r.label)));
        TaskDB.loadDecisionOptions(t.id).then(rows => setDecisionOptions((rows || []).map(r => ({ body: r.body, is_chosen: r.is_chosen }))));
      ...
      const lbl = { fontSize: 11, color: sub, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, margin: '0 0 5px', fontFamily: C.fontSans };
      const inp = { width: '100%', padding: '9px 11px', background: dark ? '#06101F' : '#F7F4EE', border: `1px solid ${bord}`, borderRadius: 6, color: ink, fontSize: 14, outline: 'none', fontFamily: C.fontSans, boxSizing: 'border-box' };
      const toggle = (arr, set, id) => set(arr.includes(id) ? arr.filter(x => x !== id) : [...arr, id]);
      const chips = (arr, set) => (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {team.map(m => { const on = arr.includes(m.id); return <button key={m.id} type="button" onClick={() => toggle(arr, set, m.id)} style={{ padding: '6px 10px', borderRadius: 16, border: `1px solid ${on ? C.navy : bord}`, background: on ? C.navy : (dark ? '#06101F' : '#fff'), color: on ? '#fff' : sub, fontSize: '0.76rem', cursor: 'pointer', fontFamily: C.fontSans }}>{m.name}</button>; })}
          {!team.length && <span style={{ fontSize: '0.74rem', color: sub }}>No teammates loaded.</span>}
        </div>
      );
      const sel = (val, set, opts) => { const cur = opts.find(o => o.id === val) || {}; return <select value={val} onChange={e => set(e.target.value)} style={{ ...inp, ...(cur.bg ? { background: cur.bg, color: cur.color, fontWeight: 600 } : {}) }}>{opts.map(o => <option key={o.id} value={o.id} style={o.bg ? { background: o.bg, color: o.color, fontWeight: 600 } : undefined}>{o.label}</option>)}</select>; };
      const field = (label, node) => <div style={{ marginBottom: 16 }}><div style={lbl}>{label}</div>{node}</div>;
      const addLabel = () => { const v = labelInput.trim(); if (!v) return; setMyLabels(l => l.includes(v) ? l : [...l, v]); setLabelInput(''); };
      const removeLabel = (v) => setMyLabels(l => l.filter(x => x !== v));
      const addOpt = () => setDecisionOptions(o => [...o, { body: '', is_chosen: false }]);
      const updateOpt = (i, v) => setDecisionOptions(o => o.map((x, j) => j === i ? { ...x, body: v } : x));
      const removeOpt = (i) => setDecisionOptions(o => o.filter((_, j) => j !== i));
      const setChosen = (i) => setDecisionOptions(o => o.map((x, j) => ({ ...x, is_chosen: j === i ? !x.is_chosen : false })));

      const mondayOpts = (() => {
        const opts = [{ id: '', label: '— Pick a week —' }];
        const d = new Date(); const o = (d.getDay() + 6) % 7; d.setDate(d.getDate() - o); d.setHours(0, 0, 0, 0); d.setDate(d.getDate() - 7);
        for (let i = 0; i < 18; i++) { const iso = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0'); opts.push({ id: iso, label: 'Wk of ' + d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) }); d.setDate(d.getDate() + 7); }
        return opts;
      })();
      const RANKS = [{ id: '', label: '—' }, { id: '1', label: 'Rank 1' }, { id: '2', label: 'Rank 2' }, { id: '3', label: 'Rank 3' }];
      ...
      async function save() {
        ...
        try {
          ...
          await onSave({
            title: title.trim(), due, recurrence: repeats ? recurrence : 'none',
            ...
            planning: { wp: weeklyPriority, wr: weeklyRank, dp: dailyPriority, dr: dailyRank },
          });
        } finally { setSaving(false); }
      }

      const sectionHeader = (open, setOpen, icon, label) => (
        <button type="button" onClick={() => setOpen(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 7, width: '100%', textAlign: 'left', background: dark ? '#06101F' : '#F7F4EE', border: `1px solid ${bord}`, borderRadius: 10, padding: '10px 12px', cursor: 'pointer' }}>
          <i className={`ti ${icon}`} style={{ fontSize: 15, color: gold }} />
          <span style={{ flex: 1, fontSize: '0.64rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: gold, fontFamily: C.fontSans }}>{label}</span>
          <i className={`ti ti-chevron-${open ? 'up' : 'down'}`} style={{ fontSize: 15, color: sub }} />
        </button>
      );

      return (
        <div style={{ padding: '14px 16px 40px' }}>
          {/* W/D Planning (collapsible) */}
          <div style={{ marginBottom: 13 }}>
            {sectionHeader(planOpen, setPlanOpen, 'ti-target', 'W/D Planning')}
            {planOpen && <div style={{ marginTop: 10 }}>
              <div style={{ display: 'flex', gap: 10 }}>
                <div style={{ flex: 1.5 }}>{field('Weekly priority', sel(weeklyPriority, setWeeklyPriority, mondayOpts))}</div>
                <div style={{ flex: 1 }}>{field('Weekly rank', sel(weeklyRank, setWeeklyRank, RANKS))}</div>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <div style={{ flex: 1.5 }}>{field('Daily priority', <DateField value={dailyPriority} onChange={setDailyPriority} dark={dark} inp={inp} bord={bord} ink={ink} sub={sub} placeholder="Select date" />)}</div>
                <div style={{ flex: 1 }}>{field('Daily rank', sel(dailyRank, setDailyRank, RANKS))}</div>
              </div>
            </div>}
          </div>
          {/* Title */}
          {field('Title', <input value={title} onChange={e => setTitle(e.target.value)} placeholder="What needs doing?" style={inp} />)}
          {/* Due date (date only) + Time / Repeat toggles */}
          <div style={{ marginBottom: 13 }}>
            <div style={lbl}>Due date</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <DateField value={dueDate} onChange={setDueDate} dark={dark} inp={inp} bord={bord} ink={ink} sub={sub} placeholder="Select date" />
              <label style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', fontSize: '0.78rem', color: ink, fontFamily: C.fontSans }}><input type="checkbox" checked={hasTime} onChange={e => setHasTime(e.target.checked)} style={{ accentColor: C.navy, width: 15, height: 15 }} />Time</label>
              <label style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', fontSize: '0.78rem', color: ink, fontFamily: C.fontSans }}><input type="checkbox" checked={repeats} onChange={e => setRepeats(e.target.checked)} style={{ accentColor: C.navy, width: 15, height: 15 }} />Repeat</label>
            </div>
            {hasTime && <input type="time" value={dueTime} onChange={e => setDueTime(e.target.value)} style={{ ...inp, marginTop: 8 }} />}
            {repeats && <div style={{ marginTop: 8 }}>
              {sel(recurrence, setRecurrence, REPEAT_OPTS)}
              {recurrence === 'custom' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                  <span style={{ fontSize: '0.78rem', color: sub, fontFamily: C.fontSans }}>Every</span>
                  <input type="number" min="1" value={recurInterval} onChange={e => setRecurInterval(e.target.value)} style={{ ...inp, width: 64, padding: '8px 9px' }} />
                  <select value={recurUnit} onChange={e => setRecurUnit(e.target.value)} style={{ ...inp, flex: 1 }}>
              ...
              <div style={{ marginTop: 10 }}>
                <div style={{ ...lbl, marginBottom: 6 }}>Copy to next instance</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {RECUR_COPY_FIELDS.map(f => { const on = recurCopy.includes(f.id); return <button key={f.id} type="button" onClick={() => setRecurCopy(c => c.includes(f.id) ? c.filter(x => x !== f.id) : [...c, f.id])} style={{ padding: '5px 9px', borderRadius: 14, border: `1px solid ${on ? C.navy : bord}`, background: on ? C.navy : (dark ? '#06101F' : '#fff'), color: on ? '#fff' : sub, fontSize: '0.72rem', cursor: 'pointer', fontFamily: C.fontSans }}>{on ? '✓ ' : ''}{f.label}</button>; })}
                </div>
              </div>
              <div style={{ fontSize: '0.66rem', color: sub, lineHeight: 1.5, marginTop: 8, fontFamily: C.fontSans }}>When marked Completed, the next one is created automatically — due date moved forward, only the checked fields carried over.</div>
            </div>}
          </div>
          {/* People */}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1 }}>{field('Status', sel(status, setStatus, TASK_STATUS))}</div>
            <div style={{ flex: 1 }}>{field('Priority', sel(priority, setPriority, TASK_PRIORITY))}</div>
          </div>
          {/* Milestone. The Overview's Milestones spine reads this flag, and
              with the old "Add a milestone…" input gone this is where it is
              set, on a new task and on an existing one alike. */}
          <div style={{ marginBottom: 16 }}>
            <button type="button" onClick={() => setIsMilestone(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 12px', borderRadius: 14, border: `1px solid ${isMilestone ? C.navy : bord}`, background: isMilestone ? C.navy : (dark ? '#06101F' : '#fff'), color: isMilestone ? '#fff' : sub, fontSize: '0.76rem', cursor: 'pointer', fontFamily: C.fontSans }}>
              <i className="ti ti-flag-3" style={{ fontSize: 13 }} />{isMilestone ? '✓ Milestone' : 'Mark as a milestone'}
            </button>
          </div>
          {/* Project */}
          {field('Project', (
            <div>
              <select value={newProjMode ? '__new__' : (project || '')} onChange={e => { const v = e.target.value; if (v === '__new__') { setNewProjMode(true); setProject(''); } else { setNewProjMode(false); setProject(v); } }} style={inp}>
                <option value="">(No project)</option>
                {projects.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
                {project && !newProjMode && !projects.some(p => p.name === project) && <option value={project}>{project}</option>}
                <option value="__new__">＋ New project…</option>
              </select>
              {newProjMode && <input autoFocus value={project} onChange={e => setProject(e.target.value)} placeholder="New project name" style={{ ...inp, marginTop: 8 }} />}
            </div>
          ))}
          {/* Labels */}
          {field('Your labels', (
            <div>
              <div style={{ display: 'flex', gap: 8 }}>
                <input value={labelInput} onChange={e => setLabelInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addLabel(); } }} placeholder="Add a label…" style={inp} />
                <button type="button" onClick={addLabel} style={{ padding: '0 14px', background: dark ? '#06101F' : '#fff', border: `1px solid ${bord}`, borderRadius: 9, color: ink, fontSize: '0.8rem', cursor: 'pointer', fontFamily: C.fontSans, flexShrink: 0 }}>Add</button>
              </div>
              {myLabels.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                {myLabels.map(l => <span key={l} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 7px 4px 10px', borderRadius: 16, background: dark ? 'rgba(201,164,90,0.15)' : '#F3EBDA', color: gold, fontSize: '0.74rem', fontWeight: 500, fontFamily: C.fontSans }}>{l}<i className="ti ti-x" onClick={() => removeLabel(l)} style={{ fontSize: 12, cursor: 'pointer' }} /></span>)}
              </div>}
            </div>
          ))}
          {/* Details */}
          {field('Description', <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="Initial description (auto-filled when generated from AI mode later)" style={{ ...inp, minHeight: 80, resize: 'vertical', lineHeight: 1.6 }} />)}
          {field('Context', <textarea value={context} onChange={e => setContext(e.target.value)} rows={2} placeholder="Ongoing updates / notes" style={{ ...inp, resize: 'vertical', lineHeight: 1.5 }} />)}
          {field('Working URL', <input value={workingUrl} onChange={e => setWorkingUrl(e.target.value)} placeholder="https://…" style={inp} />)}
          {field('Email link', <input value={emailLink} onChange={e => setEmailLink(e.target.value)} placeholder="https://… (link to the email)" style={inp} />)}
          {/* Decision (collapsible) */}
          <div style={{ marginTop: 4, marginBottom: 4 }}>
            {sectionHeader(decisionOpen, setDecisionOpen, 'ti-gavel', 'Decision')}
            {decisionOpen && <div style={{ marginTop: 10 }}>
              {field('Decision maker(s)', chips(decisionMakers, setDecisionMakers))}
              <div style={{ marginBottom: 13 }}>
                <div style={lbl}>Decision due date</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  <DateField value={decisionDueDate} onChange={setDecisionDueDate} dark={dark} inp={inp} bord={bord} ink={ink} sub={sub} placeholder="Select date" />
                  <label style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', fontSize: '0.78rem', color: ink, fontFamily: C.fontSans }}><input type="checkbox" checked={decisionHasTime} onChange={e => setDecisionHasTime(e.target.checked)} style={{ accentColor: C.navy, width: 15, height: 15 }} />Time</label>
                </div>
                {decisionHasTime && <input type="time" value={decisionDueTime} onChange={e => setDecisionDueTime(e.target.value)} style={{ ...inp, marginTop: 8 }} />}
                <div style={{ fontSize: '0.66rem', color: sub, lineHeight: 1.5, marginTop: 5, fontFamily: C.fontSans }}>If set, this shows on the decision-maker's in-app calendar (not Google Calendar).</div>
              </div>
              {field('Question', <input value={decisionQuestion} onChange={e => setDecisionQuestion(e.target.value)} placeholder="What needs to be decided?" style={inp} />)}
              {field('Options', (
                <div>
                  {decisionOptions.map((o, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <i className={o.is_chosen ? 'ti ti-circle-check-filled' : 'ti ti-circle'} onClick={() => setChosen(i)} title="Mark as chosen" style={{ fontSize: 19, cursor: 'pointer', color: o.is_chosen ? (dark ? '#3FB36B' : '#1E6B40') : sub, flexShrink: 0 }} />
                      <input value={o.body} onChange={e => updateOpt(i, e.target.value)} placeholder={'Option ' + String.fromCharCode(65 + i)} style={inp} />
                      <i className="ti ti-trash" onClick={() => removeOpt(i)} style={{ fontSize: 15, cursor: 'pointer', color: sub, flexShrink: 0 }} />
                    </div>
                  ))}
                  <button type="button" onClick={addOpt} style={{ marginTop: 2, display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 12px', background: 'none', border: `1px dashed ${bord}`, borderRadius: 9, color: sub, fontSize: '0.78rem', cursor: 'pointer', fontFamily: C.fontSans }}><i className="ti ti-plus" style={{ fontSize: 13 }} />Add option</button>
                </div>
              ))}
            </div>}
          </div>
    ...
    function TaskDetail({ dark, task, people, team, user, allTasks, onEdit, onDelete, onStatus, onOpenSubtask, onSubtaskAdded, emailLinks, onEmailChanged, initialTab, onTabSettled }) {
      ...
      useEffect(() => {
        if (detTab !== 'email' || !primaryLink) { setEmailThread(null); return; }
        let cancelled = false;
        setEmailBusy(true); setEmailErr(null);
        callCalendar({ action: 'gmail_thread', threadId: primaryLink.thread_id }).then(({ ok, data }) => {
          if (cancelled) return;
          setEmailBusy(false);
          if (!ok) { setEmailErr(data?.error || 'Could not load this thread.'); return; }
      ...
      const [links, setLinks] = useState({ predecessors: [], successors: [] });
      ...
      useEffect(() => {
        if (linkQ.trim().length < 2) { setLinkHits([]); setLinkSearching(false); return; }
        setLinkSearching(true);
        let on = true;
        const t = setTimeout(() => {
          TaskDB.searchTasks(linkQ, task.id).then(r => { if (on) { setLinkHits(r); setLinkSearching(false); } });
        }, 250);
        return () => { on = false; clearTimeout(t); };
      }, [linkQ, task.id]);
      async function addDep() { if (!linkPick) return; if (linkDir === 'pred') await TaskDB.addLink(linkPick, task.id); else await TaskDB.addLink(task.id, linkPick); setLinkPick(''); setLinks(await TaskDB.loadLinks(task.id)); }
      async function removeDep(id) { await TaskDB.removeLink(id); setLinks(await TaskDB.loadLinks(task.id)); }
      // ── Thread: summary (built from the shared activity feed, not the private AI Assist)
      async function refreshSummary(actList) {
        const list = actList || activity; if (!list.length || sumBusy) return;
        setSumBusy(true);
        try {
          const log = list.map(a => '[' + (a.kind || 'log').toUpperCase() + '] ' + a.content).join('\n');
          const text = await callAI([{ role: 'user', content: 'Task: "' + task.title + '" (status: ' + statusLabel(task.status) + ')\n\nThread:\n' + log + '\n\nWrite a concise 2–3 sentence summary of where this stands and what is next. Plain text only.' }], 'You summarise a task thread in 2–3 plain sentences. Current status, key updates, next step. No bullets, no headers.');
          setSummary(text); await TaskDB.setThreadSummary(task.id, text);
        } catch (e) { /* silent — summary is best-effort */ } finally { setSumBusy(false); }
      }
      // ── Thread: Log update (shared) — auto-refreshes the summary
      async function addNote() { if (!note.trim() || busy) return; setBusy(true); try { await TaskDB.addActivity(task.id, 'update', note.trim(), user); setNote(''); const a = await TaskDB.loadActivity(task.id); setActivity(a); refreshSummary(a); } finally { setBusy(false); } }
      // ── Thread: AI Assist (private but synced) — Q&A NOT added to the shared feed
      async function sendAssist() {
        if (!note.trim() || aiBusy) return; const txt = note.trim();
        setAiBusy(true); setNote('');
        const uMsg = await TaskDB.addAssist(task.id, 'user', txt, user); setAssist(a => uMsg ? [...a, uMsg] : a);
        try {
          ...
          const hist = [...assist, uMsg].filter(Boolean).slice(-10).map(m => ({ role: m.role === 'ai' ? 'assistant' : 'user', content: m.content }));
          const reply = await callAI(hist, sys);
          const aMsg = await TaskDB.addAssist(task.id, 'ai', reply, user); setAssist(a => aMsg ? [...a, aMsg] : a);
        } catch (e) { const aMsg = await TaskDB.addAssist(task.id, 'ai', 'Error: ' + e.message, user); setAssist(a => aMsg ? [...a, aMsg] : a); }
        finally { setAiBusy(false); }
      }
      async function saveAssistEdit(id) { const item = assist.find(x => x.id === id); await TaskDB.updateAssistContent(id, editText); if (item && item.published && item.published_activity_id) { await TaskDB.updateActivityContent(item.published_activity_id, editText); setActivity(await TaskDB.loadActivity(task.id)); } setEditId(null); setEditText(''); setAssist(await TaskDB.loadAssist(task.id, user)); }
      async function togglePublish(item) { if (item.published) await TaskDB.unpublishAssist(item); else await TaskDB.publishAssist(item, task.id, user); setAssist(await TaskDB.loadAssist(task.id, user)); setActivity(await TaskDB.loadActivity(task.id)); }
      // ── Thread: improve the decision from the thread
      async function improveDecision() {
        if (decBusy) return; setDecBusy(true);
        try {
          const log = activity.map(a => a.content).join('\n');
          const reply = await callAI([{ role: 'user', content: 'Task: "' + task.title + '"\nContext: ' + (summary || 'none') + '\nCurrent decision question: ' + (task.decision_question || 'none') + '\nThread:\n' + log + '\n\nSuggest a concise decision question and 2–3 clear options reflecting the current state. Reply ONLY valid JSON: {"question":"...","options":["...","..."]}' }], 'You propose a decision question and options from a task thread. Reply only valid JSON, no preamble.');
          let parsed = null; try { parsed = JSON.parse((reply.match(/\{[\s\S]*\}/) || [reply])[0]); } catch (e) {}
          if (parsed && parsed.question) setDecDraft({ question: parsed.question, options: (parsed.options || []).map(o => ({ text: String(o), checked: true })) });
        } catch (e) { /* silent */ } finally { setDecBusy(false); }
      }
      async function acceptDecision() {
        if (!decDraft) return;
        await TaskDB.update(task.id, task, { decision_question: decDraft.question }, user);
        await TaskDB.setDecisionOptions(task.id, decDraft.options.filter(o => o.checked).map(o => ({ body: o.text, is_chosen: false })));
        setDecDraft(null);
        setDecisionOpts(await TaskDB.loadDecisionOptions(task.id));
        setActivity(await TaskDB.loadActivity(task.id));
      }
      const depRow = (linkId, otherId) => (
        <div key={linkId} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: dark ? '#06101F' : '#fff', border: `1px solid ${bord}`, borderRadius: 9, marginBottom: 6 }}>
          <span style={{ flex: 1, minWidth: 0, fontSize: '0.8rem', color: ink, fontFamily: C.fontSans, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{titleOf(otherId)}</span>
          <i className="ti ti-x" onClick={() => removeDep(linkId)} style={{ fontSize: 14, color: sub, cursor: 'pointer', flexShrink: 0 }} />
        </div>
      );
      return (
        <div style={{ padding: '22px 24px 60px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
            ...
            <button onClick={onEdit} style={{ fontSize: 12, fontWeight: 400, letterSpacing: '0.08em', textTransform: 'uppercase', color: sub, background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 6, padding: '7px 12px', cursor: 'pointer', fontFamily: C.fontSans, flexShrink: 0 }}>Edit</button>
            <button onClick={handleDelete} disabled={deleting} style={{ fontSize: 12, fontWeight: 400, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#C0392B', background: dark ? '#0A1730' : '#fff', border: '1px solid #C0392B55', borderRadius: 6, padding: '7px 12px', cursor: deleting ? 'default' : 'pointer', opacity: deleting ? 0.6 : 1, fontFamily: C.fontSans, flexShrink: 0 }}>{deleting ? 'Deleting…' : 'Delete'}</button>
          </div>
          {/* Status segmented control — ported from the prototype's .status-seg */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, marginBottom: 22 }}>
            {TASK_STATUS.map(s => { const on = task.status === s.id; return (
              <button key={s.id} onClick={async () => { await onStatus(s.id); setActivity(await TaskDB.loadActivity(task.id)); }} style={{ padding: '8px 4px', borderRadius: 6, border: `1px solid ${on ? s.border || s.color : bord}`, background: on ? (dark ? s.color + '26' : s.bg) : (dark ? '#0A1730' : '#fff'), color: on ? s.color : sub, fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer', fontFamily: C.fontSans }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: on ? s.color : sub, flexShrink: 0 }} />{s.label}
              </button>
            ); })}
          </div>
          {/* Details | Thread | Email tabs [brief §2.4] */}
          <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
            {[['details', 'Details'], ['thread', 'Thread'], ['email', 'Email']].map(([id, label]) => {
              ...
              return (
                <button key={id} onClick={() => setDetTab(id)} style={{ flex: 1, padding: '7px 8px', borderRadius: 6, border: `1px solid ${detTab === id ? C.navy : bord}`, background: detTab === id ? C.navy : (dark ? '#0A1730' : '#fff'), color: detTab === id ? '#fff' : sub, fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: C.fontSans }}>{label}{cnt ? ' · ' + cnt : ''}</button>
              );
            })}
          </div>
          ...
          {labelRows.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
              {labelRows.map(r => { const mine = r.user_id === (user && user.id); return (
                <span key={r.id} style={{ padding: '3px 9px', borderRadius: 16, fontSize: '0.7rem', fontWeight: 500, fontFamily: C.fontSans, background: mine ? (dark ? 'rgba(201,164,90,0.15)' : '#F3EBDA') : (dark ? 'rgba(255,255,255,0.06)' : '#F0EEEA'), color: mine ? gold : sub }}>{r.label}</span>
              ); })}
            </div>
          )}
          ...
          {emLinks.length > 0
            ? row('Email link', <React.Fragment>
                <span onClick={() => setDetTab('email')} style={{ color: '#185FA5', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 500 }}><i className="ti ti-mail" style={{ fontSize: 13 }} />{(emLinks[0].subject || '(no subject)').slice(0, 40)}{emLinks.length > 1 ? ' +' + (emLinks.length - 1) + ' more' : ''}</span>
                {/* Clears the link only — never touches the actual email [brief §2.6]. */}
                <i className="ti ti-x" title="Remove link" onClick={async () => { await TaskEmailDB.remove(emLinks[0].id); onEmailChanged && await onEmailChanged(); }} style={{ fontSize: 13, color: sub, cursor: 'pointer', marginLeft: 6 }} />
              </React.Fragment>)
            : task.email_link ? row('Email link', <a href={task.email_link} target="_blank" rel="noopener noreferrer" style={{ color: C.gold }}>Open ↗</a>) : null}
          {task.description ? <div style={{ marginTop: 12 }}><div style={cap}>Description</div><div style={{ fontSize: '0.84rem', color: ink, lineHeight: 1.5, whiteSpace: 'pre-wrap', fontFamily: C.fontSans }}>{task.description}</div></div> : null}
          {/* Dependencies */}
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${bord}` }}>
            {secHead('ti-arrows-split-2', 'Dependencies')}
            {links.predecessors.length > 0 && <div style={{ marginBottom: 8 }}><div style={cap}>Must come before this</div>{links.predecessors.map(l => depRow(l.id, l.predecessor_id))}</div>}
            {links.successors.length > 0 && <div style={{ marginBottom: 8 }}><div style={cap}>Comes after this</div>{links.successors.map(l => depRow(l.id, l.successor_id))}</div>}
            {!links.predecessors.length && !links.successors.length && <div style={{ fontSize: '0.78rem', color: sub, fontFamily: C.fontSans, marginBottom: 8 }}>No dependencies.</div>}
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <select value={linkDir} onChange={e => setLinkDir(e.target.value)} style={{ ...tinp, flex: '1 1 130px' }}>
                <option value="pred">Predecessor (before)</option>
                <option value="succ">Successor (after)</option>
              </select>
              <input value={linkQ} onChange={e => { setLinkQ(e.target.value); setLinkPick(''); }} placeholder="Search any task…" style={{ ...tinp, flex: '2 1 150px' }} />
              <select value={linkPick} onChange={e => setLinkPick(e.target.value)} style={{ ...tinp, flex: '2 1 150px' }}>
                <option value="">{linkSearching ? 'Searching…' : (linkQ.trim().length >= 2 && !linkHits.length ? 'No tasks match' : 'Pick a task…')}</option>
                {(linkQ.trim().length >= 2 ? linkHits : (allTasks || [])).filter(t => t.id !== task.id).map(t => <option key={t.id} value={t.id}>{t.title}</option>)}
              </select>
              <button onClick={addDep} disabled={!linkPick} style={{ padding: '8px 13px', background: C.navy, color: '#fff', border: 'none', borderRadius: 9, fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans, opacity: !linkPick ? 0.5 : 1 }}>Link</button>
            </div>
          </div>
          {/* Decision */}
          {hasDecision && (
            <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${bord}` }}>
              {secHead('ti-gavel', 'Decision')}
              {ppl('decision_maker') !== '—' ? row('Maker(s)', ppl('decision_maker')) : null}
              {task.decision_due_at ? row('Decision due', task.decision_due_has_time ? fmt(task.decision_due_at) : new Date(task.decision_due_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })) : null}
              {task.decision_question ? <div style={{ fontSize: '0.86rem', color: ink, fontWeight: 600, margin: '10px 0 8px', fontFamily: C.fontSans, lineHeight: 1.4 }}>{task.decision_question}</div> : null}
              {decisionOpts.map(o => (
                <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: `1px solid ${bord}` }}>
                  <i className={o.is_chosen ? 'ti ti-circle-check-filled' : 'ti ti-circle'} style={{ fontSize: 17, color: o.is_chosen ? (dark ? '#3FB36B' : '#1E6B40') : sub, flexShrink: 0 }} />
                  <span style={{ flex: 1, minWidth: 0, fontSize: '0.84rem', color: o.is_chosen ? ink : sub, fontWeight: o.is_chosen ? 600 : 400, fontFamily: C.fontSans }}>{o.body}</span>
                  {o.is_chosen && <span style={{ fontSize: '0.56rem', fontWeight: 700, letterSpacing: '0.08em', color: dark ? '#3FB36B' : '#1E6B40', fontFamily: C.fontSans, flexShrink: 0 }}>CHOSEN</span>}
                </div>
              ))}
            </div>
          )}
          {/* Subtasks */}
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${bord}` }}>
            {secHead('ti-subtask', subs.length ? 'Subtasks · ' + subs.length : 'Subtasks')}
            {subs.map(s => (
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <input value={subTitle} onChange={e => setSubTitle(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') addSub(); }} placeholder="Add a subtask…" style={{ ...tinp, flex: 1, padding: '9px 11px' }} />
              <button onClick={addSub} disabled={!subTitle.trim() || addingSub} style={{ padding: '9px 13px', background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 6, color: ink, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: C.fontSans, opacity: !subTitle.trim() ? 0.5 : 1 }}>Add</button>
            </div>
          </div>
          </React.Fragment>
          ) : detTab === 'thread' ? (
          <React.Fragment>
          {/* Current Summary */}
          <div style={{ background: dark ? '#0A1E44' : '#F3EBDA', border: `1px solid ${bord}`, borderRadius: 10, padding: '10px 12px', marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: gold, fontFamily: C.fontSans }}>Current Summary</span>
              <button onClick={() => refreshSummary()} disabled={sumBusy} style={{ fontSize: '0.68rem', fontWeight: 600, color: gold, background: 'none', border: `1px solid ${bord}`, borderRadius: 7, padding: '3px 9px', cursor: 'pointer', fontFamily: C.fontSans }}>{sumBusy ? '…' : '↻ Refresh'}</button>
            </div>
            <div style={{ fontSize: '0.82rem', color: summary ? ink : sub, lineHeight: 1.5, whiteSpace: 'pre-wrap', fontFamily: C.fontSans, fontStyle: summary ? 'normal' : 'italic' }}>{summary || 'No summary yet — add an update and it’ll generate.'}</div>
          </div>
          {/* Improve decision from thread */}
          <button onClick={improveDecision} disabled={decBusy} style={{ width: '100%', padding: '9px', background: dark ? '#06101F' : '#fff', border: `1px dashed ${gold}`, borderRadius: 9, color: gold, fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans, marginBottom: 12 }}>{decBusy ? 'Thinking…' : '✨ Improve decision from thread'}</button>
          {decDraft && (
            <div style={{ border: `1px solid ${bord}`, borderRadius: 10, padding: '10px 12px', marginBottom: 14, background: dark ? '#06101F' : '#fff' }}>
              <div style={cap}>Suggested decision — edit, then save</div>
              <input value={decDraft.question} onChange={e => setDecDraft(d => ({ ...d, question: e.target.value }))} style={{ ...tinp, width: '100%', fontWeight: 600 }} />
              {decDraft.options.map((o, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                  <input type="checkbox" checked={o.checked} onChange={() => setDecDraft(d => ({ ...d, options: d.options.map((x, j) => j === i ? { ...x, checked: !x.checked } : x) }))} style={{ accentColor: C.navy, width: 15, height: 15, flexShrink: 0 }} />
                  <input value={o.text} onChange={e => setDecDraft(d => ({ ...d, options: d.options.map((x, j) => j === i ? { ...x, text: e.target.value } : x) }))} style={{ ...tinp, flex: 1 }} />
                </div>
              ))}
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                <button onClick={acceptDecision} style={{ padding: '7px 13px', background: C.navy, color: '#fff', border: 'none', borderRadius: 9, fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans }}>Save to decision</button>
                <button onClick={() => setDecDraft(null)} style={{ padding: '7px 13px', background: 'none', color: sub, border: `1px solid ${bord}`, borderRadius: 9, fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans }}>Cancel</button>
              </div>
            </div>
          )}
          {/* Shared thread feed — comments AND linked-email cards, in time order [brief §2.4] */}
          {secHead('ti-message-2', 'Thread')}
          {activity.length === 0 && emLinks.length === 0 && <div style={{ fontSize: '0.78rem', color: sub, fontFamily: C.fontSans }}>No activity yet.</div>}
          {[
            ...activity.map(a => ({ _t: a.created_at, _k: 'activity', a })),
            ...emLinks.map(l => ({ _t: l.email_date || l.created_at, _k: 'email', l })),
          ].sort((x, y) => new Date(x._t || 0) - new Date(y._t || 0)).map((item, i) => {
            if (item._k === 'email') { const l = item.l; return (
              <div key={'em-' + l.id} style={{ border: `1px solid ${bord}`, borderLeft: '3px solid #185FA5', borderRadius: 8, background: dark ? '#0A1730' : '#fff', padding: '12px 14px', margin: '8px 0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  <svg viewBox="0 0 48 48" style={{ width: 18, height: 18 }}><path fill="#4285F4" d="M44 8v2L24 24 44 8z" /><path fill="#34A853" d="M4 10v28a2 2 0 002 2h6V16z" /><path fill="#FBBC05" d="M44 10v28a2 2 0 01-2 2h-6V16z" /><path fill="#C5221F" d="M4 8l20 14L44 8" /></svg>
                  <span style={{ fontSize: 13, fontWeight: 500, fontFamily: C.fontSans, color: ink }}>{l.from_addr}</span>
                  <span style={{ fontSize: 11, color: sub, marginLeft: 'auto', fontFamily: C.fontSans }}>{fmt(l.email_date || l.created_at)}</span>
                </div>
                <div style={{ fontSize: 14, color: ink, margin: '7px 0 3px', fontWeight: 500, fontFamily: C.fontSans }}>{l.subject}</div>
                <div style={{ fontSize: 13, color: sub, fontFamily: C.fontSans }}>{l.snippet}</div>
                <div style={{ display: 'flex', gap: 14, marginTop: 9 }}>
                  <a onClick={() => setDetTab('email')} style={{ fontSize: 12, color: gold, display: 'inline-flex', alignItems: 'center', gap: 5, cursor: 'pointer', fontFamily: C.fontSans }}><i className="ti ti-mail" style={{ fontSize: 13 }} />View email</a>
                  <a href={l.permalink} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: gold, fontFamily: C.fontSans }}>↗ Open in Gmail</a>
                  {l.auto_linked && <span style={{ fontSize: 11, color: '#0F6E56', background: '#E6F2EC', border: '1px solid #A0D9C4', borderRadius: 4, padding: '1px 7px', fontFamily: C.fontSans }}>◆ Auto-linked</span>}
                </div>
              </div>
            ); }
            const a = item.a, tag = kindTag[a.kind] || kindTag.update;
            return (
              <div key={a.id} style={{ display: 'flex', gap: 8, padding: '8px 0', borderBottom: `1px solid ${bord}` }}>
                <span style={{ flexShrink: 0, fontSize: '0.52rem', fontWeight: 700, letterSpacing: '0.06em', color: tag.c, background: tag.c + '1A', borderRadius: 6, padding: '2px 6px', height: 'fit-content', fontFamily: C.fontSans }}>{tag.t}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '0.8rem', color: ink, fontFamily: C.fontSans, lineHeight: 1.4, whiteSpace: 'pre-wrap' }}>{a.content}</div>
                  <div style={{ fontSize: '0.64rem', color: sub, marginTop: 2, fontFamily: C.fontSans }}>{a.author_name || 'System'} · {fmt(a.created_at)}</div>
                </div>
              </div>
            );
          })}
          {/* Composer: Log (shared) / AI Assist (private) */}
          <div style={{ marginTop: 14 }}>
            <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
              {[['log', '📝 Log'], ['assist', '✨ AI Assist']].map(([id, label]) => (
                <button key={id} onClick={() => setMode(id)} style={{ flex: 1, padding: '6px 8px', borderRadius: 8, border: `1px solid ${mode === id ? C.navy : bord}`, background: mode === id ? C.navy : (dark ? '#06101F' : '#fff'), color: mode === id ? '#fff' : sub, fontSize: '0.72rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans }}>{label}</button>
              ))}
            </div>
            {mode === 'assist' && (
              <div style={{ border: `1px solid ${bord}`, borderRadius: 10, padding: '8px 10px', marginBottom: 8, background: dark ? '#06101F' : '#FCFBF8' }}>
                <div style={{ ...cap, marginBottom: 6 }}>AI Assist · private to you</div>
                {assist.length === 0 && <div style={{ fontSize: '0.76rem', color: sub, fontFamily: C.fontSans }}>Ask anything about this task — drafts, next steps, summaries. Only you see this.</div>}
                {assist.map(m => (
                  <div key={m.id} style={{ marginBottom: 8 }}>
                    <div style={{ fontSize: '0.58rem', fontWeight: 700, letterSpacing: '0.06em', color: m.role === 'ai' ? gold : sub, fontFamily: C.fontSans, marginBottom: 2 }}>{m.role === 'ai' ? 'AI' : 'YOU'}</div>
                    {editId === m.id ? (
                      <div>
                        <textarea value={editText} onChange={e => setEditText(e.target.value)} rows={3} style={{ ...tinp, width: '100%', resize: 'vertical' }} />
                        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                          <button onClick={() => saveAssistEdit(m.id)} style={{ fontSize: '0.7rem', fontWeight: 600, color: '#fff', background: C.navy, border: 'none', borderRadius: 7, padding: '4px 10px', cursor: 'pointer', fontFamily: C.fontSans }}>Save</button>
                          <button onClick={() => { setEditId(null); setEditText(''); }} style={{ fontSize: '0.7rem', fontWeight: 600, color: sub, background: 'none', border: `1px solid ${bord}`, borderRadius: 7, padding: '4px 10px', cursor: 'pointer', fontFamily: C.fontSans }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <div>
                        <div style={{ fontSize: '0.8rem', color: ink, lineHeight: 1.45, whiteSpace: 'pre-wrap', fontFamily: C.fontSans }}>{m.role === 'ai' ? <MarkdownContent text={m.content} /> : m.content}</div>
                        {m.role === 'ai' && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
                            <span onClick={() => { setEditId(m.id); setEditText(m.content); }} style={{ fontSize: '0.68rem', color: gold, cursor: 'pointer', fontFamily: C.fontSans }}>Edit</span>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.68rem', color: sub, cursor: 'pointer', fontFamily: C.fontSans }}>
                              <input type="checkbox" checked={!!m.published} onChange={() => togglePublish(m)} style={{ accentColor: C.navy, width: 13, height: 13 }} />Publish to thread
                            </label>
                          </div>
                        )}
                      </div>
                ...
                {aiBusy && <div style={{ fontSize: '0.76rem', color: sub, fontStyle: 'italic', fontFamily: C.fontSans }}>AI is thinking…</div>}
              </div>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
              <input value={note} onChange={e => setNote(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { mode === 'assist' ? sendAssist() : addNote(); } }} placeholder={mode === 'assist' ? 'Ask AI about this task…' : 'Add an update…'} style={{ ...tinp, flex: 1, padding: '9px 11px', fontSize: '0.84rem' }} />
              <button onClick={() => mode === 'assist' ? sendAssist() : addNote()} disabled={!note.trim() || busy || aiBusy} style={{ padding: '9px 14px', background: C.navy, color: '#fff', border: 'none', borderRadius: 9, fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans, opacity: !note.trim() ? 0.5 : 1 }}>{mode === 'assist' ? 'Ask' : 'Post'}</button>
            </div>
          </div>
          </React.Fragment>
          ) : (
          <React.Fragment>
          {/* Email tab — Gmail-style reader of the primary linked thread [brief §2.4].
              Read-only mirror; Reply/Reply-all hand off to the real Gmail thread rather
              than building a second inbox [brief §2.6]. */}
          {!primaryLink ? (
            <div style={{ color: sub, fontFamily: C.serif, fontStyle: 'italic', fontSize: 16 }}>No email linked. Attach a thread from the mailbox to see the full conversation here.</div>
          ) : emailBusy ? (
            <div style={{ color: sub, fontSize: '0.82rem', fontFamily: C.fontSans }}>Loading…</div>
          ) : emailErr ? (
            <div style={{ color: sub, fontSize: '0.82rem', fontFamily: C.fontSans }}>{emailErr}</div>
          ) : emailThread ? (
            <React.Fragment>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingBottom: 12, borderBottom: `1px solid ${bord}`, marginBottom: 4 }}>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 500, color: ink, fontFamily: C.fontSans }}>{emailThread.subject}</div>
                  <div style={{ fontSize: 12, color: sub, fontFamily: C.fontSans }}>{emailThread.messages.length} message{emailThread.messages.length > 1 ? 's' : ''} · linked to this task</div>
                </div>
                <a href={emailThread.permalink} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: gold, marginLeft: 'auto', fontFamily: C.fontSans, flexShrink: 0 }}>Open in Gmail ↗</a>
              </div>
              {emailThread.messages.map((m, i) => {
                const open = expandedMsg === m.id;
                const last = i === emailThread.messages.length - 1;
                return (
                  <div key={m.id} style={{ borderBottom: `1px solid ${bord}`, padding: '13px 0' }}>
                    <div onClick={() => setExpandedMsg(open ? null : m.id)} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                      <span style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#c9a45a,#8a6a24)', color: '#fff', fontSize: 11, display: 'grid', placeItems: 'center', flexShrink: 0 }}>{(m.from || '?').trim().charAt(0).toUpperCase()}</span>
                      <div style={{ minWidth: 0 }}>
                        <span style={{ fontSize: 13, fontWeight: 500, color: ink, fontFamily: C.fontSans }}>{m.from}</span>{' '}
                        <span style={{ fontSize: 11, color: sub, fontFamily: C.fontSans }}>{m.to}</span>
                      </div>
                      <span style={{ fontSize: 11, color: sub, marginLeft: 'auto', flexShrink: 0, fontFamily: C.fontSans }}>{m.date}</span>
                    </div>
                    {open && (
                      <div style={{ fontSize: '0.84rem', color: ink, lineHeight: 1.65, padding: '10px 0 0 40px', fontFamily: C.fontSans, whiteSpace: 'pre-wrap' }}>
                        {m.bodyText || m.snippet}
                        {last && (
                          <div style={{ display: 'flex', gap: 9, marginTop: 14 }}>
                            <a href={emailThread.permalink} target="_blank" rel="noopener noreferrer" title="Opens this thread in Gmail" style={{ height: 32, padding: '0 13px', border: `1px solid ${bord}`, borderRadius: 6, fontSize: 12, letterSpacing: '0.05em', textTransform: 'uppercase', color: sub, display: 'inline-flex', alignItems: 'center', gap: 7, fontFamily: C.fontSans, textDecoration: 'none' }}>↩ Reply</a>
                            <a href={emailThread.permalink} target="_blank" rel="noopener noreferrer" title="Opens this thread in Gmail" style={{ height: 32, padding: '0 13px', border: `1px solid ${bord}`, borderRadius: 6, fontSize: 12, letterSpacing: '0.05em', textTransform: 'uppercase', color: sub, display: 'inline-flex', alignItems: 'center', gap: 7, fontFamily: C.fontSans, textDecoration: 'none' }}>↩ Reply all</a>
                            <button onClick={async () => { await TaskDB.createSubtask(task.id, (m.subject || emailThread.subject || 'Follow up').slice(0, 120), user); setSubs(await TaskDB.loadSubtasks(task.id)); }} style={{ height: 32, padding: '0 13px', border: `1px solid ${bord}`, borderRadius: 6, fontSize: 12, letterSpacing: '0.05em', textTransform: 'uppercase', color: sub, display: 'inline-flex', alignItems: 'center', gap: 7, cursor: 'pointer', background: 'none', fontFamily: C.fontSans }}>✎ Turn into subtask</button>
                          </div>
                        )}
                      </div>
                    )}
              ...
              {emLinks.length > 1 && <div style={{ marginTop: 14, fontSize: 12, color: sub, fontFamily: C.fontSans }}>+{emLinks.length - 1} more linked thread(s)</div>}
              <div style={{ fontSize: 12, color: sub, marginTop: 14, fontFamily: C.fontSans }}>Read-only mirror of Gmail. Replies open Gmail with the thread pre-loaded, so the task keeps the record without becoming a second inbox.</div>
            </React.Fragment>
          ) : null}
          </React.Fragment>
          )}
    ...
    function LabelManager({ dark, user, onClose, onSaved }) {
      const J = C.fontSans;
      const uid = user && user.id;
      const [items, setItems] = useState(null);   // [{ name, color, _orig }]
      const [origNames, setOrigNames] = useState([]);
      const [saving, setSaving] = useState(false);
      const ink = dark ? '#fff' : '#001A4A', sub = dark ? 'rgba(255,255,255,0.5)' : '#6B6B6B', bord = dark ? '#152545' : '#E4DFD4';
      const fieldBg = dark ? '#06101F' : '#FFFFFF', rowBg = dark ? '#0A1730' : '#F7F4EE';

      useEffect(() => {
        (async () => {
          let saved = [];
          try { saved = JSON.parse(localStorage.getItem(labelKey(uid)) || '[]') || []; } catch (e) {}
          const names = await TaskDB.myLabelNames(uid);
          const have = {}; saved.forEach(s => { have[s.name] = true; });
          const merged = saved.map(s => ({ name: s.name, color: s.color || LABEL_COLORS[1], _orig: s.name }));
          names.forEach(n => { if (!have[n]) merged.push({ name: n, color: LABEL_COLORS[1], _orig: n }); });
          setItems(merged); setOrigNames(names);
        })();
      }, []);

      const update = (i, patch) => setItems(its => its.map((x, j) => j === i ? { ...x, ...patch } : x));
      const add = () => setItems(its => [...(its || []), { name: '', color: LABEL_COLORS[0], _orig: null }]);
      const remove = (i) => setItems(its => its.filter((_, j) => j !== i));

      async function save() {
        setSaving(true);
        const clean = (items || []).map(x => ({ name: (x.name || '').trim(), color: x.color, _orig: x._orig })).filter(x => x.name);
        const seen = {}, final = [];
        clean.forEach(x => { const k = x.name.toLowerCase(); if (!seen[k]) { seen[k] = 1; final.push(x); } });
        try {
          for (const x of final) { if (x._orig && x._orig !== x.name && origNames.indexOf(x._orig) !== -1) await TaskDB.renameLabel(uid, x._orig, x.name); }
          const keep = new Set(final.map(x => x.name)), keepOrig = new Set(final.map(x => x._orig).filter(Boolean));
          for (const n of origNames) { if (!keep.has(n) && !keepOrig.has(n)) await TaskDB.deleteLabel(uid, n); }
        } catch (e) { console.error('[LabelManager] sync:', e); }
        try { localStorage.setItem(labelKey(uid), JSON.stringify(final.map(x => ({ name: x.name, color: x.color })))); } catch (e) {}
      ...
      return (
        <div style={{ padding: '12px 14px 24px', fontFamily: J }}>
          <div style={{ fontSize: '0.72rem', color: sub, lineHeight: 1.5, marginBottom: 12 }}>Your personal labels — names sync across your tasks, colors are just yours. Each teammate manages their own.</div>
          {items === null ? <div style={{ color: sub, fontSize: '0.82rem', padding: 8 }}>Loading…</div>
            : (<React.Fragment>
              {items.length === 0 && <div style={{ color: sub, fontSize: '0.8rem', padding: '6px 2px 12px' }}>No labels yet — tap “Add label” to create one.</div>}
              {items.map((it, i) => (
                <div key={i} style={{ background: rowBg, border: `1px solid ${bord}`, borderRadius: 12, padding: 10, marginBottom: 9 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    <span style={{ width: 26, height: 26, borderRadius: 8, flexShrink: 0, background: it.color }} />
                    <input value={it.name} onChange={e => update(i, { name: e.target.value })} placeholder="Label name" style={{ flex: 1, minWidth: 0, padding: '8px 11px', background: fieldBg, border: `1px solid ${bord}`, borderRadius: 9, color: ink, fontSize: '0.86rem', outline: 'none', fontFamily: J }} />
                    <i className="ti ti-x" onClick={() => remove(i)} title="Remove" style={{ fontSize: 16, cursor: 'pointer', color: sub, flexShrink: 0 }} />
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 9 }}>
                    {LABEL_COLORS.map(c => { const on = it.color === c; return <button key={c} onClick={() => update(i, { color: c })} aria-label="Pick color" style={{ width: 26, height: 26, borderRadius: 8, cursor: 'pointer', background: c, border: on ? `2px solid ${ink}` : '2px solid transparent', padding: 0 }} />; })}
                  </div>
                </div>
              ))}
              <button onClick={add} style={{ width: '100%', padding: 10, borderRadius: 10, border: `1px dashed ${bord}`, background: 'none', color: sub, cursor: 'pointer', fontFamily: J, fontSize: '0.8rem', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 14 }}><i className="ti ti-plus" style={{ fontSize: 14 }} />Add label</button>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={onClose} style={{ flex: 1, padding: 11, borderRadius: 11, border: `1px solid ${bord}`, background: 'none', color: sub, cursor: 'pointer', fontFamily: J, fontSize: '0.85rem', fontWeight: 600 }}>Cancel</button>
                <button onClick={save} disabled={saving} style={{ flex: 1, padding: 11, borderRadius: 11, border: 'none', background: C.navy, color: '#fff', cursor: saving ? 'default' : 'pointer', fontFamily: J, fontSize: '0.85rem', fontWeight: 600 }}>{saving ? 'Saving…' : 'Save changes'}</button>
              </div>
            </React.Fragment>)}
        </div>
      );
    ...
    function CtcEmailsTab({ dark, user, files, onFiled }) {
      ...
      async function load() {
        ...
        const q = c.from('ctc_emails').select('*').order('email_date', { ascending: false }).limit(200);
      ...
      async function act(payload, id) {
        setBusyId(id); setErr('');
        const { ok, data } = await callCtcEmails(payload);
        setBusyId(null);
        if (!ok) { setErr(data.error || 'That did not go through.'); return false; }
      ...
      async function runTriage() {
        ...
        setTriaging(false);
        if (!ok) { setErr(data.error || 'Triage failed.'); return; }
        load();
      }
      ...
      const chip = (id, label, n) => (
        <div key={id} onClick={() => setFilter(id)} style={{ padding: '6px 12px', borderRadius: 16, fontSize: 12, cursor: 'pointer', fontFamily: C.fontSans, border: `1px solid ${filter === id ? C.navy : bord}`, background: filter === id ? C.navy : 'transparent', color: filter === id ? '#fff' : sub }}>{label}{n != null ? ' · ' + n : ''}</div>
      );
          ...
          {loading ? <div style={{ fontSize: 13, color: sub, fontFamily: C.fontSans }}>Loading emails…</div>
            : !rows.length ? (
              <div style={{ fontSize: 13, color: sub, fontFamily: C.fontSans, lineHeight: 1.6, maxWidth: 620 }}>
                No emails collected yet. Mail is pulled from Sales Agent and Transaction Coordinator mailboxes every 15 minutes — but only for people an admin has switched on <em>and</em> who have connected their own Google account in the app. Nobody's mailbox is read without both.
              </div>
            )
            : !shown.length ? <div style={{ fontSize: 13, color: sub, fontFamily: C.fontSans }}>Nothing here.</div>
            : shown.map(r => {
              const suggested = r.suggested_project_id ? nameOfFile(r.suggested_project_id) : '';
              const filed = r.status === 'approved';
              const busy = busyId === r.id;
              return (
                <div key={r.id} style={{ border: `1px solid ${bord}`, borderRadius: 10, padding: 13, marginBottom: 9, background: dark ? '#0A1730' : '#fff' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 14, fontWeight: 500, color: ink, fontFamily: C.fontSans }}>{r.subject || '(no subject)'}</span>
                    <span style={{ fontSize: 11.5, color: sub, fontFamily: C.fontSans, marginLeft: 'auto' }}>{fmt(r.email_date)}</span>
                  </div>
                  <div style={{ fontSize: 12.5, color: sub, fontFamily: C.fontSans, marginTop: 3 }}>
                    {r.from_name || r.from_addr}{r.from_name && r.from_addr ? ' · ' + r.from_addr : ''} → {r.mailbox_email}
                  </div>
                  {r.snippet && <div style={{ fontSize: 12.5, color: ink, opacity: 0.8, fontFamily: C.fontSans, marginTop: 6, lineHeight: 1.5 }}>{r.snippet}</div>}

                  {filed ? (
                    <div style={{ marginTop: 9, fontSize: 12.5, color: teal, fontFamily: C.fontSans, display: 'flex', alignItems: 'center', gap: 6 }}>
                      <i className="ti ti-check" style={{ fontSize: 13 }} />Filed to {nameOfFile(r.linked_project_id) || 'a file'}
                    </div>
                  ) : r.status === 'rejected' ? (
                    <div style={{ marginTop: 9, fontSize: 12.5, color: sub, fontFamily: C.fontSans }}>Dismissed.</div>
                  ) : r.status === 'no_match' ? (
                    <div style={{ marginTop: 9, fontSize: 12.5, color: sub, fontFamily: C.fontSans }}>No matching CTC file suggested.</div>
                  ) : suggested ? (
                    <div style={{ marginTop: 10, padding: 10, borderRadius: 8, background: dark ? 'rgba(201,164,90,0.10)' : '#F7F4EE', border: `1px solid ${bord}` }}>
                      <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: gold, fontWeight: 600, fontFamily: C.fontSans, marginBottom: 5 }}>
                        Suggested · {suggested}{r.suggested_confidence != null ? ' · ' + Math.round(Number(r.suggested_confidence) * 100) + '%' : ''}
                      </div>
                      {r.suggested_note && <div style={{ fontSize: 12.5, color: ink, fontFamily: C.fontSans, lineHeight: 1.5 }}>{r.suggested_note}</div>}
                      <div style={{ display: 'flex', gap: 8, marginTop: 9 }}>
                        <button disabled={busy} onClick={() => act({ action: 'approve', email_id: r.id }, r.id)} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: C.navy, color: '#fff', fontSize: 12, cursor: busy ? 'default' : 'pointer', fontFamily: C.fontSans, opacity: busy ? 0.6 : 1 }}>{busy ? 'Filing…' : 'File it'}</button>
                        <button disabled={busy} onClick={() => act({ action: 'reject', email_id: r.id }, r.id)} style={{ padding: '6px 12px', borderRadius: 6, border: `1px solid ${bord}`, background: 'none', color: sub, fontSize: 12, cursor: busy ? 'default' : 'pointer', fontFamily: C.fontSans }}>Dismiss</button>
                        {r.permalink && <a href={r.permalink} target="_blank" rel="noreferrer" style={{ padding: '6px 12px', borderRadius: 6, border: `1px solid ${bord}`, color: sub, fontSize: 12, textDecoration: 'none', fontFamily: C.fontSans }}>Open in Gmail</a>}
                      </div>
                    </div>
                  ) : (
                    <div style={{ marginTop: 9, display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span style={{ fontSize: 12.5, color: sub, fontFamily: C.fontSans }}>Not looked at yet.</span>
                      {r.permalink && <a href={r.permalink} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: gold, textDecoration: 'none', fontFamily: C.fontSans }}>Open in Gmail</a>}
                    </div>
                  )}
                </div>
              );
    ...
    function ZohoLinkModal({ dark, project, onClose, onLinked }) {
      ...
      useEffect(() => {
        let on = true;
        (async () => {
          const { ok, data } = await callZohoProjects({ action: 'list_projects' });
          if (!on) return;
          if (!ok) {
            if (/not configured/i.test(data.error || '')) setState('not_configured');
            else { setErrMsg(data.error || 'Could not reach Zoho Projects.'); setState('error'); }
            return;
          }
      ...
      async function link(zp) {
        setState('linking');
        try {
          const tl = await callZohoProjects({ action: 'list_tasklists', project_id: zp.id });
          let tasklistId = tl.ok && tl.data.tasklists && tl.data.tasklists[0] ? tl.data.tasklists[0].id : null;
          if (!tasklistId) {
            const created = await callZohoProjects({ action: 'create_tasklist', project_id: zp.id, name: 'TMG Tasks' });
            if (created.ok) tasklistId = created.data.id;
          }
          await ProjectDB.update(project.id, { zoho_project_id: zp.id, zoho_tasklist_id: tasklistId || null, zoho_sync_enabled: true });
          onLinked();
        } catch (e) {
      ...
      return (
        <React.Fragment>
          <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(0,0,0,0.5)' }} />
          <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 81, maxHeight: '80%', background: dark ? '#0A1730' : '#FFFFFF', borderTopLeftRadius: 20, borderTopRightRadius: 20, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: `1px solid ${bord}` }}>
              <span style={{ fontFamily: C.fontSans, fontSize: 15, fontWeight: 600, color: ink }}>Link to Zoho Projects</span>
              <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: sub, fontSize: 18, display: 'flex' }}><i className="ti ti-x" /></button>
            </div>
            <div style={{ padding: 16, overflowY: 'auto' }}>
              {state === 'loading' && <div style={{ fontFamily: C.fontSans, fontSize: 13.5, color: sub }}>Loading your Zoho Projects…</div>}
              {state === 'not_configured' && (
                <div style={{ fontFamily: C.fontSans, fontSize: 13.5, color: sub, lineHeight: 1.6 }}>
                  Zoho Projects isn't connected yet — an admin needs to set up the connection first (one-time setup, done outside the app). Ask Symon.
                </div>
              )}
              {state === 'error' && <div style={{ fontFamily: C.fontSans, fontSize: 13.5, color: '#9B1C1C' }}>{errMsg}</div>}
              {state === 'linking' && <div style={{ fontFamily: C.fontSans, fontSize: 13.5, color: sub }}>Linking…</div>}
              {state === 'list' && (
                <React.Fragment>
                  <input autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder="Search Zoho projects…" style={{ width: '100%', padding: '10px 12px', border: `1px solid ${bord}`, borderRadius: 8, background: dark ? '#06101F' : '#fff', color: ink, fontSize: 14, outline: 'none', fontFamily: C.fontSans, marginBottom: 10, boxSizing: 'border-box' }} />
                  {errMsg && <div style={{ fontFamily: C.fontSans, fontSize: 12.5, color: '#9B1C1C', marginBottom: 8 }}>{errMsg}</div>}
                  {filtered.length === 0 && <div style={{ fontFamily: C.fontSans, fontSize: 13, color: sub }}>No matching Zoho projects.</div>}
                  {filtered.map(zp => (
                    <div key={zp.id} onClick={() => link(zp)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 4px', borderBottom: `1px solid ${bord}`, cursor: 'pointer', fontFamily: C.fontSans, fontSize: 13.5, color: ink }}>
                      {zp.name}
                      <i className="ti ti-link" style={{ fontSize: 14, color: C.gold }} />
                    </div>
                  ))}
                </React.Fragment>
              )}
    ...
    function ProjectsSurface({ kind, dark, user, team, titleNode, hideSidebar, openId, onOpenIdConsumed, openCtcTab, onCtcTabConsumed, onChanged }) {
      ...
      const storeKey = { project: 'tmg-projects', ctc_file: 'tmg-ctc', rock: 'tmg-rocks' }[kind] || 'tmg-projects';
      ...
      useEffect(() => {
        let on = true;
        if (!current || !current.id) { setUpdates([]); return; }
        const c = window.SupabaseAuth?._client; if (!c) return;
        c.from('project_updates').select('id,body,source,created_at').eq('project_id', current.id)
      ...
      useEffect(() => { localStorage.setItem(sKey, sort); }, [sort]);

      const bord = dark ? '#152545' : '#E4DFD4', ink = dark ? '#fff' : '#001A4A', sub = dark ? 'rgba(255,255,255,0.5)' : '#6B6B6B', gold = dark ? '#C9A45A' : '#AD832F', bg = dark ? '#000D26' : '#FCFBF8', card = dark ? '#0A1730' : '#fff';
      const lateColor = dark ? '#E07A7A' : '#9B1C1C', teal = dark ? '#7FC6AF' : '#0F6E56', blue = dark ? '#8FB6DC' : '#185FA5', dotIdle = dark ? 'rgba(255,255,255,.24)' : '#C4B9A8';

      async function reload() { setLoading(true); const r = await ProjectDB.loadFull(kind); setRows(r); setLoading(false); if (onChanged) onChanged(); return r; }
      useEffect(() => { setPview('list'); setCurrent(null); reload(); }, [kind]);
      useEffect(() => { if (pview !== 'list') setRouteMiss(null); }, [pview]);
      ...
      useEffect(() => {
        ...
        if (openId === 'new') { setCurrent(null); setPview('form'); }
          ...
          else { setPview('list'); setRouteMiss({ id: openId }); PENDING_ROUTE.id = null; PENDING_ROUTE.tab = null; }
        }
      ...
      async function openTaskFull(t) {
        ...
        // Apply even when the row re-read failed: keeping _people undefined
        // there left Edit stuck on "Loading…" with no way to retry. null means
        // "we tried and couldn't" — the form opens, and Save re-checks before
        // touching anyone.
        setOpenTask(prev => (prev && prev.id === t.id)
          ? { ...(full || prev), _people: (ppl == null ? (prev._people === undefined ? null : prev._people) : ppl), _projectName: t._projectName || prev._projectName || ((current && full && full.project_id === current.id) ? current.name : '') }
          : prev);
      }
      async function onTaskStatus(task, status) {
        const was = task.status;
        const paint = (st) => {
          setCurrent(p => (p && p._tasks) ? { ...p, _tasks: p._tasks.map(x => x.id === task.id ? { ...x, status: st } : x) } : p);
          setOpenTask(o => (o && o.id === task.id) ? { ...o, status: st } : o);
        };
        paint(status);   // same reason as My Tasks: don't make them wait to see the tick
        try { await TaskDB.update(task.id, task, { status }, user); }
      ...
      async function onTaskSave(form) {
        ...
        const fields = { title: form.title, due_at, project_id, priority: form.priority, status: form.status, description: form.description || null, context: form.context || null, working_url: form.working_url || null, email_link: form.email_link || null, decision_question: form.decisionQuestion || null, decision_due_at, decision_due_has_time: !!(form.decisionDue && form.decisionHasTime), recurrence: form.recurrence || 'none', recur_interval: form.recurInterval || null, recur_unit: form.recurUnit || null, recur_copy_fields: form.recurCopy || null, weekly_priority: pl.wp || null, weekly_rank: pl.wr ? parseInt(pl.wr, 10) : null, daily_priority: pl.dp || null, daily_rank: pl.dr ? parseInt(pl.dr, 10) : null, is_milestone: !!form.is_milestone };
        const people = { assignee: form.assignees, assigner: form.assigners, decision_maker: form.decisionMakers || [] };
        let taskId;
      ...
      async function addToTasklist(tasklistId, tasklistName) {
        const g = tlAdd; if (!g) return;
        const title = (g.val || '').trim(); if (!title) return;
        ...
        try {
          const made = await TaskDB.create({ title, status: 'todo', priority: 'medium', project_id: current ? current.id : null,
            zoho_tasklist_id: tasklistId, zoho_tasklist_name: tasklistName },
            who ? { assignee: [who.id] } : {}, user);
        ...
        catch (e) { setTlAdd(x => (x && x.key === g.key) ? { ...x, err: 'Could not add this: ' + ((e && e.message) || 'the save failed') + '. It is still here, try again.' } : x); return; }
        finally { tlAddBusy.current = false; }
        setTlAdd(x => (x && x.key === g.key) ? { ...x, val: '', err: null } : x);
        await refreshContainer(null, false);
      }

      const nameOf = (id) => (team.find(m => m.id === id) || {}).name || '';
      const initialsOf = (id) => { const n = nameOf(id); return n ? n.trim().split(/\s+/).map(w => w[0]).slice(0, 2).join('').toUpperCase() : '?'; };
      ...
      const fmtD = (d) => d ? parseD(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—';
      ...
      const heroColor = (p) => { const a = attentionOf(p); return a.tone === 'red' ? lateColor : (a.text === 'Clear' ? teal : blue); };

      // ── shared header chips (mirror the My Tasks chip system) ──
      const chipStyle = { display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 400, padding: '7px 12px', border: `1px solid ${bord}`, borderRadius: 6, background: card, color: sub, whiteSpace: 'nowrap', cursor: 'pointer', fontFamily: C.fontSans };
      const ddStyle = { position: 'absolute', zIndex: 60, top: '100%', right: 0, background: card, border: `1px solid ${bord}`, borderRadius: 8, boxShadow: '0 12px 32px rgba(0,26,74,0.16)', padding: 6, minWidth: 190, marginTop: 4 };
      // .menu button ported: 8px/10px padding, 13.5px font, checkmark suffix
      const ddItem = (label, active, onClick, key) => (
        <div key={key || label} onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '8px 10px', fontSize: 13.5, color: active ? gold : ink, fontWeight: active ? 500 : 400, borderRadius: 5, cursor: 'pointer', fontFamily: C.fontSans }}>
          {label}{active && '✓'}
        </div>
      );
      ...
      const chipOn = { borderColor: dark ? 'rgba(201,164,90,.4)' : '#C9A45A', color: ink, background: dark ? 'rgba(201,164,90,.14)' : '#F3EBDA' };
      ...
      const groupChip = (
        <div ref={groupRef} style={{ position: 'relative' }}>
          <div onClick={() => { setGroupOpen(o => !o); setSortOpen(false); }} style={group !== 'none' ? { ...chipStyle, ...chipOn } : chipStyle}>Group: <span style={{ color: gold, fontWeight: 600 }}>{(GROUP_OPTS.find(([id]) => id === group) || [])[1]}</span> <i className="ti ti-chevron-down" style={{ fontSize: 9, color: sub }} /></div>
          {groupOpen && <div style={ddStyle} onClick={e => e.stopPropagation()}>{GROUP_OPTS.map(([id, label]) => ddItem(label, group === id, () => { setGroup(id); setGroupOpen(false); }, id))}</div>}
        </div>
      );
      const sortChip = (
        <div ref={sortRef} style={{ position: 'relative' }}>
          <div onClick={() => { setSortOpen(o => !o); setGroupOpen(false); }} style={sort !== 'target' ? { ...chipStyle, ...chipOn } : chipStyle}>Sort: <span style={{ color: gold, fontWeight: 600 }}>{(SORT_OPTS.find(([id]) => id === sort) || [])[1]}</span> <i className="ti ti-chevron-down" style={{ fontSize: 9, color: sub }} /></div>
          {sortOpen && <div style={ddStyle} onClick={e => e.stopPropagation()}>{SORT_OPTS.map(([id, label]) => ddItem(label, sort === id, () => { setSort(id); setSortOpen(false); }, id))}</div>}
        </div>
      );
      const searchChip = searchOpen
        ? <input autoFocus value={search} onChange={e => setSearch(e.target.value)} onBlur={() => { if (!search) setSearchOpen(false); }} placeholder="Search…" style={{ ...chipStyle, cursor: 'text', width: 130, textTransform: 'none', letterSpacing: 'normal', fontSize: '0.78rem', outline: 'none' }} />
        : <div onClick={() => setSearchOpen(true)} style={{ ...chipStyle, width: 34, height: 34, padding: 0, justifyContent: 'center' }}><i className="ti ti-search" style={{ fontSize: 13 }} /></div>;
      const newBtn = <button onClick={() => { setCurrent(null); setPview('form'); }} aria-label={'New ' + KL.article} style={{ display: 'flex', alignItems: 'center', gap: 6, background: C.navy, color: '#fff', border: 'none', borderRadius: 22, padding: '8px 16px', fontSize: 13, fontWeight: 500, letterSpacing: '0.02em', cursor: 'pointer', fontFamily: C.fontSans, flexShrink: 0 }}><i className="ti ti-plus" style={{ fontSize: 14 }} />New</button>;

      const avatar = (id, sz) => { const s = sz || 16; return <span style={{ width: s, height: s, borderRadius: '50%', background: C.navy, color: '#fff', fontSize: s * 0.42, display: 'grid', placeItems: 'center', fontWeight: 600, flexShrink: 0 }}>{initialsOf(id)}</span>; };
      // Matches the prototype's .pill/.stat treatment (e.g. container header's <span class="pill st-done" style="font-size:11px">)
      const statusBadge = (s) => { const m = projStatusMeta(s); return <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '3px 9px', borderRadius: 20, border: `1px solid ${m.color}55`, background: dark ? m.bgD : m.bg, color: m.color, fontFamily: C.fontSans }}>{m.label}</span>; };
      ...
      const sortRows = (arr) => arr.slice().sort((a, b) => {
        ...
        if (!a.target_date && !b.target_date) return 0; if (!a.target_date) return 1; if (!b.target_date) return -1;
      ...
      function grouped() {
        ...
        filtered.forEach(p => {
          let key;
          if (group === 'status') key = projStatusMeta(p.status).label;
          else if (group === 'group') key = p.group_tag || 'No group';
          else if (group === 'agent') key = nameOf(p.agent_id) || 'Unassigned';
          else key = nameOf(p.owner_id) || 'Unassigned';
          (map[key] = map[key] || []).push(p);
        });
        return Object.keys(map).sort((a, b) => a.localeCompare(b)).map(k => [k, sortRows(map[k])]);
      }
      const groupHeadRow = (label, count, color) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '16px 0 8px' }}>
          {color && <span style={{ width: 8, height: 8, borderRadius: '50%', background: color, flexShrink: 0 }} />}
          <span style={{ fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontFamily: C.fontSans }}>{label}</span>
          <span style={{ fontSize: '0.66rem', color: gold, fontFamily: C.fontSans }}>{count}</span>
      ...
      // ── mobile project card ──
      const projCard = (p) => (
        <div key={p.id} onClick={() => { setCurrent(p); setPview('detail'); }} style={{ background: card, border: `1px solid ${bord}`, borderRadius: 6, padding: 14, marginBottom: 11, cursor: 'pointer' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 9 }}>
            {statusBadge(p.status)}
            {p.group_tag && <span style={{ fontSize: 11, letterSpacing: '0.14em', color: sub, fontWeight: 500, textTransform: 'uppercase', fontFamily: C.fontSans }}>{p.group_tag}</span>}
          </div>
          <div style={{ fontSize: 16, fontWeight: 500, color: ink, lineHeight: 1.3, marginBottom: p.outcome ? 5 : 10, fontFamily: C.fontSans }}>{p.name}</div>
          {p.outcome && <div style={{ fontSize: 12, color: sub, fontWeight: 300, lineHeight: 1.5, marginBottom: 12, fontFamily: C.fontSans }}>{p.outcome}</div>}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 10.5, color: sub, fontFamily: C.fontSans }}>
            <span>{p._doneMs} of {p._totalMs} milestones</span>
            {p.owner_id ? avatar(p.owner_id, 18) : null}
          </div>
        </div>
      );
      ...
      const projRow = (p) => (
        <div key={p.id} onClick={() => { setCurrent(p); setPview('detail'); }} style={{ display: 'flex', alignItems: 'center', padding: '11px 12px', borderBottom: `1px solid ${bord}`, gap: 12, cursor: 'pointer' }}>
          <div style={{ width: 250, flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 500, color: ink, fontFamily: C.fontSans }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: projStatusMeta(p.status).color, flexShrink: 0 }} /><span style={{ minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.name}</span></div>
            {p.outcome && <div style={{ fontSize: 11, color: sub, fontWeight: 300, marginTop: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: C.fontSans }}>{p.outcome}</div>}
          </div>
          <div style={{ flex: 1, minWidth: 0, fontSize: 11.5, color: ink, fontFamily: C.fontSans }}>
            <div style={{ fontSize: 9, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontWeight: 600, marginBottom: 3 }}>{projStatusMeta(p.status).label}</div>
            <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'block' }}>{p._nextMs ? p._nextMs.title + (p._nextMs.due_at ? ' · ' + fmtD(p._nextMs.due_at) : '') : '—'}</span>
          </div>
          <div style={{ width: 150, flexShrink: 0 }}>
            <div style={{ height: 8, background: bord, borderRadius: 20, marginBottom: 6, overflow: 'hidden' }}><div style={{ height: '100%', width: p._progress + '%', background: `linear-gradient(90deg, #C9A45A, #AD832F)`, borderRadius: 20 }} /></div>
            <div style={{ fontSize: 10, color: sub, fontFamily: C.fontSans }}>{p._doneMs} of {p._totalMs} milestones</div>
          </div>
          <div style={{ width: 110, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6, fontSize: 10.5, color: sub, fontFamily: C.fontSans }}>{p.owner_id ? <React.Fragment>{avatar(p.owner_id, 16)}{nameOf(p.owner_id).split(' ')[0]}</React.Fragment> : '—'}</div>
          <div style={{ width: 96, flexShrink: 0, textAlign: 'right', fontSize: 10.5, color: isOverdue(p.target_date) ? lateColor : sub, fontWeight: isOverdue(p.target_date) ? 500 : 400, fontFamily: C.fontSans }}>{fmtD(p.target_date)}</div>
        </div>
      );
      ...
      const ctcCard = (p) => {
        ...
        return (
          <div key={p.id} onClick={() => { setCurrent(p); setPview('detail'); }} style={{ background: card, border: `1px solid ${bord}`, borderLeft: `2.5px solid ${hc}`, borderRadius: 6, padding: 14, marginBottom: 11, cursor: 'pointer' }}>
            <div style={{ fontSize: '0.95rem', fontWeight: 600, color: ink, lineHeight: 1.25, marginBottom: 8, fontFamily: C.fontSans }}>{p.name}</div>
            {/* The person on a CTC card is the AGENT (agent_id), not the Owner/TC —
                the column header says Agent, and the 28 imported files have
                Agent set but Owner mostly blank (audit C13). No owner fallback. */}
            {(p.group_tag || p.agent_id) && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                ...
                {p.agent_id && <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.7rem', color: sub, fontFamily: C.fontSans }}>{avatar(p.agent_id, 16)}{nameOf(p.agent_id)}</span>}
              </div>
            )}
            {p._nextMs && (
              <div style={{ borderRadius: 5, padding: '9px 11px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, background: heroBg, border: `1px solid ${bord}` }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: '0.56rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: sub, fontWeight: 500, marginBottom: 3, fontFamily: C.fontSans }}>Next binding date</div>
                  <div style={{ fontSize: '0.76rem', fontWeight: 500, color: ink, fontFamily: C.fontSans, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p._nextMs.title}</div>
                </div>
                <div style={{ flexShrink: 0, textAlign: 'right', marginLeft: 10 }}>
                  <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '1.5rem', lineHeight: 1, color: hc }}>{daysTo(p._nextMs.due_at)}</div>
                  <div style={{ fontSize: '0.5rem', fontFamily: C.fontSans, letterSpacing: '0.1em', textTransform: 'uppercase', color: sub, marginTop: 3 }}>days</div>
                </div>
              </div>
            )}
            {p._milestones.length > 0 && <div style={{ display: 'flex', overflowX: 'auto', gap: 8, marginBottom: 12, paddingBottom: 2 }}>{p._milestones.map(m => msRailNode(p, m))}</div>}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTop: `1px solid ${bord}`, fontSize: '0.7rem', fontFamily: C.fontSans }}>
              <span style={{ color: att.tone === 'red' ? lateColor : sub, fontWeight: att.tone === 'red' ? 600 : 400 }}>{att.text}</span>
              <span style={{ color: sub }}>{p._doneMs} of {p._totalMs} milestones</span>
            </div>
          </div>
        );
      };
      ...
      const ctcRow = (p) => {
        const att = attentionOf(p);
        return (
          <div key={p.id} onClick={() => { setCurrent(p); setPview('detail'); }} style={{ display: 'flex', alignItems: 'center', padding: '10px 18px', borderBottom: `1px solid ${bord}`, gap: 12, cursor: 'pointer' }}>
            ...
            <div style={{ width: 146, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
              {p._nextMs ? (
                <React.Fragment>
                  <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '1.3rem', lineHeight: 1, color: heroColor(p) }}>{daysTo(p._nextMs.due_at)}</div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: '0.56rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: sub, fontWeight: 600, marginBottom: 2, fontFamily: C.fontSans }}>Days</div>
                    <div style={{ fontSize: '0.68rem', color: ink, fontFamily: C.fontSans, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p._nextMs.title}</div>
                  </div>
                </React.Fragment>
              ) : <span style={{ fontSize: '0.72rem', color: sub, fontFamily: C.fontSans }}>No dates set</span>}
            </div>
            <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'flex-start', gap: 8, overflow: 'hidden' }}>{p._milestones.map(m => msRailNode(p, m))}</div>
            <div style={{ width: 96, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.72rem', color: sub, fontFamily: C.fontSans }}>{p.agent_id ? <React.Fragment>{avatar(p.agent_id, 16)}{nameOf(p.agent_id).split(' ')[0]}</React.Fragment> : '—'}</div>
            <div style={{ width: 96, flexShrink: 0, textAlign: 'right', fontSize: '0.68rem', fontWeight: att.tone === 'red' ? 600 : 400, color: att.tone === 'red' ? lateColor : sub, fontFamily: C.fontSans }}>{att.text}</div>
          </div>
        );
      };

      // ── list body ──
      const routeMissBar = routeMiss && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: wide ? '12px 34px 0' : '10px 14px 0', padding: '9px 12px', borderRadius: 6, border: `1px solid ${bord}`, background: dark ? 'rgba(155,28,28,.14)' : '#FBF0F0', color: dark ? '#F08A8A' : '#9B1C1C', fontSize: '0.78rem', fontFamily: C.fontSans }}>
          <i className="ti ti-alert-circle" style={{ fontSize: 15, flexShrink: 0 }} />
          <span style={{ flex: 1 }}>That {KL.singular.toLowerCase()} is archived or no longer exists — the link you followed doesn't point anywhere.</span>
          <i className="ti ti-x" onClick={() => setRouteMiss(null)} aria-label="Dismiss" style={{ fontSize: 14, cursor: 'pointer', flexShrink: 0 }} />
        </div>
      );
      const listBody = (rowFn, wideList) => {
        if (loading) return <div style={{ color: sub, fontSize: '0.82rem', padding: 20, fontFamily: C.fontSans }}>Loading…</div>;
        if (!filtered.length) return <React.Fragment>{routeMissBar}<div style={{ color: sub, fontSize: '0.82rem', padding: 24, textAlign: 'center', fontFamily: C.fontSans }}>No {KL.plural.toLowerCase()} yet. Tap <b>New</b> to add one.</div></React.Fragment>;
        return (
          <React.Fragment>
            {routeMissBar}
            {grouped().map(([label, items], gi) => (
              <div key={label || gi}>
                {label && groupHeadRow(label, items.length, group === 'status' ? projStatusMeta((items[0] || {}).status).color : gold)}
                {items.map(rowFn)}
              </div>
            ))}
          </React.Fragment>
      ...
      // ── DETAIL ──
      const spineItem = (p, m, last) => { const st = msState(p, m); return (
        <div key={m.id} style={{ display: 'flex', gap: 12, padding: '0 0 14px', position: 'relative' }}>
      ...
      const detailTaskRow = (t, isChild, guessedList) => (
      ...
      async function syncNow(p) {
        setSyncingNow(true);
        try { await callZohoProjects({ action: 'sync_now', project_id: p.id }); await refreshContainer(); }
        finally { setSyncingNow(false); }
      }

      const detailFields = (p) => {
        const fld = (k, v) => <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${bord}`, fontSize: 13.5, fontFamily: C.fontSans, gap: 10 }}><span style={{ color: sub, letterSpacing: '0.1em', textTransform: 'uppercase', fontSize: 11, flexShrink: 0 }}>{k}</span><span style={{ color: ink, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 7, textAlign: 'right' }}>{v}</span></div>;
        return (
          <React.Fragment>
              ...
              <React.Fragment>
                ...
                {fld('Last synced', (
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <i onClick={() => !syncingNow && syncNow(p)} className={syncingNow ? 'ti ti-loader-2' : 'ti ti-refresh'} title="Sync now" style={{ fontSize: 14, color: gold, cursor: syncingNow ? 'default' : 'pointer', animation: syncingNow ? 'spin 1s linear infinite' : 'none' }} />
                  </span>
                ))}
              </React.Fragment>
            )}
      ...
      const taskDetailOverlay = () => (
        <React.Fragment>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 20px', borderBottom: `1px solid ${bord}`, flexShrink: 0, background: dark ? '#0A1730' : '#fff' }}>
            <i className="ti ti-chevron-left" onClick={() => { setOpenTask(null); setTaskEditing(false); }} style={{ fontSize: 16, cursor: 'pointer', color: sub, flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: 0, fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontFamily: C.fontSans, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{(current && current.name) || KL.plural} / {taskEditing ? 'Edit task' : 'Task'}</div>
          </div>
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
            {taskEditing
      ...
      const dtabBar = (
        <div style={{ display: 'flex', gap: 2, padding: wide ? '16px 34px 0' : '0 14px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
          {[['overview', 'Overview'], ['list', 'List'], ['board', 'Board'], ['timeline', 'Timeline']].map(([id, label]) => (
            <div key={id} onClick={() => setDtab(id)} style={{ padding: '10px 16px', fontSize: 13, color: dtab === id ? ink : sub, fontWeight: dtab === id ? 500 : 400, borderBottom: `2px solid ${dtab === id ? gold : 'transparent'}`, cursor: 'pointer', fontFamily: C.fontSans, marginBottom: -1 }}>{label}</div>
          ))}
        </div>
      );
      ...
      const listTab = (p) => {
        ...
        if (tasklistNames) {
          ...
          const zohoOffNote = (zohoSyncable && p.zoho_sync_enabled && p.zoho_project_id) ? null
            : (zohoSyncable && p.zoho_project_id) ? 'Zoho syncing is paused for this file, so this is saved here only. It will not reach Zoho until syncing is resumed.'
            : 'This file is not linked to Zoho, so this is saved here only.';
          ...
          const listId = {};
          p._tasks.forEach(t => {
            const n = t.zoho_tasklist_name; if (!n) return;
          ...
          Object.keys(listSize).forEach(n => {
            ...
            if (!cur || listSize[n] > listSize[cur] ||
                (listSize[n] === listSize[cur] && String(listId[n] || n) < String(listId[cur] || cur))) listForMember[m.id] = n;
          });
          ...
          p._tasks.filter(t => !isChildHere(t)).forEach(t => {
            ...
            if (!t.zoho_tasklist_name && perPerson) {
              ...
              const mine = (t._people || []).filter(x => x.role === 'assignee')
                .map(x => listForMember[x.user_id]).filter(Boolean)
                .sort((a, b) => String(listId[a] || a) < String(listId[b] || b) ? -1 : 1)[0];
              if (mine) { key = mine; guessed[t.id] = true; }
            }
                ...
                <div key={key} style={{ marginBottom: 16 }}>
                  <div data-drag-kind="tasklist" data-drag-id={gkey(key)}
                    ...
                    {/* No pointerdown handler: a drag starts on the grip's, so
                        ...
                        keyboard-operable since it was written. */}
                    {tlId && <i className={'ti ti-' + (open ? 'x' : 'plus')} title={hint} aria-label={hint}
                  ...
                  {open && (
                    <div style={{ padding: '0 0 10px' }}>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <input autoFocus value={tlAdd.val} onChange={e => setTlAdd(g => g ? { ...g, val: e.target.value, err: null } : g)}
                          onKeyDown={e => { if (e.key === 'Enter') addToTasklist(tlId, key); else if (e.key === 'Escape') setTlAdd(null); }}
                          placeholder={hint + ' and press Enter…'}
                          style={{ flex: 1, padding: '9px 12px', background: dark ? '#0A1730' : '#fff', border: `1px solid ${tlAdd.err ? lateColor : bord}`, borderRadius: 6, color: ink, fontSize: 14, outline: 'none', fontFamily: C.fontSans }} />
                      </div>
                      {tlAdd.err && <div style={{ marginTop: 5, fontSize: '0.68rem', color: lateColor, fontFamily: C.fontSans }}>{tlAdd.err}</div>}
      ...
      const detailView = () => {
        ...
        const spine = (
          <React.Fragment>
            <div style={{ fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontWeight: 600, margin: '22px 0 8px' }}>Milestones</div>
            {p._milestones.length === 0 && <div style={{ fontSize: '0.76rem', color: sub, fontFamily: C.fontSans, marginBottom: 6 }}>No milestones yet. Tick Milestone on any task, new or existing.</div>}
            <div style={{ paddingLeft: 2 }}>{p._milestones.map((m, i) => spineItem(p, m, i === p._milestones.length - 1))}</div>
          </React.Fragment>
        );
          ...
          <React.Fragment>
            ...
            {addOpen && (
              <div style={{ marginTop: 12, padding: 12, borderRadius: 8, border: `1px solid ${bord}`, background: dark ? 'rgba(255,255,255,.03)' : '#FAF8F4' }}>
                <div ref={pSugRef} style={{ position: 'relative', marginBottom: 8 }}>
        ...
        const tasksBlock = (
          <React.Fragment>
            ...
            {updates.length > 0 && (
              <React.Fragment>
                <div style={{ fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontWeight: 600, margin: '22px 0 8px' }}>Updates · {updates.length}</div>
                <div style={{ borderTop: `1px solid ${bord}` }}>
                  {updates.map(u => (
                    <div key={u.id} style={{ padding: '10px 0', borderBottom: `1px solid ${bord}` }}>
                      <div style={{ fontSize: 13.5, color: ink, fontFamily: C.fontSans, lineHeight: 1.55 }}>{u.body}</div>
                      <div style={{ fontSize: 11, color: sub, fontFamily: C.fontSans, marginTop: 3 }}>{u.source === 'email' ? 'From email' : u.source} · {fmtD(u.created_at)}</div>
                    </div>
                  ))}
                </div>
              </React.Fragment>
        ...
        const header = (
          <div style={{ padding: wide ? '26px 34px 0' : '12px 14px', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', color: sub, fontWeight: 500, marginBottom: 10, fontFamily: C.fontSans, cursor: 'pointer' }} onClick={() => { setPview('list'); setCurrent(null); }}>
              <i className="ti ti-chevron-left" style={{ fontSize: 14, color: gold }} />{KL.plural}
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 6 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                {/* Add task sits beside Link because that is where the eye
                    already is on this screen. Same box as Link, filled, since
                    it is the one thing people come here to do. */}
                <button onClick={() => setNewTaskOpen({})} title={'Add a task to this ' + KL.singular.toLowerCase()} style={{ fontSize: 12, fontWeight: 400, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#fff', background: C.navy, border: `1px solid ${C.navy}`, borderRadius: 6, padding: '7px 12px', cursor: 'pointer', fontFamily: C.fontSans, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            {p.outcome && <div style={{ fontSize: 13.5, color: sub, marginBottom: 6, fontFamily: C.fontSans }}>{p.outcome}</div>}
          </div>
        );
        const mainCol = (
          <div style={{ padding: wide ? '18px 34px' : '14px', flex: 1, minWidth: 0 }}>
            <div style={{ height: 8, background: bord, borderRadius: 20, maxWidth: 560, marginBottom: 8, overflow: 'hidden' }}><div style={{ height: '100%', width: p._progress + '%', background: `linear-gradient(90deg, #C9A45A, #AD832F)`, borderRadius: 20 }} /></div>
            <div style={{ fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase', color: sub, marginBottom: 18, fontFamily: C.fontSans }}>{p._doneMs} of {p._totalMs} milestones · {p._progress}% done</div>
            {spine}
            {tasksBlock}
          </div>
        );
          ...
          <React.Fragment>
            ...
            {dtab === 'overview' ? (
              <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
                <div style={{ flex: 1, minWidth: 0, overflowY: 'auto' }}>{mainCol}</div>
                <div style={{ width: 260, flexShrink: 0, borderLeft: `1px solid ${bord}`, padding: '16px 18px', overflowY: 'auto' }}>
                  <div style={{ fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontWeight: 600, marginBottom: 6, fontFamily: C.fontSans }}>Details</div>
                  {detailFields(p)}
                </div>
              </div>
            ...
            {dtab === 'overview' ? (
              <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                ...
                <div style={{ padding: '0 14px 24px' }}>
                  <div style={{ fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontWeight: 600, margin: '8px 0 6px', fontFamily: C.fontSans }}>Details</div>
                  {detailFields(p)}
                </div>
              </div>
            ) : dtab === 'list' ? listTab(p) : dtab === 'board' ? boardTab(p) : timelineTab(p)}
      ...
      // ── FORM (create / edit) ──
      const formView = () => <ProjectForm kind={kind} dark={dark} team={team} user={user} project={current} onCancel={() => setPview(current ? 'detail' : 'list')} onSaved={async (savedId) => { const fresh = await reload(); const sel = fresh.find(p => p.id === savedId); setCurrent(sel || null); setPview(sel ? 'detail' : 'list'); }} onArchived={async () => { await reload(); setCurrent(null); setPview('list'); }} />;

      // ── list header ──
      const listHeader = (
        <div style={{ padding: wide ? '26px 34px 0' : '12px 14px 0', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: wide ? 'flex-start' : 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 6, flexWrap: 'wrap' }}>
            {titleNode ? titleNode(wide ? undefined : 15) : <span style={{ fontSize: wide ? 27 : 15, fontWeight: wide ? 500 : 600, color: ink, fontFamily: C.fontSans }}>{KL.plural}</span>}
            <div style={{ display: 'flex', alignItems: 'center', gap: wide ? 8 : 5, flexWrap: 'wrap' }}>{groupChip}{sortChip}{searchChip}{newBtn}</div>
          </div>
        </div>
      );
      ...
      const theadRow = (
        <div style={{ display: 'flex', alignItems: 'center', padding: '0 34px 8px', fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 500, color: sub, borderBottom: `1px solid ${bord}`, gap: 12, fontFamily: C.fontSans }}>
          {isCtc ? (
            <React.Fragment>
              <span style={{ width: 224, flexShrink: 0 }}>Property</span>
              <span style={{ width: 146, flexShrink: 0 }}>Next binding date</span>
              <span style={{ flex: 1 }}>Milestone chain</span>
              <span style={{ width: 96, flexShrink: 0 }}>Agent</span>
              <span style={{ width: 96, flexShrink: 0, textAlign: 'right' }}>Attention</span>
            </React.Fragment>
          ) : (
            <React.Fragment>
              <span style={{ width: 250, flexShrink: 0 }}>{KL.singular} &amp; outcome</span>
              <span style={{ flex: 1 }}>Next milestone</span>
              <span style={{ width: 150, flexShrink: 0 }}>Progress</span>
              <span style={{ width: 110, flexShrink: 0 }}>Owner</span>
              <span style={{ width: 96, flexShrink: 0, textAlign: 'right' }}>Target</span>
            </React.Fragment>
          )}
        </div>
      );
      ...
      const ctcTabBar = isCtc && pview === 'list' ? (
        <div style={{ display: 'flex', gap: 2, padding: wide ? '0 34px' : '0 14px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
          {[['files', KL.plural], ['emails', 'Emails']].map(([id, label]) => (
            <div key={id} onClick={() => setCtcTab(id)} style={{ padding: '10px 16px', fontSize: 13, color: ctcTab === id ? ink : sub, fontWeight: ctcTab === id ? 500 : 400, borderBottom: `2px solid ${ctcTab === id ? gold : 'transparent'}`, cursor: 'pointer', fontFamily: C.fontSans, marginBottom: -1 }}>{label}</div>
          ))}
        </div>
      ) : null;
      ...
      if (isCtc && pview === 'list' && ctcTab === 'emails') {
        return (
          <React.Fragment>
            <div style={{ padding: wide ? '26px 34px 0' : '12px 14px 0', flexShrink: 0 }}>{titleNode ? titleNode(wide ? 27 : '1.05rem') : null}</div>
            {ctcTabBar}
            <CtcEmailsTab dark={dark} user={user} files={rows} onFiled={reload} />
          </React.Fragment>
        );
      }
      ...
      return (
        <React.Fragment>
          {pview === 'form' ? formView()
            : pview === 'detail' ? detailView()
            : wide && !hideSidebar ? (
              <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
                <div style={{ width: 216, flexShrink: 0, borderRight: `1px solid ${bord}`, padding: '18px 0', background: dark ? '#08132A' : '#FCFBF8', overflowY: 'auto' }}>
                  <div style={{ fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: sub, padding: '0 18px', marginBottom: 8, fontFamily: C.fontSans }}>{KL.plural}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 18px', fontSize: '0.82rem', fontWeight: 500, color: ink, background: dark ? 'rgba(255,255,255,0.06)' : '#F7F4EE', boxShadow: `inset 2px 0 0 ${gold}`, fontFamily: C.fontSans }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: gold, flexShrink: 0 }} />{'All ' + KL.plural.toLowerCase()}<span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: sub }}>{rows.length}</span>
                  </div>
                </div>
                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                  ...
                  {ctcTabBar}
                  {theadRow}
                  <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 18px 24px' }}>{listBody(wideRowFn, true)}</div>
                </div>
              </div>
            ) : wide ? (
              <React.Fragment>
                ...
                {ctcTabBar}
                {theadRow}
                <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 34px 24px' }}>{listBody(wideRowFn, true)}</div>
              </React.Fragment>
            ) : (
              <React.Fragment>
                {listHeader}
                {ctcTabBar}
                <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 14px 24px' }}>{listBody(isCtc ? ctcCard : projCard, false)}</div>
              </React.Fragment>
            )}
          {zohoLinkOpen && current && <ZohoLinkModal dark={dark} project={current} onClose={() => setZohoLinkOpen(false)} onLinked={async () => { setZohoLinkOpen(false); await refreshContainer(); }} />}
        </React.Fragment>
      );
    }

    // Create / edit a project (or CTC file). record_type is fixed by `kind`.
    function ProjectForm({ kind, dark, team, user, project, onCancel, onSaved, onArchived }) {
      const p = project || {};
      ...
      useEffect(() => {
        if (!isCtc) return;
        const c = window.SupabaseAuth?._client; if (!c) return;
        c.from('profiles').select('id,first_name,last_name,email,access').eq('status', 'active').overlaps('access', ['agent'])
          .then(({ data }) => setAgents((data || []).map(x => ({ id: x.id, name: [x.first_name, x.last_name].filter(Boolean).join(' ') || x.email }))));
      }, [isCtc]);
      async function searchDeal(q) {
        const query = (q || dealQuery || name).trim(); if (query.length < 2) return;
        setDealBusy(true);
        const { ok, data } = await callZoho({ action: 'search_deals', query });
        setDealBusy(false);
      ...
      async function save() {
        ...
        setSaving(true);
        try {
          const fields = { name: name.trim(), record_type: kind, status, outcome: outcome.trim() || null, group_tag: groupTag.trim() || null, owner_id: ownerId || null, started_at: startedAt || null, target_date: targetDate || null, agent_id: isCtc ? (agentId || null) : undefined, zoho_deal_id: isCtc ? (zohoDealId || null) : undefined, zoho_deal_name: isCtc ? (zohoDealName || null) : undefined };
          Object.keys(fields).forEach(k => { if (fields[k] === undefined) delete fields[k]; });
          let id = p.id;
          if (id) await ProjectDB.update(id, fields);
          else { const created = await ProjectDB.create(fields, user); id = created && created.id; }
          onSaved(id);
        } finally { setSaving(false); }
      }
      ...
      return (
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
            <i className="ti ti-chevron-left" onClick={onCancel} style={{ fontSize: 16, cursor: 'pointer', color: sub }} />
            <span style={{ fontFamily: C.fontSans, fontSize: 13, fontWeight: 600, color: ink }}>{p.id ? 'Edit' : 'New'} {KL.article}</span>
          </div>
          <div style={{ padding: '16px 16px 40px', maxWidth: 640 }}>
            {field(KL.singular + ' name', <input value={name} onChange={e => setName(e.target.value)} placeholder={KL.namePlaceholder} style={inp} />)}
            {field('Outcome', <textarea value={outcome} onChange={e => setOutcome(e.target.value)} rows={2} placeholder="One sentence — what this is trying to achieve" style={{ ...inp, resize: 'vertical', lineHeight: 1.5 }} />)}
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ flex: 1 }}>{field('Status', <select value={status} onChange={e => setStatus(e.target.value)} style={inp}>{PROJECT_STATUS.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}</select>)}</div>
              <div style={{ flex: 1 }}>{field(KL.groupLabel, isCtc
                ? <select value={groupTag} onChange={e => setGroupTag(e.target.value)} style={inp}>
                    <option value="">(Not set)</option>
                    {DEAL_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                    {groupTag && !DEAL_TYPES.includes(groupTag) && <option value={groupTag}>{groupTag}</option>}
                  </select>
                : <input value={groupTag} onChange={e => setGroupTag(e.target.value)} placeholder={KL.groupPlaceholder} style={inp} />)}</div>
            </div>
            {isCtc && field('Agent', <select value={agentId} onChange={e => setAgentId(e.target.value)} style={inp}>
              <option value="">(No agent)</option>
              {agents.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
              {agentId && !agents.some(a => a.id === agentId) && <option value={agentId}>{(team.find(m => m.id === agentId) || {}).name || 'Current agent'}</option>}
            </select>)}
            {isCtc && field('Zoho deal', (
              <div>
                {zohoDealId ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 11px', background: dark ? '#06101F' : '#F7F4EE', border: `1px solid ${bord}`, borderRadius: 6 }}>
                    <i className="ti ti-link" style={{ fontSize: 13, color: dark ? '#C9A45A' : '#AD832F', flexShrink: 0 }} />
                    <span style={{ flex: 1, minWidth: 0, fontSize: 13.5, color: ink, fontFamily: C.fontSans, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{zohoDealName || zohoDealId}</span>
                    <button type="button" onClick={() => { setZohoDealId(''); setZohoDealName(''); setDealResults(null); }} style={{ background: 'none', border: 'none', color: sub, cursor: 'pointer', fontSize: 12, fontFamily: C.fontSans }}>Unlink</button>
                  </div>
                ) : (
                  <div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <input value={dealQuery} onChange={e => setDealQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); searchDeal(); } }} placeholder={name ? ('Search Zoho deals — e.g. ' + name) : 'Search Zoho deals by address or client'} style={inp} />
                      <button type="button" onClick={() => searchDeal()} disabled={dealBusy} style={{ padding: '0 14px', background: dark ? '#06101F' : '#fff', border: `1px solid ${bord}`, borderRadius: 6, color: ink, fontSize: '0.8rem', cursor: 'pointer', fontFamily: C.fontSans, flexShrink: 0 }}>{dealBusy ? '…' : 'Find'}</button>
                    </div>
                    {dealResults && dealResults.length === 0 && <div style={{ fontSize: 12, color: sub, fontFamily: C.fontSans, marginTop: 6 }}>No deal matched. Buyer deals are often named after the client, not the address — try the client's surname.</div>}
                    {dealResults && dealResults.map(dl => (
                      <div key={dl.id} onClick={() => { setZohoDealId(dl.id); setZohoDealName(dl.name || ''); setDealResults(null); }} style={{ marginTop: 6, padding: '9px 11px', border: `1px solid ${bord}`, borderRadius: 6, cursor: 'pointer', fontFamily: C.fontSans }}>
                        <div style={{ fontSize: 13.5, color: ink }}>{dl.name}</div>
                        <div style={{ fontSize: 11.5, color: sub, marginTop: 2 }}>{[dl.stage, dl.Type || dl.type, dl.owner].filter(Boolean).join(' · ')}</div>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {field('Owner', <select value={ownerId} onChange={e => setOwnerId(e.target.value)} style={inp}><option value="">(No owner)</option>{team.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select>)}
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ flex: 1 }}>{field('Started', <input type="date" value={startedAt || ''} onChange={e => setStartedAt(e.target.value)} style={inp} />)}</div>
              <div style={{ flex: 1 }}>{field('Target', <input type="date" value={targetDate || ''} onChange={e => setTargetDate(e.target.value)} style={inp} />)}</div>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
              <button onClick={save} disabled={saving || !name.trim()} style={{ flex: 1, padding: '11px 16px', background: C.navy, color: '#fff', border: 'none', borderRadius: 22, fontSize: 13, fontWeight: 500, letterSpacing: '0.02em', cursor: 'pointer', fontFamily: C.fontSans, opacity: !name.trim() ? 0.5 : 1 }}>{saving ? 'Saving…' : (p.id ? 'Save changes' : ('Create ' + KL.article))}</button>
              <button onClick={onCancel} style={{ flex: 1, padding: '11px 16px', background: 'none', color: sub, border: `1px solid ${bord}`, borderRadius: 22, fontSize: 13, cursor: 'pointer', fontFamily: C.fontSans }}>Cancel</button>
            </div>
            {p.id && <button onClick={async () => { if (!window.confirm('Archive this ' + KL.article + '? It will be hidden from the list.')) return; await ProjectDB.archive(p.id); onArchived(); }} style={{ width: '100%', marginTop: 8, padding: '9px', background: 'none', color: '#C0392B', border: `1px solid #C0392B55`, borderRadius: 8, fontSize: '0.82rem', cursor: 'pointer', fontFamily: C.fontSans }}><i className="ti ti-archive" style={{ marginRight: 6, fontSize: 13 }} />Archive {KL.article}</button>}
          </div>
        </div>
      );
    }
                ...
                {colItems.map(t => {
                  ...
                  <div key={t.id} draggable onDragStart={(e) => { setDragId(t.id); e.dataTransfer.effectAllowed = 'move'; }} onDragEnd={() => setDragId(null)}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, flexWrap: 'wrap' }}>
                      {showSource && t._projectName ? <span style={{ fontSize: 11, color: sub, fontFamily: C.fontSans, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t._projectName}</span> : <span />}
    ...
    function TaskTimeline({ items, dark, onOpen }) {
      const bord = dark ? '#152545' : '#E4DFD4', ink = dark ? '#fff' : '#001A4A', sub = dark ? 'rgba(255,255,255,0.5)' : '#6B6B6B';
      const dated = items.filter(t => t.due_at).slice().sort((a, b) => new Date(a.due_at) - new Date(b.due_at));
      if (!dated.length) return <div style={{ padding: 24, color: sub, fontSize: '0.82rem', fontFamily: C.fontSans, textAlign: 'center' }}>No dated tasks to place on a timeline.</div>;
      const today = new Date();
      const dueDates = dated.map(t => new Date(t.due_at));
      const start = new Date(Math.min(today, ...dueDates)); start.setDate(1); start.setHours(0, 0, 0, 0);
      ...
      const months = Array.from({ length: monthsSpan }, (_, i) => new Date(start.getFullYear(), start.getMonth() + i, 1));
      const end = new Date(start.getFullYear(), start.getMonth() + monthsSpan, 1);
      const totalMs = end - start;
      return (
        <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '14px 18px 24px' }}>
          <div style={{ border: `1px solid ${bord}`, borderRadius: 8, overflow: 'hidden', minWidth: 640 }}>
            <div style={{ display: 'flex', background: dark ? '#08132A' : '#FCFBF8', borderBottom: `1px solid ${bord}` }}>
              <div style={{ width: 200, flexShrink: 0, padding: '9px 14px', fontSize: '0.6rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontWeight: 600, fontFamily: C.fontSans }}>Task</div>
              <div style={{ flex: 1, display: 'flex' }}>
                {months.map((m, i) => <div key={i} style={{ flex: 1, padding: '9px 10px', fontSize: '0.6rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: sub, fontWeight: 600, borderLeft: i ? `1px solid ${bord}` : 'none', fontFamily: C.fontSans }}>{m.toLocaleDateString('en-US', { month: 'short' })}</div>)}
              </div>
            </div>
            {dated.map(t => {
    ...
    function TaskDrawer({ dark, onClose, children, width }) {
      useEffect(() => {
      ...
      return (
        <React.Fragment>
          <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,13,38,0.32)', zIndex: 90 }} />
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: width || 'min(560px, 94vw)', background: dark ? '#000D26' : '#FCFBF8', boxShadow: '-14px 0 44px rgba(0,13,38,.22)', zIndex: 91, display: 'flex', flexDirection: 'column' }}>
            {children}
          </div>
        </React.Fragment>
      );
    ...
    function TaskMailbox({ dark, user, tasks, linksByTask, onAttached, onOpenTask, onClose, onCreateTask }) {
      ...
      async function load() {
        ...
        const { ok, data } = await callCalendar({ action: 'gmail_threads', q: q || undefined, maxResults: 30 });
        setLoading(false);
        if (!ok) { setErr(data?.error === 'needs_connect' ? 'needs_connect' : (data?.error || 'Could not load mail.')); setThreads([]); return; }
      ...
      async function doAttach(threadId, taskId) {
        const th = threads.find(t => t.id === threadId); if (!th) return;
        setPickerFor(null);
      ...
      const tabBtn = (id, label) => (
        <div key={id} onClick={() => setTab(id)} style={{ fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase', color: tab === id ? ink : sub, padding: '8px 0', borderBottom: `2px solid ${tab === id ? gold : 'transparent'}`, marginBottom: -1, fontWeight: tab === id ? 500 : 400, cursor: 'pointer', fontFamily: C.fontSans }}>{label}</div>
      );

      return (
        <TaskDrawer dark={dark} onClose={onClose} width="min(460px, 96vw)">
          <div style={{ background: card, borderBottom: `1px solid ${bord}`, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <svg viewBox="0 0 48 48" style={{ width: 22, height: 22 }}><path fill="#4285F4" d="M44 8v2L24 24 44 8z" /><path fill="#34A853" d="M4 10v28a2 2 0 002 2h6V16z" /><path fill="#FBBC05" d="M44 10v28a2 2 0 01-2 2h-6V16z" /><path fill="#C5221F" d="M4 8l20 14L44 8" /><path fill="#EA4335" d="M4 8a2 2 0 012-2h2l16 11L40 6h2a2 2 0 012 2z" /></svg>
              <h3 style={{ fontSize: 14, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#001A4A', fontFamily: C.fontSans, fontWeight: 500 }}>Mailbox</h3>
            </div>
            <i className="ti ti-x" onClick={onClose} aria-label="Close" style={{ fontSize: 16, cursor: 'pointer', color: sub }} />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${bord}`, borderRadius: 22, background: card, padding: '9px 14px', margin: '14px 16px', flexShrink: 0 }}>
            <i className="ti ti-search" style={{ fontSize: 13, color: sub }} />
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search mail — sender, subject…" style={{ border: 'none', outline: 'none', flex: 1, fontSize: '0.85rem', background: 'transparent', color: ink, fontFamily: C.fontSans }} />
          </div>
          <div style={{ display: 'flex', gap: 16, padding: '0 18px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
            {tabBtn('inbox', 'Inbox')}{tabBtn('attached', 'Attached')}{tabBtn('sent', 'Sent')}
          </div>
          <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
            {loading ? <div style={{ padding: 24, textAlign: 'center', color: sub, fontSize: '0.82rem', fontFamily: C.fontSans }}>Loading…</div>
              : err === 'needs_connect' ? (
                <div style={{ padding: 24, textAlign: 'center', fontFamily: C.fontSans }}>
                  <div style={{ color: sub, fontSize: '0.82rem', marginBottom: 10 }}>Connect Google to read mail here.</div>
                  <button onClick={() => window.SupabaseAuth.connectCalendar()} style={{ background: '#001A4A', color: '#fff', border: 'none', borderRadius: 22, padding: '8px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: C.fontSans }}>Connect Google</button>
                </div>
              ) : err ? <div style={{ padding: 24, textAlign: 'center', color: sub, fontSize: '0.82rem', fontFamily: C.fontSans }}>{err}</div>
              : visible.length === 0 ? <div style={{ padding: 24, textAlign: 'center', color: sub, fontSize: '0.82rem', fontFamily: C.fontSans }}>No mail here.</div>
              : visible.map(th => {
                const linked = linkedTasksFor(th.id);
                return (
                  <div key={th.id} ref={pickerFor === th.id ? pickerRef : null} style={{ padding: '13px 18px', borderBottom: `1px solid ${dark ? '#152037' : '#efeee7'}`, position: 'relative', fontFamily: C.fontSans }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                      <span style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#3a4a72,#1a2748)', color: '#fff', fontSize: 11, display: 'grid', placeItems: 'center', flexShrink: 0 }}>{(th.from || '?').trim().charAt(0).toUpperCase()}</span>
                      <span style={{ fontSize: 13, fontWeight: 500, color: ink, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{th.from}</span>
                      <span style={{ fontSize: 11, color: sub, marginLeft: 'auto', flexShrink: 0 }}>{th.date}</span>
                    </div>
                    <div onClick={() => linked[0] && onOpenTask(linked[0], th.id)} style={{ fontSize: 13.5, color: '#001A4A', margin: '6px 0 2px', fontWeight: 500, cursor: linked[0] ? 'pointer' : 'default' }}>{th.subject}</div>
                    <div style={{ fontSize: 12.5, color: sub, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{th.snippet}</div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 9 }}>
                      {linked.length
                        ? <span style={{ fontSize: 11, color: '#0F6E56', background: '#E6F2EC', border: '1px solid #A0D9C4', borderRadius: 20, padding: '4px 10px', display: 'inline-flex', gap: 6, alignItems: 'center', cursor: 'pointer' }} onClick={() => onOpenTask(linked[0], th.id)}>✓ Linked to "{linked[0].title}"{linked.length > 1 ? ' +' + (linked.length - 1) : ''}</span>
                        : <button onClick={() => setPickerFor(p => p === th.id ? null : th.id)} style={{ fontSize: 11, letterSpacing: '0.05em', textTransform: 'uppercase', color: gold, border: '1px solid #E8D9BC', background: '#F3EBDA', borderRadius: 20, padding: '4px 11px', display: 'inline-flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontFamily: C.fontSans }}><i className="ti ti-link" style={{ fontSize: 12 }} />Attach to task</button>}
                      <a href={th.permalink} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: gold, fontFamily: C.fontSans }}>Open in Gmail ↗</a>
                    </div>
                    {pickerFor === th.id && (
                      <div style={{ position: 'absolute', right: 16, bottom: 44, background: card, border: `1px solid ${bord}`, borderRadius: 8, boxShadow: '0 12px 32px rgba(0,26,74,0.16)', padding: 6, minWidth: 240, zIndex: 30 }}>
                        <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, padding: '7px 9px 4px' }}>Attach to which task?</div>
    ...
    function AccountabilitySurface({ dark, team, titleNode }) {
      const bord = dark ? '#152545' : '#E4DFD4', ink = dark ? '#fff' : '#001A4A';
      const sub = dark ? 'rgba(255,255,255,0.5)' : '#6B6B6B', gold = dark ? '#C9A45A' : '#AD832F';
      const red = '#C0392B', green = '#0F6E56';
      const J = C.fontSans;
      const WEEKS = 8;
      ...
      useEffect(() => {
        ...
        if (!c) { setRows([]); return; }
        c.rpc('accountability_weeks', { week_count: WEEKS }).then(({ data, error }) => {
      ...
      useEffect(() => { setOpenId(null); setOpenTasks(null); }, [offset]);
      ...
      const th = { fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 500, color: sub, fontFamily: J };
      const numCell = { width: 74, flexShrink: 0, textAlign: 'right', fontFamily: J, fontSize: 14 };
      const navBtn = (icon, on) => (
        <i className={`ti ti-${icon}`} onClick={on} style={{ fontSize: 16, color: gold, cursor: 'pointer', padding: '2px 4px' }} />
      );

      const totals = people.reduce((a, m) => {
        const s = statFor(m.id);
        return { assigned: a.assigned + s.assigned, completed: a.completed + s.completed, still_open: a.still_open + s.still_open, overdue: a.overdue + s.overdue };
      }, { assigned: 0, completed: 0, still_open: 0, overdue: 0 });
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
          <div style={{ padding: '14px 20px 0', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
              {navBtn('chevron-left', () => setOffset(o => Math.min(WEEKS - 1, o + 1)))}
              <span style={{ fontFamily: J, fontSize: 14, fontWeight: 600, color: ink, minWidth: 108, textAlign: 'center' }}>{weekLabel}</span>
              {navBtn('chevron-right', () => setOffset(o => Math.max(0, o - 1)))}
              {offset !== 0 && <button onClick={() => setOffset(0)} style={{ fontFamily: J, fontSize: 12, color: gold, background: 'none', border: `1px solid ${bord}`, borderRadius: 14, padding: '3px 10px', cursor: 'pointer' }}>This week</button>}
              <span style={{ fontFamily: J, fontSize: 11.5, color: sub, marginLeft: 4 }}>Monday to Sunday</span>
            </div>
          </div>

          <div style={{ padding: '14px 20px 28px' }}>
            {err && <div style={{ fontFamily: J, fontSize: 12.5, color: red, marginBottom: 12 }}>{err}</div>}
            {rows === null ? <div style={{ fontFamily: J, fontSize: 13, color: sub, padding: 12 }}>Loading…</div> : (
              <React.Fragment>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0 10px 8px', borderBottom: `1px solid ${bord}` }}>
                  <span style={{ ...th, flex: 1 }}>Person</span>
                  <span style={{ ...th, ...numCell }}>Assigned</span>
                  <span style={{ ...th, ...numCell }}>Completed</span>
                  <span style={{ ...th, ...numCell }}>Still open</span>
                  <span style={{ ...th, ...numCell }}>Overdue</span>
                </div>
                {people.map(m => {
                  const s = statFor(m.id);
                  ...
                  return (
                    <React.Fragment key={m.id}>
                      <div onClick={() => togglePerson(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 10px', borderBottom: `1px solid ${bord}`, cursor: 'pointer', background: on ? (dark ? 'rgba(201,164,90,.08)' : '#F7F4EE') : 'transparent' }}>
                        <span style={{ flex: 1, minWidth: 0, fontFamily: J, fontSize: 14, color: quiet ? sub : ink, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.name}</span>
                        <span style={{ ...numCell, color: ink }}>{s.assigned || '—'}</span>
                        <span style={{ ...numCell, color: s.completed ? green : sub }}>{s.completed || '—'}</span>
                        <span style={{ ...numCell, color: ink }}>{s.still_open || '—'}</span>
                        <span style={{ ...numCell, color: s.overdue ? red : sub }}>{s.overdue || '—'}</span>
                      </div>
                      {on && (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 10px' }}>
                  <span style={{ flex: 1, fontFamily: J, fontSize: 13, fontWeight: 600, color: ink }}>Team</span>
                  <span style={{ ...numCell, fontWeight: 600, color: ink }}>{totals.assigned}</span>
                  <span style={{ ...numCell, fontWeight: 600, color: totals.completed ? green : sub }}>{totals.completed}</span>
                  <span style={{ ...numCell, fontWeight: 600, color: ink }}>{totals.still_open}</span>
                  <span style={{ ...numCell, fontWeight: 600, color: totals.overdue ? red : sub }}>{totals.overdue}</span>
                </div>
                <div style={{ fontFamily: J, fontSize: 11.5, color: sub, lineHeight: 1.6, marginTop: 14 }}>
                  A task is credited to the week it was <b>completed</b>, whenever it was handed out. Still open and Overdue are today's picture, not that week's.
                </div>
              </React.Fragment>
            )}
          </div>
    ...
    function TasksScreen({ dark, user, onClose, initialTask }) {
      ...
      const [cols, setCols] = useState(() => { try { const s = JSON.parse(localStorage.getItem('tmg-cols-tasks') || 'null'); return Array.isArray(s) && s.length ? s : ['source', 'status', 'due']; } catch (e) { return ['source', 'status', 'due']; } });
      useEffect(() => { localStorage.setItem('tmg-tasks-view', viewMode); }, [viewMode]);
      useEffect(() => { localStorage.setItem('tmg-cols-tasks', JSON.stringify(cols)); }, [cols]);
      const [search, setSearch] = useState('');
      const [searchOpen, setSearchOpen] = useState(false);
      ...
      const [surface, setSurface] = useState(() => { const s = localStorage.getItem('tmg-tasks-surface'); return s === 'ctc' ? 'my' : (s || 'my'); });   // CTC Files moved to More — see index.jsx CtcFilesFrame
      ...
      useEffect(() => {
        if (routeFromUrl.current) { routeFromUrl.current = false; return; }
      ...
      useEffect(() => { if (view !== 'list' || surface !== 'my') setRouteNotice(null); }, [view, surface]);
      useEffect(() => { const r = requestAnimationFrame(() => setVis(true)); return () => cancelAnimationFrame(r); }, []);
      ...
      useEffect(() => {
        ...
        if (surface !== 'my') { routeFromUrl.current = true; setSurface('my'); }
        setCurrent(initialTask); setView('detail');
      }, [initialTask]);
      const bg = dark ? '#000D26' : '#FCFBF8', bord = dark ? '#152545' : '#E4DFD4', ink = dark ? '#fff' : '#001A4A', sub = dark ? 'rgba(255,255,255,0.5)' : '#6B6B6B', gold = dark ? '#C9A45A' : '#AD832F';
      ...
      // One slot, not a map: caching every foreign task forever meant re-opening
      // one showed the assignees it had when you first looked, and Save wrote
      // that stale list back over a teammate's change.
      const [extraPeople, setExtraPeople] = useState({ id: null, people: undefined });
      // undefined = not fetched (yet, or the read failed). [] = genuinely nobody.
      // The two must stay distinguishable: Save may only write people it seeded.
      const peopleFor = id => (data.peopleByTask[id] !== undefined ? data.peopleByTask[id] : (extraPeople.id === id ? extraPeople.people : undefined));
      const enrich = (t, projById) => ({ ...t, _projectName: t.project_id ? (projById[t.project_id] || '') : '' });
      const withMetaFrom = (t, d) => { const ppl = (d.peopleByTask[t.id] !== undefined ? d.peopleByTask[t.id] : (extraPeople.id === t.id ? extraPeople.people : undefined)) || []; return { ...enrich(t, d.projById), _assignees: ppl.filter(p => p.role === 'assignee').map(p => p.user_id), _assigners: ppl.filter(p => p.role === 'assigner').map(p => p.user_id), _decisionMakers: ppl.filter(p => p.role === 'decision_maker').map(p => p.user_id) }; };
      ...
      async function onEmailAttached() { setLinksByTask(await TaskEmailDB.loadAllByTask()); }
      ...
      async function onCreateTaskFromEmail(thread) {
        const created = await TaskDB.create({ title: (thread.subject || 'New task').slice(0, 200) }, TaskDB.ownedBy({}, user), user);
        if (!created) return;
      ...
      // `tried` marks a finished attempt, successful or not. Without it a failed
      // read left the Edit form on "Loading…" with nothing to re-trigger the
      // fetch — a dead end with no way out but the back arrow.
      const [peopleTried, setPeopleTried] = useState(null);
      useEffect(() => {
        const id = current && current.id;
        if (!id || peopleFor(id) !== undefined || peopleTried === id) return;
        let on = true;
        TaskDB.loadPeopleFor(id).then(ppl => {
          if (!on) return;
          setPeopleTried(id);
          if (ppl) setExtraPeople({ id, people: ppl });
        });
        return () => { on = false; };
      }, [current && current.id, data.peopleByTask, extraPeople, peopleTried]);
      // ── Deep-link router ──────────────────────────────────────────────
      ...
      useEffect(() => {
        ...
        const goSurface = (s) => setSurface(prev => { if (prev !== s) routeFromUrl.current = true; return s; });
        const applyRoute = async (hash) => {
          const r = parseTaskRoute(hash);
          if (!r) return;
          // A plain list route is the user asking for a surface (the top-bar
          // Tasks icon, a bookmarked #projects) — remember it like any other
          // switch. Only deep links to one record stay a visit, below.
          if (r.type === 'list') { setOpenCtcTab(r.ctcTab || null); setOpenItemId(null); setSurface(r.surface); setView('list'); setCurrent(null); setRouteNotice(null); return; }
          if (r.type === 'record') {
            ...
            PENDING_ROUTE.kind = r.kind; PENDING_ROUTE.id = r.id; PENDING_ROUTE.tab = r.tab;
            goSurface(r.surface);
            setOpenItemId(r.id);
            return;
          }
          ...
          goSurface('my'); setRoutePending(true);
          const found = (data.tasks || []).find(t => t.id === r.id) || await TaskDB.getById(r.id);
          setRoutePending(false);
          if (found) { setCurrent(found); setView('detail'); }
          else { setView('list'); setCurrent(null); setRouteNotice('That task no longer exists, or you don’t have access to it.'); }
        };
        ...
        if (!routeApplied.current) { routeApplied.current = true; if (!initialTask) applyRoute(initial); }
      ...
      useEffect(() => {
        ...
        const t = setTimeout(() => {   // coalesce: Safari throws past ~100 replaceState/10s
          try { if (location.hash.replace(/^#\/?/, '') !== hash) history.replaceState(null, '', '#' + hash); } catch (e) {}
        }, 120);
        return () => clearTimeout(t);
      }, [surface, view, current && current.id, routePending, routeNotice]);
      useEffect(() => { if (view === 'list') { try { const p = JSON.parse(localStorage.getItem(labelKey(user && user.id)) || '[]') || []; const m = {}; p.forEach(x => { m[x.name] = x.color; }); setLabelColors(m); } catch (e) {} } }, [view]);
      async function handleSave(form) {
        ...
        await reload(); refreshNav(); setView('list'); setCurrent(null);
        // Drop the cached people last — doing it mid-save flipped the still-open
        // form back to "Loading…" while the write was in flight.
        setExtraPeople({ id: null, people: undefined }); setPeopleTried(null);
      }
      ...
      const patchStatus = (id, st) => {
        setData(d => ({ ...d, tasks: (d.tasks || []).map(x => x.id === id ? { ...x, status: st } : x) }));
        setCurrent(cur => (cur && cur.id === id) ? { ...cur, status: st } : cur);
      };
      async function changeStatus(task, status) {
        const was = task.status;
        patchStatus(task.id, status);
        try { await TaskDB.update(task.id, task, { status }, user); }
        catch (e) { patchStatus(task.id, was); return; }
      ...
      async function quickAddMyTask() {
        const title = quickAddVal.trim(); if (!title) return;
        ...
        await TaskDB.create({ title, status: 'todo', priority: 'medium' }, TaskDB.ownedBy({}, user), user);
        setQuickAddVal('');
        await reload();
      }
      const quickAddRow = (
        <div style={{ display: 'flex', gap: 8, padding: '12px 0 0' }}>
          <input value={quickAddVal} onChange={e => setQuickAddVal(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') quickAddMyTask(); }} placeholder="＋ Add a task and press Enter…" style={{ flex: 1, padding: '10px 12px', background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 6, color: ink, fontSize: 14, outline: 'none', fontFamily: C.fontSans }} />
        </div>
      );
      ...
      async function addToGroup() {
        const g = groupAdd; if (!g) return;
        const title = (g.val || '').trim(); if (!title) return;
        const seed = g.seed || {};
        const people = seed.assigneeId ? { assignee: [seed.assigneeId] } : TaskDB.ownedBy({}, user);
        await TaskDB.create({ title, status: seed.status || 'todo', priority: seed.priority || 'medium', project_id: seed.projectId || null }, people, user);
        setGroupAdd(x => (x && x.key === g.key) ? { ...x, val: '' } : x);
      ...
      const back = () => { if (view === 'form') setView(current ? 'detail' : 'list'); else { setView('list'); setCurrent(null); } };
      ...
      const groupHead = (color, text, add) => {
        ...
        const toggleAdd = () => setGroupAdd(g => (g && g.key === add.key) ? null : { key: add.key, val: '', seed: add });
        return (
          <React.Fragment>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 7 }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: color }} />
              <span style={{ fontSize: '0.64rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: sub, fontFamily: C.fontSans }}>{text}</span>
              {/* Focus and Enter/Space, same as the reorder grips elsewhere on
                  this screen: an <i> with only an onClick is mouse-only. */}
              {add && <i className={'ti ti-' + (open ? 'x' : 'plus')} onClick={(e) => { e.stopPropagation(); toggleAdd(); }}
                role="button" tabIndex={0}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); toggleAdd(); } }}
                title={add.hint} aria-label={add.hint} style={{ fontSize: 13, color: sub, opacity: open ? 0.9 : 0.4, cursor: 'pointer', flexShrink: 0 }} />}
            </div>
            {open && (
              <div style={{ display: 'flex', gap: 8, padding: '0 0 10px' }}>
                <input autoFocus value={groupAdd.val} onChange={e => setGroupAdd(g => g ? { ...g, val: e.target.value } : g)}
                  onKeyDown={e => { if (e.key === 'Enter') addToGroup(); else if (e.key === 'Escape') setGroupAdd(null); }}
                  placeholder={add.hint + ' and press Enter…'}
                  style={{ flex: 1, padding: '9px 12px', background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 6, color: ink, fontSize: 14, outline: 'none', fontFamily: C.fontSans }} />
              </div>
            )}
          </React.Fragment>
        );
      ...
      const taskRow = (t, showStatus) => {
        ...
        return (
          <div key={t.id} onClick={() => { setCurrent(t); setView('detail'); }} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 12px', background: (wide && current && current.id === t.id) ? (dark ? '#0A1E44' : '#F3EBDA') : (dark ? '#0A1730' : '#fff'), border: `1px solid ${(wide && current && current.id === t.id) ? gold : bord}`, borderLeft: `2.5px solid ${statusColor(t.status)}`, borderRadius: 12, marginBottom: 8, cursor: 'pointer', opacity: t.status === 'done' ? 0.75 : 1 }}>
            ...
            {showStatus && <span style={{ fontSize: '0.56rem', fontWeight: 700, padding: '2px 7px', borderRadius: 12, background: statusBg(t.status), color: statusColor(t.status), fontFamily: C.fontSans, flexShrink: 0 }}>{statusLabel(t.status)}</span>}
            <i className="ti ti-chevron-right" style={{ fontSize: 16, color: sub, flexShrink: 0 }} />
          </div>
        );
      };
      const sortTabs = (
        <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
          {[['status', 'Status'], ['due', 'Due date'], ['label', 'Label']].map(([id, label]) => (
            <button key={id} onClick={() => setSortBy(id)} style={{ flex: 1, padding: '6px 8px', borderRadius: 8, border: `1px solid ${sortBy === id ? C.navy : bord}`, background: sortBy === id ? C.navy : (dark ? '#0A1730' : '#fff'), color: sortBy === id ? '#fff' : sub, fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer', fontFamily: C.fontSans }}>{label}</button>
          ))}
        </div>
      );
      const renderList = () => {
        ...
        if (sortBy === 'label') {
          ...
          data.tasks.forEach(t => { const ls = myLabelsFor(t.id); if (!ls.length) unlabeled.push(t); else ls.forEach(l => { (groups[l] = groups[l] || []).push(t); }); });
      ...
      const listInner = (compact) => (
        <div style={{ padding: compact ? '10px 10px 24px' : '12px 14px 24px' }}>
          {routeNoticeBar}
          {loading ? <div style={{ color: sub, fontSize: '0.82rem', padding: 12 }}>Loading…</div>
            : data.tasks.length === 0 ? <div style={{ color: sub, fontSize: '0.82rem', padding: 24, textAlign: 'center' }}>No tasks yet. Tap <b>New</b> to add one.</div>
            : <React.Fragment>{renderList()}</React.Fragment>}
        </div>
      );
      const detailPane = (
        view === 'form' ? (current && peopleFor(current.id) === undefined && peopleTried !== current.id
          ...
          // query, so this placeholder is sub-second — and it gives up once the
      ...
      const containerStyle = embed
        ? { position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: bg, overflow: 'hidden', opacity: vis ? 1 : 0, transition: 'opacity 150ms ease' }
        : { position: 'fixed', top: POPOUT_TOP, left: 0, right: 0, bottom: 0, zIndex: 60, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: bg, opacity: vis ? 1 : 0, transition: 'opacity 150ms ease' };
      ...
      async function toggleMilestone(t) { await TaskDB.update(t.id, t, { is_milestone: !t.is_milestone }, user); await reload(); }
      ...
      const COLUMN_DEFS = [
        { id: 'source', label: 'Source' }, { id: 'status', label: 'Status' },
        { id: 'due', label: 'Due' }, { id: 'assignee', label: 'Assignee' }, { id: 'labels', label: 'Labels' },
        { id: 'created', label: 'Created' }, { id: 'modified', label: 'Last modified' },
      ];
      const filteredTasks = search.trim() ? data.tasks.filter(t => (t.title || '').toLowerCase().includes(search.trim().toLowerCase())) : data.tasks;

      const chipStyle = { display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 400, padding: '7px 12px', border: `1px solid ${bord}`, borderRadius: 6, background: dark ? '#0A1730' : '#fff', color: sub, whiteSpace: 'nowrap', cursor: 'pointer', fontFamily: C.fontSans };
      const ddStyle = { position: 'absolute', zIndex: 60, top: '100%', right: 0, background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 8, boxShadow: '0 12px 32px rgba(0,26,74,0.16)', padding: 6, minWidth: 190, marginTop: 4 };
      const ddItem = (label, active, onClick, key) => (
        <div key={key || label} onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '8px 10px', fontSize: 13.5, color: active ? gold : ink, fontWeight: active ? 500 : 400, borderRadius: 5, cursor: 'pointer', fontFamily: C.fontSans }}>
          {label}{active && '✓'}
        </div>
      ...
      const groupChip = (
        <div ref={groupRef} style={{ position: 'relative' }}>
          <div onClick={() => { setGroupOpen(o => !o); setSortOpen(false); setColsOpen(false); }} style={sortBy !== 'status' ? { ...chipStyle, ...chipOn } : chipStyle}>Group: <span style={{ color: gold, fontWeight: 600 }}>{(GROUP_OPTS.find(([id]) => id === sortBy) || [])[1]}</span> <i className="ti ti-chevron-down" style={{ fontSize: 9, color: sub }} /></div>
          {groupOpen && <div style={ddStyle} onClick={e => e.stopPropagation()}>{GROUP_OPTS.map(([id, label]) => ddItem(label, sortBy === id, () => { setSortBy(id); setGroupOpen(false); }, id))}</div>}
        </div>
      );
      const sortChip = (
        <div ref={sortRef} style={{ position: 'relative' }}>
          <div onClick={() => { setSortOpen(o => !o); setGroupOpen(false); setColsOpen(false); }} style={thenBy !== 'due' ? { ...chipStyle, ...chipOn } : chipStyle}>Sort: <span style={{ color: gold, fontWeight: 600 }}>{(SORT_OPTS.find(([id]) => id === thenBy) || [])[1]}</span> <i className="ti ti-chevron-down" style={{ fontSize: 9, color: sub }} /></div>
          {sortOpen && <div style={ddStyle} onClick={e => e.stopPropagation()}>{SORT_OPTS.map(([id, label]) => ddItem(label, thenBy === id, () => { setThenBy(id); setSortOpen(false); }, id))}</div>}
        </div>
      );
      const searchChip = searchOpen
        ? <input autoFocus value={search} onChange={e => setSearch(e.target.value)} onBlur={() => { if (!search) setSearchOpen(false); }} placeholder="Search tasks…" style={{ ...chipStyle, cursor: 'text', width: 130, textTransform: 'none', letterSpacing: 'normal', fontSize: '0.78rem', outline: 'none' }} />
        : <div onClick={() => setSearchOpen(true)} style={{ ...chipStyle, width: 34, height: 34, padding: 0, justifyContent: 'center' }}><i className="ti ti-search" style={{ fontSize: 13 }} /></div>;
      ...
      // ══════════════════════════════════════════════════════════════════════
      const iconBtnStyle = { width: 34, height: 34, border: `1px solid ${bord}`, background: dark ? '#0A1730' : '#fff', borderRadius: 6, display: 'grid', placeItems: 'center', color: sub, position: 'relative', flexShrink: 0 };
      const linkedEmailCount = Object.keys(linksByTask).filter(id => (linksByTask[id] || []).length).length;
      const emailIconBtn = (
        <div title="Email — open mailbox" onClick={() => setMailboxOpen(true)} style={{ ...iconBtnStyle, cursor: 'pointer' }}>
          <i className="ti ti-mail" style={{ fontSize: 15 }} />
          {linkedEmailCount > 0 && <span style={{ position: 'absolute', top: -6, right: -6, background: gold, color: '#fff', fontSize: 9, minWidth: 16, height: 16, borderRadius: 8, display: 'grid', placeItems: 'center', padding: '0 3px' }}>{linkedEmailCount}</span>}
        </div>
      );
      const dispMenuStyle = { position: 'absolute', top: 42, right: 0, background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 8, boxShadow: '0 8px 28px rgba(0,13,38,.16)', padding: 8, minWidth: 250, zIndex: 30, fontFamily: C.fontSans };
      const dispSubStyle = { position: 'absolute', top: 0, right: '100%', marginRight: 2, background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 8, boxShadow: '0 8px 28px rgba(0,13,38,.16)', padding: 6, minWidth: 170, zIndex: 31 };
      const dispMenuLabel = (text) => <div key={'ml-' + text} style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, padding: '8px 8px 4px' }}>{text}</div>;
      const dispDiv = <div style={{ height: 1, background: bord, margin: '6px 2px' }} />;
      const dispSubBtn = (label, selected, onClick, key) => (
        <button key={key || label} onClick={onClick} style={{ display: 'flex', width: '100%', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '7px 9px', borderRadius: 5, fontSize: 13, textAlign: 'left', color: selected ? gold : ink, fontWeight: selected ? 500 : 400, fontFamily: C.fontSans }}>
          {label}{selected && '✓'}
        </button>
      );
      ...
      const dispRow = (label, valueText, subKey, submenuContent) => (
        <div key={subKey} onClick={(e) => { e.stopPropagation(); setDispSub(s => s === subKey ? null : subKey); }}
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 8, borderRadius: 5, fontSize: 13.5, position: 'relative', cursor: 'pointer', color: ink, fontFamily: C.fontSans }}>
          <span>{label}</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: gold, fontWeight: 500, fontSize: 12.5 }}>{valueText} ▸</span>
          {dispSub === subKey && <div style={dispSubStyle} onClick={e => e.stopPropagation()}>{submenuContent}</div>}
        </div>
      );
      // A static (non-clickable) row — Filter and Labels are stubs for v1 [brief §1.2].
      const dispStaticRow = (label, valueText) => (
        <div key={label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 8, borderRadius: 5, fontSize: 13.5, color: ink, fontFamily: C.fontSans }}>
          <span>{label}</span><span style={{ color: gold, fontWeight: 500, fontSize: 12.5 }}>{valueText}</span>
        </div>
      );
      const dispActive = sortBy !== 'status' || thenBy !== 'due' || viewMode !== 'list';
      const displayBtn = (
        <div style={{ position: 'relative' }}>
          <div onClick={(e) => { e.stopPropagation(); setDispOpen(o => !o); setDispSub(null); }}
            style={{ height: 34, padding: '0 12px', border: `1px solid ${dispActive ? (dark ? 'rgba(201,164,90,.4)' : '#C9A45A') : bord}`, borderRadius: 6, fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase', display: 'inline-flex', alignItems: 'center', gap: 7, cursor: 'pointer', fontFamily: C.fontSans, background: dispActive ? (dark ? 'rgba(201,164,90,.14)' : '#F3EBDA') : (dark ? '#0A1730' : '#fff'), color: dispActive ? ink : sub }}>
            <i className="ti ti-adjustments-horizontal" style={{ fontSize: 14 }} />Display ▾
          </div>
          {dispOpen && (
            <div style={dispMenuStyle} onClick={e => e.stopPropagation()}>
              {dispMenuLabel('View')}
              {/* Internal viewMode value is 'kanban' (pre-existing, also read by
                  TaskBoard's render check below) — only its on-screen label is "Board". */}
              {dispRow('Layout', LAYOUT_LABELS[viewMode], 'layout',
                ['list', 'kanban', 'timeline'].map(m => dispSubBtn(LAYOUT_LABELS[m], viewMode === m, () => { setViewMode(m); setDispOpen(false); setDispSub(null); }, m)))}
              {dispDiv}
              {dispMenuLabel('Organize')}
              {dispRow('Group by', (GROUP_OPTS.find(([id]) => id === sortBy) || [])[1], 'group',
                GROUP_OPTS.filter(([id]) => DISPLAY_GROUP_IDS.includes(id)).map(([id, label]) => dispSubBtn(label, sortBy === id, () => { setSortBy(id); setDispOpen(false); setDispSub(null); }, id)))}
              {dispRow('Sort by', (SORT_OPTS.find(([id]) => id === thenBy) || [])[1], 'sort',
                SORT_OPTS.map(([id, label]) => dispSubBtn(label, thenBy === id, () => { setThenBy(id); setDispOpen(false); setDispSub(null); }, id)))}
              {dispDiv}
              {dispStaticRow('Filter', 'None')}
              {dispStaticRow('Labels', 'Show')}
              {dispDiv}
              {/* Not in the prototype (which has no per-column visibility system) — kept
                  so the existing Source/Assignee/Labels/etc. column toggles aren't lost. */}
              {dispRow('Columns', 'Choose', 'cols',
                COLUMN_DEFS.map(c => dispSubBtn(c.label, cols.includes(c.id), () => setCols(cs => cs.includes(c.id) ? cs.filter(x => x !== c.id) : [...cs, c.id]), c.id)))}
            </div>
          )}
        </div>
      );
      useEffect(() => {
        if (!dispOpen) return;
        const onKey = (e) => { if (e.key === 'Escape') { setDispOpen(false); setDispSub(null); } };
        document.addEventListener('keydown', onKey);
        return () => document.removeEventListener('keydown', onKey);
      }, [dispOpen]);
      const dispScrim = dispOpen && <div onClick={() => { setDispOpen(false); setDispSub(null); }} style={{ position: 'fixed', inset: 0, zIndex: 29 }} />;
      ...
      const viewTabsRow = (
        <div style={{ display: 'flex', gap: 2, borderBottom: `1px solid ${bord}`, marginTop: 14 }}>
          {['list', 'kanban', 'timeline'].map(id => (
            <div key={id} onClick={() => setViewMode(id)} style={{ padding: '9px 15px', fontSize: 13, color: viewMode === id ? ink : sub, borderBottom: `2px solid ${viewMode === id ? gold : 'transparent'}`, marginBottom: -1, fontWeight: viewMode === id ? 500 : 400, cursor: 'pointer', fontFamily: C.fontSans }}>{LAYOUT_LABELS[id]}</div>
          ))}
        </div>
      );
      ...
      const milestoneFlag = (t, size) => (
        <i className={t.is_milestone ? 'ti ti-flag-3-filled' : 'ti ti-flag-3'}
           onClick={(e) => { e.stopPropagation(); toggleMilestone(t); }}
           title={t.is_milestone ? 'Unmark as milestone' : 'Mark as milestone'}
           style={{ fontSize: size || 15, color: t.is_milestone ? gold : (dark ? 'rgba(255,255,255,0.22)' : '#D8D2C6'), flexShrink: 0, cursor: 'pointer' }} />
      );
      const sourceChip = (t) => t._projectName ? (
        <span style={{ fontSize: '0.6rem', fontWeight: 500, padding: '2.5px 7px', borderRadius: 4, background: dark ? 'rgba(173,131,47,.14)' : '#FAF4E8', border: `1px solid ${dark ? 'rgba(173,131,47,.3)' : '#E7D8B4'}`, color: dark ? '#C9A45A' : '#8C6A24', fontFamily: C.fontSans, whiteSpace: 'nowrap' }}>▲ {t._projectName}</span>
      ) : null;
      ...
      const statusPillEl = (t, sz) => (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: sz || 12, fontWeight: 400, padding: '3px 9px', borderRadius: 20, border: `1px solid ${statusColor(t.status)}55`, background: statusBg(t.status), color: statusColor(t.status), fontFamily: C.fontSans, whiteSpace: 'nowrap', flexShrink: 0 }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: statusColor(t.status), flexShrink: 0 }} />{statusLabel(t.status)}</span>
      );
      const milestoneBadge = (
        <span style={{ color: gold, fontSize: '0.62rem', fontWeight: 600, border: `1px solid ${dark ? 'rgba(201,164,90,.35)' : '#E8D9BC'}`, background: dark ? 'rgba(201,164,90,.14)' : '#F3EBDA', borderRadius: 4, padding: '1px 6px', letterSpacing: '0.04em', flexShrink: 0, whiteSpace: 'nowrap', fontFamily: C.fontSans }}>◆ Milestone</span>
      );
      const openCtx = (e, t) => { e.preventDefault(); setCtxMenu({ x: e.clientX, y: e.clientY, task: t }); };
      ...
      const emailIconMini = (t) => (linksByTask[t.id] || []).length > 0 ? (
        <i className="ti ti-mail" title="Has a linked email" onClick={(e) => { e.stopPropagation(); setEmailTabFocus((linksByTask[t.id] || [])[0].thread_id); setCurrent(t); setView('detail'); }}
          style={{ fontSize: 14, color: '#185FA5', flexShrink: 0, cursor: 'pointer' }} />
      ) : null;
      ...
      const surfaceKind = { ctc: 'ctc_file', projects: 'project', rocks: 'rock' };
      ...
      useEffect(() => () => { if (!TASKS_EMBED) docTitle('Tasks'); }, []);
      const surfaceRef = useCloseOnOutside(surfaceMenuOpen, () => setSurfaceMenuOpen(false));
      const surfaceTitle = (fontSize) => (
        <div ref={surfaceRef} style={{ position: 'relative' }}>
          <div onClick={() => setSurfaceMenuOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            <span style={{ fontFamily: C.fontSans, fontSize: fontSize || 17, fontWeight: 600, color: ink, letterSpacing: '-0.01em' }}>{surfaceLabel[surface]}</span>
            <i className="ti ti-chevron-down" style={{ fontSize: 11, color: gold }} />
          </div>
          {surfaceMenuOpen && (
            <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', zIndex: 62, top: '100%', left: 0, background: dark ? '#0A1730' : '#fff', border: `1px solid ${bord}`, borderRadius: 8, boxShadow: '0 12px 32px rgba(0,26,74,0.18)', padding: 6, minWidth: 168, marginTop: 6 }}>
              {SURFACE_OPTS.map(([id, label]) => (
                <div key={id} onClick={() => { setSurface(id); setSurfaceMenuOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', fontSize: '0.82rem', color: surface === id ? ink : sub, fontWeight: surface === id ? 600 : 400, borderRadius: 5, background: surface === id ? (dark ? 'rgba(255,255,255,0.06)' : '#F7F4EE') : 'none', cursor: 'pointer', fontFamily: C.fontSans }}>
                  <i className="ti ti-check" style={{ fontSize: 12, color: gold, visibility: surface === id ? 'visible' : 'hidden', flexShrink: 0 }} />{label}
                </div>
              ))}
            </div>
          )}
      ...
      const plainSurfaceTitle = (fontSize) => (
        <span style={{ fontFamily: C.fontSans, fontSize: fontSize || 27, fontWeight: 500, color: ink, letterSpacing: '0.01em' }}>{surfaceLabel[surface]}</span>
      );
      ...
      const sbSectionHead = (label, first) => (
        <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.16em', textTransform: 'uppercase', color: sub, padding: '0 20px 8px', fontFamily: C.fontSans }}>{label}</div>
      );
      const sbCount = (n) => (
        <span style={{ fontSize: 12, color: sub, background: dark ? '#08132A' : '#FCFBF8', border: `1px solid ${bord}`, borderRadius: 20, padding: '1px 8px', marginLeft: 8, flexShrink: 0, fontFamily: C.fontSans }}>{n}</span>
      );
      const sbRow = (id, label, count) => (
        // Drop any pending record open: clicking a Rock then "All projects"
        // before the rocks finished loading handed the rock's id to the
        // projects list, which reported it as missing (a false error banner).
        <div key={id} onClick={() => { setOpenItemId(null); setSurface(id); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 20px', fontSize: 14.5, color: ink, fontWeight: surface === id ? 500 : 400, borderLeft: `2.5px solid ${surface === id ? gold : 'transparent'}`, background: surface === id ? `linear-gradient(90deg, ${dark ? 'rgba(201,164,90,.1)' : 'rgba(173,131,47,.08)'}, transparent)` : 'transparent', cursor: 'pointer', fontFamily: C.fontSans }}>
          <span style={{ display: 'flex', alignItems: 'center', minWidth: 0 }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: gold, marginRight: 9, flexShrink: 0 }} />
      ...
      const surfaceForRecordType = { ctc_file: 'ctc', project: 'projects', rock: 'rocks' };
      const sbItemRow = (item) => (
        <div key={item.id} onClick={() => { setSurface(surfaceForRecordType[item.record_type] || 'projects'); setOpenItemId(item.id); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 20px', fontSize: 14.5, color: ink, borderLeft: '2.5px solid transparent', cursor: 'pointer', fontFamily: C.fontSans }}>
          <span style={{ marginLeft: 16, color: sub, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 0 }}>{item.name}</span>
          {sbCount(item.taskCount)}
        </div>
      );
      const sbAddRow = (label, onClick) => (
        <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 20px', color: sub, fontSize: 13, cursor: 'pointer', fontFamily: C.fontSans }}>
          <i className="ti ti-plus" style={{ fontSize: 12 }} />{label}
        </div>
      );
      const projNavItems = navItems.filter(i => i.record_type === 'project');
      const rockNavItems = navItems.filter(i => i.record_type === 'rock');
      const sbGroup = (children) => <div style={{ marginBottom: 20 }}>{children}</div>;
      const navSidebar = (
        <div style={{ width: 230, flexShrink: 0, borderRight: `1px solid ${bord}`, padding: '18px 0', background: dark ? '#08132A' : '#FCFBF8', overflowY: 'auto' }}>
          {sbGroup(<React.Fragment>{sbSectionHead('Tasks', true)}{sbRow('my', 'My Tasks', data.tasks.length)}{sbRow('accountability', 'My Accountabilities', groupMembers(team, 'all').length)}</React.Fragment>)}
          {sbGroup(<React.Fragment>{sbSectionHead('Company')}{sbRow('rocks', 'Rocks', rockNavItems.length)}{rockNavItems.map(sbItemRow)}{sbAddRow('New rock', () => { setSurface('rocks'); setOpenItemId('new'); })}</React.Fragment>)}
          {sbGroup(<React.Fragment>{sbSectionHead('Projects')}{sbRow('projects', 'All projects', projNavItems.length)}{projNavItems.map(sbItemRow)}{sbAddRow('New project', () => { setSurface('projects'); setOpenItemId('new'); })}</React.Fragment>)}
        </div>
      );
          ...
          <div key={t.id} onClick={() => { setCurrent(t); setView('detail'); }} onContextMenu={(e) => openCtx(e, t)}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginTop: 5 }}>
                ...
                <span style={{ width: 6, height: 6, borderRadius: '50%', flexShrink: 0, background: priorityColor(t.priority) }} title={priorityLabel(t.priority)} />
                {sourceChip(t)}
                {t.due_at && <span style={{ fontSize: '0.68rem', color: late ? lateColor : sub, fontWeight: late ? 600 : 400, fontFamily: C.fontSans }}>{new Date(t.due_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>}
              </div>
              {myLabels.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 5 }}>{myLabels.map(l => { const lc = labelColors[l]; return <span key={l} style={{ padding: '1px 6px', borderRadius: 10, background: lc ? lc + '26' : (dark ? 'rgba(201,164,90,0.15)' : '#F3EBDA'), color: lc || (dark ? '#C9A45A' : '#AD832F'), fontSize: '0.58rem', fontWeight: 600, fontFamily: C.fontSans }}>{l}</span>; })}</div>}
            </div>
            {milestoneFlag(t, 16)}
      ...
      const deskRow = (t, isChild) => {
        ...
        return (
          <div key={t.id} onClick={() => { setCurrent(t); setView('detail'); }} onContextMenu={(e) => openCtx(e, t)}
            ...
            {cols.includes('status') && <div style={{ width: 130, flexShrink: 0 }}>{statusPillEl(t)}</div>}
            <div style={{ width: 90, flexShrink: 0, fontSize: 11, letterSpacing: '0.04em', textTransform: 'uppercase', color: t.priority === 'high' ? lateColor : t.priority === 'medium' ? gold : sub }}>{priorityLabel(t.priority)}</div>
            {cols.includes('due') && <div style={{ width: 96, flexShrink: 0, textAlign: 'right', fontSize: 13, color: late ? lateColor : sub, fontWeight: late ? 500 : 400 }}>{t.due_at ? new Date(t.due_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}</div>}
            {cols.includes('assignee') && <div style={{ width: 110, flexShrink: 0, fontSize: '0.7rem', color: sub, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{assignees.join(', ') || '—'}</div>}
            {cols.includes('labels') && <div style={{ width: 120, flexShrink: 0, display: 'flex', flexWrap: 'wrap', gap: 3 }}>{myLabels.length ? myLabels.map(l => <span key={l} style={{ fontSize: '0.58rem', color: gold, background: dark ? 'rgba(201,164,90,0.15)' : '#F3EBDA', padding: '1px 5px', borderRadius: 8 }}>{l}</span>) : <span style={{ color: sub, fontSize: '0.7rem' }}>—</span>}</div>}
            {cols.includes('created') && <div style={{ width: 88, flexShrink: 0, textAlign: 'right', fontSize: '0.68rem', color: sub }}>{t.created_at ? new Date(t.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}</div>}
            {cols.includes('modified') && <div style={{ width: 88, flexShrink: 0, textAlign: 'right', fontSize: '0.68rem', color: sub }}>{t.updated_at ? new Date(t.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}</div>}
          </div>
        );
      };
      ...
      function renderGroupedList(rowFn) {
        ...
        if (sortBy === 'none') {
          ...
          body = <div>{groupHead(dark ? '#C9A45A' : '#AD832F', 'By due date · ' + sorted.length)}{sorted.map(rowFn)}</div>;
        } else if (sortBy === 'label') {
          const groups = {}; const unlabeled = [];
          active.forEach(t => { const ls = myLabelsFor(t.id); if (!ls.length) unlabeled.push(t); else ls.forEach(l => { (groups[l] = groups[l] || []).push(t); }); });
          const names = Object.keys(groups).sort((a, b) => a.localeCompare(b));
          body = (
            <React.Fragment>
              {names.map(n => <div key={n} style={{ marginBottom: 16 }}>{groupHead(dark ? '#C9A45A' : '#AD832F', n + ' · ' + groups[n].length)}{groups[n].slice().sort(byThen).map(rowFn)}</div>)}
              {unlabeled.length > 0 && <div style={{ marginBottom: 16 }}>{groupHead(sub, 'Unlabeled · ' + unlabeled.length)}{unlabeled.slice().sort(byThen).map(rowFn)}</div>}
            </React.Fragment>
          );
          ...
          active.forEach(t => {
            ...
            if (!ids.length) unassigned.push(t); else ids.forEach(id => { (groups[id] = groups[id] || { name: (team.find(m => m.id === id) || {}).name, items: [] }).items.push(t); });
          });
          const ids = Object.keys(groups).sort((a, b) => groups[a].name.localeCompare(groups[b].name));
          body = (
            <React.Fragment>
              {ids.map(id => <div key={id} style={{ marginBottom: 16 }}>{groupHead(dark ? '#C9A45A' : '#AD832F', groups[id].name + ' · ' + groups[id].items.length, { key: 'assignee:' + id, hint: 'Add a task for ' + groups[id].name, assigneeId: id, assigneeName: groups[id].name })}{groups[id].items.slice().sort(byThen).map(rowFn)}</div>)}
              {/* No plus on Unassigned: a task with nobody on it is invisible
                  the moment it saves, on this surface above all. */}
              {unassigned.length > 0 && <div style={{ marginBottom: 16 }}>{groupHead(sub, 'Unassigned · ' + unassigned.length)}{unassigned.slice().sort(byThen).map(rowFn)}</div>}
            </React.Fragment>
          );
          ...
          body = TASK_PRIORITY.slice().sort((a, b) => PRI[a.id] - PRI[b.id]).map(p => {
            ...
            if (!items.length) return null;
            return <div key={p.id} style={{ marginBottom: 16 }}>{groupHead(p.color, priorityLabel(p.id) + ' priority · ' + items.length, { key: 'priority:' + p.id, hint: 'Add a ' + priorityLabel(p.id).toLowerCase() + ' priority task', priority: p.id })}{items.map(rowFn)}</div>;
          });
        } else if (sortBy === 'project') {
          const groups = {};
          active.forEach(t => { const n = t._projectName || 'No project'; (groups[n] = groups[n] || []).push(t); });
          ...
          <React.Fragment>
            ...
            <div style={{ marginTop: completed.length ? 16 : 0 }}>
              {completed.length > 0 && <button onClick={() => setCompletedOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', background: 'none', border: 'none', padding: '6px 0', cursor: 'pointer', fontFamily: C.fontSans }}>
                <i className={'ti ti-chevron-' + (completedOpen ? 'down' : 'right')} style={{ fontSize: 13, color: sub }} />
          ...
          <div onClick={(e) => e.stopPropagation()} style={containerStyle}>
            {embed && surface !== 'my' ? (
              ...
              surface === 'accountability'
                ? <AccountabilitySurface key="acct" dark={dark} team={team} titleNode={plainSurfaceTitle} />
                : <ProjectsSurface key={surface} kind={surfaceKind[surface] || 'project'} dark={dark} user={user} team={team} titleNode={plainSurfaceTitle} openId={openItemId} onOpenIdConsumed={() => setOpenItemId(null)} openCtcTab={openCtcTab} onCtcTabConsumed={() => setOpenCtcTab(null)} onChanged={refreshNav} />
            ) : embed ? (
              /* ══════ EMBEDDED POPOUT — unchanged, out of scope for the My Tasks redesign ══════ */
              wide ? (
                <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
                  <div style={{ width: 212, flexShrink: 0, borderRight: `1px solid ${bord}`, background: dark ? '#08132A' : '#FCFBF8', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
                      <i className="ti ti-checklist" style={{ fontSize: 15, color: gold, flexShrink: 0 }} />
                      <span style={{ flex: 1, fontFamily: C.fontSans, fontSize: 12.5, fontWeight: 600, color: ink }}>Tasks</span>
                      {searchBtn}
                      {labelsBtn}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>{sortSelect}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>{thenBySelect}</div>
                      {newBtn}
                    </div>
                    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>{listInner(true)}</div>
                  </div>
                  <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
                      {detailPane || (
                        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', textAlign: 'center', color: sub, fontFamily: C.fontSans }}>
                          <i className="ti ti-list-check" style={{ fontSize: 30, color: dark ? '#22324A' : '#D4D0CC', marginBottom: 12 }} />
                          <div style={{ fontSize: '0.86rem' }}>Select a task to see its details</div>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <React.Fragment>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '9px 12px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                      {view === 'list'
                        ? <i className="ti ti-checklist" style={{ fontSize: 15, color: gold }} />
                        : <i className="ti ti-chevron-left" onClick={back} aria-label="Back" style={{ fontSize: 16, cursor: 'pointer', color: sub }} />}
                      <span style={{ fontFamily: C.fontSans, fontSize: 12.5, fontWeight: 600, letterSpacing: '0.03em', color: ink, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{titleText}</span>
                    </div>
                    {view === 'list'
                      ...
                      : (!embed && <i className="ti ti-x" onClick={onClose} aria-label="Close" style={{ fontSize: 15, cursor: 'pointer', color: dark ? 'rgba(255,255,255,0.4)' : '#B4B2A9', flexShrink: 0 }} />)}
                  </div>
                  <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
                    {detailPane || listInner(false)}
                  </div>
                </React.Fragment>
              )
            ) : (
              /* ══════ STANDALONE /tasks — My Tasks / CTC Files / Projects ══════ */
              wide ? (
                ...
                <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                    {surface !== 'my' ? (
                      ...
                      surface === 'accountability'
                        ? <AccountabilitySurface key="acct" dark={dark} team={team} titleNode={plainSurfaceTitle} />
                        : <ProjectsSurface key={surface} kind={surfaceKind[surface] || 'project'} dark={dark} user={user} team={team} titleNode={plainSurfaceTitle} hideSidebar openId={openItemId} onOpenIdConsumed={() => setOpenItemId(null)} openCtcTab={openCtcTab} onCtcTabConsumed={() => setOpenCtcTab(null)} onChanged={refreshNav} />
                    ) : (
                      <React.Fragment>
                        {/* List/Kanban/Timeline always stays mounted and visible — opening a
                            task no longer replaces it, it overlays a drawer on top (below). */}
                        <div style={{ padding: '26px 34px 0', flexShrink: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                            <div>
                              {plainSurfaceTitle()}
                              <div style={{ fontFamily: C.fontSans, fontSize: 13.5, color: sub, marginTop: 3 }}>{dueTodayCount} due today · {overdueCount} overdue · {openCount} open</div>
                            </div>
                        ...
                        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch', display: 'flex', flexDirection: 'column', padding: '14px 34px 0' }}>
                          {routeNoticeBar}
                          {loading ? <div style={{ color: sub, fontSize: '0.82rem', padding: 20, fontFamily: C.fontSans }}>Loading…</div>
                            ...
                            : (
                              <React.Fragment>
                                <div style={{ display: 'flex', alignItems: 'center', padding: '0 12px 8px', fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 500, color: sub, borderBottom: `1px solid ${bord}`, gap: 12, fontFamily: C.fontSans }}>
                                  <span style={{ width: 14, flexShrink: 0 }} /><span style={{ flex: 1 }}>Task</span>
                                  {cols.includes('source') && <span style={{ width: 150, flexShrink: 0 }}>Source</span>}
                                  {cols.includes('status') && <span style={{ width: 130, flexShrink: 0 }}>Status</span>}
                                  <span style={{ width: 90, flexShrink: 0 }}>Priority</span>
                                  {cols.includes('due') && <span style={{ width: 96, flexShrink: 0, textAlign: 'right' }}>Due</span>}
                                  {cols.includes('assignee') && <span style={{ width: 110, flexShrink: 0 }}>Assignee</span>}
                                  {cols.includes('labels') && <span style={{ width: 120, flexShrink: 0 }}>Labels</span>}
                        ...
                        {view !== 'list' && (
                          <TaskDrawer dark={dark} onClose={back}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 20px', borderBottom: `1px solid ${bord}`, flexShrink: 0, background: dark ? '#0A1730' : '#fff' }}>
                              <i className="ti ti-chevron-left" onClick={back} aria-label="Back" style={{ fontSize: 16, cursor: 'pointer', color: sub, flexShrink: 0 }} />
                              <div style={{ flex: 1, minWidth: 0, fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: sub, fontFamily: C.fontSans, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{(current && current._projectName) || 'My Tasks'} / {titleText}</div>
                              <i className="ti ti-x" onClick={back} aria-label="Close" style={{ fontSize: 15, cursor: 'pointer', color: sub, flexShrink: 0 }} />
                            </div>
                            <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>{detailPane}</div>
                          </TaskDrawer>
                        )}
                        {mailboxOpen && (
                          <TaskMailbox dark={dark} user={user} tasks={data.tasks} linksByTask={linksByTask}
                            onAttached={onEmailAttached}
                            onCreateTask={onCreateTaskFromEmail}
                            onOpenTask={(t, threadId) => { setMailboxOpen(false); setEmailTabFocus(threadId); setCurrent(t); setView('detail'); }}
                ...
                surface !== 'my' ? (
                  surface === 'accountability'
                    ? <AccountabilitySurface key="acct" dark={dark} team={team} titleNode={surfaceTitle} />
                    : <ProjectsSurface key={surface} kind={surfaceKind[surface] || 'project'} dark={dark} user={user} team={team} titleNode={surfaceTitle} openId={openItemId} onOpenIdConsumed={() => setOpenItemId(null)} openCtcTab={openCtcTab} onCtcTabConsumed={() => setOpenCtcTab(null)} onChanged={refreshNav} />
                ) : view !== 'list' ? (
                  <React.Fragment>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
                      <i className="ti ti-chevron-left" onClick={back} aria-label="Back" style={{ fontSize: 16, cursor: 'pointer', color: sub }} />
                      <span style={{ fontFamily: C.fontSans, fontSize: 12.5, fontWeight: 600, letterSpacing: '0.03em', color: ink }}>{titleText}</span>
                    </div>
                    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>{detailPane}</div>
                  </React.Fragment>
                ) : (
                  <React.Fragment>
                    <div style={{ padding: '12px 14px 0', borderBottom: `1px solid ${bord}`, flexShrink: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
                        {surfaceTitle(15)}
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                          {groupChip}{sortChip}{searchChip}
                          {labelsBtn}
                          {newBtn}
                        </div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', padding: '2px 0 10px' }}>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, paddingRight: 14, marginRight: 14, borderRight: `1px solid ${bord}` }}><span style={{ fontSize: 15, fontWeight: 600, color: ink, fontFamily: C.fontSans }}>{dueTodayCount}</span><span style={{ fontSize: 10, color: sub, fontFamily: C.fontSans }}>due today</span></div>
                        {overdueCount > 0 && <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, paddingRight: 14, marginRight: 14, borderRight: `1px solid ${bord}` }}><span style={{ fontSize: 15, fontWeight: 600, color: lateColor, fontFamily: C.fontSans }}>{overdueCount}</span><span style={{ fontSize: 10, color: lateColor, fontFamily: C.fontSans }}>overdue</span></div>}
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}><span style={{ fontSize: 15, fontWeight: 600, color: ink, fontFamily: C.fontSans }}>{openCount}</span><span style={{ fontSize: 10, color: sub, fontFamily: C.fontSans }}>open</span></div>
                      </div>
                    </div>
    ...
    // ─── Top Bar ─────────────────────────────────────────────────────
    function TopBar({ dark, onToggleDark, onCalendar, calendarOpen, onTasks, tasksOpen, onDecisions, decisionsOpen, avatar, name, dotColor, onProfile }) {
      // Gold-tinted rounded-square chip; same tint + gold icon in light and dark.
      const iconBtn = (onClick, iconClass, title, active) => (
        <button
          onClick={onClick}
          title={title}
          aria-label={title}
          style={{
            ...
            width: 32, height: 32, borderRadius: 9, cursor: 'pointer', padding: 0,
            background: active ? 'rgba(201,164,90,0.30)' : 'rgba(201,164,90,0.16)', border: 'none',
            boxShadow: active ? '0 0 0 2px #C9A45A' : 'none',
            color: '#C9A45A', fontSize: 16, lineHeight: 1, transition: 'background 0.15s, box-shadow 0.15s',
          }}
        >
      ...
      return (
        <div style={{
          ...
          background: dark ? '#0A1730' : '#001A4A', backdropFilter: 'blur(20px) saturate(180%)',
          WebkitBackdropFilter: 'blur(20px) saturate(180%)',
          borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <a href="index.html" title="Back to TMG App" style={{ color: 'rgba(255,255,255,.7)', textDecoration: 'none', display: 'flex', alignItems: 'center', marginRight: 3 }}><i className="ti ti-arrow-left" style={{ fontSize: 18 }} /></a>
            <img src="m-monogram.png" alt="" style={{ height: 22, width: 'auto', display: 'block' }} />
            <div style={{ height: 22, fontSize: '14.4px', lineHeight: '22px', fontWeight: 600, color: C.gold, letterSpacing: '0.18em', textTransform: 'uppercase', fontFamily: "'Jost', sans-serif", whiteSpace: 'nowrap' }}>TMG APP</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {iconBtn(onTasks, 'ti ti-checklist', 'Tasks', tasksOpen)}
            <button onClick={onDecisions} title="Decisions" aria-label="Decisions" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 9, cursor: 'pointer', padding: 0, background: decisionsOpen ? 'rgba(201,164,90,0.30)' : 'rgba(201,164,90,0.16)', border: 'none', boxShadow: decisionsOpen ? '0 0 0 2px #C9A45A' : 'none', color: '#C9A45A', transition: 'background 0.15s, box-shadow 0.15s' }}>
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 4v6" /><path d="M9 7l3-3 3 3" /><path d="M4 13l-2 2 2 2" /><path d="M2 15h7" /><path d="M20 13l2 2-2 2" /><path d="M22 15h-7" /></svg>
            </button>
            {iconBtn(onCalendar, 'ti ti-calendar', 'Calendar', calendarOpen)}
            <button onClick={onProfile} aria-label="Your profile" style={{ position: 'relative', width: 32, height: 32, borderRadius: 9, border: 'none', padding: 0, cursor: 'pointer', background: 'rgba(201,164,90,0.16)', flexShrink: 0 }}>
              {avatar
                ? <img src={avatar} alt="" style={{ width: 32, height: 32, borderRadius: 9, objectFit: 'cover', display: 'block' }} />
                : <span style={{ display: 'flex', width: 32, height: 32, borderRadius: 9, alignItems: 'center', justifyContent: 'center', color: C.goldSoft, fontFamily: C.fontSans, fontWeight: 600, fontSize: 13 }}>{(name || '?').trim().charAt(0).toUpperCase()}</span>}
              <span style={{ position: 'absolute', right: -2, bottom: -2, width: 11, height: 11, borderRadius: '50%', background: dotColor, border: `2px solid ${dark ? '#0A1730' : '#001A4A'}` }} />
            </button>
          </div>
        </div>
      );
    ...
    function ProfilePanel({ dark, user, profile, name, avatar, away, statusText, statusEmoji, onToggleAway, onOpenStatus, onClose, historySide, setHistorySide, setDark, fontScale, setFontScale }) {
      ...
      useEffect(() => { ProfileDB.loadAll().then(setAllUsers); }, []);
      ...
      useEffect(() => {
        let c = false;
        callCalendar({ action: 'calendars' })
          .then(({ ok, data }) => { if (!c) setGcalState(ok && Array.isArray(data.calendars) && data.calendars.length ? 'connected' : 'disconnected'); })
          .catch(() => { if (!c) setGcalState('disconnected'); });
        return () => { c = true; };
      }, []);

      const email = (me && me.email) || (user && user.email) || '';
      const roleLine = (me && me.title) || '';
      const managerName = (id) => { if (!id) return ''; const m = allUsers.find(u => u.id === id); return m ? ((((m.first_name || '') + ' ' + (m.last_name || '')).trim()) || m.email) : ''; };
      const locationText = () => { if (!me) return ''; const c = COUNTRIES.find(x => x.c === me.country); const cn = c ? c.n : (me.country || ''); const city = me.timezone ? tzCity(me.timezone) : ''; return [city, cn].filter(Boolean).join(', '); };
      async function saveMine() {
        setSaving(true);
        try { await ProfileDB.updateMine({ first_name: first, last_name: last, phone, country, timezone, dob: dob || null, pets }); setMe(prev => ({ ...(prev || {}), first_name: first, last_name: last, phone, country, timezone, dob: dob || null, pets })); setEditing(false); }
      ...
      const inputStyle  = { width: '100%', padding: '10px 12px', background: ctrlBg, border: `1px solid ${ctrlBorder}`, borderRadius: 10, color: ink, fontSize: 13, outline: 'none', fontFamily: J };

      const detailRow = (label, value, opts) => {
        opts = opts || {};
        return (
          <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: opts.last ? 'none' : `1px solid ${rowBorder}` }}>
            <span style={{ fontFamily: J, fontSize: 10, color: keyMuted, flexShrink: 0 }}>{label}</span>
            <span style={{ fontFamily: J, fontSize: opts.small ? 8.5 : 10, fontWeight: 600, color: opts.muted ? valMuted : ink, textAlign: 'right', marginLeft: 12, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value}</span>
          </div>
        );
      };
      ...
      return (
        <React.Fragment>
          ...
          {/* WebkitOverflowScrolling + translateZ force this fixed, self-scrolling drawer onto its
              own compositing layer — without it, iOS Safari can repaint stale content behind the
              new layout mid-scroll, showing as the panel's cards briefly overlapping. */}
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '88%', maxWidth: 390, zIndex: 71, background: slideBg, boxShadow: '-12px 0 30px rgba(0,0,0,0.28)', display: 'flex', flexDirection: 'column', overflowY: 'auto', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', transform: 'translateZ(0)' }}>
            <div style={{ padding: 'calc(14px + env(safe-area-inset-top)) 16px 0' }}>
              {/* ─── one cohesive profile panel ─── */}
              <div style={{ borderRadius: 18, overflow: 'hidden', border: `1px solid ${panelBorder}`, boxShadow: '0 12px 40px rgba(0,26,74,0.12)', background: panelBg }}>
                {/* 1 · accent bar */}
                <div style={{ height: 4, background: accentBar }} />

                {/* 2 · merged identity header (no role badge) */}
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 13, padding: '18px 18px 16px' }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    {avatar
                      ? <img src={avatar} alt="" style={{ width: 58, height: 58, borderRadius: '50%', objectFit: 'cover' }} />
                      : <div style={{ width: 58, height: 58, borderRadius: '50%', background: C.navy, color: C.goldSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: J, fontWeight: 600, fontSize: 22 }}>{(name || '?').trim().charAt(0).toUpperCase()}</div>}
                    <span style={{ position: 'absolute', right: 2, bottom: 2, width: 14, height: 14, borderRadius: '50%', background: presence, border: `2.5px solid ${panelBg}` }} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: J, fontSize: 16, fontWeight: 600, color: ink, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</div>
                    {roleLine && <div style={{ fontFamily: J, fontSize: 10, color: keyMuted, marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{roleLine}</div>}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
                      <span style={{ width: 6, height: 6, borderRadius: '50%', background: presence }} />
                      <span style={{ fontFamily: J, fontSize: 8, letterSpacing: '0.08em', fontWeight: 500, color: presence }}>{presenceTxt}</span>
                    </div>
                  </div>
                  <button onClick={onClose} aria-label="Close" style={{ flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', color: valMuted, fontSize: 17, display: 'flex' }}><i className="ti ti-x" /></button>
                </div>
                ...
                {!editing ? (
                  <React.Fragment>
                    ...
                    <div style={{ padding: '0 14px 12px' }}>
                      <button onClick={onOpenStatus} style={{ display: 'flex', alignItems: 'center', gap: 9, width: '100%', textAlign: 'left', borderRadius: 11, border: `1px solid ${ctrlBorder}`, background: ctrlBg, padding: '10px 12px', marginBottom: 8, cursor: 'pointer', fontFamily: J, fontSize: 10, color: (statusText || statusEmoji) ? ink : keyMuted }}>
                        <i className="ti ti-mood-smile" style={{ fontSize: 15, color: gold }} />
                        {statusText ? ((statusEmoji ? statusEmoji + ' ' : '') + statusText) : "What's your status?"}
                      </button>
                      <button onClick={onToggleAway} style={{ display: 'flex', alignItems: 'center', gap: 9, width: '100%', textAlign: 'left', borderRadius: 11, border: `1px solid ${awayBorder}`, background: ctrlBg, padding: '9px 12px', cursor: 'pointer', fontFamily: J, fontSize: 10, fontWeight: 500, color: ink }}>
                        <i className="ti ti-clock-pause" style={{ fontSize: 15, color: away ? '#E0A93B' : gold }} />
                        {away ? 'Set yourself as active' : 'Set yourself as away'}
                      </button>
                    </div>

                    {/* 4 · Details label */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 18px 8px' }}>
                      <span style={{ fontFamily: J, fontSize: 7, letterSpacing: '0.18em', fontWeight: 600, textTransform: 'uppercase', color: gold }}>Details</span>
                      <span style={{ flex: 1, height: 1, background: ruleCol }} />
                    </div>

                    {/* 5 · field rows */}
                    <div style={{ padding: '0 18px' }}>
                      {detailRow('Employee ID', (me && me.employee_id) || '—', { muted: !(me && me.employee_id) })}
                      {detailRow('Title', roleLine || '—', { muted: !roleLine })}
                      {detailRow('Reports to', mgr || '—', { muted: !mgr })}
                      {detailRow('Phone', (me && me.phone) ? contactLink('Phone', me.phone) : '—', { muted: !(me && me.phone) })}
                      {detailRow('Email', email ? contactLink('Email', email) : '—', { small: true, muted: !email })}
                      {detailRow('Location', loc || '—', { muted: !loc })}
                      {detailRow('Local time', (me && me.timezone) ? localTimeIn(me.timezone) : '—', { muted: !(me && me.timezone) })}
                      {detailRow('Pets', petStr || '—', { last: true, muted: !petStr })}
                    </div>

                    {/* 6 · edit button */}
                    <div style={{ padding: '14px 18px 18px' }}>
                      <button onClick={() => { setFirst((me && me.first_name) || ''); setLast((me && me.last_name) || ''); setPhone((me && me.phone) || ''); setCountry((me && me.country) || ''); setTimezone((me && me.timezone) || ''); setDob((me && me.dob) || ''); setPets((me && me.pets) || []); setEditing(true); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, width: '100%', padding: 13, borderRadius: 11, border: 'none', cursor: 'pointer', fontFamily: J, fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', background: dark ? '#AD832F' : '#001A4A', color: '#fff' }}>
                        <i className="ti ti-pencil" style={{ fontSize: 14 }} />Edit my info
                      </button>
                    </div>
                  </React.Fragment>
                ) : (
                  <div style={{ padding: '4px 18px 18px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <input placeholder="First name" value={first} onChange={e => setFirst(e.target.value)} style={inputStyle} />
                    <input placeholder="Last name" value={last} onChange={e => setLast(e.target.value)} style={inputStyle} />
                    <input placeholder="Phone" value={phone} onChange={e => setPhone(e.target.value)} style={inputStyle} />
                    <div style={{ fontFamily: J, fontSize: 10, color: keyMuted, marginTop: 2 }}>Location</div>
                    <select value={country} onChange={e => { setCountry(e.target.value); setTimezone(''); }} style={inputStyle}><option value="">Country…</option>{COUNTRIES.map(c => <option key={c.c} value={c.c}>{c.n}</option>)}</select>
                    {country && (COUNTRY_TZ[country] || []).length > 0 && <select value={timezone} onChange={e => setTimezone(e.target.value)} style={inputStyle}><option value="">City / time zone…</option>{(COUNTRY_TZ[country] || []).map(z => <option key={z} value={z}>{tzCity(z)}</option>)}</select>}
                    {timezone && <div style={{ fontFamily: J, fontSize: 10, color: keyMuted }}>Local time: <b style={{ color: ink }}>{localTimeIn(timezone)}</b></div>}
                    <div style={{ fontFamily: J, fontSize: 10, color: keyMuted, marginTop: 2 }}>Date of birth <span style={{ opacity: 0.7 }}>(optional)</span></div>
                    <input type="date" value={dob || ''} onChange={e => setDob(e.target.value)} style={inputStyle} />
                    <div style={{ fontFamily: J, fontSize: 10, color: keyMuted, marginTop: 2 }}>Pets</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>{PETS.map(p => { const on = pets.includes(p.k); return <button key={p.k} type="button" onClick={() => setPets(on ? pets.filter(x => x !== p.k) : [...pets, p.k])} style={{ padding: '7px 11px', borderRadius: 20, border: `1px solid ${on ? C.navy : ctrlBorder}`, background: on ? C.navy : ctrlBg, color: on ? '#fff' : keyMuted, fontSize: 12, cursor: 'pointer', fontFamily: J }}>{p.e} {p.label}</button>; })}</div>
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                      <button onClick={saveMine} disabled={saving} style={{ flex: 1, padding: 11, background: dark ? '#AD832F' : C.navy, color: '#fff', border: 'none', borderRadius: 11, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: J }}>{saving ? 'Saving…' : 'Save'}</button>
                      <button onClick={() => setEditing(false)} style={{ flex: 1, padding: 11, background: 'none', color: keyMuted, border: `1px solid ${ctrlBorder}`, borderRadius: 11, fontSize: 13, cursor: 'pointer', fontFamily: J }}>Cancel</button>
                    </div>
                    <div style={{ fontFamily: J, fontSize: 9, color: keyMuted, lineHeight: 1.4 }}>Employee ID, title, reports-to, and access are set by an admin.</div>
                  </div>
                )}
              </div>
            </div>

            {/* Google Calendar & Tasks — standalone card, matches Appearance/etc. below
                [ported from index.html's ProfilePanel, which this was missing entirely]. */}
            <div style={{ background: ctrlBg, border: dark ? `1px solid ${panelBorder}` : 'none', boxShadow: dark ? 'none' : '0 2px 8px rgba(0,26,74,0.07), 0 1px 2px rgba(0,26,74,0.04)', borderRadius: 14, padding: 16, margin: '14px 16px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontFamily: J, fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: ink }}>Google Email, Calendar &amp; Tasks</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto', fontFamily: J, fontSize: 8, fontWeight: 600, letterSpacing: '0.04em', color: gcalState === 'connected' ? presence : keyMuted }}>
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: gcalState === 'connected' ? presence : valMuted }} />
                  {gcalState === 'checking' ? '…' : gcalState === 'connected' ? 'CONNECTED' : 'NOT CONNECTED'}
                </span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 12 }}>
                {[
                  'Gmail is read to update CTC files or connect it to tasks',
                  'Google Calendar is shown in the Calendar tab',
                  'TMG App tasks are synced to your Google Tasks',
                ].map((line, i) => (
                  <div key={i} style={{ display: 'flex', gap: 6, fontFamily: J, fontSize: 10, color: keyMuted, lineHeight: 1.5 }}>
                    <span style={{ color: gold, flexShrink: 0 }}>•</span>
                    <span>{line}</span>
                  </div>
                ))}
              </div>
              <button onClick={() => { try { window.SupabaseAuth.connectCalendar(); } catch (e) {} }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, width: '100%', padding: 12, borderRadius: 11, border: `1px solid ${ctrlBorder}`, cursor: 'pointer', fontFamily: J, fontSize: 11, fontWeight: 600, letterSpacing: '0.04em', background: dark ? '#0A1730' : '#FCFBF8', color: ink }}>
                <i className="ti ti-brand-google" style={{ fontSize: 14, color: gold }} />
                {gcalState === 'connected' ? 'Reconnect Google Calendar & Tasks' : 'Connect Google Calendar & Tasks'}
              </button>
            </div>

            {/* ─── below the panel (untouched): Appearance · Chat History · Sign out ─── */}
            <div style={{ paddingTop: 14 }}>
              <SettingsTab embedded historySide={historySide} setHistorySide={setHistorySide} dark={dark} setDark={setDark} fontScale={fontScale} setFontScale={setFontScale} />
            </div>
          </div>
        </React.Fragment>
      );
    ...
    // ─── Set-status sheet ────────────────────────────────────────────
    function StatusModal({ dark, initialText, initialEmoji, onSave, onClose }) {
      const J = "'Jost', sans-serif";
      ...
      const SUGGEST = [
        { e: '📅', t: 'In a meeting' }, { e: '🍔', t: 'Out at lunch' }, { e: '🚌', t: 'Commuting' },
        { e: '🏡', t: 'Working remotely' }, { e: '🤒', t: 'Out sick' }, { e: '🏖️', t: 'On vacation' },
      ];
      return (
        <React.Fragment>
          <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(0,0,0,0.5)' }} />
          <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 81, maxHeight: '88%', background: dark ? '#0A1730' : '#FFFFFF', borderTopLeftRadius: 20, borderTopRightRadius: 20, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px' }}>
              <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: J, fontSize: 15, color: accent }}>Cancel</button>
              <button onClick={() => onSave(text.trim(), emoji)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: J, fontSize: 15, fontWeight: 600, color: accent }}>Done</button>
            </div>
            <div style={{ padding: '0 16px 24px', overflowY: 'auto' }}>
              <div style={{ fontFamily: J, fontSize: 22, fontWeight: 600, color: ink, marginBottom: 6 }}>Set your status</div>
              <div style={{ fontFamily: J, fontSize: 13, color: sub, lineHeight: 1.5, marginBottom: 16 }}>Let your team know what you're up to.</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, border: `1px solid ${bord}`, borderRadius: 12, padding: '4px 8px 4px 4px', marginBottom: 18 }}>
                <input value={emoji} onChange={e => setEmoji(e.target.value.slice(0, 4))} placeholder="🙂" style={{ width: 44, height: 40, textAlign: 'center', fontSize: 20, border: 'none', outline: 'none', background: 'transparent' }} />
                <input value={text} onChange={e => setText(e.target.value)} placeholder="What's your status?" style={{ flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent', fontFamily: J, fontSize: 15, color: ink }} />
                {(text || emoji) && <button onClick={() => { setText(''); setEmoji(''); }} aria-label="Clear" style={{ background: 'none', border: 'none', cursor: 'pointer', color: sub, fontSize: 18, display: 'flex' }}><i className="ti ti-x" /></button>}
              </div>
              <div style={{ fontFamily: J, fontSize: 12, color: sub, letterSpacing: '0.04em', marginBottom: 4 }}>Suggestions</div>
              {SUGGEST.map((s, i) => (
                <button key={i} onClick={() => { setEmoji(s.e); setText(s.t); }} style={{ display: 'flex', alignItems: 'center', gap: 13, width: '100%', textAlign: 'left', padding: '12px 4px', background: 'none', border: 'none', borderTop: i ? `1px solid ${dark ? '#152545' : '#F0EBE3'}` : 'none', cursor: 'pointer', fontFamily: J, fontSize: 15, color: ink }}>
                  <span style={{ fontSize: 20, width: 24, textAlign: 'center' }}>{s.e}</span>{s.t}
                </button>
              ))}
            </div>
          </div>
    ...
    function CalendarPopover({ dark, zoneH, onOpenTask, onClose }) {
      const J = "'Jost', sans-serif";
      const p2 = (n) => String(n).padStart(2, '0');
      ...
      const K = dark ? {
        pop: '#070F1E', bd: '#152545', bd2: '#0D1E3A', shadow: '0 6px 22px rgba(0,0,0,.5)', navBg: '#040C1C',
        date: '#fff', tBtnTx: '#5AA0E6', tBtnBg: '#0C1830', tBtnBd: '#22324A',
        npBg: '#0C1830', npBd: '#22324A', npIc: 'rgba(255,255,255,.4)',
        sep: '#22324A', gold: '#C9A45A', circBd: 'rgba(255,255,255,.3)', doneBg: '#0F6E56', doneName: 'rgba(255,255,255,.35)', prioName: '#9FC0F0', reg: 'rgba(255,255,255,.55)',
        chipBd: '#22324A', chipBg: '#0C1830', chipIc: 'rgba(255,255,255,.4)', goldBd: '#3A2E14', goldBg: '#1E1808',
        segBg: '#0C1830', segTx: 'rgba(255,255,255,.4)', segABg: '#152545', segATx: '#fff',
        taskBg: '#0A1730', cellBd: '#0D1E3A', muted: 'rgba(255,255,255,.25)', rowBd: '#0D1E3A',
        dueOver: '#E07A7A', dueToday: '#C9A45A', dueUp: 'rgba(255,255,255,.35)',
        tzBg: '#040C1C', tx: '#5AA0E6', ph: '#C9A45A', dayLetter: 'rgba(255,255,255,.4)', weekend: 'rgba(255,255,255,.25)',
        dot: '#C9A45A', todBg: '#0A1E44', circBg: '#152545', circTx: '#fff',
        slot: '#0D1E3A', gline: '#0D1E3A',
        evNavyBd: '#3F6FA8', evNavyBg: '#1A2A3A', evGoldBd: '#C9A45A', evGoldBg: '#2A1F0A', evTitle: '#fff', evTime: 'rgba(255,255,255,.5)',
        weekTodayCol: 'rgba(201,164,90,.06)', weekendCell: '#0A1322',
        cellBg: '#070F1E', monthBd: '#152545', todCellBg: '#0A1E44', todCellBd: '#3A2E14', dotEvent: '#C9A45A', dotTask: '#3F6FA8',
      } : {
        pop: '#fff', bd: '#EDE7DC', bd2: '#EDE7DC', shadow: '0 6px 22px rgba(0,26,74,.1)', navBg: '#fff',
        date: '#001A4A', tBtnTx: '#185FA5', tBtnBg: '#EEF3FB', tBtnBd: '#C5D8F0',
        npBg: '#FCFBF8', npBd: '#E4DFD4', npIc: '#6B6B6B',
        sep: '#E4DFD4', gold: '#AD832F', circBd: '#C4B9A8', doneBg: '#0F6E56', doneName: '#B4B2A9', prioName: '#001A4A', reg: '#6B7280',
        chipBd: '#E4DFD4', chipBg: '#FCFBF8', chipIc: '#6B6B6B', goldBd: '#E8D9BC', goldBg: '#F3EBDA',
        segBg: '#F0EBE3', segTx: '#888', segABg: '#001A4A', segATx: '#fff',
        taskBg: '#FDFAF5', cellBd: '#F0EBE3', muted: '#B4B2A9', rowBd: '#F5F2EE',
        dueOver: '#9B1C1C', dueToday: '#AD832F', dueUp: '#B4B2A9',
        tzBg: '#FCFBF8', tx: '#185FA5', ph: '#AD832F', dayLetter: '#B4B2A9', weekend: '#D4D0CC',
        dot: '#AD832F', todBg: '#F3EBDA', circBg: '#001A4A', circTx: '#fff',
        slot: '#F0EBE3', gline: '#F5F2EE',
        evNavyBd: '#001A4A', evNavyBg: '#EEF2F8', evGoldBd: '#AD832F', evGoldBg: '#FDF8F0', evTitle: '#001A4A', evTime: '#6B6B6B',
        weekTodayCol: '#FBF8F2', weekendCell: '#FAFAF8',
        cellBg: '#fff', monthBd: '#F0EBE3', todCellBg: '#F3EBDA', todCellBd: '#E8D9BC', dotEvent: '#AD832F', dotTask: '#001A4A',
      };

      // ── data ──
      useEffect(() => {
        let cancelled = false; setGcal('loading');
        let tMin, tMax;
        if (view === 'month') { tMin = mondayOf(new Date(anchor.getFullYear(), anchor.getMonth(), 1)); tMax = addDays(tMin, 42); }
        else if (view === 'week') { tMin = mondayOf(anchor); tMax = addDays(tMin, 7); }
        else { tMin = new Date(anchor); tMin.setHours(0, 0, 0, 0); tMax = addDays(tMin, 1); }
        ...
        const personalP = fetchGoogleEvents(tMin.toISOString(), tMax.toISOString(), calList.map(c => c.id))
          .then(list => ({ ok: true, list: list || [] }))
      ...
      useEffect(() => { let c = false;
        TaskDB.loadAll().then(d => { if (!c && d) { setTasks(d.tasks || []); setPeopleByTask(d.peopleByTask || {}); } }).catch(() => {});
        ProfileDB.loadAll().then(ps => { if (!c && ps) {
          const m = {}; ps.forEach(p => { m[p.id] = (((p.first_name || '') + ' ' + (p.last_name || '')).trim()) || p.email || 'User'; }); setRoster(m);
          ...
          if (mine && mine.calendar_prefs && typeof mine.calendar_prefs === 'object') {
            const cp = { ...CAL_PREFS_DEFAULT, ...mine.calendar_prefs };
            setCalPrefs(cp); try { localStorage.setItem(calPrefsKey, JSON.stringify(cp)); } catch (e) {}
          }
        } }).catch(() => {});
        return () => { c = true; }; }, []);

      // ── timezone helpers (TX=Austin/America-Chicago, PH=Manila/Asia-Manila, live) ──
      const tzOffMin = (date, tz) => { const f = new Intl.DateTimeFormat('en-US', { timeZone: tz, hour12: false, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }); const m = {}; f.formatToParts(date).forEach(p => m[p.type] = p.value); return (Date.UTC(+m.year, +m.month - 1, +m.day, +m.hour, +m.minute, +m.second) - date.getTime()) / 60000; };
      // The instant at wall-clock hour `h` on date `d` in zone `tz` (rail anchors to the PRIMARY zone).
      const tzInstant = (d, h, tz) => { const off = tzOffMin(new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), h)), tz); return new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), h) - off * 60000); };
      const fmtHour = (inst, tz) => new Intl.DateTimeFormat('en-US', { timeZone: tz, hour: 'numeric', hour12: true }).format(inst);
      const fmtClock = (inst, tz) => new Intl.DateTimeFormat('en-US', { timeZone: tz, hour: 'numeric', minute: '2-digit', hour12: true }).format(inst);
      // Wall-clock hour (as a float, e.g. 9.5) of `inst` in zone `tz` — used to position events.
      const tzFloat = (inst, tz) => { const f = new Intl.DateTimeFormat('en-US', { timeZone: tz, hour: '2-digit', minute: '2-digit', hour12: false }); const m = {}; f.formatToParts(inst).forEach(p => m[p.type] = p.value); return ((+m.hour) % 24) + (+m.minute) / 60; };
      // 'YYYY-MM-DD' / 'HH:mm' of an instant as read in zone `tz` (for prefilling the event form's date/time inputs).
      const dateInTz = (inst, tz) => { const m = {}; new Intl.DateTimeFormat('en-CA', { timeZone: tz, year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(inst).forEach(p => m[p.type] = p.value); return m.year + '-' + m.month + '-' + m.day; };
      const timeInTz = (inst, tz) => { const m = {}; new Intl.DateTimeFormat('en-GB', { timeZone: tz, hour: '2-digit', minute: '2-digit', hour12: false }).formatToParts(inst).forEach(p => m[p.type] = p.value); return (m.hour === '24' ? '00' : m.hour) + ':' + m.minute; };
      const rruleToSimple = (rec) => { const r = (rec || []).find(x => /^RRULE:/.test(x)) || ''; if (/FREQ=DAILY/.test(r)) return 'daily'; if (/FREQ=WEEKLY/.test(r)) return 'weekly'; if (/FREQ=MONTHLY/.test(r)) return 'monthly'; return 'none'; };

      const setF = (patch) => setEditing(prev => ({ ...prev, ...patch }));
      // Open the form to create a new event (defaults: the focused day, next round hour).
      const openCreate = (day) => {
        const d = day || anchor;
        let h = sameDay(d, todayD) ? Math.floor(tzFloat(new Date(), primaryTz)) + 1 : 9; if (h > 22 || h < 6) h = 9;
        const hh = String(h).padStart(2, '0'); const eh = String((h + 1) % 24).padStart(2, '0');
        setEventErr(''); setConfirmDel(false);
        setEditing({ mode: 'create', eventId: null, calendarId: (writableCals[0] || {}).id || 'primary',
          title: '', allDay: false, date: isoOf(d), endDate: isoOf(d), start: hh + ':00', end: eh + ':00',
          location: '', notes: '', guests: '', repeat: 'none', readOnly: false, recurring: false });
      };
      ...
      const openEdit = (ev) => {
        ...
        setEventErr(''); setConfirmDel(false);
        setEditing({ mode: 'edit', eventId: ev.id, calendarId: ev.calendarId || 'primary',
          ...
          repeat: rruleToSimple(ev.recurrence), recurring: !!(ev.recurrence || ev.recurringEventId),
          readOnly: !writable });
      };
      ...
      const buildEventBody = (f) => {
        ...
        const ev = { summary: (f.title || '').trim() || '(no title)' };
        // On edit, send '' / [] to actively CLEAR — a PATCH leaves omitted keys unchanged. On create, omit empties.
        const loc = (f.location || '').trim(); const desc = (f.notes || '').trim();
        if (loc || edit) ev.location = loc;
        if (desc || edit) ev.description = desc;
        if (f.allDay) {
          const endIncl = (f.endDate && f.endDate >= f.date) ? f.endDate : f.date;
          ev.start = { date: f.date };
          ev.end = { date: isoOf(addDays(new Date(endIncl + 'T00:00:00'), 1)) };  // end date is exclusive
        } else {
          const endDate = (f.endDate && f.endDate >= f.date) ? f.endDate : f.date;
          ev.start = { dateTime: f.date + 'T' + f.start + ':00', timeZone: primaryTz };
          ev.end = { dateTime: endDate + 'T' + f.end + ':00', timeZone: primaryTz };
        }
        const guests = (f.guests || '').split(/[,;\s]+/).map(s => s.replace(/[<>]/g, '').trim()).filter(s => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s));
        if (guests.length || edit) ev.attendees = guests.map(email => ({ email }));  // [] removes all guests on edit
        // Only manage recurrence for non-recurring events. (Changing an existing series' rule is done in Google.)
        if (!f.recurring) {
          if (f.repeat && f.repeat !== 'none') ev.recurrence = ['RRULE:FREQ=' + (f.repeat === 'daily' ? 'DAILY' : f.repeat === 'weekly' ? 'WEEKLY' : 'MONTHLY')];
          else if (edit) ev.recurrence = [];
        }
        return ev;
      };
      const submitEvent = async () => {
        const f = editing; if (!f || f.readOnly) return;
        if (!f.allDay) {
          if (!/^\d{2}:\d{2}$/.test(f.start || '') || !/^\d{2}:\d{2}$/.test(f.end || '')) { setEventErr('Please set a start and end time.'); return; }
          ...
          if (effEnd === f.date) {
            ...
            if (eMin <= sMin) { setEventErr('End time must be after the start time.'); return; }
          }
        }
        setSavingEvent(true); setEventErr('');
        try {
          await saveGoogleEvent({ mode: f.mode, calendarId: f.calendarId, eventId: f.eventId, event: buildEventBody(f) });
          setEditing(null); setReloadKey(k => k + 1);
        } catch (err) {
          const code = err && err.code, msg = err && err.message;
          setEventErr(code === 'needs_connect' ? 'Reconnect your Google account in Settings, then try again.'
            : (msg && msg !== 'save_failed' ? msg : 'Could not save — please try again.'));
        } finally { setSavingEvent(false); }
      };
      const removeEvent = async () => {
        const f = editing; if (!f || f.mode !== 'edit') return;
        setSavingEvent(true); setEventErr('');
        try { await deleteGoogleEvent(f.calendarId, f.eventId); setEditing(null); setConfirmDel(false); setReloadKey(k => k + 1); }
        catch (err) { setEventErr('Could not delete — please try again.'); setConfirmDel(false); }
        finally { setSavingEvent(false); }
      };
      ...
      const prioTasks = () => {
        ...
        const dy = isoOf(anchor); return (tasks || []).filter(t => t.daily_priority && String(t.daily_priority).slice(0, 10) === dy && t.daily_rank).sort((a, b) => a.daily_rank - b.daily_rank).slice(0, 3);
      };
      const dueInfo = (t) => {
        if (!t.due_at) return { label: '', color: K.dueUp };
        const d = new Date(t.due_at); const t0 = new Date(todayD); t0.setHours(0, 0, 0, 0); const d0 = new Date(d); d0.setHours(0, 0, 0, 0);
        if (d0.getTime() === t0.getTime()) return { label: 'Today', color: K.dueToday };
        if (d0 < t0) return { label: d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), color: K.dueOver };
        return { label: d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), color: K.dueUp };
      };
      const calColor = (id) => { const c = calList.find(x => x.id === id); return (c && c.color) || null; };
      ...
      const dayEvents = (day) => visEvents().filter(e => e.start && !e.allDay && sameDay(new Date(e.start), day)).map(e => ({ title: e.title || '(busy)', start: new Date(e.start), end: e.end ? new Date(e.end) : null, color: e.color || calColor(e.calendarId), gold: (e.colorId === '5' || e.colorId === '6'), raw: e }));
      ...
      const layoutDayEvents = (evs) => {
        ...
        const items = evs.map((e, i) => ({ i, s: e.start.getTime(), en: endMs(e) })).sort((a, b) => a.s - b.s || a.en - b.en);
        const out = {}; let cluster = [], clusterEnd = -Infinity;
        const flush = () => {
          if (!cluster.length) return;
          const cols = [];                                  // cols[c] = last end time placed in column c
          cluster.forEach(it => { let c = 0; for (; c < cols.length; c++) { if (it.s >= cols[c]) break; } cols[c] = it.en; it.col = c; });
          cluster.forEach(it => { out[it.i] = { col: it.col, n: cols.length }; });
      ...
      useEffect(() => {
        if (!decOpen) return; let c = false;
        decisionTasks().forEach(t => {
          if (decData[t.id] && decData[t.id].loaded) return;
          Promise.all([TaskDB.loadDecisionOptions(t.id), TaskDB.loadActivity(t.id)]).then(([opts, act]) => { if (!c) setDecData(prev => ({ ...prev, [t.id]: { options: opts || [], comments: (act || []).filter(a => a.kind === 'update'), loaded: true } })); }).catch(() => {});
        });
        return () => { c = true; };
      }, [decOpen, tasks, peopleByTask]);
      const chooseOption = async (taskId, idx) => {
        const d = decData[taskId]; if (!d) return;
        const opts = d.options.map((o, i) => ({ ...o, is_chosen: i === idx ? !o.is_chosen : false }));
        setDecData(prev => ({ ...prev, [taskId]: { ...prev[taskId], options: opts } }));
        try { await TaskDB.setDecisionOptions(taskId, opts.map(o => ({ body: o.body, is_chosen: o.is_chosen }))); } catch (e) {}
      };
      const addComment = async (taskId) => {
        const text = (decDraft[taskId] || '').trim(); if (!text || !user) return;
        setDecDraft(prev => ({ ...prev, [taskId]: '' }));
        try { await TaskDB.addActivity(taskId, 'update', text, user); const act = await TaskDB.loadActivity(taskId); setDecData(prev => ({ ...prev, [taskId]: { ...(prev[taskId] || {}), comments: (act || []).filter(a => a.kind === 'update') } })); } catch (e) {}
      };
      ...
      const dateLabel = () => {
        ...
        if (view === 'week') { const s = mondayOf(anchor), e = addDays(s, 6); return s.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) + ' – ' + e.toLocaleDateString('en-US', { day: 'numeric' }) + ', ' + e.getFullYear(); }
        return anchor.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
      };

      const stripDays = []; { const s = mondayOf(anchor); for (let i = 0; i < 7; i++) stripDays.push(addDays(s, i)); }
      ...
      const sep = <div style={{ width: 1, height: 15, background: K.sep, flexShrink: 0 }} />;
      const connectBar = (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '5px 12px', borderBottom: `0.5px solid ${K.bd2}`, flexShrink: 0 }}>
          <span style={{ fontFamily: J, fontSize: 9, color: K.muted }}>Google Calendar not connected</span>
          <button onClick={() => { try { window.SupabaseAuth.connectCalendar(); } catch (e) {} }} style={{ fontFamily: J, fontSize: 9, fontWeight: 600, padding: '3px 9px', borderRadius: 6, border: 'none', background: K.gold, color: '#fff', cursor: 'pointer' }}>Connect</button>
        </div>
      );

      // ── Section 1: nav bar (horizontally scrollable so nothing clips on phone width) ──
      const navBar = (
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 10px', borderBottom: `0.5px solid ${K.bd2}`, background: K.navBg, flexShrink: 0, overflowX: 'auto' }}>
          <button onClick={goToday} style={{ flexShrink: 0, fontFamily: J, fontSize: 9, fontWeight: 600, color: K.tBtnTx, background: K.tBtnBg, border: `0.5px solid ${K.tBtnBd}`, borderRadius: 6, padding: '3px 8px', cursor: 'pointer' }}>Today</button>
          <div style={{ display: 'flex', flexShrink: 0 }}>
            <button onClick={() => step(-1)} style={{ width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', background: K.npBg, border: `0.5px solid ${K.npBd}`, borderRight: 'none', borderRadius: '4px 0 0 4px', cursor: 'pointer', padding: 0 }}><i className="ti ti-chevron-left" style={{ fontSize: 11, color: K.npIc }} /></button>
            <button onClick={() => step(1)} style={{ width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', background: K.npBg, border: `0.5px solid ${K.npBd}`, borderRadius: '0 4px 4px 0', cursor: 'pointer', padding: 0 }}><i className="ti ti-chevron-right" style={{ fontSize: 11, color: K.npIc }} /></button>
          </div>
          <span onClick={() => { const el = dateInputRef.current; if (!el) return; if (el.showPicker) el.showPicker(); else el.focus(); }} style={{ flexShrink: 0, fontFamily: J, fontSize: 11, fontWeight: 600, color: K.date, cursor: 'pointer', whiteSpace: 'nowrap' }}>{dateLabel()}</span>
          <input
            ref={dateInputRef}
            type="date"
            value={isoOf(anchor)}
          ...
          />
          {sep}
          {navChip('ti-search', false, () => {})}
          {navChip('ti-settings-2', true, () => setShowSettings(true))}
          {navChip('ti-plus', true, () => openCreate())}
          <div style={{ display: 'flex', background: K.segBg, borderRadius: 6, padding: 2, gap: 1, flexShrink: 0 }}>
            {['day', 'week', 'month'].map(v => (
              <button key={v} onClick={() => setView(v)} style={{ fontFamily: J, fontSize: 7.5, fontWeight: 600, padding: '3px 6px', borderRadius: 4, border: 'none', cursor: 'pointer', textTransform: 'capitalize', background: v === view ? K.segABg : 'transparent', color: v === view ? K.segATx : K.segTx }}>{v}</button>
            ))}
          </div>
        </div>
      );
      ...
      const taskStrip = (
        <React.Fragment>
          <div onClick={() => setTasksOpen(o => !o)} style={{ display: 'flex', background: tasksOpen ? K.navBg : K.taskBg, borderBottom: `0.5px solid ${K.bd2}`, cursor: 'pointer', flexShrink: 0 }}>
            <div style={{ width: GUTTER_W, flexShrink: 0, background: K.tzBg, borderRight: `0.5px solid ${K.bd2}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontFamily: J, fontSize: 7, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: K.gold }}>tasks</span>
            </div>
            {tasksOpen ? (
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', padding: '7px 10px' }}>
                <span style={{ fontFamily: J, fontSize: 9, fontStyle: 'italic', color: K.muted }}>All tasks</span>
              </div>
              ...
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', padding: '7px 10px' }}>
                <span style={{ fontFamily: J, fontSize: 9, fontStyle: 'italic', color: K.muted }}>No priorities set — tap for all tasks</span>
              </div>
            )}
            <div style={{ width: 32, flexShrink: 0, borderLeft: `0.5px solid ${K.cellBd}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className={'ti ' + (tasksOpen ? 'ti-chevron-up' : 'ti-chevron-down')} style={{ fontSize: 12, color: tasksOpen ? K.gold : K.muted }} />
            </div>
          </div>
          {tasksOpen && (
            <div style={{ flexShrink: 0, maxHeight: 200, overflowY: 'auto', borderBottom: `0.5px solid ${K.bd2}` }}>
              ...
              {expandedTasks.length === 0 && <div style={{ padding: '10px 12px 10px 68px', fontFamily: J, fontSize: 9, color: K.muted }}>No tasks yet.</div>}
            </div>
          )}
        </React.Fragment>
      );
      ...
      const decIcon = (
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M12 4v6" /><path d="M9 7l3-3 3 3" />
          <path d="M4 13l-2 2 2 2" /><path d="M2 15h7" />
          <path d="M20 13l2 2-2 2" /><path d="M22 15h-7" />
        </svg>
      );
      const statusPill = (st) => <span style={{ fontFamily: J, fontSize: 7, fontWeight: 700, padding: '2px 6px', borderRadius: 10, background: statusBg(st), color: statusColor(st), flexShrink: 0 }}>{statusLabel(st)}</span>;
      const decCard = (t) => {
        const d = decData[t.id] || {}; const dd = decDueInfo(t); const maker = roleNames(t.id, 'decision_maker'); const asg = roleNames(t.id, 'assignee'); const extra = t.description || '';
        return (
          <div key={t.id} style={{ borderBottom: `0.5px solid ${K.rowBd}`, padding: '9px 12px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 5 }}>
              {maker && <span style={{ fontFamily: J, fontSize: 8.5, color: K.reg }}><b style={{ color: K.gold }}>Maker:</b> {maker}</span>}
              {dd && <span style={{ fontFamily: J, fontSize: 8.5, fontWeight: 600, color: dd.color }}>Decide by {dd.label}</span>}
              {asg && <span style={{ fontFamily: J, fontSize: 8.5, color: K.reg }}>Assignee: {asg}</span>}
            </div>
            {t.context && <div style={{ fontFamily: J, fontSize: 8.5, color: K.reg, lineHeight: 1.5, marginTop: 4, whiteSpace: 'pre-wrap' }}><b style={{ color: K.gold }}>Context:</b> {t.context}</div>}
            {t.decision_question && <div style={{ fontFamily: J, fontSize: 10.5, fontWeight: 600, color: K.date, margin: '8px 0 6px', lineHeight: 1.4 }}>{t.decision_question}</div>}
            {(d.options || []).map((o, i) => (
              <div key={o.id || i} onClick={() => chooseOption(t.id, i)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '5px 8px', marginBottom: 4, borderRadius: 7, cursor: 'pointer', background: o.is_chosen ? (dark ? 'rgba(15,110,86,.18)' : '#E6F2EC') : (dark ? '#0C1830' : '#FAF8F3'), border: `1px solid ${o.is_chosen ? (dark ? '#1E6B4E' : '#9FD3BC') : K.cellBd}` }}>
                <i className={o.is_chosen ? 'ti ti-circle-check-filled' : 'ti ti-circle'} style={{ fontSize: 15, color: o.is_chosen ? (dark ? '#3FB36B' : '#0F6E56') : K.muted, flexShrink: 0 }} />
                <span style={{ flex: 1, minWidth: 0, fontFamily: J, fontSize: 9.5, fontWeight: o.is_chosen ? 600 : 400, color: K.date }}>{o.body}</span>
                {o.is_chosen && <span style={{ fontFamily: J, fontSize: 6.5, fontWeight: 700, letterSpacing: '.06em', color: dark ? '#3FB36B' : '#0F6E56', flexShrink: 0 }}>CHOSEN</span>}
              </div>
            ))}
            {extra && <div style={{ fontFamily: J, fontSize: 8.5, color: K.reg, lineHeight: 1.5, marginTop: 4, whiteSpace: 'pre-wrap' }}>{extra}</div>}
            {(d.comments || []).slice(-2).map(cm => (
              <div key={cm.id} style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                <span style={{ fontFamily: J, fontSize: 8, fontWeight: 700, color: K.gold, flexShrink: 0 }}>{(cm.author_name || 'User').split(' ')[0]}:</span>
                <span style={{ fontFamily: J, fontSize: 9, color: K.date, minWidth: 0 }}>{cm.content}</span>
              </div>
            ))}
            <div style={{ display: 'flex', gap: 6, marginTop: 7 }}>
              <input value={decDraft[t.id] || ''} onChange={e => setDecDraft(prev => ({ ...prev, [t.id]: e.target.value }))} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addComment(t.id); } }} placeholder="Add a comment…" style={{ flex: 1, minWidth: 0, fontFamily: J, fontSize: 9, padding: '6px 9px', borderRadius: 7, border: `1px solid ${K.cellBd}`, background: dark ? '#040C1C' : '#FCFBF8', color: K.date, outline: 'none' }} />
              <button onClick={() => addComment(t.id)} style={{ flexShrink: 0, fontFamily: J, fontSize: 9, fontWeight: 600, padding: '0 12px', borderRadius: 7, border: 'none', background: K.gold, color: '#fff', cursor: 'pointer' }}>Send</button>
            </div>
          </div>
        );
      };
      const decStrip = (
        <React.Fragment>
          <div onClick={() => setDecOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', background: decOpen ? K.navBg : K.taskBg, borderBottom: `0.5px solid ${K.bd2}`, cursor: 'pointer', flexShrink: 0 }}>
            <div style={{ width: 56, flexShrink: 0, background: K.tzBg, borderRight: `0.5px solid ${K.bd2}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: K.gold, padding: '7px 0' }}>{decIcon}</div>
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, padding: '7px 10px' }}>
              <span style={{ fontFamily: J, fontSize: 8, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: K.gold }}>Decisions</span>
              <span style={{ fontFamily: J, fontSize: 9, color: K.muted }}>{decs.length ? decs.length + ' open' : 'none'}</span>
            </div>
            <div style={{ width: 32, flexShrink: 0, borderLeft: `0.5px solid ${K.cellBd}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className={'ti ' + (decOpen ? 'ti-chevron-up' : 'ti-chevron-down')} style={{ fontSize: 12, color: decOpen ? K.gold : K.muted }} />
            </div>
          </div>
          {decOpen && (
            <div style={{ flexShrink: 0, maxHeight: 280, overflowY: 'auto', borderBottom: `0.5px solid ${K.bd2}` }}>
              {decs.length ? decs.map(decCard) : <div style={{ padding: '12px', fontFamily: J, fontSize: 9, color: K.muted, textAlign: 'center' }}>No decisions yet. Add a decision on any task.</div>}
            </div>
          )}
        </React.Fragment>
      );
      ...
      const dateStrip = (
        <div style={{ display: 'flex', borderBottom: `0.5px solid ${K.bd2}`, flexShrink: 0 }}>
          <div style={{ width: GUTTER_W, flexShrink: 0, background: K.tzBg, borderRight: `0.5px solid ${K.bd2}`, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 6px' }}>
            <span style={{ fontFamily: J, fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', color: K.tx }}>TX</span>
            {showSecondaryTz && <span style={{ fontFamily: J, fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', color: K.ph }}>PH</span>}
          </div>
          {stripDays.map((d, i) => { const wknd = i >= 5, isTod = sameDay(d, todayD), isSel = sameDay(d, anchor), hasEv = hasEvents(d); return (
            <div key={i} onClick={() => { setAnchor(new Date(d)); if (view === 'month') setView('day'); }} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '4px 1px 5px', borderRight: i < 6 ? `0.5px solid ${K.gline}` : 'none', cursor: 'pointer', background: isTod ? K.todBg : (isSel && view === 'day' ? (dark ? 'rgba(255,255,255,.05)' : '#F7F4EE') : 'transparent') }}>
              <span style={{ fontFamily: J, fontSize: 6, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, color: isTod ? K.gold : (wknd ? K.weekend : K.dayLetter) }}>{SL[i]}</span>
              <div style={{ width: 21, height: 21, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: J, fontSize: isTod ? 10 : 11, fontWeight: 600, background: isTod ? K.circBg : 'transparent', color: isTod ? K.circTx : (wknd ? K.weekend : K.date) }}>{d.getDate()}</div>
              <div style={{ width: 3, height: 3, borderRadius: '50%', background: hasEv ? K.dot : 'transparent' }} />
            </div>
          ); })}
        </div>
      );
      ...
      const LANE_CATEGORY_STYLE = {
        ooo:     { border: '#9B1C1C', fill: dark ? 'rgba(155,28,28,.16)' : '#F7EEEE', text: dark ? '#E4A0A0' : '#7C3030' },
        team:    { border: '#185FA5', fill: dark ? 'rgba(24,95,165,.16)' : '#EAF0F8', text: dark ? '#8FB6DC' : '#1B4C82' },
        holiday: { border: '#0F6E56', fill: dark ? 'rgba(15,110,86,.16)' : '#E9F4F0', text: dark ? '#7FC6AF' : '#0B5744' },
      };
      const LANE_CATEGORY_LABEL = { ooo: 'Out of office', team: 'Team event', holiday: 'Holiday' };
      ...
      const laneEvents = (view === 'month') ? [] : visEvents()
        .filter(e => laneQualifies(e) && laneTrueEnd(e) >= laneRangeStart && laneTrueStart(e) <= laneRangeEnd)
        .map(e => ({ raw: e, cat: laneCategoryOf(e), trueStart: laneTrueStart(e), trueEnd: laneTrueEnd(e) }));
      const [laneExpanded, setLaneExpanded] = useState(false);
      useEffect(() => { setLaneExpanded(false); }, [view, anchor]);
      ...
      const laneGridRef = useCallback((node) => {
        if (laneRO.current) { laneRO.current.disconnect(); laneRO.current = null; }
        if (!node || typeof ResizeObserver === 'undefined') return;
        const ro = new ResizeObserver((entries) => { const w = entries[0] && entries[0].contentRect && entries[0].contentRect.width; if (w) setLaneGridW(w); });
        ro.observe(node);
        laneRO.current = ro;
      }, []);
      ...
      const laneFmtRange = (b) => {
        ...
        const df = (d) => d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
      ...
      const laneLabel = (b, dayLabel) => {
        ...
        const range = laneFmtRange(b); if (range) s += ' · ' + range;
      ...
      const laneAssignRows = (bars) => {
        const rowEnds = [];
        bars.forEach((b) => { let r = 0; while (r < rowEnds.length && rowEnds[r] >= b.startIdx) r++; rowEnds[r] = b.endIdx; b.row = r; });
        return bars;
      };
      const laneWeekBars = view === 'week' ? laneEvents.map((b) => {
        ...
        return { ...b, startIdx: Math.max(0, rawStart), endIdx: Math.min(6, rawEnd), clipStart: rawStart < 0, clipEnd: rawEnd > 6 };
      }).sort((a, b) => a.startIdx - b.startIdx || (b.endIdx - b.startIdx) - (a.endIdx - a.startIdx)) : [];
      const laneDayBars = view === 'day' ? laneEvents.map((b) => ({ ...b, startIdx: 0, endIdx: 0 })).sort((a, b) => a.trueStart - b.trueStart) : [];
      ...
      const laneMorePill = (extraStyle) => (
        <div onClick={(e) => { e.stopPropagation(); setLaneExpanded((x) => !x); }} style={{ fontFamily: J, fontSize: 10, fontWeight: 600, color: K.muted, background: K.chipBg, borderRadius: 20, height: 20, padding: '0 10px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap', ...extraStyle }}>+{laneOverflowCount} more</div>
      );
      const laneWeekBarEl = (b) => {
        ...
        const short = pxW < 60;
        const radTL = b.clipStart ? 0 : 4, radTR = b.clipEnd ? 0 : 4;
        return (
          <div key={b.raw.id + b.raw.calendarId} onClick={() => openEdit(b.raw)} style={{
            position: 'absolute', top: b.row * 24, left: leftPct + '%', width: widthPct + '%', height: 20,
            borderRadius: `${radTL}px ${radTR}px ${radTR}px ${radTL}px`,
            borderLeft: b.clipStart ? 'none' : `3px solid ${cs.border}`,
            background: cs.fill, color: cs.text, display: 'flex', alignItems: 'center',
            padding: '0 8px', fontFamily: J, fontSize: 10.5, fontWeight: 500, whiteSpace: 'nowrap',
            overflow: 'hidden', textOverflow: 'ellipsis', cursor: 'pointer', boxSizing: 'border-box',
          }}>
            {b.clipStart && <span style={{ fontWeight: 700, marginRight: 3, flexShrink: 0 }}>‹</span>}
            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{short ? laneInitials(b) : laneLabel(b, null)}</span>
            {b.clipEnd && <span style={{ fontWeight: 700, marginLeft: 3, flexShrink: 0 }}>›</span>}
          </div>
        );
      };
      const laneDayRowEl = (b, isLast) => {
        ...
        return (
          <div key={b.raw.id + b.raw.calendarId} style={{ display: 'flex', alignItems: 'center', gap: 6, height: 20, marginBottom: isLast ? 0 : 4 }}>
            <div onClick={() => openEdit(b.raw)} style={{
              flex: 1, minWidth: 0, height: 20, borderRadius: 4, borderLeft: `3px solid ${cs.border}`,
              background: cs.fill, color: cs.text, display: 'flex', alignItems: 'center', padding: '0 8px',
              fontFamily: J, fontSize: 10.5, fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden',
              textOverflow: 'ellipsis', cursor: 'pointer', boxSizing: 'border-box',
            }}>{laneLabel(b, dayLabel)}</div>
            {laneCapped && isLast && laneMorePill({})}
          </div>
        );
      };
      const teamLane = laneEvents.length > 0 ? (
        <div style={{ display: 'flex', flexShrink: 0, background: LANE_BG, borderBottom: `1px solid ${K.bd2}` }}>
          <div style={{ width: GUTTER_W, flexShrink: 0, borderRight: `1px solid ${K.bd2}`, fontFamily: J, fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 600, color: K.muted, lineHeight: 1.4, padding: '9px 0 0 16px' }}>Team<br />All-day</div>
          <div ref={laneGridRef} style={{ flex: 1, position: 'relative', height: laneHeight }}>
            <div style={{ position: 'absolute', inset: 0, display: 'flex' }}>
              {(view === 'week' ? stripDays : [anchor]).map((d, i) => (
                <div key={i} style={{ flex: 1, borderLeft: i > 0 ? `1px solid ${K.gline}` : 'none', background: sameDay(d, todayD) ? K.weekTodayCol : 'transparent' }} />
              ))}
            </div>
            {view === 'week' ? (
              <div style={{ position: 'absolute', top: 6, bottom: 6, left: 0, right: 0 }}>
                {laneVisibleBars.map(laneWeekBarEl)}
                {laneCapped && laneMorePill({ position: 'absolute', top: 2 * 24, right: 0 })}
              </div>
            ) : (
              <div style={{ position: 'absolute', top: 6, bottom: 6, left: 0, right: 0 }}>
                {laneVisibleBars.map((b, i) => laneDayRowEl(b, i === laneVisibleBars.length - 1))}
              </div>
            )}
          </div>
        </div>
      ...
      const dayGrid = (
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          {gcal === 'needs_connect' && connectBar}
          <div ref={scrollRef} style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
            <div style={{ display: 'flex', position: 'relative' }}>
              <div style={{ width: GUTTER_W, flexShrink: 0, background: K.tzBg, borderRight: `0.5px solid ${K.bd2}` }}>
                {HOURS.map(h => { const inst = tzInstant(anchor, h, primaryTz); const hot = dayEvents(anchor).some(e => Math.floor(tzFloat(e.start, primaryTz)) === h); return (
                  <div key={h} style={{ height: 48, borderBottom: `0.5px solid ${K.slot}`, padding: '5px 6px 0' }}>
                    <div style={{ fontFamily: J, fontSize: 11, fontWeight: hot ? 700 : 600, color: hot ? K.gold : K.tx }}>{fmtHour(inst, primaryTz)}</div>
                    {showSecondaryTz && <div style={{ fontFamily: J, fontSize: 9, fontWeight: hot ? 600 : 400, color: K.ph, opacity: hot ? 1 : 0.85 }}>{fmtHour(inst, secondaryTz)}</div>}
                  </div>
                ); })}
              </div>
              <div style={{ flex: 1, position: 'relative' }}>
                {HOURS.map(h => <div key={h} style={{ height: 48, borderBottom: `0.5px solid ${K.gline}` }} />)}
                {(() => { const dEvs = dayEvents(anchor); const lay = layoutDayEvents(dEvs); return dEvs.map((e, i) => { const top = tzFloat(e.start, primaryTz) * 48; const dur = e.end ? Math.max((e.end - e.start) / 3600000, 0.5) : 0.75; const p = lay[i] || { col: 0, n: 1 }; return (
                  <div key={i} onClick={() => openEdit(e.raw)} style={{ position: 'absolute', top: top + 1, left: `calc(4px + (100% - 8px) * ${p.col} / ${p.n})`, width: `calc((100% - 8px) / ${p.n} - 2px)`, height: Math.max(dur * 48 - 2, 16), background: evBg(e), borderLeft: `3px solid ${evBd(e)}`, borderRadius: 5, padding: '4px 7px', overflow: 'hidden', cursor: 'pointer', boxSizing: 'border-box' }}>
                    <div style={{ fontFamily: J, fontSize: 9, fontWeight: 600, color: K.evTitle, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e.title}</div>
                    <div style={{ fontFamily: J, fontSize: 7.5, color: K.evTime, marginTop: 1 }}>{fmtClock(e.start, primaryTz)}</div>
                  </div>
                ); }); })()}
              </div>
            </div>
      ...
      const weekGrid = (
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div ref={scrollRef} style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
            {HOURS.map(h => { const inst = tzInstant(mondayOf(anchor), h, primaryTz); return (
              <div key={h} style={{ display: 'flex' }}>
                <div style={{ width: GUTTER_W, flexShrink: 0, background: K.tzBg, borderRight: `0.5px solid ${K.bd2}`, padding: '4px 6px 0' }}>
                  <div style={{ fontFamily: J, fontSize: 11, fontWeight: 600, color: K.tx }}>{fmtHour(inst, primaryTz)}</div>
                  {showSecondaryTz && <div style={{ fontFamily: J, fontSize: 9, fontWeight: 400, color: K.ph, opacity: 0.85 }}>{fmtHour(inst, secondaryTz)}</div>}
                </div>
                {stripDays.map((d, di) => { const isTod = sameDay(d, todayD), wknd = di >= 5; const evs = dayEvents(d).filter(e => Math.floor(tzFloat(e.start, primaryTz)) === h); return (
                  <div key={di} style={{ flex: 1, minWidth: 0, height: 44, borderBottom: `0.5px solid ${K.gline}`, borderRight: `0.5px solid ${K.gline}`, background: isTod ? K.weekTodayCol : (wknd ? K.weekendCell : 'transparent'), padding: '1px 2px', overflow: 'hidden' }}>
                    {evs.map((e, ei) => (
                      <div key={ei} onClick={() => openEdit(e.raw)} style={{ fontFamily: J, fontSize: 9.5, fontWeight: 600, padding: '2px 4px', borderRadius: 3, marginBottom: 1, background: evBg(e), borderLeft: `2px solid ${evBd(e)}`, color: K.evTitle, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', cursor: 'pointer' }}>{e.title}</div>
                    ))}
                  </div>
                ); })}
              </div>
      ...
      const monthGrid = (() => {
        ...
        const cells = []; for (let i = 0; i < rows * 7; i++) cells.push(addDays(gridStart, i));
        return (
          <div style={{ flex: '0 0 auto', padding: '8px 10px 10px', overflowY: 'auto' }}>
            {gcal === 'needs_connect' && <div style={{ marginBottom: 8 }}>{connectBar}</div>}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3, marginBottom: 4 }}>
              {SL.map((l, i) => <div key={i} style={{ textAlign: 'center', fontFamily: J, fontSize: 7, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: i >= 5 ? K.weekend : K.dayLetter }}>{l}</div>)}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3 }}>
              {cells.map((d, i) => { const inM = d.getMonth() === anchor.getMonth(); const wknd = (i % 7) >= 5; const isTod = sameDay(d, todayD); const ev = monthHasEvent(d), tk = monthHasTask(d); return (
                <div key={i} onClick={() => { setAnchor(new Date(d)); setView('day'); }} style={{ minHeight: 42, padding: 3, border: `1px solid ${isTod ? K.todCellBd : K.monthBd}`, borderRadius: 4, background: isTod ? K.todCellBg : (wknd ? K.weekendCell : K.cellBg), opacity: inM ? 1 : 0.4, cursor: 'pointer' }}>
                  {isTod
                    ? <div style={{ width: 16, height: 16, borderRadius: '50%', background: K.circBg, color: K.circTx, fontFamily: J, fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{d.getDate()}</div>
                    : <div style={{ fontFamily: J, fontSize: 9, fontWeight: 500, color: wknd ? K.weekend : K.date }}>{d.getDate()}</div>}
                  <div style={{ display: 'flex', gap: 2, marginTop: 2 }}>
                    {ev && <div style={{ width: 4, height: 4, borderRadius: '50%', background: K.dotEvent }} />}
                    {tk && <div style={{ width: 4, height: 4, borderRadius: '50%', background: K.dotTask }} />}
                  </div>
                </div>
              ); })}
            </div>
      ...
      const settHead = (t) => <div style={{ fontFamily: J, fontSize: 8.5, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: K.muted, padding: '16px 18px 7px' }}>{t}</div>;
      const settCard = { background: dark ? '#0A1730' : '#fff', border: `0.5px solid ${K.bd2}`, borderRadius: 12, margin: '0 14px', overflow: 'hidden' };
      const settRow = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '13px 15px' };
      const settPill = { fontFamily: J, fontSize: 10, fontWeight: 600, padding: '6px 14px', borderRadius: 8, border: `0.5px solid ${K.chipBd}`, background: K.chipBg, color: K.tBtnTx, cursor: 'pointer', flexShrink: 0 };
      const toggle = (on, onClick) => (
        <button onClick={onClick} style={{ width: 42, height: 25, borderRadius: 13, border: 'none', cursor: 'pointer', background: on ? '#34C759' : (dark ? '#39393D' : '#E4DFD4'), position: 'relative', flexShrink: 0, transition: 'background 150ms', padding: 0 }}>
          <span style={{ position: 'absolute', top: 2, left: on ? 19 : 2, width: 21, height: 21, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,.3)', transition: 'left 150ms' }} />
        </button>
      );
      const tzSelect = (val, onChange) => (
        <select value={val} onChange={e => onChange(e.target.value)} style={{ fontFamily: J, fontSize: 12, fontWeight: 600, color: K.date, background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'right', maxWidth: 190, outline: 'none' }}>
          {!CAL_TZ_OPTIONS.some(o => o.tz === val) && <option value={val}>{tzLabel(val)}</option>}
          {CAL_TZ_OPTIONS.map(o => <option key={o.tz} value={o.tz}>{o.label}</option>)}
        </select>
      );
      const settingsPanel = (
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: K.pop, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 15px', borderBottom: `0.5px solid ${K.bd2}`, flexShrink: 0, background: K.navBg }}>
            <span style={{ width: 48 }} />
            <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: K.date }}>Calendar Settings</span>
            <button onClick={() => setShowSettings(false)} style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: K.gold, background: 'none', border: 'none', cursor: 'pointer', width: 48, textAlign: 'right' }}>Done</button>
          </div>
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', paddingBottom: 'calc(24px + env(safe-area-inset-bottom))' }}>
            {settHead('Google Calendar & Tasks')}
            <div style={settCard}>
              <div style={settRow}>
            ...
            {settHead('Calendars to Show')}
            <div style={settCard}>
              {calList.length === 0 ? (
                <div style={{ padding: '14px 15px', fontFamily: J, fontSize: 11, color: K.muted, lineHeight: 1.55 }}>
                  {gcal === 'needs_connect' ? 'Connect your Google account to choose which calendars appear.' : 'No calendars to choose yet. If you just connected, give it a moment, or tap Reconnect above.'}
                </div>
              ) : calList.map((c, i) => { const on = !isHidden(c.id); return (
                <div key={c.id} style={{ ...settRow, borderTop: i ? `0.5px solid ${K.rowBd}` : 'none' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0 }}>
                    <span style={{ width: 15, height: 15, borderRadius: 4, background: c.color || K.muted, flexShrink: 0 }} />
                    <span style={{ fontFamily: J, fontSize: 12, color: K.date, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.summary}{c.primary ? ' · primary' : ''}</span>
                  </span>
                  {toggle(on, () => { const set = new Set(calPrefs.hidden || []); on ? set.add(c.id) : set.delete(c.id); savePrefs({ hidden: Array.from(set) }); })}
                </div>
              ); })}
            </div>

            {settHead('Primary Timezone (shown on top)')}
            <div style={settCard}><div style={settRow}><span style={{ fontFamily: J, fontSize: 12, color: K.date }}>Primary</span>{tzSelect(primaryTz, v => savePrefs({ primaryTz: v }))}</div></div>

            {settHead('Secondary Timezone (shown below)')}
            <div style={settCard}>
              <div style={settRow}>
                <span style={{ fontFamily: J, fontSize: 12, color: K.date }}>Show secondary timezone</span>
                {toggle(showSecondaryTz, () => savePrefs({ secondaryTzEnabled: !showSecondaryTz }))}
              </div>
              <div style={{ ...settRow, borderTop: `0.5px solid ${K.rowBd}` }}>
                <span style={{ fontFamily: J, fontSize: 12, color: K.date }}>Secondary</span>
                {tzSelect(secondaryTz, v => savePrefs({ secondaryTz: v }))}
              </div>
            </div>
            <div style={{ padding: '11px 18px 0', fontFamily: J, fontSize: 10.5, color: K.muted, lineHeight: 1.5 }}>{showSecondaryTz ? `The calendar shows ${tzShort(primaryTz)} over ${tzShort(secondaryTz)} on the time rail.` : `The calendar shows ${tzShort(primaryTz)} only. Turn this on to also show ${tzShort(secondaryTz)}.`}</div>
          </div>
        </div>
      );

      // ── Event create / edit / delete form ──
      const fld = { width: '100%', boxSizing: 'border-box', fontFamily: J, fontSize: 13, color: K.date, background: dark ? '#06101F' : '#FCFBF8', border: `1px solid ${K.cellBd}`, borderRadius: 9, padding: '10px 12px', outline: 'none' };
      const fldLbl = { fontFamily: J, fontSize: 8.5, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: K.muted, margin: '0 0 5px 2px', display: 'block' };
      const fldWrap = { padding: '12px 16px 0' };
      const eventForm = editing ? (() => {
        const ed = editing; const ro = !!ed.readOnly;
        const calOf = calList.find(c => c.id === ed.calendarId) || {};
        return (
          <div style={{ position: 'fixed', inset: 0, zIndex: 10001, background: K.pop, display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 15px', borderBottom: `0.5px solid ${K.bd2}`, flexShrink: 0, background: K.navBg }}>
              <button onClick={() => setEditing(null)} style={{ fontFamily: J, fontSize: 12, color: K.tBtnTx, background: 'none', border: 'none', cursor: 'pointer', width: 60, textAlign: 'left' }}>Cancel</button>
              <span style={{ fontFamily: J, fontSize: 13, fontWeight: 700, color: K.date }}>{ed.mode === 'create' ? 'New event' : (ro ? 'Event' : 'Edit event')}</span>
              <button onClick={submitEvent} disabled={savingEvent || ro} style={{ fontFamily: J, fontSize: 12, fontWeight: 700, color: ro ? K.muted : K.gold, background: 'none', border: 'none', cursor: ro ? 'default' : 'pointer', width: 60, textAlign: 'right', opacity: savingEvent ? 0.5 : 1 }}>{savingEvent ? '…' : 'Save'}</button>
            </div>
            <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', paddingBottom: 'calc(28px + env(safe-area-inset-bottom))' }}>
              {ro && <div style={{ margin: '12px 16px 0', padding: '9px 12px', borderRadius: 9, background: dark ? '#2A1F0A' : '#FDF8F0', border: `1px solid ${K.evGoldBd}`, fontFamily: J, fontSize: 10.5, color: K.date }}>This calendar is read-only — you can view this event but not change it.</div>}
              {eventErr && <div style={{ margin: '12px 16px 0', padding: '9px 12px', borderRadius: 9, background: dark ? '#2A0F0F' : '#FDEDED', border: '1px solid #E07A7A', fontFamily: J, fontSize: 10.5, color: dark ? '#E89A9A' : '#9B1C1C' }}>{eventErr}</div>}
              {ed.recurring && <div style={{ margin: '12px 16px 0', fontFamily: J, fontSize: 10, color: K.muted, lineHeight: 1.5, padding: '0 2px' }}>This is a repeating event — changes apply to this occurrence.</div>}

              <div style={fldWrap}><label style={fldLbl}>Title</label>
                <input value={ed.title} disabled={ro} onChange={e => setF({ title: e.target.value })} placeholder="Add a title" autoFocus={ed.mode === 'create'} style={{ ...fld, fontWeight: 600 }} />
              </div>

              <div style={fldWrap}><label style={fldLbl}>Calendar</label>
                {ed.mode === 'create'
                  ? <select value={ed.calendarId} onChange={e => setF({ calendarId: e.target.value })} style={fld}>
                      {writableCals.map(c => <option key={c.id} value={c.id}>{c.summary}{c.primary ? ' · primary' : ''}</option>)}
                      {!writableCals.length && <option value="primary">Primary</option>}
                    </select>
                  : <div style={{ ...fld, display: 'flex', alignItems: 'center', gap: 9 }}>
                      <span style={{ width: 13, height: 13, borderRadius: 4, background: calColor(ed.calendarId) || K.muted, flexShrink: 0 }} />
                      <span>{calOf.summary || 'Calendar'}</span>
                    </div>}
              </div>

              <div style={{ ...fldWrap, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: J, fontSize: 12.5, color: K.date }}>All day</span>
                {toggle(ed.allDay, () => { if (!ro) setF({ allDay: !ed.allDay }); })}
              </div>

              <div style={fldWrap}><label style={fldLbl}>Starts</label>
                <div style={{ display: 'flex', gap: 8 }}>
                  <input type="date" value={ed.date} disabled={ro} onChange={e => setF({ date: e.target.value, endDate: (ed.endDate < e.target.value ? e.target.value : ed.endDate) })} style={{ ...fld, flex: 1 }} />
                  {!ed.allDay && <input type="time" value={ed.start} disabled={ro} onChange={e => setF({ start: e.target.value })} style={{ ...fld, width: 120 }} />}
                </div>
              </div>
              <div style={fldWrap}><label style={fldLbl}>Ends</label>
                <div style={{ display: 'flex', gap: 8 }}>
                  <input type="date" value={ed.endDate} disabled={ro} min={ed.date} onChange={e => setF({ endDate: e.target.value })} style={{ ...fld, flex: 1 }} />
                  {!ed.allDay && <input type="time" value={ed.end} disabled={ro} onChange={e => setF({ end: e.target.value })} style={{ ...fld, width: 120 }} />}
                </div>
              </div>
              {!ed.allDay && <div style={{ padding: '6px 18px 0', fontFamily: J, fontSize: 10, color: K.muted }}>Times are in {tzShort(primaryTz)} ({tzLabel(primaryTz)}).</div>}

              <div style={fldWrap}><label style={fldLbl}>Location</label>
                <input value={ed.location} disabled={ro} onChange={e => setF({ location: e.target.value })} placeholder="Add a location" style={fld} />
              </div>
              <div style={fldWrap}><label style={fldLbl}>Notes</label>
                <textarea value={ed.notes} disabled={ro} onChange={e => setF({ notes: e.target.value })} placeholder="Add notes" style={{ ...fld, minHeight: 70, resize: 'vertical' }} />
              </div>
              <div style={fldWrap}><label style={fldLbl}>Guests (emails)</label>
                <input value={ed.guests} disabled={ro} onChange={e => setF({ guests: e.target.value })} placeholder="name@email.com, another@email.com" style={fld} />
                <div style={{ fontFamily: J, fontSize: 9.5, color: K.muted, margin: '5px 0 0 2px' }}>Separate with commas. Google emails them an invite.</div>
              </div>
              <div style={fldWrap}><label style={fldLbl}>Repeat</label>
                {(ed.mode === 'edit' && ed.recurring)
                  ? <div style={{ ...fld, color: K.muted }}>Repeating event — manage the series in Google Calendar.</div>
                  : <select value={ed.repeat} disabled={ro} onChange={e => setF({ repeat: e.target.value })} style={fld}>
              ...
              {ed.mode === 'edit' && !ro && (
                <div style={{ padding: '20px 16px 0' }}>
                  {confirmDel ? (
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span style={{ flex: 1, fontFamily: J, fontSize: 11.5, color: K.date }}>Delete this event?</span>
                      <button onClick={() => setConfirmDel(false)} disabled={savingEvent} style={{ fontFamily: J, fontSize: 11, fontWeight: 600, padding: '8px 14px', borderRadius: 9, border: `0.5px solid ${K.chipBd}`, background: K.chipBg, color: K.date, cursor: 'pointer' }}>Keep</button>
                      <button onClick={removeEvent} disabled={savingEvent} style={{ fontFamily: J, fontSize: 11, fontWeight: 700, padding: '8px 16px', borderRadius: 9, border: 'none', background: '#C0392B', color: '#fff', cursor: 'pointer' }}>{savingEvent ? '…' : 'Delete'}</button>
                    </div>
                  ) : (
                    <button onClick={() => setConfirmDel(true)} style={{ width: '100%', fontFamily: J, fontSize: 12, fontWeight: 600, padding: '11px', borderRadius: 10, border: `1px solid ${dark ? '#5A2A2A' : '#E8C9C9'}`, background: 'none', color: dark ? '#E07A7A' : '#9B1C1C', cursor: 'pointer' }}>Delete event</button>
                  )}
                </div>
              )}
            </div>
      ...
      const calStyle = TASKS_EMBED
        ? { position: 'fixed', inset: 0, zIndex: 10, background: K.pop, opacity: vis ? 1 : 0, transition: 'opacity 150ms ease', display: 'flex', flexDirection: 'column', overflow: 'hidden' }
        : {
            position: 'fixed', top: POPOUT_TOP, right: 10, width: 'min(calc(100% - 20px), 560px)', bottom: view === 'month' ? 'auto' : (zoneH || 0), zIndex: 10,
            background: K.pop, border: `1px solid ${K.bd}`, borderTop: 'none', borderRadius: '0 0 14px 14px', boxShadow: K.shadow,
            opacity: vis ? 1 : 0, transition: 'opacity 150ms ease', display: 'flex', flexDirection: 'column', overflow: 'hidden',
            maxHeight: view === 'month' ? 'calc(100vh - 90px - env(safe-area-inset-top))' : 'none',
          };
      return (
        <React.Fragment>
          <div onClick={onClose} style={{ position: 'fixed', top: POPOUT_TOP, left: 0, right: 0, bottom: 0, zIndex: 9, background: dark ? 'rgba(0,0,0,.35)' : 'rgba(0,26,74,.18)' }} />
          <div onClick={(e) => e.stopPropagation()} style={calStyle}>
            {navBar}
            {view !== 'month' && taskStrip}
            {view !== 'month' && dateStrip}
            {view !== 'month' && teamLane}
            {view === 'day' && dayGrid}
            {view === 'week' && weekGrid}
    ...
    function DecisionsPopover({ dark, zoneH, onOpenTask, onClose }) {
      const J = "'Jost', sans-serif";
      const todayD = new Date();
      const [tasks, setTasks] = useState([]);
      const [peopleByTask, setPeopleByTask] = useState({});
      ...
      useEffect(() => { let c = false;
        TaskDB.loadAll().then(d => { if (!c && d) { setTasks(d.tasks || []); setPeopleByTask(d.peopleByTask || {}); } }).catch(() => {});
        ProfileDB.loadAll().then(ps => { if (!c && ps) { const m = {}; ps.forEach(p => { m[p.id] = (((p.first_name || '') + ' ' + (p.last_name || '')).trim()) || p.email || 'User'; }); setRoster(m); } }).catch(() => {});
        return () => { c = true; }; }, []);

      const K = dark ? {
        pop: '#070F1E', bd: '#152545', bd2: '#0D1E3A', shadow: '0 6px 22px rgba(0,0,0,.5)',
        date: '#fff', gold: '#C9A45A', reg: 'rgba(255,255,255,.55)', muted: 'rgba(255,255,255,.3)',
        cellBd: '#0D1E3A', rowBd: '#0D1E3A', dueToday: '#C9A45A', dueOver: '#E07A7A', dueUp: 'rgba(255,255,255,.4)',
      } : {
        pop: '#fff', bd: '#EDE7DC', bd2: '#EDE7DC', shadow: '0 6px 22px rgba(0,26,74,.1)',
        date: '#001A4A', gold: '#AD832F', reg: '#6B7280', muted: '#B4B2A9',
        cellBd: '#F0EBE3', rowBd: '#F5F2EE', dueToday: '#AD832F', dueOver: '#9B1C1C', dueUp: '#B4B2A9',
      };
      ...
      useEffect(() => { if (wide && decs.length && !decs.some(t => t.id === selId)) setSelId((openDecs[0] || decs[0]).id); }, [wide, tasks, decData]);
      const selTask = decs.find(t => t.id === selId) || null;
      const secHead = (color, label, count) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 14px 6px' }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: color, flexShrink: 0 }} />
          <span style={{ fontFamily: J, fontSize: 8, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: K.reg }}>{label}</span>
          <span style={{ fontFamily: J, fontSize: 8, color: K.muted }}>· {count}</span>
          <span style={{ flex: 1, height: 0.5, background: K.bd2 }} />
        </div>
      );
      useEffect(() => { let c = false;
        decs.forEach(t => { if (decData[t.id] && decData[t.id].loaded) return; Promise.all([TaskDB.loadDecisionOptions(t.id), TaskDB.loadActivity(t.id)]).then(([opts, act]) => { if (!c) setDecData(prev => ({ ...prev, [t.id]: { options: opts || [], comments: (act || []).filter(a => a.kind === 'update'), loaded: true } })); }).catch(() => {}); });
        return () => { c = true; }; }, [tasks, peopleByTask]);
      ...
      const chooseOption = async (taskId, idx) => { const d = decData[taskId]; if (!d) return; const opts = d.options.map((o, i) => ({ ...o, is_chosen: i === idx ? !o.is_chosen : false })); setDecData(prev => ({ ...prev, [taskId]: { ...prev[taskId], options: opts } })); try { await TaskDB.setDecisionOptions(taskId, opts.map(o => ({ body: o.body, is_chosen: o.is_chosen }))); } catch (e) {} };
      const addComment = async (taskId) => { const text = (decDraft[taskId] || '').trim(); if (!text || !user) return; setDecDraft(prev => ({ ...prev, [taskId]: '' })); try { await TaskDB.addActivity(taskId, 'update', text, user); const act = await TaskDB.loadActivity(taskId); setDecData(prev => ({ ...prev, [taskId]: { ...(prev[taskId] || {}), comments: (act || []).filter(a => a.kind === 'update') } })); } catch (e) {} };
      const statusPill = (st) => <span style={{ fontFamily: J, fontSize: 7.5, fontWeight: 700, padding: '2px 7px', borderRadius: 10, background: statusBg(st), color: statusColor(st), flexShrink: 0 }}>{statusLabel(st)}</span>;
      ...
      const decCard = (t) => {
        ...
        return (
          <div key={t.id} style={{ margin: '0 12px 8px', borderRadius: 10, border: `1px solid ${K.cellBd}`, borderLeft: `2.5px solid ${accentOf(t)}`, background: dark ? '#0B1526' : '#fff', padding: '11px 13px', opacity: isResolved(t) ? 0.78 : 1 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 5 }}>
              {maker && <span style={{ fontFamily: J, fontSize: 9, color: K.reg }}><b style={{ color: K.gold }}>Maker:</b> {maker}</span>}
              {dd && <span style={{ fontFamily: J, fontSize: 9, fontWeight: 600, color: dd.color }}>Decide by {dd.label}</span>}
              {asg && <span style={{ fontFamily: J, fontSize: 9, color: K.reg }}>Assignee: {asg}</span>}
            </div>
            {t.context && <div style={{ fontFamily: J, fontSize: 9, color: K.reg, lineHeight: 1.5, marginTop: 4, whiteSpace: 'pre-wrap' }}><b style={{ color: K.gold }}>Context:</b> {t.context}</div>}
            {t.decision_question && <div style={{ fontFamily: J, fontSize: 11, fontWeight: 600, color: K.date, margin: '8px 0 6px', lineHeight: 1.4 }}>{t.decision_question}</div>}
            {(d.options || []).map((o, i) => (
              <div key={o.id || i} onClick={() => chooseOption(t.id, i)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 9px', marginBottom: 4, borderRadius: 7, cursor: 'pointer', background: o.is_chosen ? (dark ? 'rgba(15,110,86,.18)' : '#E6F2EC') : (dark ? '#0C1830' : '#FAF8F3'), border: `1px solid ${o.is_chosen ? (dark ? '#1E6B4E' : '#9FD3BC') : K.cellBd}` }}>
                <i className={o.is_chosen ? 'ti ti-circle-check-filled' : 'ti ti-circle'} style={{ fontSize: 16, color: o.is_chosen ? (dark ? '#3FB36B' : '#0F6E56') : K.muted, flexShrink: 0 }} />
                <span style={{ flex: 1, minWidth: 0, fontFamily: J, fontSize: 10, fontWeight: o.is_chosen ? 600 : 400, color: K.date }}>{o.body}</span>
                {o.is_chosen && <span style={{ fontFamily: J, fontSize: 7, fontWeight: 700, letterSpacing: '.06em', color: dark ? '#3FB36B' : '#0F6E56', flexShrink: 0 }}>CHOSEN</span>}
              </div>
            ))}
            {extra && <div style={{ fontFamily: J, fontSize: 9, color: K.reg, lineHeight: 1.5, marginTop: 4, whiteSpace: 'pre-wrap' }}>{extra}</div>}
            {(d.comments || []).slice(-3).map(cm => (
              <div key={cm.id} style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                <span style={{ fontFamily: J, fontSize: 8.5, fontWeight: 700, color: K.gold, flexShrink: 0 }}>{(cm.author_name || 'User').split(' ')[0]}:</span>
                <span style={{ fontFamily: J, fontSize: 9.5, color: K.date, minWidth: 0 }}>{cm.content}</span>
              </div>
            ))}
            <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
              <input value={decDraft[t.id] || ''} onChange={e => setDecDraft(prev => ({ ...prev, [t.id]: e.target.value }))} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addComment(t.id); } }} placeholder="Add a comment…" style={{ flex: 1, minWidth: 0, fontFamily: J, fontSize: 9.5, padding: '7px 10px', borderRadius: 7, border: `1px solid ${K.cellBd}`, background: dark ? '#040C1C' : '#FCFBF8', color: K.date, outline: 'none' }} />
              <button onClick={() => addComment(t.id)} style={{ flexShrink: 0, fontFamily: J, fontSize: 9.5, fontWeight: 600, padding: '0 13px', borderRadius: 7, border: 'none', background: K.gold, color: '#fff', cursor: 'pointer' }}>Send</button>
            </div>
          </div>
        );
      };

      const emptyMsg = <div style={{ padding: '24px 16px', fontFamily: J, fontSize: 11, color: K.muted, textAlign: 'center', lineHeight: 1.6 }}>No decisions yet.<br />Add a decision question, maker, or due date on any task to see it here.</div>;
      const stackedList = (
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', paddingBottom: 14 }}>
          {!decs.length ? emptyMsg : (
            <React.Fragment>
              {openDecs.length > 0 && secHead('#9B1C1C', 'Open', openDecs.length)}
              {openDecs.map(decCard)}
              {resolvedDecs.length > 0 && secHead('#0F6E56', 'Resolved', resolvedDecs.length)}
              {resolvedDecs.map(decCard)}
            </React.Fragment>
          )}
        </div>
      );
      const containerStyle = embed
        ? { position: 'fixed', inset: 0, background: K.pop, opacity: vis ? 1 : 0, transition: 'opacity 150ms ease', display: 'flex', flexDirection: 'column', overflow: 'hidden' }
        : { position: 'fixed', top: POPOUT_TOP, right: 10, width: wide ? 'min(calc(100% - 20px), 820px)' : 'min(calc(100% - 20px), 560px)', bottom: (zoneH || 0), zIndex: 10, background: K.pop, border: `1px solid ${K.bd}`, borderTop: 'none', borderRadius: '0 0 14px 14px', boxShadow: K.shadow, opacity: vis ? 1 : 0, transition: 'opacity 150ms ease', display: 'flex', flexDirection: 'column', overflow: 'hidden' };
      return (
        <React.Fragment>
          {!embed && <div onClick={onClose} style={{ position: 'fixed', top: POPOUT_TOP, left: 0, right: 0, bottom: 0, zIndex: 9, background: dark ? 'rgba(0,0,0,.35)' : 'rgba(0,26,74,.18)' }} />}
          <div onClick={(e) => e.stopPropagation()} style={containerStyle}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 14px', borderBottom: `0.5px solid ${K.bd2}`, flexShrink: 0 }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={K.gold} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M12 4v6" /><path d="M9 7l3-3 3 3" /><path d="M4 13l-2 2 2 2" /><path d="M2 15h7" /><path d="M20 13l2 2-2 2" /><path d="M22 15h-7" /></svg>
              <span style={{ flex: 1, fontFamily: J, fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: K.gold }}>Decisions</span>
              {openDecs.length > 0 && <span style={{ fontFamily: J, fontSize: 8.5, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: dark ? 'rgba(155,28,28,0.28)' : '#FEE2E2', color: dark ? '#F4A6A6' : '#9B1C1C', flexShrink: 0 }}>{openDecs.length} open</span>}
              <i className="ti ti-search" title="Search (coming soon)" style={{ fontSize: 15, color: K.muted, cursor: 'default', flexShrink: 0 }} />
              {!embed && <i className="ti ti-x" onClick={onClose} style={{ fontSize: 17, color: K.muted, cursor: 'pointer', flexShrink: 0 }} />}
            </div>
            {wide ? (
              <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
                {/* LEFT — compact decision list */}
                <div style={{ width: 232, flexShrink: 0, borderRight: `0.5px solid ${K.bd2}`, background: dark ? '#08121F' : '#FCFBF8', overflowY: 'auto', paddingBottom: 12 }}>
                  {!decs.length ? emptyMsg : (
                    <React.Fragment>
                      {openDecs.length > 0 && secHead('#9B1C1C', 'Open', openDecs.length)}
                      {openDecs.map(decRow)}
                      {resolvedDecs.length > 0 && secHead('#0F6E56', 'Resolved', resolvedDecs.length)}
                      {resolvedDecs.map(decRow)}
                    </React.Fragment>
                  )}
                </div>
                {/* RIGHT — full decision detail */}
                <div style={{ flex: 1, minWidth: 0, overflowY: 'auto', paddingTop: 4 }}>
                  {selTask ? decCard(selTask) : (
                    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', textAlign: 'center', color: K.muted, fontFamily: J }}>
                      <i className="ti ti-gavel" style={{ fontSize: 28, color: dark ? '#22324A' : '#D4D0CC', marginBottom: 12 }} />
                      <div style={{ fontSize: 11 }}>Select a decision to see its options</div>
                    </div>
                  )}
                </div>
              </div>
    ...
    function shortTime(iso) {
      ...
      if (Math.floor((now - d) / 86400000) < 7) return d.toLocaleDateString('en-US', { weekday: 'short' });
    ...
    function App({ user, profile }) {
      ...
      const moreItems = []
        ...
        .concat([{ id: 'projects', label: 'Projects', icon: 'ti-list-check' }])
        .concat([{ id: 'expsurvey', label: 'Exp Survey', icon: 'ti-qrcode' }])
        .concat((!appSet || appSet.has('sffu')) ? [{ id: 'sffu', label: 'SFFU', icon: 'ti-messages' }] : [])
      ...
      const toggleAway = () => { const v = !away; setAway(v); ProfileDB.setPresence({ away: v }); };
      const saveStatus = (text, emoji) => { setStatusText(text); setStatusEmoji(emoji); ProfileDB.setPresence({ status_text: text || null, status_emoji: emoji || null }); setShowStatus(false); };
      useEffect(() => {
        if (!visibleTabs.some(t => t.id === activeTab)) setActiveTab((visibleTabs[0] || { id: 'more' }).id);
      }, [profile]);
      ...
      useEffect(() => {
        const apply = () => {
          ...
          setMoreMenuOpen(false); setDrawerOpen(false); setShowProfile(false); setShowStatus(false);
      ...
      async function onSelectFiles(fileList) {
        ...
        for (const file of files) {
          ...
          let kind = null, textContent = null, previewUrl = null;
          try {
            if (type.startsWith('image/')) { kind = 'image'; previewUrl = URL.createObjectURL(file); }
            else if (type === 'application/pdf' || /\.pdf$/i.test(name)) { kind = 'pdf'; }
            else if (/\.(xlsx|xls)$/i.test(name)) { kind = 'text'; textContent = await readXlsxAsText(file); }
            else if (/\.docx$/i.test(name)) { kind = 'text'; textContent = await readDocxAsText(file); }
            else if (/\.pptx$/i.test(name)) { kind = 'text'; textContent = await readPptxAsText(file); }
            else if (/\.(doc|ppt)$/i.test(name)) { alert('"' + name + '" is an older Office format the app can\'t read. Please re-save it as .docx or .pptx and try again.'); continue; }
            else if (type.startsWith('text/') || /\.(txt|csv|md|json|log)$/i.test(name)) { kind = 'text'; textContent = await file.text(); }
            else { alert('The AI can\'t read "' + name + '" yet (unsupported type).'); continue; }
          } catch (e) { alert('Could not read "' + name + '": ' + (e.message || e)); continue; }
          additions.push({ id: crypto.randomUUID(), file, kind, name, mediaType: type, size: file.size, previewUrl, textContent });
        }
        if (additions.length) setPendingAttachments(prev => [...prev, ...additions]);
      }
      function removeAttachment(id) {
        setPendingAttachments(prev => prev.filter(a => {
          if (a.id === id && a.previewUrl) { try { URL.revokeObjectURL(a.previewUrl); } catch (e) {} }
          return a.id !== id;
        }));
      }
      ...
      useEffect(() => {
        ...
        (async () => {
          try {
            for (let i = 0; i < FALLBACK_PILLS.length; i++) await PillDB.create({ ...FALLBACK_PILLS[i], sortOrder: i });
            localStorage.setItem('tmg-pills-seeded', '1');
            refreshPills();
          } catch (e) { console.warn('[pills] seed:', e); }
        })();
      }, [pills, isAdmin]);
      ...
      function updateInput(val) {
        setInput(val);
        try { localStorage.setItem(draftKey(currentConvId), val); } catch (e) {}
      }
      ...
      useEffect(() => { AccessDB.load().then(c => { if (c) setAccessCfg(normalizeAccess(c)); }); }, []);

      // Presence heartbeat: mark "online" now + every minute while the app is open/foreground.
      useEffect(() => {
        ProfileDB.touchPresence();
        const iv = setInterval(() => { if (!document.hidden) ProfileDB.touchPresence(); }, 60000);
        const onVis = () => { if (!document.hidden) ProfileDB.touchPresence(); };
        document.addEventListener('visibilitychange', onVis);
        window.addEventListener('focus', onVis);
      ...
      async function runSend(text, opts = {}) {
        ...
        const localAtts = (pend || []).map(a => ({ kind: a.kind, name: a.name, mediaType: a.mediaType, size: a.size, url: a.previewUrl, textContent: a.textContent }));
        const userMsg = { role: 'user', content: text || '', attachments: localAtts };
        ...
        try {
          if (!convId) {
            const conv = await ConvDB.createConversation({ type: startType, workRequest: startReq });
            convId = conv.id;
          ...
          for (const a of (pend || [])) {
            let path = null;
            if (a.file) { try { path = await ConvDB.uploadAttachment(a.file, convId); } catch (e) {} }
            const url = path ? await ConvDB.signedUrl(path) : a.previewUrl;
            uploaded.push({ kind: a.kind, name: a.name, mediaType: a.mediaType, size: a.size, path, url, textContent: a.textContent });
          }
          const history = next.map((m, idx) => (idx === next.length - 1 ? { ...m, attachments: uploaded } : m));
          setMessages(history);

          await ConvDB.insertMessage(convId, 'user', text || '', uploaded);
          const apiMsgs = history.map(m => ({ role: m.role, content: toApiContent(m) }));
            ...
            if (mm) {
              ...
              try {
                ...
                if (parsed && String(parsed.type).toUpperCase() === 'KPI' && parsed.payload) {
                  const payload = { ...parsed.payload, owner: myName }; // owner is app-side, never from the model
                  setKpiInstruction(null); // got a full payload → leave note mode; confirm via the card
                  setKpiFlow({ step: 'summary', payload });
          ...
          if (isFirst) {
            ...
            await ConvDB.updateConversation(convId, { title });
          } else {
            await ConvDB.touchConversation(convId);
          }
        } catch (e) {
          setMessages(prev => [...prev, { role: 'assistant', content: String(e.message || e), error: true }]);
        } finally {
          setLoading(false);
          refreshConversations();
        }
      ...
      function send() {
        ...
        try { localStorage.removeItem(draftKey(currentConvId)); } catch (e) {}
        setInput('');
        runSend(text);
      }
      ...
      function startKpiNote() {
        ...
        setInput('KPI: ');
        setTimeout(() => { const el = document.querySelector('.tmg-ai-input'); if (el) { el.focus(); try { const n = el.value.length; el.setSelectionRange(n, n); } catch (e) {} } }, 30);
      }
      ...
      async function submitKpi(rawPayload) {
        ...
        const payload = { ...rawPayload, owner: myName, owner_email: (profile && profile.email) || null };
        setKpiFlow(null);
        ...
        if (!cid) {
          try {
            const conv = await ConvDB.createConversation({ type: 'kpi', workRequest: 'Enter KPI', title: 'KPI — ' + (payload.kpi_date || kpiTodayStr()) });
            cid = conv.id;
            setCurrentConvId(cid);
            refreshConversations();
          } catch (e) { /* best-effort — the confirmation above still shows on screen either way */ }
        }
        if (cid) { try { await ConvDB.insertMessage(cid, 'assistant', content); await ConvDB.touchConversation(cid); } catch (e) {} }
      }
      ...
      function dispatchPill(pill) {
        ...
        if (pill.id === 'fb-kpis' || (pill.config && pill.config.kpi)) {
          ...
          setKpiFlow({ step: 'choice' });
          return;
        }
        if (pill.type === 'form') { setActivePill({ pill, kind: 'form' }); return; }
        if (pill.type === 'steps') { setActivePill({ pill, kind: 'steps' }); return; }
        setKpiFlow(null); setKpiInstruction(null);
        if (pill.type === 'task') {
      ...
      function startWorkRequest(key) {
        ...
        setCurrentConvId(null);
        setMessages([]);
        runSend(req.label, { type: 'work', workRequest: key });
      }

      async function pinConversation(id, pinned) { await ConvDB.updateConversation(id, { pinned }); refreshConversations(); }
      async function archiveConversation(id, archived) {
        await ConvDB.updateConversation(id, { archived });
      ...
      async function renameConversation(id) {
        ...
        if (title && title.trim()) { await ConvDB.updateConversation(id, { title: title.trim() }); refreshConversations(); }
      }
      ...
      if (TASKS_EMBED) {
        return (
          <div style={{ height: '100%', background: dark ? '#000D26' : C.bg, color: C.textPrimary }}>
            {showCalendar && <CalendarPopover dark={dark} zoneH={0} onOpenTask={(t) => { setShowCalendar(false); setShowTasks(true); setTasksFocus(t); }} onClose={() => {}} />}
            {showTasks && <TasksScreen dark={dark} user={user} initialTask={tasksFocus} onClose={() => {}} />}
            {showDecisions && <DecisionsPopover dark={dark} zoneH={0} onOpenTask={(t) => { setShowDecisions(false); setShowTasks(true); setTasksFocus(t); }} onClose={() => {}} />}
          </div>
        );
      }
      ...
      if (TASKS_STANDALONE) {
        return (
          <div style={{ height: '100%', background: dark ? '#000D26' : C.bg, display: 'flex', flexDirection: 'column', color: C.textPrimary }}>
            {/* Top-bar icons SELECT a surface, never toggle it off: toggling
                left all three false and the page blank below the bar (audit
                C5/C22). Tasks while already on Tasks = go home to My Tasks via
                the hash, so both hash readers (App's and TasksScreen's) react. */}
            <TopBar dark={dark} onToggleDark={() => setDark(d => !d)}
              onCalendar={() => { setTasksFocus(null); if (showCalendar) { setShowCalendar(false); setShowTasks(true); } else { setShowCalendar(true); setShowTasks(false); setShowDecisions(false); } }} calendarOpen={showCalendar}
              onTasks={() => { setShowCalendar(false); setShowDecisions(false); setTasksFocus(null); if (showTasks) { try { if (location.hash.replace(/^#\/?/, '') !== 'tasks') location.hash = 'tasks'; else window.dispatchEvent(new HashChangeEvent('hashchange')); } catch (e) {} } else setShowTasks(true); }} tasksOpen={showTasks}
              onDecisions={() => { setTasksFocus(null); if (showDecisions) { setShowDecisions(false); setShowTasks(true); } else { setShowDecisions(true); setShowCalendar(false); setShowTasks(false); } }} decisionsOpen={showDecisions}
              avatar={myAvatar} name={myName} dotColor={dotColor} onProfile={() => setShowProfile(true)} />
            {/* tasksFocus is a one-shot handoff. Leaving it set meant closing a
                popover without clicking anything re-opened the last task you
                ever opened from it, instead of the view you came from (C18). */}
            {showCalendar && <CalendarPopover dark={dark} zoneH={0} onOpenTask={(t) => { setShowCalendar(false); setShowTasks(true); setTasksFocus(t); }} onClose={() => { setTasksFocus(null); setShowCalendar(false); setShowTasks(true); }} />}
            {showTasks && <TasksScreen dark={dark} user={user} initialTask={tasksFocus} onClose={() => { setTasksFocus(null); }} />}
            {showDecisions && <DecisionsPopover dark={dark} zoneH={0} onOpenTask={(t) => { setShowDecisions(false); setShowTasks(true); setTasksFocus(t); }} onClose={() => { setTasksFocus(null); setShowDecisions(false); setShowTasks(true); }} />}
            {!showCalendar && !showTasks && !showDecisions && <div style={{ flex: 1 }} />}
            {showProfile && <ProfilePanel dark={dark} user={user} profile={profile} name={myName} avatar={myAvatar} dotColor={dotColor} away={away} statusText={statusText} statusEmoji={statusEmoji} onToggleAway={toggleAway} onOpenStatus={() => setShowStatus(true)} onClose={() => setShowProfile(false)} historySide={historySide} setHistorySide={setHistorySide} setDark={setDark} fontScale={fontScale} setFontScale={setFontScale} />}
    ...
    (function mount() {
      ...
      function _showScreen(id) {
        ['splash-screen', 'login-screen', 'denied-screen', 'pending-screen'].forEach(function (s) { var el = document.getElementById(s); if (el) el.hidden = (s !== id); });
      ...
      if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.protocol === 'file:') {
        root.render(<App user={window.SupabaseAuth._state.session?.user} profile={{ id: 'dev-user-id', email: 'symon@morshedgroup.com', first_name: 'Symon', last_name: 'Yongco', title: 'Operations Manager', employee_id: 'TMG-001', access: 'admin', status: 'active', avatar_url: '' }} />);
        hideOverlay();
        return;
      }
      ...
      window.SupabaseAuth.onAuthStateChange(async function({ session }) {
        ...
        if (grt) { window.SupabaseAuth._googleRefresh = null; connectGoogleCalendar(grt); }
