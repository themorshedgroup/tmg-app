...
function useIsWide() {
  const [w, setW] = useState(() => typeof window !== 'undefined' && window.matchMedia('(min-width:769px)').matches);
  useEffect(() => {
...
_devAttachUrls['dev/sample-floorplan.svg'] = 'data:image/svg+xml;utf8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="220" height="150" viewBox="0 0 220 150">' + '<rect width="220" height="150" fill="#F3EBDA"/>' + '<rect x="12" y="12" width="120" height="90" fill="none" stroke="#AD832F" stroke-width="2"/>' + '<rect x="132" y="12" width="76" height="44" fill="none" stroke="#AD832F" stroke-width="2"/>' + '<rect x="132" y="56" width="76" height="46" fill="none" stroke="#AD832F" stroke-width="2"/>' + '<line x1="12" y1="102" x2="208" y2="102" stroke="#001A4A" stroke-width="2"/>' + '<text x="110" y="132" font-family="Jost,sans-serif" font-size="13" fill="#001A4A" text-anchor="middle">Sample Floor Plan</text>' + '</svg>');
...
const ChatDB = {
  ...
  uid() {
    return window.SupabaseAuth?._state?.session?.user?.id || (this.client() ? null : 'dev-user-id');
  },
  _ws: null,
  _devData: null,
  _devRead: {},
  _dev() {
    ...
    this._devData = {
      ...
      conversations: [{
        ...
        type: 'dm',
        title: null,
        memberIds: ['dev-user-id', 'dev-tarek']
      }, {
        id: 'devg1',
        type: 'group_dm',
        title: 'Ops Team',
        memberIds: ['dev-user-id', 'dev-tarek', 'dev-alex']
      }],
      messages: {
        devc1: [{
          id: 'm1',
          channel_id: 'devc1',
          author_id: 'dev-tarek',
          content: 'Welcome to the new Chat! 🎉 Notes at https://themorshedgroup.com',
          created_at: ago(3600000),
          reactions: [{
            emoji: '🎉',
            user_id: 'dev-user-id'
          }, {
          ...
          attachments: [{
            id: 'devatt1',
            message_id: 'm1',
            storage_path: 'dev/sample-floorplan.svg',
          ...
          id: 'm2',
          channel_id: 'devc1',
          author_id: 'dev-user-id',
          content: 'Looks great. @Tarek can you review the #deals pipeline?',
          ...
          channel_id: 'devc1',
          author_id: 'dev-tarek',
          content: 'XSS test: <img src=x onerror=alert(1)> <script>alert(1)<\/script> javascript:alert(1)',
          created_at: ago(600000),
          edited_at: ago(300000),
          reactions: [],
          reply_count: 0
        }, {
          id: 'm4',
          channel_id: 'devc1',
          author_id: 'dev-tarek',
          content: 'Hey team, quick voice note — the buyer wants to push closing to Friday, let me know if that works.',
          created_at: ago(240000),
          reactions: [],
          reply_count: 0,
          attachments: [{
            id: 'devatt2',
            message_id: 'm4',
            storage_path: 'dev/sample-voice.wav',
        ...
        devd1: [{
          id: 'd1',
          conversation_id: 'devd1',
          author_id: 'dev-tarek',
          content: 'Hey — got a sec to sync on Forest Hill?',
  ...
  async loadRoster() {
    if (!this.client()) return [{
      ...
      away: false,
      status_emoji: '☕',
      status_text: 'Coffee run',
      last_seen_at: new Date().toISOString()
    }];
    try {
      return await ProfileDB.loadAll();
    } catch (e) {
      return [];
    }
  },
  async defaultWorkspaceId() {
    ...
    const {
      data
    } = await this.client().from('chat_workspaces').select('id').limit(1).maybeSingle();
  ...
  async listChannels() {
    ...
    const {
      data,
      error
    } = await this.client().from('chat_channels').select('*').eq('is_archived', false).order('name', {
      ascending: true
    });
    if (error) {
      console.error('[ChatDB] listChannels:', error.message);
      return [];
    }
  ...
  async listConversations() {
    ...
    const {
      data: mine,
      error: e1
    } = await this.client().from('chat_conversation_members').select('conversation_id').eq('user_id', me);
    if (e1) {
      console.error('[ChatDB] convs:', e1.message);
      return [];
    }
    ...
    const {
      data: convs
    } = await this.client().from('chat_conversations').select('*').in('id', ids);
    const {
      data: mems
    } = await this.client().from('chat_conversation_members').select('conversation_id,user_id').in('conversation_id', ids);
    const byConv = {};
    (mems || []).forEach(m => {
    ...
    return (convs || []).map(c => ({
      ...
      type: c.type,
      title: c.title || null,
      memberIds: byConv[c.id] || []
    }));
  },
  _scopeCol(scope) {
    return scope.kind === 'channel' ? 'channel_id' : 'conversation_id';
  },
  async loadMessages(scope, opts = {}) {
    ...
    const {
      data,
      error
    } = await this.client().from('chat_messages').select('*').eq(col, scope.id).is('thread_parent_id', null).order('created_at', {
      ascending: true
    }).limit(limit);
    if (error) {
      console.error('[ChatDB] loadMessages:', error.message);
      return [];
    }
    const msgs = data || [];
  ...
  async _enrich(scope, msgs) {
    ...
    const {
      data: rx
    } = await this.client().from('chat_message_reactions').select('message_id,emoji,user_id').in('message_id', ids);
    const rxBy = {};
    (rx || []).forEach(r => {
      (rxBy[r.message_id] = rxBy[r.message_id] || []).push({
        emoji: r.emoji,
        user_id: r.user_id
      });
    });
    const {
      data: rep
    } = await this.client().from('chat_messages').select('thread_parent_id').eq(col, scope.id).not('thread_parent_id', 'is', null).in('thread_parent_id', ids);
    const repBy = {};
    (rep || []).forEach(r => {
      repBy[r.thread_parent_id] = (repBy[r.thread_parent_id] || 0) + 1;
    });
    const byAtt = await this.loadAttachments(ids);
    msgs.forEach(m => {
      m.reactions = rxBy[m.id] || [];
      m.reply_count = repBy[m.id] || 0;
      m.attachments = byAtt[m.id] || [];
    });
  },
  async threadReplies(scope, parentId) {
    ...
    const {
      data,
      error
    } = await this.client().from('chat_messages').select('*').eq('thread_parent_id', parentId).order('created_at', {
      ascending: true
    });
    if (error) {
      console.error('[ChatDB] threadReplies:', error.message);
      return [];
    }
    const msgs = data || [];
    if (msgs.length) await this._enrich(scope, msgs);
  ...
  async sendMessage(scope, {
    ...
    if (!this.client()) {
      ...
      const row = {
        ...
        reactions: [],
        reply_count: 0,
        channel_id: scope.kind === 'channel' ? scope.id : null,
        conversation_id: scope.kind === 'conversation' ? scope.id : null,
        thread_parent_id: threadParentId || null
      };
      if (threadParentId) {
        (d.threads[threadParentId] = d.threads[threadParentId] || []).push(row);
        const pm = (d.messages[scope.id] || []).find(m => m.id === threadParentId);
        if (pm) pm.reply_count = (pm.reply_count || 0) + 1;
    ...
    const {
      data,
      error
    } = await this.client().from('chat_messages').insert(base).select().single();
    if (error) {
      console.error('[ChatDB] send:', error.message);
      throw error;
    }
    return data;
  },
  ...
  async uploadAttachment(scope, file) {
    ...
    const {
      error
    } = await this.client().storage.from(ATTACH_BUCKET).upload(path, file, {
      contentType: file.type || 'application/octet-stream',
      upsert: false
    });
    if (error) {
      console.error('[ChatDB] uploadAttachment:', error.message);
      throw error;
    }
    return {
      storage_path: path,
  ...
  async addAttachmentRow(messageId, meta) {
    ...
    if (!this.client()) {
      const d = this._dev();
      const rec = {
        id: 'devatt-' + (crypto.randomUUID ? crypto.randomUUID() : Date.now()),
        ...row
      };
      const push = m => {
        if (m.id === messageId) m.attachments = [...(m.attachments || []), rec];
      };
    ...
    const {
      data,
      error
    } = await this.client().from('chat_message_attachments').insert(row).select().single();
    if (error) {
      console.error('[ChatDB] addAttachmentRow:', error.message);
      throw error;
    }
    return data;
  },
  ...
  async signedUrl(storagePath) {
    ...
    if (!this.client()) {
      const u = _devAttachUrls[storagePath] || '';
      if (u) _signedUrlCache.set(storagePath, u);
      return u;
    }
    const {
      data,
      error
    } = await this.client().storage.from(ATTACH_BUCKET).createSignedUrl(storagePath, 3600);
    if (error || !data?.signedUrl) {
      console.error('[ChatDB] signedUrl:', error && error.message);
      return '';
    }
  ...
  async loadAttachments(messageIds) {
    ...
    if (!this.client()) {
      const d = this._dev();
      const scan = m => {
        if (ids.includes(m.id) && (m.attachments || []).length) out[m.id] = m.attachments.slice();
      };
    ...
    const {
      data,
      error
    } = await this.client().from('chat_message_attachments').select('*').in('message_id', ids);
    if (error) {
      console.error('[ChatDB] loadAttachments:', error.message);
      return out;
    }
    (data || []).forEach(r => {
  ...
  async transcribe(storagePath) {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('[ChatDB] transcribe HTTP', res.status);
        return '';
      }
      const data = await res.json().catch(() => ({}));
      if (data && data.error) {
        console.error('[ChatDB] transcribe:', data.error);
        return '';
      }
      return data && data.transcript || '';
    } catch (e) {
      console.error('[ChatDB] transcribe:', e && e.message);
      return '';
    }
  },
  async editMessage(id, content) {
    if (!this.client()) {
      ...
      Object.keys(d.messages).forEach(k => d.messages[k].forEach(m => {
        if (m.id === id) {
          m.content = content;
          m.edited_at = stamp;
        }
      }));
      Object.keys(d.threads).forEach(k => d.threads[k].forEach(m => {
        if (m.id === id) {
          m.content = content;
          m.edited_at = stamp;
        }
    ...
    const {
      error
    } = await this.client().from('chat_messages').update({
      content,
      edited_at: new Date().toISOString()
    }).eq('id', id);
    if (error) {
      console.error('[ChatDB] edit:', error.message);
      throw error;
    }
  },
  async react(messageId, emoji) {
    ...
    const {
      error
    } = await this.client().from('chat_message_reactions').insert({
      message_id: messageId,
      user_id: this.uid(),
      emoji
    });
  ...
  async unreact(messageId, emoji) {
    ...
    const {
      error
    } = await this.client().from('chat_message_reactions').delete().eq('message_id', messageId).eq('user_id', this.uid()).eq('emoji', emoji);
  ...
  async insertMentions(messageId, mentions) {
    ...
    const rows = mentions.map(m => ({
      ...
      mentioned_channel_id: m.channelId || null
    }));
    const {
      error
    } = await this.client().from('chat_mentions').insert(rows);
    if (error) console.error('[ChatDB] mentions:', error.message);
  },
  async markRead(scope, lastMessageId) {
    if (!this.client()) {
      this._devRead[scope.id] = Date.now();
      return;
    }
    ...
    const {
      error
    } = await this.client().from(tbl).update(patch).eq(col, scope.id).eq('user_id', this.uid());
  ...
  async loadMemberReads(scope) {
    ...
    const {
      data,
      error
    } = await this.client().from(tbl).select('user_id,last_read_at,last_read_message_id').eq(col, scope.id);
    if (error) {
      console.error('[ChatDB] loadMemberReads:', error.message);
      return [];
    }
    return data || [];
  },
  ...
  subscribeMemberReads(scope, onUpdate) {
    ...
    const tbl = scope.kind === 'channel' ? 'chat_channel_members' : 'chat_conversation_members';
    const col = scope.kind === 'channel' ? 'channel_id' : 'conversation_id';
    const ch = c.channel('chatreads:' + scope.kind + ':' + scope.id).on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: tbl,
      filter: col + '=eq.' + scope.id
    }, p => onUpdate && onUpdate(p.new)).subscribe();
    return ch;
  },
  ...
  async unreadSummary() {
    if (!this.client()) {
      ...
      const me = this.uid();
      const scan = (id, msgs) => {
        const since = this._devRead[id] || 0;
        const u = (msgs || []).filter(m => m.author_id !== me && new Date(m.created_at).getTime() > since).length;
        if (u) {
          byScope[id] = {
            unread: u,
            mentions: 0
    ...
    try {
      ...
      const scopes = [].concat((cm.data || []).map(r => ({
        kind: 'channel',
        id: r.channel_id,
        col: 'channel_id',
        since: r.last_read_at
      })), (vm.data || []).map(r => ({
        kind: 'conversation',
      ...
      await Promise.all(scopes.map(async s => {
        let q = this.client().from('chat_messages').select('id', {
          count: 'exact',
          head: true
        }).eq(s.col, s.id).neq('author_id', me);
      ...
      const sinceById = {};
      scopes.forEach(s => {
        sinceById[s.id] = s.since ? new Date(s.since).getTime() : 0;
      });
      const {
        data: mn
      } = await this.client().from('chat_mentions').select('message:chat_messages(channel_id,conversation_id,created_at,author_id)').eq('mentioned_user_id', me).limit(200);
      (mn || []).forEach(r => {
        const m = r.message;
        if (!m || m.author_id === me) return;
        const sid = m.channel_id || m.conversation_id;
        const t = new Date(m.created_at).getTime();
        if (t > (sinceById[sid] || 0)) {
          out.mentions++;
          if (out.byScope[sid]) out.byScope[sid].mentions++;
        }
      });
    } catch (e) {
      console.error('[ChatDB] unreadSummary:', e.message || e);
    }
    return out;
  },
  async createChannel({
    ...
    const {
      data,
      error
    } = await this.client().from('chat_channels').insert({
      workspace_id: ws,
      name,
      type: type || 'public',
      topic: topic || null,
    ...
    if (error) {
      console.error('[ChatDB] createChannel:', error.message);
      throw error;
    }
    const {
      error: me
  ...
  async leaveChannel(channelId) {
    ...
    const {
      error
    } = await this.client().from('chat_channel_members').delete().eq('channel_id', channelId).eq('user_id', this.uid());
  ...
  async createDM(otherId) {
    if (!this.client()) {
      const cv = {
        ...
        type: 'dm',
        title: null,
        memberIds: [this.uid(), otherId]
      };
      this._dev().conversations.push(cv);
      return cv;
    }
    const me = this.uid();
    const existing = (await this.listConversations()).find(cv => cv.type === 'dm' && cv.memberIds.length === 2 && cv.memberIds.includes(me) && cv.memberIds.includes(otherId));
    if (existing) return existing;
    const ws = await this.defaultWorkspaceId();
    const {
      data: conv,
      error
    } = await this.client().from('chat_conversations').insert({
      workspace_id: ws,
      type: 'dm',
      created_by: me
    }).select().single();
    if (error) {
      console.error('[ChatDB] createDM:', error.message);
      throw error;
    }
    const {
      error: me2
    } = await this.client().from('chat_conversation_members').insert([{
  ...
  async createGroupDM(userIds, title) {
    ...
    if (error) {
      console.error('[ChatDB] createGroupDM:', error.message);
      throw error;
    }
    const {
      error: me2
    } = await this.client().from('chat_conversation_members').insert(members.map(u => ({
      conversation_id: conv.id,
      user_id: u
    })));
  ...
  subscribeScope(scope, {
    ...
    if (!c || !scope) return null;
    const col = this._scopeCol(scope);
    const ch = c.channel('chat:' + scope.kind + ':' + scope.id).on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'chat_messages',
      filter: col + '=eq.' + scope.id
    }, p => onInsert && onInsert(p.new)).on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: 'chat_messages',
      filter: col + '=eq.' + scope.id
    }, p => onUpdate && onUpdate(p.new)).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'chat_message_reactions'
    }, p => onReaction && onReaction(p)).subscribe();
    return ch;
  },
  unsubscribe(ch) {
    const c = this.client();
    if (ch && c) c.removeChannel(ch);
  }
};
...
function parseChatMentions(text, scope, roster, channels) {
  ...
  while (m = MEN.exec(text)) {
    ...
    if (h === 'channel') {
      if (!seen.channel && scope.kind === 'channel') {
        out.push({
          type: 'channel',
          channelId: scope.id
        });
...
function renderMessageContent(text, ctx) {
  ...
  while (m = URL_RE.exec(s)) matches.push({
    start: m.index,
  ...
  while (m = MEN_RE.exec(s)) matches.push({
    start: m.index,
    end: m.index + m[0].length,
    type: 'mention',
    id: m[1] || null,
  ...
  const CODE_RE = /`([^`\n]+)`/g;
  while (m = CODE_RE.exec(s)) matches.push({
    start: m.index,
    end: m.index + m[0].length,
    type: 'code',
    inner: m[1]
  ...
  const LINK_RE = /\[([^\]\n]+)\]\((https?:\/\/[^\s)]+)\)/g;
  while (m = LINK_RE.exec(s)) matches.push({
    start: m.index,
    end: m.index + m[0].length,
    type: 'mdlink',
    inner: m[1],
  ...
  const BOLD_RE = /\*\*([^*\n]+)\*\*/g;
  while (m = BOLD_RE.exec(s)) matches.push({
    start: m.index,
    end: m.index + m[0].length,
    type: 'bold',
    inner: m[1]
  ...
  while (m = ITAL_RE.exec(s)) {
    const st = s.indexOf('_', m.index);
    matches.push({
      start: st,
      end: st + 2 + m[1].length,
      type: 'italic',
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (t.type === 'url') {
      ...
      out.push(href ? React.createElement('a', {
        key: key++,
        href,
        target: '_blank',
      ...
      out.push(React.createElement('span', {
        ...
        style: {
          ...
          fontSize: '0.92em',
          background: ctx.dark ? 'rgba(255,255,255,0.12)' : 'rgba(0,26,74,0.08)',
          padding: '1px 5px',
          borderRadius: 5
        }
      }, t.inner));
      ...
      }, renderMessageContent(t.inner, ctx)));
...
function AttachmentImage({
  ...
  useEffect(() => {
    let active = true;
    ChatDB.signedUrl(att.storage_path).then(u => {
  ...
  const ph = dark ? '#0A1730' : '#F3EBDA',
    phBord = dark ? '#152545' : '#E4DFD4',
    phCol = dark ? 'rgba(255,255,255,0.4)' : '#8A8676';
  if (err || !url && !att.previewUrl) {
    ...
    }), att.file_name || 'Image unavailable');
    return /*#__PURE__*/React.createElement("div", {
      style: {
        width: 160,
        height: 110,
...
function AttachmentAudio({
  ...
  useEffect(() => {
    let active = true;
    if (!att.previewUrl) ChatDB.signedUrl(att.storage_path).then(u => {
      if (active && u) setUrl(u);
    }).catch(() => {});
    return () => {
      active = false;
  ...
  const bord = dark ? '#152545' : '#E4DFD4',
    bg = dark ? '#0A1730' : '#fff',
    col = dark ? 'rgba(255,255,255,0.5)' : '#8A8676';
  if (!url) {
    return /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'inline-flex',
...
function NewChannelModal({
  ...
  const panelBg = dark ? '#0A1730' : '#FFFFFF',
    txt = dark ? '#fff' : '#001A4A',
    sub = dark ? 'rgba(255,255,255,0.55)' : '#6B6B6B',
    bord = dark ? '#152545' : '#E4DFD4',
    inpBg = dark ? '#06101F' : '#F3EBDA';
  const [name, setName] = useState('');
  const [type, setType] = useState(isAdmin ? 'public' : 'private');
  ...
  const create = async () => {
    ...
    if (!nm) {
      alert('Give the channel a name.');
      return;
    }
    setBusy(true);
    try {
      const ch = await ChatDB.createChannel({
        name: nm,
        type,
        topic: topic.trim()
      });
      ...
      alert('Could not create channel: ' + (e.message || e));
    } finally {
      setBusy(false);
    }
  };
  const tBtn = (val, label, icon, disabled) => /*#__PURE__*/React.createElement("button", {
    onClick: () => !disabled && setType(val),
    disabled: disabled,
    style: {
      flex: 1,
      display: 'flex',
      ...
      fontSize: '0.8rem',
      fontWeight: 600,
      border: `1px solid ${type === val ? dark ? '#C9A45A' : '#AD832F' : bord}`,
      background: type === val ? dark ? 'rgba(173,131,47,0.18)' : '#F3EBDA' : 'transparent',
      color: disabled ? dark ? 'rgba(255,255,255,0.25)' : '#B4B2A9' : type === val ? dark ? '#C9A45A' : '#AD832F' : txt
    }
  }, /*#__PURE__*/React.createElement("i", {
    className: `ti ${icon}`,
    style: {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "New channel"), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    "aria-label": "Close",
    style: {
      background: 'none',
    ...
    style: {
      display: 'flex',
      gap: 8,
      marginBottom: type === 'public' && !isAdmin ? 6 : 12
    }
  }, tBtn('public', 'Public', 'ti-hash', !isAdmin), tBtn('private', 'Private', 'ti-lock', false)), !isAdmin && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: '0.68rem',
      color: sub,
      marginBottom: 12,
  ...
  }, "Only admins can create public channels. You can create a private one."), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: '0.62rem',
      letterSpacing: '0.05em',
      color: sub,
...
function NewDMModal({
  ...
  const start = async () => {
    ...
    setBusy(true);
    try {
      const cv = picked.length === 1 ? await ChatDB.createDM(picked[0]) : await ChatDB.createGroupDM(picked);
      onCreated(cv);
    } catch (e) {
      alert('Could not start chat: ' + (e.message || e));
    } finally {
      setBusy(false);
    }
  };
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        cursor: 'pointer',
        borderRadius: 10,
        background: on ? dark ? 'rgba(173,131,47,0.12)' : '#F3EBDA' : 'transparent'
      }
    }, /*#__PURE__*/React.createElement(Avatar, {
      photo: p.avatar_url,
      name: nameOf(p),
    ...
    }, nameOf(p)), p.title && /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: J,
        fontSize: '0.7rem',
        color: sub
      }
    }, p.title)), /*#__PURE__*/React.createElement("i", {
      className: `ti ${on ? 'ti-circle-check-filled' : 'ti-circle'}`,
      style: {
        fontSize: 20,
        color: on ? dark ? '#C9A45A' : '#AD832F' : dark ? 'rgba(255,255,255,0.2)' : '#D8D2C6'
      }
    }));
  })), /*#__PURE__*/React.createElement("button", {
    onClick: start,
    ...
    style: {
      ...
      padding: 12,
      marginTop: 10,
      background: picked.length ? dark ? '#AD832F' : '#001A4A' : dark ? '#1A2A45' : '#C9CDD6',
      color: '#fff',
      border: 'none',
      borderRadius: 12,
      fontSize: '0.9rem',
  ...
  }, busy ? 'Starting…' : picked.length > 1 ? `Start group chat (${picked.length})` : 'Start chat')));
}
function ChatConversation({
  ...
  const J = "'Jost', sans-serif";
  const barBg = dark ? '#070F1E' : '#FFFFFF',
    barBord = dark ? '#0D1E3A' : '#F0EBE3',
    msgBg = dark ? '#070F1E' : '#FCFBF8';
  const backCol = dark ? 'rgba(255,255,255,0.4)' : 'rgba(0,26,74,0.4)',
    titleCol = dark ? '#fff' : '#001A4A',
    subCol = dark ? 'rgba(255,255,255,0.4)' : '#6B6B6B';
  const theirBg = dark ? '#0A1E44' : '#FFFFFF',
    theirCol = dark ? '#fff' : '#001A4A',
    theirBord = dark ? 'none' : '1px solid #E4DFD4';
  const mineBg = dark ? '#AD832F' : '#001A4A',
    timeCol = dark ? 'rgba(255,255,255,0.3)' : '#B4B2A9',
    senderGold = dark ? '#C9A45A' : '#AD832F';
  const inBarBg = dark ? '#070F1E' : '#F5F0E8',
    inBarBord = dark ? '#0D1E3A' : '#E0D8CC',
    inFieldBg = dark ? '#0A1730' : '#FFFFFF',
    inFieldBord = dark ? '#152545' : '#E0D8CC',
    sendBg = dark ? '#AD832F' : '#001A4A';
  const reactBg = dark ? 'rgba(173,131,47,0.15)' : '#F3EBDA',
    reactBord = dark ? '#152545' : '#E4DFD4',
    reactMineBd = dark ? '#C9A45A' : '#AD832F';
  ...
  const byIdNames = {};
  Object.keys(rosterById).forEach(id => {
    byIdNames[id] = nameOf(id);
  });
  const ctxFor = mine => mine ? {
    dark,
    byId: byIdNames,
    chipColor: '#fff',
    linkColor: '#fff'
  ...
  const dedupeAdd = (prev, row) => prev.some(x => row.client_id && x.client_id === row.client_id || x.id === row.id) ? prev.map(x => row.client_id && x.client_id === row.client_id || x.id === row.id ? {
    ...x,
    ...row
  } : x) : [...prev, row];
  const applyReaction = (list, p) => {
    const r = p.new && p.new.message_id ? p.new : p.old;
    if (!r || !r.message_id) return list;
    return list.map(mm => {
      if (mm.id !== r.message_id) return mm;
      let rx = mm.reactions || [];
      if (p.eventType === 'DELETE') rx = rx.filter(x => !(x.emoji === r.emoji && x.user_id === r.user_id));else if (!rx.some(x => x.emoji === r.emoji && x.user_id === r.user_id)) rx = [...rx, {
        emoji: r.emoji,
        user_id: r.user_id
      }];
      return {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setMemberReads([]);
    ChatDB.loadMessages(scope).then(ms => {
      if (active) {
        setMessages(ms);
        setLoading(false);
        setTimeout(scrollDown, 40);
        ChatDB.markRead(scope, ms.length ? ms[ms.length - 1].id : null).then(() => onRead && onRead());
      }
    });
    ChatDB.loadMemberReads(scope).then(rows => {
      if (active) setMemberReads((rows || []).filter(r => r.user_id !== me));
    });
    ...
    const ch = ChatDB.subscribeScope(scope, {
      onInsert: row => {
        ...
        const patchAttachments = () => {
          ...
          ChatDB.loadAttachments([row.id]).then(byId => {
            const atts = byId[row.id] || [];
            if (!atts.length) return;
            const apply = l => l.map(x => x.id === row.id ? {
              ...x,
              attachments: atts
            } : x);
            setMessages(apply);
        ...
        if (row.thread_parent_id) {
          setMessages(prev => prev.map(x => x.id === row.thread_parent_id ? {
            ...x,
            reply_count: (x.reply_count || 0) + 1
          } : x));
          if (threadRef.current && threadRef.current.id === row.thread_parent_id) {
            setThreadMsgs(prev => dedupeAdd(prev, row));
            patchAttachments();
          }
        } else {
      ...
      onUpdate: row => {
        setMessages(prev => prev.map(x => x.id === row.id ? {
          ...x,
          ...row
        } : x));
        setThreadMsgs(prev => prev.map(x => x.id === row.id ? {
          ...x,
          ...row
        } : x));
      },
      onReaction: p => {
        setMessages(prev => applyReaction(prev, p));
        setThreadMsgs(prev => applyReaction(prev, p));
      }
    });
    return () => {
      active = false;
      ChatDB.unsubscribe(ch);
      ChatDB.unsubscribe(readCh);
    };
  }, [scope.id]);
  ...
  const doSend = async (body, threadParentId, clear, setBusy, staged) => {
    ...
    const optimisticAtt = ready.map(p => ({
      id: 'tmpatt-' + p.id,
      message_id: 'tmp-' + clientId,
      ...p.meta
    }));
    const optimistic = {
      id: 'tmp-' + clientId,
    ...
    try {
      ...
      const row = await ChatDB.sendMessage(scope, {
        content: body,
        clientId,
        threadParentId
      });
      ...
      for (const p of ready) {
        try {
          const ar = await ChatDB.addAttachmentRow(row.id, p.meta);
          attachments.push(ar);
        } catch (e) {
          /* keep the optimistic preview so the user still sees their file */attachments.push({
            id: 'tmpatt-' + p.id,
            message_id: row.id,
            ...p.meta
          });
      ...
      if (threadParentId) {
        setThreadMsgs(prev => prev.map(x => x.client_id === clientId ? merged : x));
        setMessages(prev => prev.map(x => x.id === threadParentId ? {
          ...x,
          reply_count: (x.reply_count || 0) + 1
        } : x));
      } else setMessages(prev => prev.map(x => x.client_id === clientId ? merged : x));
    } catch (e) {
      const mark = l => l.map(x => x.client_id === clientId ? {
        ...x,
        _pending: false,
        _failed: true
      } : x);
  ...
  const send = () => doSend(text, null, () => {
    setText('');
    setPending([]);
  }, setSending, pending);
  const sendThread = () => doSend(threadText, thread && thread.id, () => {
    setThreadText('');
    setPending([]);
  }, setTSending, pending);
  const onKey = fn => e => {
  ...
  const onPickFiles = async e => {
    ...
    for (const file of files) {
      ...
      if (!okType(file)) {
        alert(`"${file.name}" — that file type isn't supported.`);
        continue;
      }
      ...
      try {
        ...
        setPending(prev => prev.map(p => p.id === id ? {
          ...
          meta,
          status: 'ready',
          previewUrl: p.previewUrl || meta.previewUrl || ''
        } : p));
      } catch (err) {
        setPending(prev => prev.map(p => p.id === id ? {
          ...p,
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      setRecError('Microphone access was blocked. Enable it in your browser settings to send a voice message.');
      return;
    }
    try {
      ...
      const mr = mime ? new MediaRecorder(stream, {
        mimeType: mime
  ...
  const stopAndSend = () => {
    ...
    mr.onstop = async () => {
      ...
      if (typeof URL !== 'undefined' && URL.createObjectURL) {
        ...
        voiceUrlsRef.current.push(previewUrl);
      }
      ...
      try {
        ...
        if (inThread) await doSend(transcript, threadRef.current.id, () => {}, () => {}, staged);else await doSend(transcript, null, () => {}, () => {}, staged);
      } catch (e) {
  ...
  useEffect(() => () => {
    ...
    stopTracks();
    voiceUrlsRef.current.forEach(u => {
      try {
  ...
  const onComposerChangeFor = (target, setVal) => e => {
    ...
    const sym = mm[2],
      qy = mm[3].toLowerCase(),
      tokenStart = pos - (mm[3].length + 1);
    let items = [];
    if (sym === '@') {
      if (scope.kind === 'channel' && 'channel'.startsWith(qy)) items.push({
        label: '@channel',
        sub: 'Notify the channel',
        insert: '@channel'
      });
      if ('here'.startsWith(qy)) items.push({
        label: '@here',
        sub: 'Notify those active',
        insert: '@here'
      });
      (roster || []).filter(p => p.id !== me).forEach(p => {
        const nm = nameOf(p.id);
        if (!qy || nm.toLowerCase().includes(qy)) items.push({
          label: nm,
          photo: p.avatar_url,
          insert: '@' + (p.first_name || nm).replace(/\s+/g, '')
        });
      });
    } else {
      (channels || []).forEach(c => {
        if (!qy || c.name.toLowerCase().includes(qy)) items.push({
          label: '#' + c.name,
          insert: '#' + c.name
        });
      });
  ...
  const pickMention = item => {
    ...
    setTimeout(() => {
      if (ta) {
        ta.focus();
        const c = menu.tokenStart + item.insert.length + 1;
        try {
  ...
  const toggleReaction = (mm, emoji) => {
    ...
    const upd = l => l.map(x => x.id !== mm.id ? x : {
      ...x,
      reactions: mine ? (x.reactions || []).filter(r => !(r.emoji === emoji && r.user_id === me)) : [...(x.reactions || []), {
        emoji,
        user_id: me
      }]
    });
  ...
  const saveEdit = async () => {
    ...
    setEditId(null);
    try {
      await ChatDB.editMessage(id, body);
    } catch (e) {
  ...
  const openThread = mm => {
    setActionFor(null);
    if (threadRef.current && threadRef.current.id === mm.id) {
      setThread(null);
      return;
    }
    setThread(mm);
  ...
  const groupRx = rx => {
    ...
    (rx || []).forEach(r => {
      ...
      g[r.emoji].count++;
      if (r.user_id === me) g[r.emoji].mine = true;
    });
    return g;
  };
  ...
  const renderRow = (mm, i, list, inThread) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: mm.id,
      id: inThread ? undefined : 'chat-row-' + mm.id,
      onMouseEnter: () => setHoverId(mm.id),
      onMouseLeave: () => setHoverId(h => h === mm.id ? null : h),
      style: {
        display: 'flex',
        flexDirection: 'column',
        gap: 2,
        borderRadius: threadOpen ? 10 : 0,
        background: threadOpen ? dark ? 'rgba(173,131,47,0.07)' : 'rgba(173,131,47,0.06)' : 'transparent',
        padding: threadOpen ? '4px 4px 2px' : 0,
        margin: threadOpen ? '0 -4px' : 0
      }
    ...
    }, nameOf(mm.author_id)), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
      ...
      photo: photoOf(mm.author_id),
      name: nameOf(mm.author_id),
      size: 22
    }) : /*#__PURE__*/React.createElement("div", {
      style: {
        width: 1
    ...
    }, "Save"))) : hasAudioAtt && !(mm.content || '').trim() ?
    /*#__PURE__*/
    ...
    React.createElement("div", {
      ...
      onClick: () => setActionFor(actionFor === mm.id ? null : mm.id),
      style: {
        maxWidth: '78%',
        padding: '7px 10px',
        fontFamily: J,
        ...
        opacity: mm._pending ? 0.6 : 1
      }
    }, hasAudioAtt && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 4,
    ...
    }), "AI transcript"), renderMessageContent(mm.content, ctxFor(mine)), mm.edited_at && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 8,
        opacity: 0.6,
        marginLeft: 5
      }
    }, "(edited)")), canReact && /*#__PURE__*/React.createElement("button", {
      type: "button",
      "aria-label": "React",
      onClick: e => {
        e.stopPropagation();
        setActionFor(actionFor === mm.id ? null : mm.id);
      },
      style: {
        width: 22,
        height: 22,
      ...
      className: "ti ti-mood-plus",
      style: {
        fontSize: 13
      }
    }))), (mm.attachments || []).length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        flexWrap: 'wrap',
        gap: 6,
    ...
    }, (mm.attachments || []).map(att => isAudioMime(att.mime_type) ? /*#__PURE__*/React.createElement(AttachmentAudio, {
      key: att.id || att.storage_path,
      att: att,
      dark: dark
    }) : isImageMime(att.mime_type) ? /*#__PURE__*/React.createElement(AttachmentImage, {
      key: att.id || att.storage_path,
      att: att,
      dark: dark
    }) : /*#__PURE__*/React.createElement("button", {
      ...
      type: "button",
      onClick: async () => {
        const u = att.previewUrl || (await ChatDB.signedUrl(att.storage_path));
        if (u) window.open(u, '_blank', 'noopener,noreferrer');
      },
      style: {
        display: 'inline-flex',
    ...
    }, att.file_name || 'File'), /*#__PURE__*/React.createElement("span", {
      style: {
        fontFamily: J,
        fontSize: 8,
        color: subCol
      }
    }, humanFileSize(att.file_size)))))), Object.keys(rg).length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        flexWrap: 'wrap',
        gap: 4,
    ...
    }, Object.keys(rg).map(em => /*#__PURE__*/React.createElement("button", {
      key: em,
      onClick: () => toggleReaction(mm, em),
      style: {
        display: 'inline-flex',
        alignItems: 'center',
        gap: 3,
        ...
        fontSize: 10,
        cursor: 'pointer',
        background: rg[em].mine ? reactBg : 'transparent',
        border: `1px solid ${rg[em].mine ? reactMineBd : reactBord}`,
        color: dark ? '#fff' : '#001A4A'
      }
    ...
    }, rg[em].count)))), !inThread && mm.reply_count > 0 && /*#__PURE__*/React.createElement("button", {
      onClick: () => openThread(mm),
      style: {
        alignSelf: mine ? 'flex-end' : 'flex-start',
        marginLeft: mine ? 0 : 28,
        marginTop: 2,
    ...
    }), mm.reply_count, " ", mm.reply_count === 1 ? 'reply' : 'replies'), actionFor === mm.id && !editing && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 2,
    ...
    }, CHAT_EMOJIS.map(em => /*#__PURE__*/React.createElement("button", {
      key: em,
      onClick: () => toggleReaction(mm, em),
      style: {
        background: 'none',
    ...
    }, em)), !inThread && /*#__PURE__*/React.createElement("button", {
      onClick: () => openThread(mm),
      ...
      style: {
        ...
        fontFamily: J,
        fontSize: 7,
        color: mm._failed ? '#C0392B' : timeCol,
        justifyContent: mine ? 'flex-end' : 'flex-start',
        marginLeft: mine ? 0 : 28,
        marginRight: mine ? 4 : 0
      }
    }, /*#__PURE__*/React.createElement("span", null, mm._failed ? 'Failed to send' : mm._pending ? 'Sending…' : fmtTime(mm.created_at)), mine && !mm._failed && (() => {
      const rcpt = receiptFor(mm);
      if (rcpt === 'sent') return /*#__PURE__*/React.createElement("i", {
      ...
      if (rcpt === 'seen') return /*#__PURE__*/React.createElement("i", {
        className: "ti ti-checks",
        style: {
          fontSize: 10,
          color: dark ? '#2FBF6B' : '#1E6B40'
        },
        "aria-label": "Seen"
      });
  ...
  const renderThreadDock = () => /*#__PURE__*/React.createElement("div", {
    style: {
      marginLeft: 28,
      marginTop: 4,
      marginBottom: 2,
      background: dark ? 'rgba(173,131,47,0.05)' : 'rgba(173,131,47,0.04)',
      border: `1px solid ${reactBord}`,
      borderLeft: `2px solid ${reactMineBd}`,
      borderRadius: 10,
  ...
  }, threadMsgs.length, " ", threadMsgs.length === 1 ? 'reply' : 'replies')), /*#__PURE__*/React.createElement("div", {
    style: {
      maxHeight: 240,
      overflowY: 'auto',
  ...
  }, threadMsgs.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 10,
      color: subCol,
  ...
  }, "No replies yet \u2014 start the thread below."), threadMsgs.map((tm, i) => renderRow(tm, i, threadMsgs, true))));
  const composer = (val, onChange, onSend, busy, kRef, placeholder, autoMenu, pickerKey) => {
    ...
    const fmtBtn = (icon, before, after, ph) => /*#__PURE__*/React.createElement("button", {
      type: "button",
      onMouseDown: e => e.preventDefault(),
      onClick: () => wrapSelection(kRef, val, kRef === composerRef ? setText : setThreadText, before, after, ph),
      "aria-label": icon,
      style: {
        width: 26,
        height: 26,
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, autoMenu && menu && menu.items.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        position: 'absolute',
        left: 12,
        right: 12,
    ...
    }, menu.items.map((it, k) => /*#__PURE__*/React.createElement("button", {
      key: k,
      ...
      style: {
        ...
        background: 'none',
        border: 'none',
        borderBottom: k < menu.items.length - 1 ? `1px solid ${dark ? '#152545' : '#F0EBE3'}` : 'none',
        cursor: 'pointer',
        textAlign: 'left'
      }
    }, it.photo !== undefined ? /*#__PURE__*/React.createElement(Avatar, {
      photo: it.photo,
      name: it.label,
      size: 22
    }) : /*#__PURE__*/React.createElement("i", {
    ...
    }, it.sub)))), emojiPicker === pickerKey && /*#__PURE__*/React.createElement("div", {
      style: {
        position: 'absolute',
        left: 12,
        right: 12,
    ...
    }, COMPOSER_EMOJIS.map((em, k) => /*#__PURE__*/React.createElement("button", {
      key: k,
      type: "button",
      onMouseDown: e => e.preventDefault(),
      onClick: () => insertEmoji(em, kRef, setterFor, val),
    ...
    }, fmtBtn('ti-bold', '**', '**', 'bold'), fmtBtn('ti-italic', '_', '_', 'italic'), fmtBtn('ti-code', '`', '`', 'code'), /*#__PURE__*/React.createElement("button", {
      type: "button",
      onMouseDown: e => e.preventDefault(),
      onClick: () => wrapSelection(kRef, val, setterFor, '[', '](url)', 'text'),
      "aria-label": "ti-link",
      style: {
        width: 26,
        height: 26,
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          ...
          alignItems: 'center',
          gap: 6,
          padding: img ? 0 : '6px 8px',
          paddingRight: img ? 0 : 22,
          borderRadius: 9,
          background: img ? 'transparent' : dark ? '#0A1730' : '#fff',
          border: `1px solid ${errored ? '#C0392B' : reactBord}`,
          maxWidth: 200,
      ...
      }, p.previewUrl ? /*#__PURE__*/React.createElement("img", {
        src: p.previewUrl,
        alt: p.file.name,
        style: {
          width: '100%',
        ...
        className: `ti ${errored ? 'ti-alert-triangle' : uploading ? 'ti-loader-2' : 'ti-file'}`,
        style: {
          fontSize: 16,
          color: errored ? '#C0392B' : senderGold,
          flexShrink: 0,
          animation: uploading ? 'tmg-spin 0.8s linear infinite' : 'none'
        }
      }), /*#__PURE__*/React.createElement("span", {
        style: {
          fontFamily: J,
      ...
      }, p.file ? p.file.name : 'File')), /*#__PURE__*/React.createElement("button", {
        type: "button",
        ...
        style: {
          ...
          top: img ? -6 : '50%',
          right: -6,
          transform: img ? 'none' : 'translateY(-50%)',
          width: 17,
          height: 17,
          borderRadius: '50%',
          border: 'none',
          cursor: 'pointer',
          background: dark ? '#1A2A45' : '#001A4A',
          color: '#fff',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
    ...
    })), recError && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 6,
        ...
        padding: '6px 9px',
        borderRadius: 9,
        background: dark ? 'rgba(192,57,43,0.14)' : '#FBEAE7',
        border: `1px solid ${dark ? 'rgba(192,57,43,0.4)' : '#E8C4BE'}`,
        color: dark ? '#F0A99F' : '#A2342A',
        fontFamily: J,
        fontSize: 9.5
      }
    }, /*#__PURE__*/React.createElement("i", {
      className: "ti ti-microphone-off",
      style: {
        fontSize: 13,
        flexShrink: 0
      }
    ...
    }, recError), /*#__PURE__*/React.createElement("button", {
      type: "button",
      onClick: () => setRecError(''),
      "aria-label": "Dismiss",
      style: {
    ...
    React.createElement("div", {
    ...
    }, rec === 'processing' ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("i", {
      className: "ti ti-loader-2",
      style: {
        fontSize: 16,
        color: senderGold,
    ...
    }, "Transcribing\u2026")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
      style: {
        width: 9,
        height: 9,
        borderRadius: '50%',
        background: '#C0392B',
        flexShrink: 0,
        animation: 'tmg-pulse-ring 1.4s ease infinite'
      }
    }), /*#__PURE__*/React.createElement("span", {
      style: {
        flex: 1,
    ...
    }, fmtRecTime(recSecs)), /*#__PURE__*/React.createElement("button", {
      type: "button",
      ...
      style: {
        ...
        flexShrink: 0,
        background: 'transparent',
        color: dark ? 'rgba(255,255,255,0.55)' : '#8A8676',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center'
      }
    ...
    }, autoMenu && /*#__PURE__*/React.createElement("button", {
      type: "button",
      disabled: true,
      "aria-label": "Screen recording (coming soon)",
      title: "Screen recording \u2014 coming soon",
      style: {
        width: 32,
        height: 32,
        borderRadius: '50%',
        ...
        flexShrink: 0,
        cursor: 'not-allowed',
        color: dark ? 'rgba(255,255,255,0.30)' : '#B4B2A9',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center'
      }
      ...
      onClick: () => {
        setMenu(null);
        setEmojiPicker(p => p === pickerKey ? null : pickerKey);
      },
      "aria-label": "Emoji",
      style: {
        width: 32,
        ...
        flexShrink: 0,
        cursor: 'pointer',
        color: emojiPicker === pickerKey ? senderGold : iconCol,
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center'
      }
      ...
      if (!canSend && canRecord) return /*#__PURE__*/React.createElement("button", {
        type: "button",
      ...
      return /*#__PURE__*/React.createElement("button", {
        ...
        style: {
          ...
          flexShrink: 0,
          background: canSend ? sendBg : dark ? '#1A2A45' : '#C9CDD6',
          color: '#fff',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center'
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("input", {
    ...
    const iconCircleBg = dark ? '#0A1730' : '#F5F2EE',
      iconCircleCol = dark ? '#C9A45A' : '#AD832F';
    ...
    let subLine = null,
      subTint = subCol;
    if (peerId) {
      if (hasStatus) subLine = `${ps.status_emoji || ''} ${ps.status_text || ''}`.trim();else {
        const away = awayOf(peerId);
        subLine = away ? 'Away' : 'Active';
        if (away) subTint = awayCol;
      }
    ...
    return /*#__PURE__*/React.createElement("div", {
      style: {
        ...
        gap: 9,
        padding: '10px 14px',
        borderBottom: `1px solid ${barBord}`,
        background: barBg,
        flexShrink: 0
      }
    ...
    })), peerId ? /*#__PURE__*/React.createElement("div", {
      style: {
        position: 'relative',
        flexShrink: 0
      }
    }, /*#__PURE__*/React.createElement(Avatar, {
      photo: photoOf(peerId),
      name: nameOf(peerId),
      size: 28
    }), /*#__PURE__*/React.createElement("span", {
      style: {
        ...
        height: 9,
        borderRadius: '50%',
        background: awayOf(peerId) ? '#E0A93B' : '#2FBF6B',
      ...
      className: `ti ${scope.kind === 'channel' ? scope.channel && scope.channel.type === 'private' ? 'ti-lock' : 'ti-hash' : 'ti-users'}`,
      style: {
        fontSize: 14
      }
    })), /*#__PURE__*/React.createElement("div", {
    ...
    }, scope.title), subLine ? /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: J,
        fontSize: 8,
        color: subTint,
    ...
    }, subLine) : null), /*#__PURE__*/React.createElement("button", {
      "aria-label": "Tasks",
      style: {
        background: 'none',
        border: 'none',
  ...
  }, "Loading\u2026"), !loading && messages.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 11,
      color: subCol,
  ...
  }, "No messages yet. Say hello \uD83D\uDC4B"), messages.map((mm, i) => /*#__PURE__*/React.createElement(React.Fragment, {
    key: mm.id
  }, renderRow(mm, i, messages, false), thread && thread.id === mm.id && renderThreadDock()))), thread && /*#__PURE__*/React.createElement("div", {
    style: {
      flexShrink: 0,
      display: 'flex',
      alignItems: 'center',
      gap: 8,
      padding: '6px 12px',
      background: dark ? 'rgba(173,131,47,0.08)' : '#F3EBDA',
      borderTop: `1px solid ${inBarBord}`
    }
  }, /*#__PURE__*/React.createElement("i", {
    className: "ti ti-message-circle",
  ...
  }, "Replying in thread"), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setThread(null);
  ...
  }))), thread ? composer(threadText, onThreadComposerChange, sendThread, tSending, threadComposerRef, 'Reply in thread…', true, 'thread') : composer(text, onComposerChange, send, sending, composerRef, 'Message ' + scope.title, true, 'main'));
}
function ChatTab({
  dark,
  profile,
  ...
  const bg = dark ? '#000D26' : '#FCFBF8',
    headTitle = dark ? '#FFFFFF' : '#001A4A';
  const composeBg = dark ? 'rgba(173,131,47,0.15)' : '#F3EBDA',
    composeCol = dark ? '#C9A45A' : '#AD832F';
  const searchBg = dark ? '#0A1730' : '#F5F2EE',
    searchCol = dark ? 'rgba(255,255,255,0.4)' : '#8A8676';
  const rowBord = dark ? '#0D1E3A' : '#F5F2EE',
    sectionCol = dark ? '#C9A45A' : '#AD832F';
  const nameCol = dark ? '#FFFFFF' : '#001A4A',
    previewCol = dark ? 'rgba(255,255,255,0.35)' : '#888888';
  ...
  const refresh = () => {
    setLoadingList(true);
    Promise.all([ChatDB.loadRoster(), ChatDB.listChannels(), ChatDB.listConversations()]).then(([r, ch, cv]) => {
      setRoster(r || []);
      setChannels(ch || []);
      setConvs(cv || []);
  ...
  const openChannel = async ch => {
    if (ch.type === 'public') {
      try {
        await ChatDB.joinChannel(ch.id);
      } catch (e) {}
    }
    setScope({
      kind: 'channel',
  ...
  const publicCh = fch.filter(c => c.type === 'public'),
    privateCh = fch.filter(c => c.type === 'private');
  ...
  const UnreadDot = ({
    ...
    return /*#__PURE__*/React.createElement("div", {
      style: {
        ...
        justifyContent: 'center',
        padding: '0 4px',
        boxShadow: u.mentions ? '0 0 0 1.5px ' + bg + ', 0 0 0 3px #AD832F' : 'none'
      }
  ...
  const ChannelRow = c => /*#__PURE__*/React.createElement("div", {
    key: c.id,
    ...
    style: {
      ...
      background: isSel(c.id) ? selBg : 'transparent'
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      width: 34,
    ...
    className: `ti ${c.type === 'private' ? 'ti-lock' : 'ti-hash'}`,
    style: {
      fontSize: 16
    }
  })), /*#__PURE__*/React.createElement("div", {
  ...
  }, c.name), c.topic ? /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 9,
      color: previewCol,
  ...
  }, c.topic) : null), /*#__PURE__*/React.createElement(UnreadDot, {
    id: c.id
  }));
  const ConvRow = cv => {
    const others = cv.memberIds.filter(id => id !== me);
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        cursor: 'pointer',
        borderBottom: `1px solid ${rowBord}`,
        background: isSel(cv.id) ? selBg : 'transparent'
      }
      ...
      photo: photoOf(others[0]),
      name: nameOf(others[0]),
      size: 34
    }), !awayOf(others[0]) && /*#__PURE__*/React.createElement("span", {
      style: {
        position: 'absolute',
        bottom: 0,
        right: 0,
    ...
    }, convTitle(cv)), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: J,
        fontSize: 9,
        color: previewCol,
    ...
    }, cv.type === 'group_dm' ? cv.memberIds.length + ' members' : 'Direct message')), /*#__PURE__*/React.createElement(UnreadDot, {
      id: cv.id
    }));
  };
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      zIndex: 40,
      background: dark ? '#0A1730' : '#fff',
      border: `1px solid ${dark ? '#152545' : '#E4DFD4'}`,
      borderRadius: 12,
    ...
    style: {
      ...
      background: 'none',
      border: 'none',
      borderTop: `1px solid ${dark ? '#152545' : '#F0EBE3'}`,
      cursor: 'pointer',
      fontFamily: J,
      fontSize: 12,
      color: dark ? '#fff' : '#001A4A',
  ...
  }, loadingList && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 10,
      color: searchCol,
  ...
  }, "Loading\u2026"), nothing && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 11,
      color: previewCol,
  ...
  }, "No conversations yet.", /*#__PURE__*/React.createElement("br", null), "Tap ", /*#__PURE__*/React.createElement("i", {
    className: "ti ti-edit",
    style: {
      fontSize: 12
    }
  }), " to start a message or create a channel."), publicCh.length > 0 && /*#__PURE__*/React.createElement(Section, {
    label: "CHANNELS"
  }), publicCh.map(ChannelRow), privateCh.length > 0 && /*#__PURE__*/React.createElement(Section, {
    label: "PRIVATE CHANNELS"
  }), privateCh.map(ChannelRow), fcv.length > 0 && /*#__PURE__*/React.createElement(Section, {
    label: "DIRECT MESSAGES"
  }), fcv.map(ConvRow), /*#__PURE__*/React.createElement("div", {
    style: {
      height: 12
    }
  }))), /*#__PURE__*/React.createElement("div", {
    style: wide ? {
      ...
      flexDirection: 'column',
      background: dark ? '#070F1E' : '#FCFBF8',
      transform: scope ? 'translateX(0)' : 'translateX(100%)',
      transition: 'transform 220ms ease',
      boxShadow: scope && !dark ? '-10px 0 28px rgba(0,0,0,0.12)' : 'none',
      pointerEvents: scope ? 'auto' : 'none',
      overflow: 'hidden'
    }
  }, scope ? /*#__PURE__*/React.createElement(ChatConversation, {
    key: scope.id,
    dark: dark,
    scope: scope,
    me: me,
  ...
  }, "Select a conversation")) : null)), modal === 'newchannel' && /*#__PURE__*/React.createElement(NewChannelModal, {
    dark: dark,
    isAdmin: isAdmin,
  ...
  }), modal === 'newdm' && /*#__PURE__*/React.createElement(NewDMModal, {
    dark: dark,
    roster: roster,
    me: me,
    onClose: () => setModal(null),
...
(function mount() {
  ...
  if (window.TMG_DEV) {
    ...
    }, /*#__PURE__*/React.createElement(ChatTab, {
      dark: dark,
      profile: devProfile,
      chatUnread: 0,
      onRead: () => {}
