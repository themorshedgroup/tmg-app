...
- Resolve relative dates using today's date given below. Do NOT use markdown formatting.`;
    ...
    const NEW_CHAT_PILL = { id: 'new-chat', label: 'New Chat', icon: 'ti-message-plus', colorLight: '#001A4A', colorDark: 'rgba(255,255,255,0.6)', type: 'newchat', config: {} };
        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
          ...
          {allPills.length > 0 && (
            <div style={{
              ...
              <button onClick={() => setShowActions(true)} title="More actions" style={{
                flexShrink: 0, width: 30, height: 30, borderRadius: '50%', cursor: 'pointer',
                border: `0.5px solid ${AI_UI.border}`, background: AI_UI.tint,
                display: 'flex', alignItems: 'center', justifyContent: 'center', color: AI_UI.primary,
              }}>
          ...
          {/* All quick actions — bottom sheet from the ⋯ button, bounded to the chat message area. */}
          {!empty && showActions && (
            <React.Fragment>
              <div onClick={() => setShowActions(false)} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: AI_COMPACT_ROW_H, zIndex: 24, background: dark ? 'rgba(0,0,0,0.28)' : 'rgba(0,13,38,0.12)' }} />
              <div style={{ position: 'absolute', left: 0, right: 0, bottom: AI_COMPACT_ROW_H, zIndex: 25, maxHeight: '60%', overflowY: 'auto', background: '#F5F0E8', borderRadius: '16px 16px 0 0', boxShadow: '0 -8px 28px rgba(0,0,0,0.18)', padding: '8px 8px calc(8px + env(safe-area-inset-bottom))' }}>
                {allPills.map(p => {
                  const soon = p.type === 'soon';
                  return (
                    <button key={p.id} onClick={() => { setShowActions(false); onPill(p); }} style={{
                      display: 'flex', alignItems: 'center', gap: 10, width: '100%', textAlign: 'left', cursor: 'pointer',
                      padding: '12px 12px', borderRadius: 10, border: 'none', background: 'none',
                    }}>
                      <i className={`ti ${p.icon}`} style={{ fontSize: 16, color: '#1A1A1A', flexShrink: 0 }} />
                      <span style={{ flex: 1, minWidth: 0, fontFamily: "'Jost', sans-serif", fontSize: 13, fontWeight: 500, color: '#001A4A' }}>{p.label}</span>
                      {soon && (
                        <span style={{
                          flexShrink: 0, padding: '2px 7px', borderRadius: 8, background: '#1A1A1A', color: '#fff',
                          fontFamily: "'Jost', sans-serif", fontSize: 8, fontWeight: 700, letterSpacing: '0.08em',
                        }}>SOON</span>
                      )}
                    </button>
                  );
    ...
    function CallsTab({ dark, ownerName, ownerEmail, isAdmin }) {
      ...
      useEffect(() => { localStorage.setItem('tmg-coach-open', briefOpen ? 'true' : 'false'); }, [briefOpen]);
      ...
      const brief = useMemo(() => {
        ...
        const lines = [];
        const mix = {};
        list.forEach(t => { const g = callTier(t.Subject); mix[g] = (mix[g] || 0) + 1; });
        const mixTxt = ['A', 'B', 'C', 'EO'].filter(g => mix[g]).map(g => mix[g] + ' ' + g).join(' · ');
        const openList = list.filter(t => !/completed/i.test(t.Status || ''));
        if (!list.length) lines.push(who ? who + ' no calls on the board for today.' : 'No calls on the board for today.');
        else lines.push((who ? who + ' ' : '') + list.length + ' call' + (list.length === 1 ? '' : 's') + ' today' + (mixTxt ? ' — ' + mixTxt : '') + '. ' + (list.length - openList.length) + ' already logged.');
      ...
      const botMsg = (t, key) => <div key={key} style={{ alignSelf: 'flex-start', marginRight: '8%', background: dark ? '#0A1E44' : '#F3EBDA', color: dark ? '#fff' : '#001A4A', borderRadius: '8px 8px 8px 2px', padding: '6px 8px', fontFamily: J, fontSize: 8, lineHeight: 1.45 }}>{t}</div>;
        ...
        return names.map(o => {
          ...
          return (
            <div key={o}>
              ...
              {!shut && list.map((t, i) => callRow(t, i === list.length - 1))}
            </div>
          );
        });
      };
      ...
      return (
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
            ...
            {!err && buckets !== null && (team && !agent ? groupedRows() : shown.map((t, i) => callRow(t, i === shown.length - 1)))}
          </div>
          {/* Call Brief — derived from the real list, no model in the loop */}
          <div style={{ flexShrink: 0, background: coachBg, borderTop: `1px solid ${coachBord}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <i className="ti ti-sparkles" style={{ fontSize: 11, color: coachLabel }} />
                <span style={{ fontFamily: J, fontSize: 8, letterSpacing: '0.16em', fontWeight: 500, color: coachLabel, textTransform: 'uppercase' }}>Call Brief</span>
              </div>
              <button onClick={() => setBriefOpen(o => !o)} style={{ width: 22, height: 22, borderRadius: '50%', border: 'none', cursor: 'pointer', background: addBg, color: addCol, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <i className={`ti ti-chevron-${briefOpen ? 'down' : 'up'}`} style={{ fontSize: 13 }} />
              </button>
            </div>
            {briefOpen && (
              <div style={{ padding: '7px 12px 9px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                {brief.length ? brief.map((l, i) => botMsg(l, 'b' + i)) : botMsg('Loading the board…', 'b0')}
              </div>
            )}
          </div>
          </React.Fragment>
