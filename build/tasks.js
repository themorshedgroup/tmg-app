...
const OrderDB = {
  ...
  async setRank(id, col, val) {
    ...
    if (error) {
      console.error('[OrderDB] setRank:', error.message);
      throw error;
    }
  },
  ...
  async loadMyOrder(userId) {
    ...
    const {
      data
    } = await pageAll(c, 'task_user_order', 'task_id,rank', {
      eq: ['user_id', userId]
    });
    const out = {};
    (data || []).forEach(r => {
      out[r.task_id] = r.rank;
    });
    return out;
  },
  async setMyRank(userId, taskId, val) {
    ...
    const {
      error
    } = await c.from('task_user_order').upsert({
      user_id: userId,
      task_id: taskId,
      rank: val,
      updated_at: new Date().toISOString()
    ...
    if (error) {
      console.error('[OrderDB] setMyRank:', error.message);
      throw error;
    }
  },
  async setMyRanks(userId, rows) {
    const c = this.client();
    if (!c || !userId || !rows.length) return;
    const {
      error
    } = await c.from('task_user_order').upsert(rows.map(r => ({
      user_id: userId,
      task_id: r.id,
      rank: r.rank,
      updated_at: new Date().toISOString()
    ...
    if (error) {
      console.error('[OrderDB] setMyRanks:', error.message);
      throw error;
    }
  },
  ...
  async setRanks(col, rows) {
    ...
    const {
      error
    } = await c.rpc('reorder_tasks', {
      p_col: col,
      p_rows: rows
    });
    if (error) {
      console.error('[OrderDB] setRanks:', error.message);
      throw error;
    }
  },
  async loadTasklistOrder(projectId) {
    ...
    const {
      data,
      error
    } = await c.from('project_tasklist_order').select('tasklist_key,rank').eq('project_id', projectId);
    if (error) return {}; // no table yet: fall back to the old order
    const out = {};
    (data || []).forEach(r => {
      out[r.tasklist_key] = r.rank;
    });
    return out;
  },
  ...
  async setTasklistRank(projectId, key, rank, userId) {
    ...
    const {
      error
    } = await c.from('project_tasklist_order').upsert({
    ...
    if (error) {
      console.error('[OrderDB] setTasklistRank:', error.message);
      throw error;
    }
  },
  async setTasklistRanks(projectId, rows, userId) {
    ...
    const {
      error
    } = await c.from('project_tasklist_order').upsert(rows.map(r => ({
      project_id: projectId,
      tasklist_key: r.key,
      rank: r.rank,
      updated_by: userId || null,
    ...
    if (error) {
      console.error('[OrderDB] setTasklistRanks:', error.message);
      throw error;
    }
  }
};
const TaskDB = {
  ...
  async loadAll(userId) {
    ...
    const [minePpl, prRes, myOrd] = await Promise.all([pageAll(c, 'task_people', 'task_id,role', {
      eq: ['user_id', uid]
    }), pageAll(c, 'projects', 'id,name,archived'), OrderDB.loadMyOrder(uid)]);
    ...
    .map(t => myOrd[t.id] != null ? {
      ...t,
      my_rank: myOrd[t.id]
    } : t).sort(rankCmp('my_rank'));
    return {
      tasks,
      peopleByTask,
      projById,
...
function rankBetween(prev, next) {
  // neighbour ranks; null at either end
...
async function pageAll(c, table, cols, opts) {
  ...
  for (;;) {
    ...
    const {
      data,
      error
    } = await q.range(from, from + SIZE - 1);
    if (error) {
      console.error('[pageAll] ' + table + ':', error.message);
      break;
    }
    out = out.concat(data || []);
...
const ProjectDB = {
  ...
  async loadFull(recordType) {
    ...
    const [msRows, statsRes] = await Promise.all([chunked(projIds, 100, ids => pageAll(c, 'tasks', MS_COLS, {
      in: ['project_id', ids],
      eq: ['is_milestone', true]
    })), c.rpc('project_task_stats')]);
    const statsBy = {};
    (statsRes && statsRes.data || []).forEach(r => {
      statsBy[r.project_id] = r;
    });
    const tRes = {
        data: msRows
      },
    ...
    return projects.map(p => {
      ...
      const ms = (tasksByProj[p.id] || []).map(t => ({
        ...t,
        _projectName: p.name
      }));
      ...
      return {
        ...
        _openTasks: [],
        _collaborators: [],
        _taskCount: st.total || 0,
        _doneCount: st.done || 0,
        _stuck: st.stuck || 0,
        _overdue: st.overdue || 0,
        _anyDates: !!st.any_dates
      };
    });
  },
  ...
  async hydrate(p) {
    ...
    const [ppl, tlOrder] = await Promise.all([chunked(ts0.map(t => t.id), 150, ids => pageAll(c, 'task_people', 'task_id,user_id,role', {
      in: ['task_id', ids]
    })), OrderDB.loadTasklistOrder(p.id)]);
    const peopleByTask = {};
    ppl.forEach(x => {
      (peopleByTask[x.task_id] = peopleByTask[x.task_id] || []).push(x);
    });
    const dueCmp = (a, b) => {
      if (!a.due_at && !b.due_at) return 0;
      if (!a.due_at) return 1;
...
function MailboxAdminPanel({
  ...
  async function load() {
    ...
    setLoading(false);
    if (!ok) {
      setErr(data.error || 'Could not load the mailbox list.');
      return;
    }
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Mailboxes we read"), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      background: 'none',
      border: 'none',
  ...
  }, m.name || m.email), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11.5,
      color: sub,
      fontFamily: C.fontSans,
  ...
  }, m.email, m.enabled && !m.connected && /*#__PURE__*/React.createElement("span", {
    style: {
      color: '#9B6B1C'
    }
  }, " \xB7 hasn't connected Google yet"), m.enabled && m.connected && m.last_polled_at && /*#__PURE__*/React.createElement("span", null, " \xB7 last checked ", new Date(m.last_polled_at).toLocaleString('en-US', {
    month: 'short',
    day: 'numeric',
    hour: 'numeric',
    minute: '2-digit'
  })), m.enabled && m.last_error && /*#__PURE__*/React.createElement("span", {
    style: {
      color: '#9B1C1C'
    }
  }, " \xB7 ", m.last_error))), /*#__PURE__*/React.createElement("button", {
    onClick: () => toggle(m),
    ...
    style: {
      ...
      fontSize: 12,
      fontFamily: C.fontSans,
      cursor: busy === m.user_id ? 'default' : 'pointer',
      border: `1px solid ${m.enabled ? teal : bord}`,
...
function CtcEmailsTab({
  ...
  useEffect(() => {
    ...
    if (!c || !user) return;
    c.from('profiles').select('access').eq('id', user.id).maybeSingle().then(({
      data
    }) => {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "ti ti-mailbox",
    style: {
      fontSize: 13
    }
  }), "Mailboxes"), /*#__PURE__*/React.createElement("button", {
    onClick: runTriage,
    disabled: triaging,
    style: {
      padding: '7px 12px',
  ...
  }), triaging ? 'Reading…' : 'Suggest matches'))), mbOpen && /*#__PURE__*/React.createElement(MailboxAdminPanel, {
    dark: dark,
    onClose: () => {
      setMbOpen(false);
      load();
...
function ProjectsSurface({
  ...
  useEffect(() => {
    if (!orderErr) return;
    const t = setTimeout(() => setOrderErr(null), 6000);
  ...
  useEffect(() => {
    ...
    let on = true;
    zohoPortalBase().then(b => {
      if (on && b) setPortalBase(b);
    });
    return () => {
      on = false;
    };
  ...
  useEffect(() => {
    ...
    let on = true;
    ProjectDB.hydrate(current).then(full => {
      if (on && full) setCurrent(prev => prev && prev.id === full.id ? full : prev);
    });
    return () => {
      on = false;
    };
  ...
  async function refreshContainer(focusTaskId, reopen) {
    ...
    if (!proj) return;
    setCurrent(proj);
    setRows(rs => rs.map(r => r.id === proj.id ? {
      ...r,
      _milestones: proj._milestones,
      _doneMs: proj._doneMs,
      _totalMs: proj._totalMs,
      _nextMs: proj._nextMs,
      _progress: proj._progress,
      _taskCount: proj._taskCount,
      _doneCount: proj._doneCount,
      _stuck: proj._stuck,
      _overdue: proj._overdue,
      _anyDates: proj._anyDates
    } : r));
    ...
    if (reopen && focusTaskId) openTaskFull(proj._tasks.find(x => x.id === focusTaskId) || {
      id: focusTaskId
    });
    if (onChanged) onChanged();
  }
  ...
  async function openTaskFull(t) {
    ...
    setOpenTask(prev => prev && prev.id === t.id ? {
      ...(full || prev),
      _people: ppl == null ? prev._people === undefined ? null : prev._people : ppl,
      _projectName: t._projectName || prev._projectName || (current && full && full.project_id === current.id ? current.name : '')
    } : prev);
  }
  ...
  const attentionOf = p => {
    ...
    const overdue = p._hydrated ? p._openTasks.filter(t => t.due_at && new Date(t.due_at) < today0).length : p._overdue || 0;
    if (overdue) return {
      text: overdue + (overdue === 1 ? ' task overdue' : ' tasks overdue'),
      tone: 'red'
    };
    if (!p._taskCount) return {
      text: 'No tasks yet',
      tone: 'sub'
    };
  ...
  async function disconnectProject(p) {
    ...
    const {
      data: rows,
      error: qErr
    } = await c.from('tasks').select('id,parent_task_id').eq('project_id', p.id).not('zoho_task_id', 'is', null).is('created_by', null);
    if (qErr) {
      window.alert("Couldn't check which tasks came from Zoho, so nothing was changed.");
      return;
    }
    const ids = (rows || []).map(r => r.id);
    const n = ids.length;
    if (!window.confirm('Disconnect ' + (p.name || 'this file') + ' from Zoho Projects?\n\n' + (n ? 'This deletes ' + n + ' task' + (n === 1 ? '' : 's') + ' that were created in Zoho. Anything you added here in the app stays.' : 'Nothing was created in Zoho, so no tasks will be deleted.') + '\n\nThe project itself stays in Zoho — this only stops TMG syncing with it.')) return;
    setZBusy('projects');
    try {
      ...
      for (const id of order) {
        try {
          ...
          kept.push(e && e.message ? e.message : 'could not be deleted');
        }
      }
      const {
        data,
      ...
      if (!linkCleared) {
        window.alert((removed ? 'Removed ' + removed + ' task' + (removed === 1 ? '' : 's') + ', but the' : 'The') + ' link itself would not clear, so this file is still connected to Zoho. Tell Claude — this is usually a database permission.');
      } else if (kept.length) {
        window.alert('Disconnected, and removed ' + removed + ' of ' + n + ' Zoho task' + (n === 1 ? '' : 's') + '.\n\n' + kept.length + (kept.length === 1 ? ' task is' : ' tasks are') + ' still on the file — usually because ' + (kept.length === 1 ? 'it has a subtask' : 'they have subtasks') + ' of their own. Delete those first if you want them gone.');
      }
  ...
  const zohoChip = (which, {
    ...
    onClick: () => !zBusy && setZMenu(m => m === which ? null : which),
    title: alt + ' actions',
    style: {
      display: 'flex',
      alignItems: 'center',
      ...
      background: 'none',
      cursor: zBusy ? 'default' : 'pointer',
      color: '#5A6B85'
    }
  }, /*#__PURE__*/React.createElement("i", {
    className: zBusy === which ? 'ti ti-loader-2' : 'ti ti-chevron-down',
    style: {
      fontSize: 11,
      animation: zBusy === which ? 'spin 1s linear infinite' : 'none'
    }
  }))), zMenu === which && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    onClick: () => setZMenu(null),
    style: {
      position: 'fixed',
      inset: 0,
      zIndex: 55
  ...
  const paintTasks = arr => setCurrent(p => p && p._tasks ? {
    ...p,
    _tasks: arr
  } : p);
  ...
  async function moveTaskTo(id, prevRank, nextRank, nextId, unranked) {
    ...
    paintTasks((current && current._tasks || []).map(t => t.id === id ? {
      ...t,
      list_rank: rank
    } : t).slice().sort(rankCmp('list_rank')));
    try {
      await OrderDB.setRank(id, 'list_rank', rank);
    }
  ...
  async function renormaliseList(id, nextId) {
    ...
    const {
      data
    } = await c.from('tasks').select('id,list_rank').eq('project_id', current.id);
    ...
    try {
      ...
      paintTasks((current && current._tasks || []).map(t => byId[t.id] != null ? {
        ...t,
        list_rank: byId[t.id]
      } : t).slice().sort(rankCmp('list_rank')));
    } catch (e) {
  ...
  function neighboursInGroup(id, dir) {
    ...
    const prev = row(prevId),
      next = row(nextId);
    const unranked = !!prevId && (!prev || prev.list_rank == null) || !!nextId && (!next || next.list_rank == null);
    return {
      prev: prev ? prev.list_rank : null,
      next: next ? next.list_rank : null,
      nextId,
      unranked
    };
  }
  const moveTaskBy = (id, dir) => {
    const n = neighboursInGroup(id, dir);
    if (n) moveTaskTo(id, n.prev, n.next, n.nextId, n.unranked);
  };
  ...
  const rowDrag = useRowDrag((id, o) => {
    ...
    const rows = Array.from(o.el.parentElement.querySelectorAll(':scope > [data-drag-kind="task"]')).map(el => el.getAttribute('data-drag-id')).filter(x => x !== id);
    ...
    const prev = prevId ? byId(prevId) : null,
      next = nextId ? byId(nextId) : null;
    const unranked = !!prevId && (!prev || prev.list_rank == null) || !!nextId && (!next || next.list_rank == null);
    moveTaskTo(id, prev ? prev.list_rank : null, next ? next.list_rank : null, nextId, unranked);
  }, {
    sameParent: true,
    onReject: () => setOrderErr('A task can only move inside its own list.')
  });
  const tlDrop = async (key, o) => {
    if (!current || key === o.id) return;
    const keys = Array.from(document.querySelectorAll('[data-drag-kind="tasklist"]')).map(el => el.getAttribute('data-drag-id')).filter(k => k !== key);
    ...
    if (!known) {
      ...
      try {
        await OrderDB.setTasklistRanks(current.id, rows, user && user.id);
      } catch (e) {
        setCurrent(p => p ? {
          ...p,
          _tlOrder: tlo
    ...
    try {
      await OrderDB.setTasklistRank(current.id, key, rank, user && user.id);
    } catch (e) {
      setCurrent(p => p ? {
        ...p,
        _tlOrder: tlo
  ...
  const moveTasklistBy = (key, dir) => {
    ...
    tlDrop(key, {
      id: target,
      before: dir < 0,
      el: document.querySelector('[data-drag-kind="tasklist"][data-drag-id="' + target + '"]')
    });
  };
  ...
  const detailFields = p => {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, fld('Owner', p.owner_id ? /*#__PURE__*/React.createElement(React.Fragment, null, avatar(p.owner_id, 16), nameOf(p.owner_id)) : '—'), isCtc && fld('Agent', p.agent_id ? /*#__PURE__*/React.createElement(React.Fragment, null, avatar(p.agent_id, 16), nameOf(p.agent_id)) : '—'), isCtc && fld('Zoho deal', p.zoho_deal_id ? /*#__PURE__*/React.createElement("a", {
      href: zohoRecordUrl('Deals', p.zoho_deal_id) || undefined,
      target: "_blank",
      rel: "noopener noreferrer",
    ...
    }, p.zoho_deal_name || 'Linked', /*#__PURE__*/React.createElement("i", {
    ...
    }, avatar(id, 18)))) : '—'), zohoSyncable && p.zoho_project_id && /*#__PURE__*/React.createElement(React.Fragment, null, fld('Zoho Project', (() => {
      const u = zohoProjectUrl(portalBase, p.zoho_project_id);
      return u ? /*#__PURE__*/React.createElement("a", {
        href: u,
        target: "_blank",
        rel: "noopener noreferrer",
        title: "Open this project in Zoho Projects",
        style: {
          display: 'flex',
          alignItems: 'center',
          gap: 5,
      ...
      }, p.zoho_sync_enabled ? 'Linked' : 'Linked · paused', /*#__PURE__*/React.createElement("i", {
        className: "ti ti-external-link",
        style: {
          fontSize: 12
        }
      ...
      }), p.zoho_sync_enabled ? 'Linked' : 'Linked · paused');
    })()), fld('Last synced', /*#__PURE__*/React.createElement("span", {
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 8
  ...
  const taskDetailOverlay = () => /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    onDelete: () => {
      ...
      setCurrent(p => p && p._tasks ? {
        ...p,
        _tasks: p._tasks.filter(t => t.id !== id),
        _milestones: p._milestones.filter(m => m.id !== id)
      } : p);
      setOpenTask(null);
      setTaskEditing(false);
  ...
  const applyRow = n => setCurrent(p => p && p._tasks ? {
    ...p,
    _tasks: p._tasks.map(t => t.id === n.id ? {
      ...t,
      ...n,
      _people: t._people,
  ...
  useEffect(() => {
    ...
    let ch;
    try {
      ch = c.channel('proj-order-' + current.id).on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'tasks',
        ...
        if (!n || !n.id) return;
        if (rowDrag.active.current) {
          heldRows.current.push(n);
          return;
        }
        applyRow(n);
      }).on('postgres_changes', {
  ...
  refreshRef.current = () => {
    if (current && current.id && !rowDrag.active.current) refreshContainer(null, false);
  };
  useEffect(() => {
    let hiddenAt = 0;
    const onVis = () => {
  ...
  const orderErrBar = orderErr ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8,
  ...
  const listTab = p => {
    ...
    }, orderErrBar, !p._hydrated ? /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '0.8rem',
        color: sub,
        fontFamily: C.fontSans
      }
    }, "Loading tasks\u2026") : p._tasks.length === 0 ? /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '0.8rem',
        color: sub,
        fontFamily: C.fontSans
  ...
  const detailView = () => {
    ...
    }), !addOpen && /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        resetPartyForm();
        setAddOpen(true);
      },
    ...
    }, partiesErr), parties === null ? /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 13,
        color: sub,
        fontFamily: C.fontSans,
    ...
    const header = /*#__PURE__*/React.createElement("div", {
      ...
      items: /*#__PURE__*/React.createElement(React.Fragment, null, ddItem('Open in Zoho Projects', false, () => {
        const u = zohoProjectUrl(portalBase, p.zoho_project_id);
        setZMenu(null);
        if (u) window.open(u, '_blank', 'noopener');
      }, 'open'), ddItem(syncingNow ? 'Syncing…' : 'Sync now', false, () => {
        setZMenu(null);
        if (!syncingNow) syncNow(p);
      }, 'sync'), ddItem(p.zoho_sync_enabled ? 'Pause syncing' : 'Resume syncing', false, async () => {
        setZMenu(null);
        const next = !p.zoho_sync_enabled;
        await ProjectDB.update(p.id, {
          zoho_sync_enabled: next
        });
      ...
      }, 'pause'), ddItem('Change project…', false, () => {
        setZMenu(null);
        setZohoLinkOpen(true);
      }, 'change'), ddItem('Disconnect', false, () => disconnectProject(p), 'off'))
    }), !p.zoho_sync_enabled && /*#__PURE__*/React.createElement("span", {
      title: "Linked, but syncing is paused",
      style: {
        width: 7,
        height: 7,
        borderRadius: '50%',
    ...
    }), "Link to Zoho Projects")), isCtc && zohoChip('deal', {
      logo: 'zoho-crm.svg',
      alt: 'Zoho CRM',
      ...
      items: p.zoho_deal_id ? /*#__PURE__*/React.createElement(React.Fragment, null, ddItem('Open in Zoho CRM', false, () => {
        const u = zohoRecordUrl('Deals', p.zoho_deal_id);
        setZMenu(null);
        if (u) window.open(u, '_blank', 'noopener');
      }, 'open'), ddItem('Change deal…', false, () => {
        setZMenu(null);
        setPview('form');
      }, 'change'), ddItem('Disconnect', false, () => disconnectDeal(p), 'off')) : ddItem('Connect a deal…', false, () => {
        setZMenu(null);
        setPview('form');
      }, 'connect')
    }), /*#__PURE__*/React.createElement("button", {
  ...
  const start = (e, kind, id) => {
    ...
    const tick = () => {
      ...
      if (c.pane) {
        ...
        const dTop = c.y - r.top,
          dBot = r.bottom - c.y;
        const d = dTop < 64 ? -(1 + (64 - Math.max(dTop, 0)) / 64 * 14) : dBot < 64 ? 1 + (64 - Math.max(dBot, 0)) / 64 * 14 : 0;
    ...
    const end = ok => {
      ...
      if (ok && o && o.id !== id) cb.current.onDrop(id, o);else if (ok && s.current.reject && cb.current.onReject) cb.current.onReject();
      setDrag(null);
      setOver(null);
    ...
    const up = ev => {
      if (ev.pointerId === pid) end(true);
    };
    const cancel = ev => {
      if (ev.pointerId === pid) end(false);
    }; // an incoming call on iOS fires this
    const esc = ev => {
...
function dragGrip(rd, kind, id, opts) {
  const o = opts || {};
  if (o.spacer) return /*#__PURE__*/React.createElement("span", {
    key: "g",
    style: {
      width: 14,
      flexShrink: 0
  ...
  return /*#__PURE__*/React.createElement("i", {
    ...
    onPointerDown: e => {
      if (o.dim) {
        e.preventDefault();
        e.stopPropagation();
        if (o.onPromote) o.onPromote();
      } else rd.start(e, kind, id);
    },
    onClick: e => e.stopPropagation(),
    ...
    style: {
      ...
      fontSize: 14,
      color: o.color,
      opacity: o.dim ? 0.3 : 0.5,
      cursor: o.dim ? 'pointer' : 'grab',
      touchAction: 'none',
      padding: '9px 0',
      margin: '-9px 0'
    }
...
function TasksScreen({
  ...
  const patchRank = (id, r) => setData(d => ({
    ...d,
  ...
  async function moveMyTaskTo(id, prevRank, nextRank, nextId, unranked) {
    ...
    patchRank(id, rank);
    try {
      await OrderDB.setMyRank(user && user.id, id, rank);
    } catch (e) {
      patchRank(id, was);
      setOrderNote({
        text: 'Could not save the new order — it has been put back.',
        bad: true
      });
    }
  }
  ...
  async function renormaliseMy(id, nextId) {
    ...
    try {
      ...
      setData(d => ({
        ...d,
        tasks: (d.tasks || []).map(t => byId[t.id] != null ? {
          ...t,
          my_rank: byId[t.id]
        } : t)
      }));
  ...
  function myNeighbours(id, dir) {
    ...
    if (!el || !el.parentElement) return {
      err: 'nodom'
    }; // Board and Timeline draw no handles
    ...
    return {
      prev: prev ? prev.my_rank : null,
      next: next ? next.my_rank : null,
      nextId,
      unranked
    };
  }
  const moveMyTaskBy = (id, dir) => {
    if (!manualOn) {
      setThenBy('manual');
      setOrderNote({
        text: 'Sorted manually — now use Move up / Move down, or drag the handle.'
      });
      return;
    }
    ...
    if (n.err === 'nodom') {
      setOrderNote({
        text: 'Move up and Move down only work in the List layout.'
      });
      return;
    }
  ...
  const myDrag = useRowDrag((id, o) => {
    ...
    const rows = Array.from(o.el.parentElement.querySelectorAll(':scope > [data-drag-kind="mytask"]')).map(el => el.getAttribute('data-drag-id')).filter(x => x !== id);
    const i = rows.indexOf(o.id);
    const prevId = o.before ? rows[i - 1] : rows[i];
    const nextId = o.before ? rows[i] : rows[i + 1];
    ...
    sameParent: true,
    onReject: () => setOrderNote({
      text: 'A task can only move inside its own group.'
    })
  });
  ...
  const myGrip = (t, isChild) => dragGrip(myDrag, 'mytask', t.id, {
    ...
    onPromote: () => {
      setThenBy('manual');
      setOrderNote({
        text: 'Sorted manually. Drag the handle to rearrange.'
      });
    },
  ...
  useEffect(() => {
    if (!orderNote) return;
    const t = setTimeout(() => setOrderNote(null), 6000);
    return () => clearTimeout(t);
  }, [orderNote]);
  ...
  const withMetaFrom = (t, d) => {
    const ppl = (d.peopleByTask[t.id] !== undefined ? d.peopleByTask[t.id] : extraPeople.id === t.id ? extraPeople.people : undefined) || [];
    return {
      ...enrich(t, d.projById),
  ...
  const thenBySelect = /*#__PURE__*/React.createElement("select", {
    ...
    value: "manual"
  }, "\u21B3 Manual (drag)"), /*#__PURE__*/React.createElement("option", {
    value: "due"
  }, "\u21B3 Due date"), /*#__PURE__*/React.createElement("option", {
    value: "priority"
  ...
  const routeNoticeBar = (routeNotice || bulkNote || orderNote) && /*#__PURE__*/React.createElement(React.Fragment, null, routeNotice && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 10,
  ...
  })), orderNote && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 10,
    ...
    className: 'ti ' + (orderNote.bad ? 'ti-alert-circle' : 'ti-arrows-sort'),
    style: {
      fontSize: 15,
      flexShrink: 0
    }
  ...
  const detailPane = view === 'form' ? current && peopleFor(current.id) === undefined && peopleTried !== current.id
  ...
  const taskRowNew = (t, isChild) => {
    ...
    }, myGrip(t, isChild)), /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 1
      }
  ...
  const deskRow = (t, isChild) => {
    ...
    }), myGrip(t, isChild), doneCheck(t, 17), milestoneFlag(t, 14), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1,
        minWidth: 0,
        display: 'flex',
  ...
  function renderGroupedList(rowFn) {
    ...
    const dragOffNote = dragOff ? /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '0.68rem',
        color: sub,
        fontFamily: C.fontSans,
    ...
    }, sortBy === 'due' ? 'Dragging is off while grouped by Due date — the list is in date order, so there is nowhere for a hand-set position to sit.' : 'Dragging is off while grouped by ' + (sortBy === 'label' ? 'Label' : 'Assignee') + ' — a task can appear in more than one group at a time.') : null;
    let body;
    if (sortBy === 'none') {
  ...
  }, (routeNotice || bulkNote || orderNote) && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '10px 14px 0'
    }
  }, routeNoticeBar), loading ? /*#__PURE__*/React.createElement("div", {
  ...
  }, !dragOff && viewMode === 'list' && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      moveMyTaskBy(ctxMenu.task.id, -1);
      setCtxMenu(null);
    },
    style: {
      width: '100%',
  ...
  }), "Move up"), !dragOff && viewMode === 'list' && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      moveMyTaskBy(ctxMenu.task.id, 1);
      setCtxMenu(null);
    },
...
function CalendarPopover({
  ...
  const loadGtsState = async () => {
    ...
    const {
      data
    } = await c.from('google_tasks_sync_state').select('*').eq('user_id', user.id).maybeSingle();
  ...
  }, "Sync my tasks with Google Tasks"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: J,
      fontSize: 10,
      color: K.muted,
  ...
  }, "Anything assigned to you shows up in Google Tasks, and anything assigned to you in a meeting agenda shows up here. Runs by itself every 15 minutes.")), toggle(calPrefs.autoSyncTasks !== false, () => savePrefs({
    autoSyncTasks: calPrefs.autoSyncTasks === false
  }))), /*#__PURE__*/React.createElement("div", {
    style: {
      ...settRow,
      borderTop: `0.5px solid ${K.rowBd}`
  ...
  }, gtsState && gtsState.last_ok_at ? 'Last synced ' + sinceLabel(gtsState.last_ok_at) : 'Not synced yet'), gtsState && (gtsState.pushed || gtsState.pulled) ? /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: J,
      fontSize: 10,
      color: K.muted
    }
  }, gtsState.pushed, " sent to Google \xB7 ", gtsState.pulled, " brought in from an agenda") : null, gtsState && gtsState.last_error ? /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: J,
      fontSize: 10,
      color: '#C0392B',
  ...
  }, gtsState.last_error) : null), /*#__PURE__*/React.createElement("button", {
    onClick: syncTasksNow,
    disabled: gtsBusy,
    style: {
      ...settPill,
      opacity: gtsBusy ? 0.5 : 1
    }
  }, gtsBusy ? 'Syncing…' : 'Sync now'))), settHead('Calendars to Show'), /*#__PURE__*/React.createElement("div", {
    style: settCard
