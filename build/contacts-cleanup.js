...
function ZohoLink({
  ...
  return /*#__PURE__*/React.createElement("a", {
    ...
    title: title || 'Open in Zoho CRM',
    style: {
      ...style,
      color: style && style.color || C.navy,
      textDecoration: 'none',
      cursor: 'pointer'
    },
...
function Spinner({
  ...
  return /*#__PURE__*/React.createElement("span", {
    style: {
      ...
      height: size,
      borderRadius: '50%',
      border: '2px solid rgba(201,164,90,.25)',
      borderTopColor: color || C.goldSoft,
...
const DEV_CONTACTS = [{
  ...
  Prospect_Form_Type: ['Buyer']
}, {
  id: '102',
  First_Name: 'Dean',
  Last_Name: 'Kinney',
  Full_Name: 'Dean Kinney',
  ...
  Phone: '212-555-0190',
  Other_Phone: '',
  Client_Classification: 'B',
  Owner: {
    id: '1',
    name: 'Brett Silverman'
  },
  Spouse: null,
  Prospect_Form_Type: ['Seller']
}, {
  id: '104',
  First_Name: 'Alok',
  Last_Name: 'Mody',
  ...
  Spouse: null,
  Prospect_Form_Type: ['Investor', 'Buyer']
}, {
  id: '106',
  First_Name: 'Wendy',
  Last_Name: 'Sato',
  Full_Name: 'Wendy Sato',
  ...
  Spouse: null,
  Prospect_Form_Type: ['Seller', 'Investor']
}];
const DEV_CALLS = {
  '101': {
    id: 't1',
...
async function discoverFields() {
  ...
  return {
    spouse: spouse ? {
      api: spouse.api_name,
      label: spouse.field_label
    } : null,
    prospect: prospects.length === 1 ? pick(prospects[0]) : null,
    cls: pick(fields.find(f => f.api_name === CLS_FIELD)) || {
      api: CLS_FIELD,
      label: 'Client Classification',
      dataType: 'picklist',
      options: null
...
async function fetchContacts(meta, onPage) {
  if (isDev()) {
    onPage(DEV_CONTACTS, DEV_CONTACTS.length, false);
    return {
      ok: true,
  ...
  let fields = BASE_FIELDS.concat(extras),
    dropped = false;
  let page = 1,
    token = null,
    more = true,
  ...
  while (more && guard++ < 80) {
    ...
    if (!ok) {
      if (status === 400 && !dropped && extras.length) {
        dropped = true;
        fields = BASE_FIELDS;
        continue;
      }
      return {
        ok: false,
        error: data && data.error || 'Could not load contacts from Zoho.',
        contacts: all,
        dropped
      };
    }
...
async function fetchNextCalls() {
  ...
  while (more && guard++ < 80) {
    const args = {
      ...
      fields: ['Subject', 'Status', 'Due_Date', 'Who_Id', 'Owner'],
      per_page: 200
    };
    if (token) args.page_token = token;else args.page = page;
    const {
    ...
    (data.tasks || []).forEach(t => {
      ...
      const cur = map[cid];
      if (!cur || t.Due_Date < cur.date) map[cid] = {
        id: t.id,
        date: t.Due_Date,
        subject: t.Subject || ''
      };
...
function SpousePicker({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (isDev()) {
        const found = DEV_CONTACTS.filter(c => c.Full_Name.toLowerCase().includes(term.toLowerCase()) && c.id !== selfId).map(c => ({
          id: c.id,
  ...
  if (value) {
    ...
    }, value.name || 'Linked'), /*#__PURE__*/React.createElement("button", {
      type: "button",
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }), open && q.trim().length >= 2 && rect && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'fixed',
      top: rect.bottom + 3,
      left: rect.left,
      ...
      borderRadius: 9,
      boxShadow: '0 10px 26px rgba(0,26,74,.14)',
      zIndex: 80,
      maxHeight: 210,
      overflowY: 'auto'
    }
    ...
    size: 11
  }), " Searching\u2026"), !busy && !hits.length && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '9px 11px',
      fontSize: '0.74rem',
      color: C.textMuted
    }
  }, "No match"), !busy && hits.map(h => /*#__PURE__*/React.createElement("div", {
    key: h.id,
    onMouseDown: () => {
      onChange({
        id: h.id,
...
function DeleteModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      maxWidth: 420,
      padding: '22px 24px',
      boxShadow: '0 24px 60px rgba(0,26,74,.24)'
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
  ...
  }, "Delete this contact?")), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: '0.84rem',
      color: C.textSecondary,
      lineHeight: 1.6
  ...
  }, contact.Full_Name || 'This contact'), " will be deleted from Zoho CRM, along with the calls, notes and deal links attached to them. This cannot be undone from here."), error && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12,
      fontSize: '0.78rem',
      color: C.red
  ...
  }, "Keep contact"), /*#__PURE__*/React.createElement("button", {
    onClick: onConfirm,
    disabled: busy,
    style: {
      ...btn,
...
function App({
  ...
  async function load(force) {
    ...
    if (!m) {
      setErr('Could not read the Contacts field list from Zoho.');
      setLoading(false);
      return;
    }
    setMeta(m);
    const res = await fetchContacts(m, (rows, n) => {
      setContacts(rows);
      setProgress(n);
    });
    if (!res.ok) {
  ...
  const agents = useMemo(() => {
    const m = {};
    contacts.forEach(c => {
      const n = c.Owner && c.Owner.name || '';
      if (n) m[n] = (m[n] || 0) + 1;
    });
    return Object.keys(m).sort().map(n => ({
      name: n,
      n: m[n]
    }));
  }, [contacts]);
  const classes = useMemo(() => {
    ...
    Object.keys(seen).forEach(v => {
      if (!fromField.includes(v)) fromField.push(v);
    });
    return fromField;
  }, [contacts, meta]);
  ...
  function beginEdit(c) {
    ...
    setEditId(c.id);
    setDraft({
      First_Name: txt(c.First_Name),
      Last_Name: txt(c.Last_Name),
      Mobile: txt(c.Mobile),
      Phone: txt(c.Phone),
      Email: txt(c.Email),
      cls: clsOf(c),
      spouse: spouseOf(c, meta),
      prospect: prospectOf(c, meta)
    });
  }
  function cancelEdit() {
    setEditId(null);
  ...
  async function saveEdit(c) {
    ...
    if (meta?.spouse && !sameVal(draft.spouse, spouseOf(c, meta))) {
      ...
      record[meta.spouse.api] = draft.spouse ? {
        id: draft.spouse.id
      } : null;
    }
    if (meta?.prospect && !sameVal(draft.prospect, prospectOf(c, meta))) {
      record[meta.prospect.api] = meta.prospect.dataType === 'multiselectpicklist' ? draft.prospect || [] : draft.prospect && draft.prospect[0] || null;
    }
    ...
    if (meta?.spouse && meta.spouse.api in record) patch[meta.spouse.api] = draft.spouse ? {
      id: draft.spouse.id,
      name: draft.spouse.name
    } : null;
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Contacts Cleanup"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: '0.74rem',
      color: C.textMuted,
      marginTop: 3
    ...
    value: ""
  }, "All agents (", contacts.length, ")"), agents.map(a => /*#__PURE__*/React.createElement("option", {
    key: a.name,
    value: a.name
    ...
    value: ""
  }, "All classifications"), classes.map(v => /*#__PURE__*/React.createElement("option", {
    key: v,
    value: v
  }, v)), /*#__PURE__*/React.createElement("option", {
    value: "__none"
  }, "No classification")), /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'relative',
      marginLeft: 'auto'
    }
  ...
  })))), (err || notice || loading) && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '9px 22px',
      fontSize: '0.76rem',
      background: err ? C.red + '0F' : C.surfaceAlt,
      color: err ? C.red : C.textSecondary,
      borderBottom: '1px solid ' + C.border,
      display: 'flex',
      alignItems: 'center',
      gap: 8
    ...
    size: 12
  }), err || notice || (progress ? `Loading contacts from Zoho… ${progress} so far` : 'Loading contacts from Zoho…')), /*#__PURE__*/React.createElement("main", {
    style: {
      flex: 1,
      overflow: 'auto'
    }
    ...
    style: th
  }, "Linked spouse"), meta?.prospect && /*#__PURE__*/React.createElement("th", {
    style: th
  }, meta.prospect.label), /*#__PURE__*/React.createElement("th", {
    style: th
  }, "Next call"), /*#__PURE__*/React.createElement("th", {
    style: {
      ...th,
      position: 'sticky',
      right: 0,
    ...
    return /*#__PURE__*/React.createElement("tr", {
      ...
      value: ""
    }, "\u2014"), classes.map(v => /*#__PURE__*/React.createElement("option", {
      key: v,
      value: v
    }, v))) : /*#__PURE__*/React.createElement(ClsPill, {
      value: clsOf(c)
    })), /*#__PURE__*/React.createElement("td", {
      style: {
        ...td,
      ...
      style: inp
    }) : txt(c.Mobile) || /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.textMuted
      }
    }, "\u2014")), /*#__PURE__*/React.createElement("td", {
      ...
      style: inp
    }) : txt(c.Phone) || /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.textMuted
      }
    }, "\u2014")), /*#__PURE__*/React.createElement("td", {
      ...
      style: inp
    }) : txt(c.Email) || /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.textMuted
      }
    }, "\u2014")), /*#__PURE__*/React.createElement("td", {
    ...
    }, editing ? meta?.spouse ? /*#__PURE__*/React.createElement(SpousePicker, {
      value: draft.spouse,
      selfId: c.id,
    ...
    }, "no spouse field") : sp ? /*#__PURE__*/React.createElement(ZohoLink, {
      module: "Contacts",
      id: sp.id,
      style: {
        fontSize: '0.8rem',
    ...
    }, sp.name || 'Linked') : /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.textMuted
      }
    }, "\u2014")), meta?.prospect && /*#__PURE__*/React.createElement("td", {
      style: {
        ...td,
        minWidth: 160
      }
    }, editing ? meta.prospect.dataType === 'multiselectpicklist' ? /*#__PURE__*/React.createElement(MultiPick, {
      value: draft.prospect,
      options: meta.prospect.options,
      onChange: v => setDraft(d => ({
        ...d,
        prospect: v
      }))
    }) : /*#__PURE__*/React.createElement("select", {
      value: draft.prospect && draft.prospect[0] || '',
      onChange: e => setDraft(d => ({
        ...d,
        prospect: e.target.value ? [e.target.value] : []
      })),
      style: {
        ...inp,
        cursor: 'pointer'
      ...
      value: ""
    }, "\u2014"), (meta.prospect.options || []).map(o => /*#__PURE__*/React.createElement("option", {
      key: o,
      value: o
    }, o))) : prospectOf(c, meta).length ? /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        flexWrap: 'wrap',
        gap: 4
    ...
    }, call ? /*#__PURE__*/React.createElement(ZohoLink, {
      module: "Tasks",
      id: call.id,
      title: call.subject,
      style: {
        fontSize: '0.8rem',
        color: overdue ? C.red : C.textPrimary,
        fontWeight: overdue ? 600 : 400
      }
    }, fmtDate(call.date), overdue ? ' (overdue)' : '') : /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.textMuted
      }
      ...
      style: {
        ...
        position: 'sticky',
        right: 0,
        background: editing ? C.surfaceAlt : C.surface,
    ...
    }, rowError && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: '0.7rem',
        color: C.red,
        maxWidth: 160,
    ...
    }, rowError), /*#__PURE__*/React.createElement("button", {
      onClick: cancelEdit,
      disabled: saving,
      style: {
        ...btn,
