...
async function matchZohoContacts(name) {
  if (callsIsDev()) {
    await new Promise(r => setTimeout(r, 200));
    return devContactHits(name).map(c => ({
      ...c,
      score: 100
    }));
  }
...
async function writeCallKpisAsTasks(payload, hooks) {
  const adopt = hooks && hooks.adopt,
    onAdopted = hooks && hooks.onAdopted;
  ...
  for (const r of rows) {
    ...
    const adopted = [],
      items = [];
    r.calls.forEach(k => {
      const subject = k === 'Touch Call' ? (tier ? tier + ' ' : '') + 'Touch Call: ' + r.person.name : 'Follow Up Call : ' + r.person.name;
      const open = canAdopt ? adopt(r.person.contact_id, k) : null;
      if (open && !adopted.some(a => a.id === open.id)) {
        adopted.push(open);
        return;
      }
      items.push({
...
async function createZohoContact({
  ...
  const record = {
    Last_Name: l,
    Mobile: digits,
    [CONTACT_CLASS_FIELD]: classification
  };
...
function KpiSummary({
  ...
  useEffect(() => {
    ...
    (async () => {
      for (let i = 0; i < persons.length; i++) {
        ...
        let cad = null;
        try {
          cad = await touchCallCadence(payload.owner, st.chosenId);
        } catch (e) {
          cad = null;
        }
        if (!alive) return;
  ...
  const switched = i => {
    const st = res[i] || {};
    return !!(st.cad && st.cad.tooSoon && wantsTouch(i));
  };
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, effectiveKpis(i).map((k, n) => /*#__PURE__*/React.createElement("span", {
      key: k + '-' + n,
      style: {
        fontSize: '0.72rem',
        fontWeight: 600,
    ...
    }, k === 'Hotzone Action/s' ? 'Hotzone × ' + (p.hotzone_count || 0) : k))), st.cadBusy && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '0.72rem',
        color: sub,
        fontFamily: C.fontSans,
    ...
    }, "Checking the call cadence\u2026"), switched(i) && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '0.72rem',
        lineHeight: 1.5,
        marginBottom: 7,
    ...
    }, "Too soon for a touch call, so this is going in as a ", /*#__PURE__*/React.createElement("b", null, "Follow Up Call"), ". They were last called ", st.cad.gapDays, " day", st.cad.gapDays === 1 ? '' : 's', " ago and the ", st.cad.tier, " cadence is ", st.cad.interval, " days", st.cad.dueIso ? ', so the next touch is due ' + st.cad.dueIso : '', "."), st.status === 'loading' && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '0.76rem',
        color: sub,
        fontFamily: C.fontSans,
    ...
    }, st.matches && st.matches.length ? 'None of these?' : 'No matching contact found.'), !st.newC && /*#__PURE__*/React.createElement("button", {
      ...
      className: "ti ti-user-plus"
    }), "Create contact \u201C", p.name, "\u201D"), st.newC && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'grid',
        gap: 6,
        marginTop: 7
      ...
      style: {
        ...
        border: `1px solid ${dark ? C.navyHover : C.border}`,
        borderRadius: 8,
        padding: '7px 9px',
        fontSize: '0.78rem',
        color: dark ? '#fff' : C.textPrimary,
    ...
    }, "Classification"), CONTACT_CLASS_OPTIONS.map(o => /*#__PURE__*/React.createElement("button", {
      key: o,
      onClick: () => setNewC(i, {
        classification: o
      }),
      style: {
        ...
        color: st.newC.classification === o ? dark ? C.goldSoft : C.gold : dark ? '#fff' : C.textPrimary
      }
      ...
      style: {
        ...
        cursor: 'pointer',
        fontFamily: C.fontSans,
        opacity: st.creating ? 0.55 : 1
      }
    }, st.creating ? 'Saving…' : 'Create contact'))), st.error && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '0.74rem',
        color: C.red,
        marginTop: 5,
    ...
    onClick: confirm,
    disabled: !allResolved || submitting || anyBusy,
    style: {
      flex: 1,
      padding: 11,
      background: !allResolved || submitting || anyBusy ? dark ? '#23344f' : '#C9C3B4' : dark ? C.gold : C.navy,
      color: '#fff',
      border: 'none',
      borderRadius: 10,
      fontSize: '0.88rem',
      fontWeight: 600,
      cursor: !allResolved || submitting || anyBusy ? 'not-allowed' : 'pointer',
      fontFamily: C.fontSans,
      display: 'inline-flex',
      alignItems: 'center',
      justifyContent: 'center',
  ...
  }), submitting ? 'Sending…' : anyBusy ? 'Checking…' : 'Confirm' + (KPI_LIVE ? ' & send' : ''))));
}
...
function ZohoReconnect() {
  ...
  }, "Re-authorize Zoho if it stops reading the CRM.")), /*#__PURE__*/React.createElement("i", {
  ...
  })), !open ? null : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: '0.74rem',
      color: C.textSecondary,
      lineHeight: 1.65
...
function EmailNotificationsTab({
  ...
  useEffect(() => {
    ...
    Promise.all([c.from('tmg_meeting_notices').select('*').order('notified_at', {
      ascending: false
    }).limit(100), c.from('tmg_task_digests').select('*').order('sent_at', {
      ascending: false
    }).limit(100)]).then(([m, d]) => {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "What was sent, and why \u2014 visible only to you")), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      overflowY: 'auto',
  ...
  }, "No notification emails have gone out yet."), (rows || []).map((r, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      border: `1px solid ${cardBord}`,
      background: cardBg,
    ...
    style: {
      ...
      letterSpacing: '0.04em',
      textTransform: 'uppercase',
      color: badgeFg(r.kind),
      background: badgeBg(r.kind),
      borderRadius: 6,
      padding: '2px 8px'
    }
  ...
  }, r.detail), r.sub && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 11,
      color: metaC,
...
function pickProspectField(fields) {
  ...
  return hits.length === 1 ? {
    api: hits[0].api_name,
    label: hits[0].field_label || 'Prospect form'
  } : null;
}
...
const DEV_FORMS = {
  ...
  'Residential Seller': {
    fields: [{
      ...
      type: 'picklist',
      value: 'Relocating',
      options: ['Relocating', 'Upsizing', 'Downsizing', 'Investment']
    }, {
      api: 'List_Timeline',
      label: 'Listing timeline',
      type: 'picklist',
      value: '30-60 days',
      options: ['ASAP', '30-60 days', '60-90 days']
    }, {
      api: 'Occupied',
      ...
      label: "Spouse's Email",
      type: 'email',
      value: 'ronan.okafor@example.com'
    }, {
      api: 'Mortgage_Balance',
  ...
  'Open House': {
    fields: [{
      ...
      type: 'picklist',
      value: 'Just looking',
      options: ['Just looking', '0-3 months', '3-6 months']
    }, {
      api: 'Financing',
      label: 'Financing',
      type: 'picklist',
  ...
  'Investor': {
    fields: [{
      ...
      label: 'Asset class',
      type: 'multiselectpicklist',
      value: 'Multifamily, Retail',
      options: ['Multifamily', 'Retail', 'Industrial', 'Land']
    }, {
      api: 'Cash_Available',
...
function devProspect(type, contactId) {
  ...
  const build = f => {
    const v = Object.prototype.hasOwnProperty.call(saved, f.api) ? saved[f.api] === true ? 'Yes' : saved[f.api] === false ? '' : String(saved[f.api] == null ? '' : saved[f.api]) : String(f.value || '');
    return {
      api: f.api,
      label: f.label,
      type: f.type,
      ...
      read_only: false,
      link_module: f.link_module || null,
  ...
  if (callsIsDev()) {
    ...
    if (k >= 10) {
      ...
      out = good({
        ...
        zoho_email_span: mail.length ? {
          first: mail[mail.length - 1].time,
          last: mail[0].time
        } : null,
        last_touch: k % 4 === 3 ? null : {
          type: ['Call', 'Pop-by', 'Meeting', 'Email'][k % 4],
          date: '2026-09-0' + (1 + k % 9),
          subject: null
        }
      });
    } else if (k === 1) out = good({
      mailboxes: [{
        role: 'agent',
        name: 'Brad Baker',
        state: 'searched',
      ...
      mailboxes: [{
        ...
        role: 'tc',
        name: '',
        state: 'no_tc_assigned',
        threads: 0
      }]
    });else if (k === 9) fail('ai_unavailable');else out = good();
    return out;
  }
  const {
    ok,
...
const DEV_PEOPLE = [{
  ...
  cls: 'A',
  pform: null,
  tags: ['Sphere', 'Past Client', 'Newsletter', 'Luxury', 'Referral'],
  phone: 1,
  email: 1,
  spouse: 1
}, {
  name: 'Jonathan Hewitt',
  cls: 'B',
  pform: 'Residential Seller',
  tags: [],
  phone: 1,
  email: 1,
  spouse: 0
  ...
  name: 'Courtney Gill',
  cls: 'A',
  pform: 'Residential Buyer',
  tags: ['Sphere', 'Past Client', 'Newsletter', 'Luxury', 'Referral'],
  phone: 1,
  email: 0,
  spouse: 1
}, {
  name: 'Adriana Culling',
  cls: 'B',
  pform: null,
  tags: ['Sphere', 'Past Client', 'Newsletter', 'Luxury', 'Referral'],
  phone: 0,
...
function devContactHits(term) {
  ...
  return DEV_PEOPLE.map((p, k) => ({
    id: devContactId(k),
    full_name: p.name,
    email: p.email ? 'contact' + k + '@example.com' : ''
...
const devApplyStatus = list => list.map(t => DEV_TASK_STATUS.has(t.id) ? {
  ...t,
  Status: DEV_TASK_STATUS.get(t.id)
} : t);
...
async function lastCallFor(contactId) {
  ...
  if (callsIsDev()) {
    ...
    const v = days === null ? null : {
      iso: cIso(new Date(Date.now() - days * 86400000)),
      tier: 'B'
    };
...
function ProspectSheet({
  ...
  useEffect(() => {
    ...
    if (callsIsDev()) {
      ...
      setTimeout(() => {
        if (!dead) {
          setData(devProspect(t, contactId));
          setState('ready');
        }
      }, 400);
      return () => {
...
function CallsTab({
  ...
  const loadBrief = (id, force) => {
    ...
    fetchContactBrief(id).then(data => {
      ...
      setBrief({
        id,
        state: 'error',
        code: e && e.code || 'brief_failed',
        ownerName: e && e.ownerName || null
      });
    });
  };
  const openInfo = b => {
  ...
  useEffect(() => {
    ...
    const onBack = () => {
      if (!document.hidden && !busy && stale()) load(true);
    };
    ...
    if (callsIsDev()) {
      ...
      ids.forEach(id => {
        ...
        out[id] = {
          ...
          spouse: p.spouse ? {
            id: '55000000000009' + String(100 + k),
            name: 'Pat Example'
          } : null,
  ...
  const openCallFor = (contactId, kind) => {
    ...
    return [].concat(overdue && overdue.list || [], buckets && buckets.yesterday || [], buckets && buckets.today || [], buckets && buckets.tomorrow || []).find(t => t && t.Who_Id && t.Who_Id.id === contactId && !/completed/i.test(t.Status || '') && want.test(t.Subject || '')) || null;
  };
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onSaved: (cid, repatch) => {
      ...
      if (memo && memo.then) memo.then(d => {
        if (d && d.prospect) d.prospect = repatch(d.prospect);
      }).catch(() => {});
      setBrief(b => b && b.id === cid && b.state === 'ready' && b.data && b.data.prospect ? {
        ...b,
        data: {
          ...b.data,
          prospect: repatch(b.data.prospect)
        }
      } : b);
    },
  ...
  }))), (!brief || brief.id !== infoFor.id || brief.state === 'loading') && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 12,
      fontWeight: 300,
    ...
    style: {
      ...
      fontSize: 12.5,
      fontWeight: 300,
      color: brief.code === 'not_permitted' ? mutedCol : redCol,
      lineHeight: 1.55,
      margin: '10px 0 12px'
    }
  }, brief.code === 'owner_unresolved' ? brief.ownerName ? brief.ownerName + ' owns this contact in Zoho but has no TMG account — usually somebody who has left. There is no mailbox to read.' : BRIEF_ERROR_NOTE.owner_unresolved : BRIEF_ERROR_NOTE[brief.code] || 'The summary couldn’t be written this time.'), !['not_permitted', 'owner_unresolved', 'bad_contact_id', 'contact_not_found'].includes(brief.code) && /*#__PURE__*/React.createElement("button", {
    onClick: () => loadBrief(infoFor.id, true),
    style: {
      background: 'none',
      border: `1px solid ${lineCol}`,
      borderRadius: 10,
...
function AddKpiSheet({
  ...
  async function submitKpi(base, resolvedPersons) {
    ...
    try {
      ...
      (rest.others || []).forEach(o => done.push({
        count: o.count,
        subject: o.kpi
      }));
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }), " Paste your notes")), mode === 'form' && !formPayload && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 9,
      color: mutedCol,
  ...
  }, "Everything entered here is credited to ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: headTitle
    }
  }, ownerName), "."), /*#__PURE__*/React.createElement(KpiInlineForm, {
    dark: dark,
    ownerName: ownerName,
    initial: formSeed,
    onReview: p => {
    ...
    onCancel: onClose
  })), mode === 'notes' && !notePayload && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 9,
      color: mutedCol,
  ...
  }, noteBusy ? 'Thinking...' : 'Send'))), (formPayload || mode === 'notes' && notePayload) && /*#__PURE__*/React.createElement("div", null, submitErr && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: J,
      fontSize: 9,
      color: redCol,
  ...
  }, submitErr), /*#__PURE__*/React.createElement(KpiSummary, {
    dark: dark,
    payload: formPayload || notePayload,
    onBack: () => {
      if (formPayload) setFormPayload(null);else setNotePayload(null);
    },
...
function LogActivitySheet({
  ...
  async function submit() {
    ...
    try {
      ...
      if (items.length) {
        res = await createActivityTasks({
          items,
          typeField: meta && meta.api,
          contact,
          dateIso: dateIso || cIso(new Date()),
          ownerId
        });
      }
...
function CapacityView({
  ...
  const months = useMemo(() => Array.from({
    length: monthsLoaded
  }, (_, i) => new Date(baseMonth.getFullYear(), baseMonth.getMonth() + i, 1)), [baseMonth, monthsLoaded]);
  ...
  }, "Zoho stops paging open tasks at 2,000 records, so the months below may be missing calls. /crm-tasks hits the same ceiling."), nameUnmatched ? /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '20px 0',
      fontFamily: J,
      fontSize: 9,
  ...
  React.createElement("div", {
  ...
  }, "\u2715"), "weekend/holiday")), /*#__PURE__*/React.createElement("button", {
    onClick: () => setMonthsLoaded(n => n + CAPACITY_MONTHS_STEP),
    style: {
      display: 'block',
      margin: '0 auto',
      padding: '9px 18px',
    ...
    key: t,
    title: 'Counted from the task subject, not the contact record',
    style: {
      fontFamily: J,
      fontSize: 8,
      fontWeight: 700,
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      title: 'From the task subject, not the contact record — the Calls list shows the record’s own classification',
      style: {
        fontFamily: J,
        fontSize: 8,
        fontWeight: 700,
...
function App({
  ...
  const isSymon = ['symon@themorshedgroup.com', 'manager@themorshedgroup.com'].includes(String(profile && profile.email || '').toLowerCase());
  ...
  const tasksNavItem = {
    ...
    label: 'Tasks',
    icon: 'ti-checklist',
    href: 'tasks.html?v=' + TASKS_POPOUT_VERSION + '#tasks'
  };
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    dark: dark,
    ownerName: myName,
    ownerEmail: profile && profile.email || null,
    isAdmin: isAdmin
    ...
    dark: dark,
    onBack: () => setMoreMenuOpen(true)
  }) : moreView === 'email-notifications' && isSymon ? /*#__PURE__*/React.createElement(EmailNotificationsTab, {
    dark: dark,
    onBack: () => setMoreMenuOpen(true)
  }) : /*#__PURE__*/React.createElement(CompanyDirectory, {
    dark: dark,
    onBack: () => setMoreMenuOpen(true)
  })), !['chat', 'calls', 'kpis', 'deals', 'more'].includes(activeTab) && /*#__PURE__*/React.createElement(Placeholder, {
