...
- Resolve relative dates using today's date given below. Do NOT use markdown formatting.`;
    ...
    async function createActivityTasks({ items, typeField, contact, dateIso, ownerId }) {
      const jobs = [];
      (items || []).forEach(it => { for (let i = 0; i < it.count; i++) jobs.push(it.type); });
      ...
      let whoShape = 'object';
      const build = (type) => {
        const rec = { Subject: activitySubject(type, contact.name), Status: 'Completed', Due_Date: dateIso };
      ...
      const post = (type) => safeZoho({ action: 'create_record', module: 'Tasks', record: build(type) }).then(r => ({ type, r }));
      ...
      useEffect(() => {
        ...
        if (callsIsDev()) {
          ...
          ids.forEach((id, i) => { out[id] = {
            ...
            spouse: i % 3 === 0 ? { id: '55000000000009' + String(100 + i), name: 'Pat Example' } : null,
          }; });
          merge(out);
          return;
        }
          ...
          for (let i = 0; i < needSpouse.length; i += 60) {
            ...
            try {
              ...
              if (r.ok && r.data && r.data.links) {
                const out = {};
                chunk.forEach(id => { out[id] = { spouse: r.data.links[id] || null }; });
                merge(out);
              }
          ...
          for (let i = 0; i < needDetail.length; i += 6) {
            const got = await Promise.all(needDetail.slice(i, i + 6).map(async id => {
              try {
                ...
                return [id, { detail: true, phone: (c && c.phone) || null, email: (c && c.email) || null }];
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 3, minWidth: 0 }}>
                  ...
                  <span style={{ fontFamily: J, fontSize: 8, color: st.num, flexShrink: 0 }}>{phone ? formatPhone(phone) : (looked ? 'No number' : '…')}</span>
              ...
              {phone
                ? <a href={'tel:' + String(phone).replace(/[^\d+]/g, '')} title="Call" style={{ ...ctrl, background: callBtnBg, color: '#fff', textDecoration: 'none' }}>
    ...
    function CapacityView({ dark, agent, setAgent, agents, team, me, meId }) {
      ...
      const projection = useMemo(
        () => showProjected ? projectCallCadence(calls, endIso, todayIso) : { byOwner: {}, unprojected: 0 },
